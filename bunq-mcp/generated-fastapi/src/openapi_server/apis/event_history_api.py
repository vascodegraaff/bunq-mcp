...
from openapi_server.apis.event_history_api_base import BaseEventHistoryApi
...
from openapi_server.models.event_history_listing import EventHistoryListing
...
@router.get(
    "/user/{userID}/event/{eventID}/event-history",
    responses={
        200: {"model": List[EventHistoryListing], "description": "Used to list event history. apiApp:LISTING"},
...
async def list_all_event_history_for_user_event(
...
) -> List[EventHistoryListing]:
    """Used to list event history. apiApp:LISTING"""
    if not BaseEventHistoryApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseEventHistoryApi.subclasses[0]().list_all_event_history_for_user_event(userID, eventID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
