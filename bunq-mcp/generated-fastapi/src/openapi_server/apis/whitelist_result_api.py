...
from openapi_server.apis.whitelist_result_api_base import BaseWhitelistResultApi
...
from openapi_server.models.whitelist_result_listing import WhitelistResultListing
from openapi_server.models.whitelist_result_read import WhitelistResultRead
...
@router.get(
    "/user/{userID}/monetary-account/{monetary-accountID}/whitelist/{whitelistID}/whitelist-result",
    responses={
        200: {"model": List[WhitelistResultListing], "description": "Whitelist an SDD so that when one comes in, it is automatically accepted. apiAdmin:READ/LISTING apiApp:READ/LISTING apiServiceInternal:READ/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["whitelist-result"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_whitelist_result_for_user_monetary_account_whitelist(
...
) -> List[WhitelistResultListing]:
    """Whitelist an SDD so that when one comes in, it is automatically accepted. apiAdmin:READ/LISTING apiApp:READ/LISTING apiServiceInternal:READ/LISTING"""
    if not BaseWhitelistResultApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseWhitelistResultApi.subclasses[0]().list_all_whitelist_result_for_user_monetary_account_whitelist(userID, monetary-accountID, whitelistID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/monetary-account/{monetary-accountID}/whitelist/{whitelistID}/whitelist-result/{itemId}",
    responses={
        200: {"model": WhitelistResultRead, "description": "Whitelist an SDD so that when one comes in, it is automatically accepted. apiAdmin:READ/LISTING apiApp:READ/LISTING apiServiceInternal:READ/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["whitelist-result"],
    summary="",
...
async def r_ead_whitelist_result_for_user_monetary_account_whitelist(
    userID: StrictInt = Path(..., description=""),
    monetary-accountID: StrictInt = Path(..., description=""),
    whitelistID: StrictInt = Path(..., description=""),
    ...
    if not BaseWhitelistResultApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseWhitelistResultApi.subclasses[0]().r_ead_whitelist_result_for_user_monetary_account_whitelist(userID, monetary-accountID, whitelistID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
