...
from openapi_server.apis.iberpay_transfer_api_base import BaseIberpayTransferApi
...
from openapi_server.models.iberpay_incoming_transfer_create import IberpayIncomingTransferCreate
...
@router.post(
    "/iberpay-transfer",
    responses={
        200: {"model": IberpayIncomingTransferCreate, "description": "Endpoint for Iberpay to send payments to bunq. Iberpay service name: WS_RECEP_INVESTIGACION. apiIberpay:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["iberpay-transfer"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_iberpay_transfer(
...
) -> IberpayIncomingTransferCreate:
    """Endpoint for Iberpay to send payments to bunq. Iberpay service name: WS_RECEP_INVESTIGACION. apiIberpay:CREATE"""
    if not BaseIberpayTransferApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseIberpayTransferApi.subclasses[0]().c_reate_iberpay_transfer(user_agent, x_bunq_client_authentication, body, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
