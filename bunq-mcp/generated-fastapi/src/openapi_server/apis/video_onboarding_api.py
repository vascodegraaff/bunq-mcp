...
from openapi_server.apis.video_onboarding_api_base import BaseVideoOnboardingApi
...
@router.get(
    "/video-onboarding/{itemId}",
    responses={
        200: {"model": object, "description": "Endpoint used only to read videoOnboarding model in other api calls. apiAdmin:READ apiServiceInternal:READ"},
...
async def r_ead_video_onboarding(
    ...
    """Endpoint used only to read videoOnboarding model in other api calls. apiAdmin:READ apiServiceInternal:READ"""
    if not BaseVideoOnboardingApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseVideoOnboardingApi.subclasses[0]().r_ead_video_onboarding(itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
