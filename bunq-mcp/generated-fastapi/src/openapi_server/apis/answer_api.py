...
from openapi_server.apis.answer_api_base import BaseAnswerApi
...
from openapi_server.models.assistant_message_answer import AssistantMessageAnswer
from openapi_server.models.assistant_message_answer_create import AssistantMessageAnswerCreate
from openapi_server.models.assistant_message_answer_read import AssistantMessageAnswerRead
...
@router.post(
    "/user/{userID}/assistant/{assistantID}/conversation/{conversationID}/answer",
    responses={
        200: {"model": AssistantMessageAnswerCreate, "description": "Endpoint to retrieve answers to questions to your personal assistant within a conversation. apiAdmin:CREATE/READ apiApp:READ"},
...
async def c_reate_answer_for_user_assistant_conversation(
    ...
    assistant_message_answer: AssistantMessageAnswer = Body(None, description=""),
...
) -> AssistantMessageAnswerCreate:
    """Endpoint to retrieve answers to questions to your personal assistant within a conversation. apiAdmin:CREATE/READ apiApp:READ"""
    if not BaseAnswerApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAnswerApi.subclasses[0]().c_reate_answer_for_user_assistant_conversation(userID, assistantID, conversationID, user_agent, x_bunq_client_authentication, assistant_message_answer, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/assistant/{assistantID}/conversation/{conversationID}/answer/{itemId}",
    responses={
        200: {"model": AssistantMessageAnswerRead, "description": "Endpoint to retrieve answers to questions to your personal assistant within a conversation. apiAdmin:CREATE/READ apiApp:READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["answer"],
    summary="",
...
async def r_ead_answer_for_user_assistant_conversation(
    userID: StrictInt = Path(..., description=""),
    assistantID: StrictInt = Path(..., description=""),
    conversationID: StrictInt = Path(..., description=""),
    ...
    x_bunq_client_request_id: Annotated[Optional[StrictStr], Field(description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer.")] = Header(None, description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer."),
    x_bunq_geolocation: Annotated[Optional[StrictStr], Field(description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued.")] = Header(None, description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued."),
) -> AssistantMessageAnswerRead:
    """Endpoint to retrieve answers to questions to your personal assistant within a conversation. apiAdmin:CREATE/READ apiApp:READ"""
    if not BaseAnswerApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAnswerApi.subclasses[0]().r_ead_answer_for_user_assistant_conversation(userID, assistantID, conversationID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
