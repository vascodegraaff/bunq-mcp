...
from openapi_server.apis.token_qr_promotion_api_base import BaseTokenQrPromotionApi
...
from openapi_server.models.token_qr_promotion import TokenQrPromotion
from openapi_server.models.token_qr_promotion_create import TokenQrPromotionCreate
...
@router.post(
    "/user/{userID}/token-qr-promotion",
    responses={
        200: {"model": TokenQrPromotionCreate, "description": "Usage of this endpoint (by scanning a QR code) results in creating a promotion for the user. apiApp:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["token-qr-promotion"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_token_qr_promotion_for_user(
    ...
    token_qr_promotion: TokenQrPromotion = Body(None, description=""),
...
) -> TokenQrPromotionCreate:
    """Usage of this endpoint (by scanning a QR code) results in creating a promotion for the user. apiApp:CREATE"""
    if not BaseTokenQrPromotionApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseTokenQrPromotionApi.subclasses[0]().c_reate_token_qr_promotion_for_user(userID, user_agent, x_bunq_client_authentication, token_qr_promotion, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
