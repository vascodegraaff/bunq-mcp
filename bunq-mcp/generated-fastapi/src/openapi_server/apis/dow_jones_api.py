...
from openapi_server.apis.dow_jones_api_base import BaseDowJonesApi
...
from openapi_server.models.dow_jones_listing import DowJonesListing
...
@router.get(
    "/dow-jones",
    responses={
        200: {"model": List[DowJonesListing], "description": "Used to get the details for a given Dow Jones Id. apiAdmin:LISTING apiServiceInternal:LISTING apiProcess:LISTING"},
...
) -> List[DowJonesListing]:
    """Used to get the details for a given Dow Jones Id. apiAdmin:LISTING apiServiceInternal:LISTING apiProcess:LISTING"""
    if not BaseDowJonesApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseDowJonesApi.subclasses[0]().list_all_dow_jones(user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
