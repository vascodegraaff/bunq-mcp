...
from openapi_server.apis.token_qr_request_payment_service_provider_api_base import BaseTokenQrRequestPaymentServiceProviderApi
...
from openapi_server.models.token_qr_request_payment_service_provider import TokenQrRequestPaymentServiceProvider
from openapi_server.models.token_qr_request_payment_service_provider_create import TokenQrRequestPaymentServiceProviderCreate
...
@router.post(
    "/user/{userID}/token-qr-request-payment-service-provider",
    responses={
        200: {"model": TokenQrRequestPaymentServiceProviderCreate, "description": "Used to create a request based on a token which is created during the creation of a payment service provider issuer transaction. apiApp:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["token-qr-request-payment-service-provider"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_token_qr_request_payment_service_provider_for_user(
    ...
    token_qr_request_payment_service_provider: TokenQrRequestPaymentServiceProvider = Body(None, description=""),
    ...
    """Used to create a request based on a token which is created during the creation of a payment service provider issuer transaction. apiApp:CREATE"""
    if not BaseTokenQrRequestPaymentServiceProviderApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseTokenQrRequestPaymentServiceProviderApi.subclasses[0]().c_reate_token_qr_request_payment_service_provider_for_user(userID, user_agent, x_bunq_client_authentication, token_qr_request_payment_service_provider, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
