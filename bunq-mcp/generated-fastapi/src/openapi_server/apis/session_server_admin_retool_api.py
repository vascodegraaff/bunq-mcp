...
from openapi_server.apis.session_server_admin_retool_api_base import BaseSessionServerAdminRetoolApi
...
from openapi_server.models.session_server_admin_retool import SessionServerAdminRetool
from openapi_server.models.session_server_admin_retool_create import SessionServerAdminRetoolCreate
...
@router.post(
    "/session-server-admin-retool",
    responses={
        200: {"model": SessionServerAdminRetoolCreate, "description": "Authenticate a Retool session. apiRetool:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["session-server-admin-retool"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_session_server_admin_retool(
    ...
    session_server_admin_retool: SessionServerAdminRetool = Body(None, description=""),
...
) -> SessionServerAdminRetoolCreate:
    """Authenticate a Retool session. apiRetool:CREATE"""
    if not BaseSessionServerAdminRetoolApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseSessionServerAdminRetoolApi.subclasses[0]().c_reate_session_server_admin_retool(user_agent, x_bunq_client_authentication, session_server_admin_retool, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
