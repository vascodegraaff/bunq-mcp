...
from openapi_server.apis.sofort_merchant_transaction_poll_api_base import BaseSofortMerchantTransactionPollApi
...
from openapi_server.models.sofort_merchant_transaction_poll import SofortMerchantTransactionPoll
from openapi_server.models.sofort_merchant_transaction_poll_create import SofortMerchantTransactionPollCreate
...
@router.post(
    "/sofort-merchant-transaction-poll",
    responses={
        200: {"model": SofortMerchantTransactionPollCreate, "description": "View to pass an transaction identifier, in which case the associated transaction status will be polled nearly immediately. apiApp:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["sofort-merchant-transaction-poll"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_sofort_merchant_transaction_poll(
    ...
    sofort_merchant_transaction_poll: SofortMerchantTransactionPoll = Body(None, description=""),
...
) -> SofortMerchantTransactionPollCreate:
    """View to pass an transaction identifier, in which case the associated transaction status will be polled nearly immediately. apiApp:CREATE"""
    if not BaseSofortMerchantTransactionPollApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseSofortMerchantTransactionPollApi.subclasses[0]().c_reate_sofort_merchant_transaction_poll(user_agent, x_bunq_client_authentication, sofort_merchant_transaction_poll, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
