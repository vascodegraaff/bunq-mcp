...
from openapi_server.apis.billing_contract_subscription_product_availability_api_base import BaseBillingContractSubscriptionProductAvailabilityApi
...
from openapi_server.models.billing_contract_subscription_product_availability_listing import BillingContractSubscriptionProductAvailabilityListing
...
@router.get(
    "/user/{userID}/billing-contract-subscription-product-availability",
    responses={
        200: {"model": List[BillingContractSubscriptionProductAvailabilityListing], "description": "Used to view product availability if user wants to switch to a different plan. apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["billing-contract-subscription-product-availability"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_billing_contract_subscription_product_availability_for_user(
...
) -> List[BillingContractSubscriptionProductAvailabilityListing]:
    """Used to view product availability if user wants to switch to a different plan. apiApp:LISTING"""
    if not BaseBillingContractSubscriptionProductAvailabilityApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseBillingContractSubscriptionProductAvailabilityApi.subclasses[0]().list_all_billing_contract_subscription_product_availability_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
