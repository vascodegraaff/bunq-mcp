...
from openapi_server.apis.monetary_account_assessment_api_base import BaseMonetaryAccountAssessmentApi
...
from openapi_server.models.monetary_account_assessment_read import MonetaryAccountAssessmentRead
...
@router.get(
    "/monetary-account-assessment/{itemId}",
    responses={
        200: {"model": MonetaryAccountAssessmentRead, "description": "View for the assessment monetary accounts apiAdmin:READ apiServiceInternal:READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["monetary-account-assessment"],
    summary="",
    response_model_by_alias=True,
)
async def r_ead_monetary_account_assessment(
...
) -> MonetaryAccountAssessmentRead:
    """View for the assessment monetary accounts apiAdmin:READ apiServiceInternal:READ"""
    if not BaseMonetaryAccountAssessmentApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseMonetaryAccountAssessmentApi.subclasses[0]().r_ead_monetary_account_assessment(itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
