...
from openapi_server.apis.invite_user_public_api_base import BaseInviteUserPublicApi
...
from openapi_server.models.invite_user_public_listing import InviteUserPublicListing
...
@router.get(
    "/invite-user-public",
    responses={
        200: {"model": List[InviteUserPublicListing], "description": "Used for listing the invite types a pointer has been invited to. apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["invite-user-public"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_invite_user_public(
...
) -> List[InviteUserPublicListing]:
    """Used for listing the invite types a pointer has been invited to. apiApp:LISTING"""
    if not BaseInviteUserPublicApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseInviteUserPublicApi.subclasses[0]().list_all_invite_user_public(user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
