...
from openapi_server.apis.quality_assurance_support_user_validation_api_base import BaseQualityAssuranceSupportUserValidationApi
...
from openapi_server.models.quality_assurance_support_user_validation_create import QualityAssuranceSupportUserValidationCreate
...
@router.post(
    "/user/{userID}/quality-assurance-support-user-validation",
    responses={
        200: {"model": QualityAssuranceSupportUserValidationCreate, "description": " apiQualityAssurance:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["quality-assurance-support-user-validation"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_quality_assurance_support_user_validation_for_user(
...
) -> QualityAssuranceSupportUserValidationCreate:
    """ apiQualityAssurance:CREATE"""
    if not BaseQualityAssuranceSupportUserValidationApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseQualityAssuranceSupportUserValidationApi.subclasses[0]().c_reate_quality_assurance_support_user_validation_for_user(userID, user_agent, x_bunq_client_authentication, body, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
