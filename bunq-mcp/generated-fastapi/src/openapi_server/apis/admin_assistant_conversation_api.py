...
from openapi_server.apis.admin_assistant_conversation_api_base import BaseAdminAssistantConversationApi
...
@router.get(
    ...
    responses={
        200: {"model": List[object], "description": "The endpoint to manage assistant conversations from admin. apiAdmin:LISTING/READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["admin-assistant-conversation"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_admin_assistant_conversation(
    ...
    """The endpoint to manage assistant conversations from admin. apiAdmin:LISTING/READ"""
    if not BaseAdminAssistantConversationApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAdminAssistantConversationApi.subclasses[0]().list_all_admin_assistant_conversation(user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/admin-assistant-conversation/{itemId}",
    responses={
        200: {"model": object, "description": "The endpoint to manage assistant conversations from admin. apiAdmin:LISTING/READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["admin-assistant-conversation"],
    summary="",
...
async def r_ead_admin_assistant_conversation(
    ...
    if not BaseAdminAssistantConversationApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAdminAssistantConversationApi.subclasses[0]().r_ead_admin_assistant_conversation(itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
