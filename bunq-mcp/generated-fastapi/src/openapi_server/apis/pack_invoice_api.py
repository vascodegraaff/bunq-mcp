...
from openapi_server.apis.pack_invoice_api_base import BasePackInvoiceApi
...
@router.get(
    "/user/{userID}/pack-invoice",
    responses={
        200: {"model": List[object], "description": "Used for listing invoices created while having a pack memberships. apiApp:LISTING"},
...
async def list_all_pack_invoice_for_user(
    ...
    """Used for listing invoices created while having a pack memberships. apiApp:LISTING"""
    if not BasePackInvoiceApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BasePackInvoiceApi.subclasses[0]().list_all_pack_invoice_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
