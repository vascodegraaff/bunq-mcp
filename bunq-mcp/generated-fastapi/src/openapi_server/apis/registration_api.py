...
from openapi_server.apis.registration_api_base import BaseRegistrationApi
...
from openapi_server.models.invite_profile_public_registration import InviteProfilePublicRegistration
from openapi_server.models.invite_profile_public_registration_create import InviteProfilePublicRegistrationCreate
...
@router.post(
    "/invite-profile-public/{invite-profile-publicUUID}/registration",
    responses={
        200: {"model": InviteProfilePublicRegistrationCreate, "description": "Public endpoint used to record number of registrations on an invite profile. apiApp:CREATE"},
...
async def c_reate_registration_for_invite_profile_public(
    invite-profile-publicUUID: StrictStr = Path(..., description=""),
    ...
    invite_profile_public_registration: InviteProfilePublicRegistration = Body(None, description=""),
...
) -> InviteProfilePublicRegistrationCreate:
    """Public endpoint used to record number of registrations on an invite profile. apiApp:CREATE"""
    if not BaseRegistrationApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseRegistrationApi.subclasses[0]().c_reate_registration_for_invite_profile_public(invite-profile-publicUUID, user_agent, x_bunq_client_authentication, invite_profile_public_registration, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
