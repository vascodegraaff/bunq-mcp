...
from openapi_server.apis.ginmon_instrument_price_api_base import BaseGinmonInstrumentPriceApi
...
from openapi_server.models.ginmon_instrument_price_read import GinmonInstrumentPriceRead
...
@router.get(
    "/user/{userID}/ginmon-instrument-price/{itemId}",
    responses={
        200: {"model": GinmonInstrumentPriceRead, "description": "Endpoint for listing price history of an instrument from Ginmon. apiApp:READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["ginmon-instrument-price"],
    summary="",
    response_model_by_alias=True,
)
async def r_ead_ginmon_instrument_price_for_user(
    ...
    """Endpoint for listing price history of an instrument from Ginmon. apiApp:READ"""
    if not BaseGinmonInstrumentPriceApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseGinmonInstrumentPriceApi.subclasses[0]().r_ead_ginmon_instrument_price_for_user(userID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
