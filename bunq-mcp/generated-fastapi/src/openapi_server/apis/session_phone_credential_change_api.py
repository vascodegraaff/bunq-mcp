...
from openapi_server.apis.session_phone_credential_change_api_base import BaseSessionPhoneCredentialChangeApi
...
from openapi_server.models.session_phone_credential_change import SessionPhoneCredentialChange
from openapi_server.models.session_phone_credential_change_create import SessionPhoneCredentialChangeCreate
...
@router.post(
    ...
    responses={
        200: {"model": SessionPhoneCredentialChangeCreate, "description": "Used to create a new session which is needed to change a user&#39;s password. apiAdmin:CREATE apiApp:CREATE apiServiceInternal:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["session-phone-credential-change"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_session_phone_credential_change(
    ...
    session_phone_credential_change: SessionPhoneCredentialChange = Body(None, description=""),
...
) -> SessionPhoneCredentialChangeCreate:
    """Used to create a new session which is needed to change a user&#39;s password. apiAdmin:CREATE apiApp:CREATE apiServiceInternal:CREATE"""
    if not BaseSessionPhoneCredentialChangeApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseSessionPhoneCredentialChangeApi.subclasses[0]().c_reate_session_phone_credential_change(user_agent, x_bunq_client_authentication, session_phone_credential_change, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
