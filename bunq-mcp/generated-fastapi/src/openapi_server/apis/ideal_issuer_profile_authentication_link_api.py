...
from openapi_server.apis.ideal_issuer_profile_authentication_link_api_base import BaseIdealIssuerProfileAuthenticationLinkApi
...
from openapi_server.models.ideal_issuer_profile_authentication_link_create import IdealIssuerProfileAuthenticationLinkCreate
...
@router.post(
    "/user/{userID}/ideal-issuer-profile-authentication-link",
    responses={
        200: {"model": IdealIssuerProfileAuthenticationLinkCreate, "description": "The endpoint to authenticate user to iDEAL profile whenever user wants to log in to iDEAL portal. apiApp:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["ideal-issuer-profile-authentication-link"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_ideal_issuer_profile_authentication_link_for_user(
...
) -> IdealIssuerProfileAuthenticationLinkCreate:
    """The endpoint to authenticate user to iDEAL profile whenever user wants to log in to iDEAL portal. apiApp:CREATE"""
    if not BaseIdealIssuerProfileAuthenticationLinkApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseIdealIssuerProfileAuthenticationLinkApi.subclasses[0]().c_reate_ideal_issuer_profile_authentication_link_for_user(userID, user_agent, x_bunq_client_authentication, body, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
