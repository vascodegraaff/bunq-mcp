...
from openapi_server.models.product_price_listing import ProductPriceListing
...
@router.get(
    "/user/{userID}/price",
    responses={
        200: {"model": List[ProductPriceListing], "description": "Used to view a product price. apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING"},
...
async def list_all_price_for_user(
...
) -> List[ProductPriceListing]:
    """Used to view a product price. apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING"""
    if not BasePriceApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BasePriceApi.subclasses[0]().list_all_price_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
