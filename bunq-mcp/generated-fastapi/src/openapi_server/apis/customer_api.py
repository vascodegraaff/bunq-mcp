...
from openapi_server.apis.customer_api_base import BaseCustomerApi
...
from openapi_server.models.customer_listing import CustomerListing
...
@router.get(
    ...
    responses={
        200: {"model": List[CustomerListing], "description": "Used to view a customer. apiAdmin:LISTING/READ/UPDATE apiApp:LISTING/READ/UPDATE apiServiceInternal:LISTING/READ/UPDATE"},
...
async def list_all_customer_for_user(
...
) -> List[CustomerListing]:
    """Used to view a customer. apiAdmin:LISTING/READ/UPDATE apiApp:LISTING/READ/UPDATE apiServiceInternal:LISTING/READ/UPDATE"""
    if not BaseCustomerApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseCustomerApi.subclasses[0]().list_all_customer_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/customer/{itemId}",
    responses={
        200: {"model": CustomerRead, "description": "Used to view a customer. apiAdmin:LISTING/READ/UPDATE apiApp:LISTING/READ/UPDATE apiServiceInternal:LISTING/READ/UPDATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["customer"],
    summary="",
...
async def r_ead_customer_for_user(
    userID: StrictInt = Path(..., description=""),
    ...
    if not BaseCustomerApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseCustomerApi.subclasses[0]().r_ead_customer_for_user(userID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.put(
    "/user/{userID}/customer/{itemId}",
    responses={
        200: {"model": CustomerUpdate, "description": "Used to view a customer. apiAdmin:LISTING/READ/UPDATE apiApp:LISTING/READ/UPDATE apiServiceInternal:LISTING/READ/UPDATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["customer"],
    summary="",
...
async def u_pdate_customer_for_user(
    userID: StrictInt = Path(..., description=""),
    itemId: StrictInt = Path(..., description=""),
    user_agent: Annotated[StrictStr, Field(description="The User-Agent header field should contain information about the user agent originating the request. There are no restrictions on the value of this header.")] = Header(None, description="The User-Agent header field should contain information about the user agent originating the request. There are no restrictions on the value of this header."),
    x_bunq_client_authentication: Annotated[StrictStr, Field(description="The authentication token is used to authenticate the source of the API call. It is required by all API calls except for POST /v1/installation. It is important to note that the device and session calls are using the token from the response of the installation call, while all the other calls use the token from the response of the session-server call")] = Header(None, description="The authentication token is used to authenticate the source of the API call. It is required by all API calls except for POST /v1/installation. It is important to note that the device and session calls are using the token from the response of the installation call, while all the other calls use the token from the response of the session-server call"),
    ...
    if not BaseCustomerApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseCustomerApi.subclasses[0]().u_pdate_customer_for_user(userID, itemId, user_agent, x_bunq_client_authentication, customer, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
