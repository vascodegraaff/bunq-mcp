...
from openapi_server.apis.company_legal_form_available_api_base import BaseCompanyLegalFormAvailableApi
...
from openapi_server.models.company_legal_form_available_listing import CompanyLegalFormAvailableListing
...
@router.get(
    "/company-legal-form-available",
    responses={
        200: {"model": List[CompanyLegalFormAvailableListing], "description": "Retrieve all possible legal forms for companies. Their presence in this list does not guarantee that we currently support signups for them. apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["company-legal-form-available"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_company_legal_form_available(
...
) -> List[CompanyLegalFormAvailableListing]:
    """Retrieve all possible legal forms for companies. Their presence in this list does not guarantee that we currently support signups for them. apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING"""
    if not BaseCompanyLegalFormAvailableApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseCompanyLegalFormAvailableApi.subclasses[0]().list_all_company_legal_form_available(user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
