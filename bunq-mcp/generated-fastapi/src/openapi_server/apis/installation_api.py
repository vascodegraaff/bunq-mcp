...
from openapi_server.apis.installation_api_base import BaseInstallationApi
...
from openapi_server.models.installation_create import InstallationCreate
from openapi_server.models.installation_listing import InstallationListing
from openapi_server.models.installation_read import InstallationRead
...
@router.post(
    "/installation",
    responses={
        200: {"model": InstallationCreate, "description": "Installation is used to tell the server about the public key of your key pair. The server uses this key to verify your subsequent calls, which need to be signed with your own private key. Additionally, you can use the token you get from an Installation to authenticate the registration of a new device. apiAdmin:CREATE/READ/LISTING apiApp:CREATE/READ/LISTING apiPublic:CREATE/READ/LISTING apiServiceInternal:CREATE/READ/LISTING apiProcess:CREATE/READ/LISTING apiTranslink:CREATE/READ/LISTING apiDeveloperPortal:CREATE/READ/LISTING"},
...
async def c_reate_installation(
    ...
    installation: Installation = Body(None, description=""),
    ...
    if not BaseInstallationApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseInstallationApi.subclasses[0]().c_reate_installation(user_agent, x_bunq_client_authentication, installation, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/installation",
    responses={
        200: {"model": List[InstallationListing], "description": "Installation is used to tell the server about the public key of your key pair. The server uses this key to verify your subsequent calls, which need to be signed with your own private key. Additionally, you can use the token you get from an Installation to authenticate the registration of a new device. apiAdmin:CREATE/READ/LISTING apiApp:CREATE/READ/LISTING apiPublic:CREATE/READ/LISTING apiServiceInternal:CREATE/READ/LISTING apiProcess:CREATE/READ/LISTING apiTranslink:CREATE/READ/LISTING apiDeveloperPortal:CREATE/READ/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["installation"],
    summary="",
...
async def list_all_installation(
    ...
    if not BaseInstallationApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseInstallationApi.subclasses[0]().list_all_installation(user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/installation/{itemId}",
    responses={
        200: {"model": InstallationRead, "description": "Installation is used to tell the server about the public key of your key pair. The server uses this key to verify your subsequent calls, which need to be signed with your own private key. Additionally, you can use the token you get from an Installation to authenticate the registration of a new device. apiAdmin:CREATE/READ/LISTING apiApp:CREATE/READ/LISTING apiPublic:CREATE/READ/LISTING apiServiceInternal:CREATE/READ/LISTING apiProcess:CREATE/READ/LISTING apiTranslink:CREATE/READ/LISTING apiDeveloperPortal:CREATE/READ/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["installation"],
    summary="",
...
async def r_ead_installation(
    ...
    if not BaseInstallationApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseInstallationApi.subclasses[0]().r_ead_installation(itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
