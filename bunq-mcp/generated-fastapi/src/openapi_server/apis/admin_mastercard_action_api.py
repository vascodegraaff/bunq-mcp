...
from openapi_server.apis.admin_mastercard_action_api_base import BaseAdminMastercardActionApi
...
from openapi_server.models.admin_master_card_action_read import AdminMasterCardActionRead
...
@router.get(
    "/admin-mastercard-action/{itemId}",
    responses={
        200: {"model": AdminMasterCardActionRead, "description": "MasterCard action view, used only by the Admin API. apiAdmin:READ apiServiceInternal:READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["admin-mastercard-action"],
    summary="",
    response_model_by_alias=True,
)
async def r_ead_admin_mastercard_action(
    ...
    """MasterCard action view, used only by the Admin API. apiAdmin:READ apiServiceInternal:READ"""
    if not BaseAdminMastercardActionApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAdminMastercardActionApi.subclasses[0]().r_ead_admin_mastercard_action(itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
