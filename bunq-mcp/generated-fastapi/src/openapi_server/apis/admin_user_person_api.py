...
from openapi_server.apis.admin_user_person_api_base import BaseAdminUserPersonApi
...
from openapi_server.models.admin_user_person_read import AdminUserPersonRead
...
@router.get(
    "/admin-user-person/{itemId}",
    responses={
        200: {"model": AdminUserPersonRead, "description": "Admin-only endpoint to manage person users. apiAdmin:READ apiServiceInternal:READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["admin-user-person"],
    summary="",
    response_model_by_alias=True,
)
async def r_ead_admin_user_person(
    ...
    """Admin-only endpoint to manage person users. apiAdmin:READ apiServiceInternal:READ"""
    if not BaseAdminUserPersonApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAdminUserPersonApi.subclasses[0]().r_ead_admin_user_person(itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
