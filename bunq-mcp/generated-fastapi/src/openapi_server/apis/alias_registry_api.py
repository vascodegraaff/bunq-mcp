...
from openapi_server.apis.alias_registry_api_base import BaseAliasRegistryApi
...
from openapi_server.models.alias_registry import AliasRegistry
from openapi_server.models.alias_registry_create import AliasRegistryCreate
from openapi_server.models.alias_registry_listing import AliasRegistryListing
from openapi_server.models.alias_registry_read import AliasRegistryRead
from openapi_server.models.alias_registry_update import AliasRegistryUpdate
...
@router.post(
    ...
    responses={
        200: {"model": AliasRegistryCreate, "description": "Used to register an alias for a given user for use with Registries (Tricount app, or the group expenses). Initially introduced to be able to support registration of external IBANs for Registry settlements. apiAdmin:CREATE/READ/LISTING/UPDATE apiApp:CREATE/READ/LISTING/UPDATE apiServiceInternal:CREATE/READ/LISTING/UPDATE"},
...
async def c_reate_alias_registry_for_user(
    ...
    alias_registry: AliasRegistry = Body(None, description=""),
    ...
    """Used to register an alias for a given user for use with Registries (Tricount app, or the group expenses). Initially introduced to be able to support registration of external IBANs for Registry settlements. apiAdmin:CREATE/READ/LISTING/UPDATE apiApp:CREATE/READ/LISTING/UPDATE apiServiceInternal:CREATE/READ/LISTING/UPDATE"""
    if not BaseAliasRegistryApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAliasRegistryApi.subclasses[0]().c_reate_alias_registry_for_user(userID, user_agent, x_bunq_client_authentication, alias_registry, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/alias-registry",
    responses={
        200: {"model": List[AliasRegistryListing], "description": "Used to register an alias for a given user for use with Registries (Tricount app, or the group expenses). Initially introduced to be able to support registration of external IBANs for Registry settlements. apiAdmin:CREATE/READ/LISTING/UPDATE apiApp:CREATE/READ/LISTING/UPDATE apiServiceInternal:CREATE/READ/LISTING/UPDATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["alias-registry"],
    summary="",
...
async def list_all_alias_registry_for_user(
    userID: StrictInt = Path(..., description=""),
    user_agent: Annotated[StrictStr, Field(description="The User-Agent header field should contain information about the user agent originating the request. There are no restrictions on the value of this header.")] = Header(None, description="The User-Agent header field should contain information about the user agent originating the request. There are no restrictions on the value of this header."),
    x_bunq_client_authentication: Annotated[StrictStr, Field(description="The authentication token is used to authenticate the source of the API call. It is required by all API calls except for POST /v1/installation. It is important to note that the device and session calls are using the token from the response of the installation call, while all the other calls use the token from the response of the session-server call")] = Header(None, description="The authentication token is used to authenticate the source of the API call. It is required by all API calls except for POST /v1/installation. It is important to note that the device and session calls are using the token from the response of the installation call, while all the other calls use the token from the response of the session-server call"),
    cache_control: Annotated[Optional[StrictStr], Field(description="The standard HTTP Cache-Control header is required for all signed requests.")] = Header(None, description="The standard HTTP Cache-Control header is required for all signed requests."),
    ...
    if not BaseAliasRegistryApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAliasRegistryApi.subclasses[0]().list_all_alias_registry_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/alias-registry/{itemId}",
    responses={
        200: {"model": AliasRegistryRead, "description": "Used to register an alias for a given user for use with Registries (Tricount app, or the group expenses). Initially introduced to be able to support registration of external IBANs for Registry settlements. apiAdmin:CREATE/READ/LISTING/UPDATE apiApp:CREATE/READ/LISTING/UPDATE apiServiceInternal:CREATE/READ/LISTING/UPDATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["alias-registry"],
    summary="",
...
async def r_ead_alias_registry_for_user(
    userID: StrictInt = Path(..., description=""),
    ...
    if not BaseAliasRegistryApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAliasRegistryApi.subclasses[0]().r_ead_alias_registry_for_user(userID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.put(
    "/user/{userID}/alias-registry/{itemId}",
    responses={
        200: {"model": AliasRegistryUpdate, "description": "Used to register an alias for a given user for use with Registries (Tricount app, or the group expenses). Initially introduced to be able to support registration of external IBANs for Registry settlements. apiAdmin:CREATE/READ/LISTING/UPDATE apiApp:CREATE/READ/LISTING/UPDATE apiServiceInternal:CREATE/READ/LISTING/UPDATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["alias-registry"],
    summary="",
...
async def u_pdate_alias_registry_for_user(
    userID: StrictInt = Path(..., description=""),
    itemId: StrictInt = Path(..., description=""),
    user_agent: Annotated[StrictStr, Field(description="The User-Agent header field should contain information about the user agent originating the request. There are no restrictions on the value of this header.")] = Header(None, description="The User-Agent header field should contain information about the user agent originating the request. There are no restrictions on the value of this header."),
    x_bunq_client_authentication: Annotated[StrictStr, Field(description="The authentication token is used to authenticate the source of the API call. It is required by all API calls except for POST /v1/installation. It is important to note that the device and session calls are using the token from the response of the installation call, while all the other calls use the token from the response of the session-server call")] = Header(None, description="The authentication token is used to authenticate the source of the API call. It is required by all API calls except for POST /v1/installation. It is important to note that the device and session calls are using the token from the response of the installation call, while all the other calls use the token from the response of the session-server call"),
    ...
    if not BaseAliasRegistryApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAliasRegistryApi.subclasses[0]().u_pdate_alias_registry_for_user(userID, itemId, user_agent, x_bunq_client_authentication, alias_registry, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
