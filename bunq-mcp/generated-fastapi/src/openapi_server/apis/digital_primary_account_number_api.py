...
from openapi_server.apis.digital_primary_account_number_api_base import BaseDigitalPrimaryAccountNumberApi
...
from openapi_server.models.card_digital_primary_account_number_listing import CardDigitalPrimaryAccountNumberListing
...
@router.get(
    "/user/{userID}/card/{cardID}/digital-primary-account-number",
    responses={
        200: {"model": List[CardDigitalPrimaryAccountNumberListing], "description": "View for getting the dPAN of the card. apiTranslink:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["digital-primary-account-number"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_digital_primary_account_number_for_user_card(
...
) -> List[CardDigitalPrimaryAccountNumberListing]:
    """View for getting the dPAN of the card. apiTranslink:LISTING"""
    if not BaseDigitalPrimaryAccountNumberApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseDigitalPrimaryAccountNumberApi.subclasses[0]().list_all_digital_primary_account_number_for_user_card(userID, cardID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
