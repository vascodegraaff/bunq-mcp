...
from openapi_server.apis.statement_api_base import BaseStatementApi
...
from openapi_server.models.export_statement_payment_create import ExportStatementPaymentCreate
from openapi_server.models.export_statement_payment_read import ExportStatementPaymentRead
...
@router.post(
    ...
    responses={
        200: {"model": ExportStatementPaymentCreate, "description": "Used to create a statement export of a single payment. apiAdmin:CREATE/READ apiApp:CREATE/READ apiPublic:CREATE/READ apiServiceInternal:CREATE/READ"},
...
async def c_reate_statement_for_user_monetary_account_event(
    userID: StrictInt = Path(..., description=""),
    monetary-accountID: StrictInt = Path(..., description=""),
    eventID: StrictInt = Path(..., description=""),
    ...
    """Used to create a statement export of a single payment. apiAdmin:CREATE/READ apiApp:CREATE/READ apiPublic:CREATE/READ apiServiceInternal:CREATE/READ"""
    if not BaseStatementApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseStatementApi.subclasses[0]().c_reate_statement_for_user_monetary_account_event(userID, monetary-accountID, eventID, user_agent, x_bunq_client_authentication, body, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/monetary-account/{monetary-accountID}/event/{eventID}/statement/{itemId}",
    responses={
        200: {"model": ExportStatementPaymentRead, "description": "Used to create a statement export of a single payment. apiAdmin:CREATE/READ apiApp:CREATE/READ apiPublic:CREATE/READ apiServiceInternal:CREATE/READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["statement"],
    summary="",
...
async def r_ead_statement_for_user_monetary_account_event(
    userID: StrictInt = Path(..., description=""),
    monetary-accountID: StrictInt = Path(..., description=""),
    eventID: StrictInt = Path(..., description=""),
    ...
    if not BaseStatementApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseStatementApi.subclasses[0]().r_ead_statement_for_user_monetary_account_event(userID, monetary-accountID, eventID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
