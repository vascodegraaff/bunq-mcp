...
from openapi_server.apis.point_mutation_entry_api_base import BasePointMutationEntryApi
...
from openapi_server.models.point_mutation_entry_listing import PointMutationEntryListing
...
@router.get(
    "/user/{userID}/point-mutation-entry",
    responses={
        200: {"model": List[PointMutationEntryListing], "description": "The listing endpoint for point mutations. apiAdmin:LISTING apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["point-mutation-entry"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_point_mutation_entry_for_user(
...
) -> List[PointMutationEntryListing]:
    """The listing endpoint for point mutations. apiAdmin:LISTING apiApp:LISTING"""
    if not BasePointMutationEntryApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BasePointMutationEntryApi.subclasses[0]().list_all_point_mutation_entry_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
