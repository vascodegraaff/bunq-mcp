...
from openapi_server.apis.support_employee_on_shift_api_base import BaseSupportEmployeeOnShiftApi
...
from openapi_server.models.support_employee_on_shift_listing import SupportEmployeeOnShiftListing
...
@router.get(
    "/user/{userID}/support-employee-on-shift",
    responses={
        200: {"model": List[SupportEmployeeOnShiftListing], "description": "Gets the Support Employees currently on shift. apiAdmin:LISTING apiServiceInternal:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["support-employee-on-shift"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_support_employee_on_shift_for_user(
...
) -> List[SupportEmployeeOnShiftListing]:
    """Gets the Support Employees currently on shift. apiAdmin:LISTING apiServiceInternal:LISTING"""
    if not BaseSupportEmployeeOnShiftApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseSupportEmployeeOnShiftApi.subclasses[0]().list_all_support_employee_on_shift_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
