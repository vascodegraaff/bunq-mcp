...
from openapi_server.apis.insight_preference_monetary_account_api_base import BaseInsightPreferenceMonetaryAccountApi
...
from openapi_server.models.insight_preference_monetary_account_listing import InsightPreferenceMonetaryAccountListing
...
@router.get(
    "/user/{userID}/insight-preference-monetary-account",
    responses={
        200: {"model": List[InsightPreferenceMonetaryAccountListing], "description": "Used to allow users to set insight/budget preferences. apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["insight-preference-monetary-account"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_insight_preference_monetary_account_for_user(
...
) -> List[InsightPreferenceMonetaryAccountListing]:
    """Used to allow users to set insight/budget preferences. apiApp:LISTING"""
    if not BaseInsightPreferenceMonetaryAccountApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseInsightPreferenceMonetaryAccountApi.subclasses[0]().list_all_insight_preference_monetary_account_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
