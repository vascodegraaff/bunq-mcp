...
from openapi_server.apis.bizum_prizum_return_debit_credit_api_base import BaseBizumPrizumReturnDebitCreditApi
...
from openapi_server.models.bizum_prizum_return_debit_credit_create import BizumPrizumReturnDebitCreditCreate
...
@router.post(
    ...
    responses={
        200: {"model": BizumPrizumReturnDebitCreditCreate, "description": "Endpoint for receiving returns of Bizum transfers between two bunq users via the Prizum protocol. apiBizum:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["bizum-prizum-return-debit-credit"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_bizum_prizum_return_debit_credit(
...
) -> BizumPrizumReturnDebitCreditCreate:
    """Endpoint for receiving returns of Bizum transfers between two bunq users via the Prizum protocol. apiBizum:CREATE"""
    if not BaseBizumPrizumReturnDebitCreditApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseBizumPrizumReturnDebitCreditApi.subclasses[0]().c_reate_bizum_prizum_return_debit_credit(user_agent, x_bunq_client_authentication, body, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
