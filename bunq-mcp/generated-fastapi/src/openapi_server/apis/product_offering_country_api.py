...
from openapi_server.apis.product_offering_country_api_base import BaseProductOfferingCountryApi
...
from openapi_server.models.sim_virtual_product_offering_country_listing import SimVirtualProductOfferingCountryListing
...
@router.get(
    "/user/{userID}/sim-virtual/{sim-virtualID}/product-offering-country",
    responses={
        200: {"model": List[SimVirtualProductOfferingCountryListing], "description": "Endpoint to retrieve and list all countries in which at least one active eSIM product offering is available. apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["product-offering-country"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_product_offering_country_for_user_sim_virtual(
...
) -> List[SimVirtualProductOfferingCountryListing]:
    """Endpoint to retrieve and list all countries in which at least one active eSIM product offering is available. apiApp:LISTING"""
    if not BaseProductOfferingCountryApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseProductOfferingCountryApi.subclasses[0]().list_all_product_offering_country_for_user_sim_virtual(userID, sim-virtualID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
