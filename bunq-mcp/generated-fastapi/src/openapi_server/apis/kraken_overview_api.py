...
from openapi_server.apis.kraken_overview_api_base import BaseKrakenOverviewApi
...
from openapi_server.models.kraken_overview_listing import KrakenOverviewListing
...
@router.get(
    "/user/{userID}/kraken-overview",
    responses={
        200: {"model": List[KrakenOverviewListing], "description": "Kraken overview. apiAdmin:LISTING apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["kraken-overview"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_kraken_overview_for_user(
...
) -> List[KrakenOverviewListing]:
    """Kraken overview. apiAdmin:LISTING apiApp:LISTING"""
    if not BaseKrakenOverviewApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseKrakenOverviewApi.subclasses[0]().list_all_kraken_overview_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
