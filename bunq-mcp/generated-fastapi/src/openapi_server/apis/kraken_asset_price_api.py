...
from openapi_server.apis.kraken_asset_price_api_base import BaseKrakenAssetPriceApi
...
from openapi_server.models.kraken_asset_price_read import KrakenAssetPriceRead
...
@router.get(
    "/user/{userID}/kraken-asset-price/{itemId}",
    responses={
        200: {"model": KrakenAssetPriceRead, "description": "Endpoint for listing price history of an asset from Kraken apiApp:READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["kraken-asset-price"],
    summary="",
    response_model_by_alias=True,
)
async def r_ead_kraken_asset_price_for_user(
...
) -> KrakenAssetPriceRead:
    """Endpoint for listing price history of an asset from Kraken apiApp:READ"""
    if not BaseKrakenAssetPriceApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseKrakenAssetPriceApi.subclasses[0]().r_ead_kraken_asset_price_for_user(userID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
