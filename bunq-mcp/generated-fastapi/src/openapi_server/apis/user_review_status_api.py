...
from openapi_server.apis.user_review_status_api_base import BaseUserReviewStatusApi
...
from openapi_server.models.admin_user_review_status_listing import AdminUserReviewStatusListing
...
@router.get(
    "/user-review-status",
    responses={
        200: {"model": List[AdminUserReviewStatusListing], "description": "Gets the user review status. apiAdmin:LISTING apiServiceInternal:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["user-review-status"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_user_review_status(
...
) -> List[AdminUserReviewStatusListing]:
    """Gets the user review status. apiAdmin:LISTING apiServiceInternal:LISTING"""
    if not BaseUserReviewStatusApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseUserReviewStatusApi.subclasses[0]().list_all_user_review_status(user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
