...
from openapi_server.apis.wheel_of_fortune_prize_available_api_base import BaseWheelOfFortunePrizeAvailableApi
...
from openapi_server.models.wheel_of_fortune_prize_available_listing import WheelOfFortunePrizeAvailableListing
...
@router.get(
    "/user/{userID}/wheel-of-fortune-prize-available",
    responses={
        200: {"model": List[WheelOfFortunePrizeAvailableListing], "description": "Get available prizes in the Wheel of Fortune. apiAdmin:LISTING apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["wheel-of-fortune-prize-available"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_wheel_of_fortune_prize_available_for_user(
...
) -> List[WheelOfFortunePrizeAvailableListing]:
    """Get available prizes in the Wheel of Fortune. apiAdmin:LISTING apiApp:LISTING"""
    if not BaseWheelOfFortunePrizeAvailableApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseWheelOfFortunePrizeAvailableApi.subclasses[0]().list_all_wheel_of_fortune_prize_available_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
