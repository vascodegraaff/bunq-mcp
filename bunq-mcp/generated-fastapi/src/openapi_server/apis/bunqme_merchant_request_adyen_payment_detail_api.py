...
from openapi_server.apis.bunqme_merchant_request_adyen_payment_detail_api_base import BaseBunqmeMerchantRequestAdyenPaymentDetailApi
...
from openapi_server.models.bunq_me_merchant_request_adyen_payment_detail import BunqMeMerchantRequestAdyenPaymentDetail
from openapi_server.models.bunq_me_merchant_request_adyen_payment_detail_update import BunqMeMerchantRequestAdyenPaymentDetailUpdate
...
@router.put(
    "/bunqme-merchant-request-adyen-payment-detail/{itemId}",
    responses={
        200: {"model": BunqMeMerchantRequestAdyenPaymentDetailUpdate, "description": "View to pass an payment details after redirect, in which case the associated transaction status will be polled nearly immediately. apiBunqMe:UPDATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["bunqme-merchant-request-adyen-payment-detail"],
    summary="",
    response_model_by_alias=True,
)
async def u_pdate_bunqme_merchant_request_adyen_payment_detail(
    ...
    bunq_me_merchant_request_adyen_payment_detail: BunqMeMerchantRequestAdyenPaymentDetail = Body(None, description=""),
...
) -> BunqMeMerchantRequestAdyenPaymentDetailUpdate:
    """View to pass an payment details after redirect, in which case the associated transaction status will be polled nearly immediately. apiBunqMe:UPDATE"""
    if not BaseBunqmeMerchantRequestAdyenPaymentDetailApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseBunqmeMerchantRequestAdyenPaymentDetailApi.subclasses[0]().u_pdate_bunqme_merchant_request_adyen_payment_detail(itemId, user_agent, x_bunq_client_authentication, bunq_me_merchant_request_adyen_payment_detail, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
