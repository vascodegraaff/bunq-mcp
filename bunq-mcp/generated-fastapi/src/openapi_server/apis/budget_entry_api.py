...
from openapi_server.apis.budget_entry_api_base import BaseBudgetEntryApi
...
from openapi_server.models.budget_entry_listing import BudgetEntryListing
...
@router.get(
    "/user/{userID}/budget-entry",
    responses={
        200: {"model": List[BudgetEntryListing], "description": "The endpoint to list BudgetEntries apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING"},
...
async def list_all_budget_entry_for_user(
...
) -> List[BudgetEntryListing]:
    """The endpoint to list BudgetEntries apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING"""
    if not BaseBudgetEntryApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseBudgetEntryApi.subclasses[0]().list_all_budget_entry_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
