...
from openapi_server.apis.freeze_api_base import BaseFreezeApi
...
from openapi_server.models.freeze import Freeze
from openapi_server.models.freeze_create import FreezeCreate
from openapi_server.models.freeze_listing import FreezeListing
from openapi_server.models.freeze_read import FreezeRead
...
@router.post(
    "/user/{userID}/freeze",
    responses={
        200: {"model": FreezeCreate, "description": "Used to freeze a user and their associated monetary accounts and cards. apiAdmin:CREATE/READ/LISTING apiApp:CREATE apiServiceInternal:CREATE/READ/LISTING"},
...
async def c_reate_freeze_for_user(
    ...
    freeze: Freeze = Body(None, description=""),
    ...
    """Used to freeze a user and their associated monetary accounts and cards. apiAdmin:CREATE/READ/LISTING apiApp:CREATE apiServiceInternal:CREATE/READ/LISTING"""
    if not BaseFreezeApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseFreezeApi.subclasses[0]().c_reate_freeze_for_user(userID, user_agent, x_bunq_client_authentication, freeze, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/freeze",
    responses={
        200: {"model": List[FreezeListing], "description": "Used to freeze a user and their associated monetary accounts and cards. apiAdmin:CREATE/READ/LISTING apiApp:CREATE apiServiceInternal:CREATE/READ/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["freeze"],
    summary="",
...
async def list_all_freeze_for_user(
    userID: StrictInt = Path(..., description=""),
    user_agent: Annotated[StrictStr, Field(description="The User-Agent header field should contain information about the user agent originating the request. There are no restrictions on the value of this header.")] = Header(None, description="The User-Agent header field should contain information about the user agent originating the request. There are no restrictions on the value of this header."),
    x_bunq_client_authentication: Annotated[StrictStr, Field(description="The authentication token is used to authenticate the source of the API call. It is required by all API calls except for POST /v1/installation. It is important to note that the device and session calls are using the token from the response of the installation call, while all the other calls use the token from the response of the session-server call")] = Header(None, description="The authentication token is used to authenticate the source of the API call. It is required by all API calls except for POST /v1/installation. It is important to note that the device and session calls are using the token from the response of the installation call, while all the other calls use the token from the response of the session-server call"),
    cache_control: Annotated[Optional[StrictStr], Field(description="The standard HTTP Cache-Control header is required for all signed requests.")] = Header(None, description="The standard HTTP Cache-Control header is required for all signed requests."),
    ...
    if not BaseFreezeApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseFreezeApi.subclasses[0]().list_all_freeze_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/freeze/{itemId}",
    responses={
        200: {"model": FreezeRead, "description": "Used to freeze a user and their associated monetary accounts and cards. apiAdmin:CREATE/READ/LISTING apiApp:CREATE apiServiceInternal:CREATE/READ/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["freeze"],
    summary="",
...
async def r_ead_freeze_for_user(
    userID: StrictInt = Path(..., description=""),
    ...
    if not BaseFreezeApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseFreezeApi.subclasses[0]().r_ead_freeze_for_user(userID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
