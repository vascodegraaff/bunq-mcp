...
from openapi_server.apis.user_occupation_category_api_base import BaseUserOccupationCategoryApi
...
from openapi_server.models.user_occupation_category_listing import UserOccupationCategoryListing
...
@router.get(
    ...
    responses={
        200: {"model": List[UserOccupationCategoryListing], "description": "Used to list occupation categories. apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["user-occupation-category"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_user_occupation_category(
...
) -> List[UserOccupationCategoryListing]:
    """Used to list occupation categories. apiApp:LISTING"""
    if not BaseUserOccupationCategoryApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseUserOccupationCategoryApi.subclasses[0]().list_all_user_occupation_category(user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
