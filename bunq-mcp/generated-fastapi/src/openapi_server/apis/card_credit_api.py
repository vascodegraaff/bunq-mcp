...
from openapi_server.apis.card_credit_api_base import BaseCardCreditApi
...
from openapi_server.models.card_credit import CardCredit
from openapi_server.models.card_credit_create import CardCreditCreate
...
@router.post(
    ...
    responses={
        200: {"model": CardCreditCreate, "description": "With bunq it is possible to order credit cards that can then be connected with each one of the monetary accounts the user has access to (including connected accounts). apiApp:CREATE apiPublic:CREATE"},
...
async def c_reate_card_credit_for_user(
    ...
    card_credit: CardCredit = Body(None, description=""),
    ...
    if not BaseCardCreditApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseCardCreditApi.subclasses[0]().c_reate_card_credit_for_user(userID, user_agent, x_bunq_client_authentication, card_credit, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
