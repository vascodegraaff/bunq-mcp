...
from openapi_server.apis.marketing_communication_api_base import BaseMarketingCommunicationApi
...
from openapi_server.models.marketing_communication import MarketingCommunication
from openapi_server.models.marketing_communication_create import MarketingCommunicationCreate
from openapi_server.models.marketing_communication_update import MarketingCommunicationUpdate
...
@router.post(
    "/user/{userID}/marketing-communication",
    responses={
        200: {"model": MarketingCommunicationCreate, "description": "Endpoint for creating a custom event and notification. apiBird:CREATE/UPDATE"},
...
async def c_reate_marketing_communication_for_user(
    ...
    marketing_communication: MarketingCommunication = Body(None, description=""),
...
) -> MarketingCommunicationCreate:
    """Endpoint for creating a custom event and notification. apiBird:CREATE/UPDATE"""
    if not BaseMarketingCommunicationApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseMarketingCommunicationApi.subclasses[0]().c_reate_marketing_communication_for_user(userID, user_agent, x_bunq_client_authentication, marketing_communication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.put(
    "/user/{userID}/marketing-communication/{itemId}",
    responses={
        200: {"model": MarketingCommunicationUpdate, "description": "Endpoint for creating a custom event and notification. apiBird:CREATE/UPDATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["marketing-communication"],
    summary="",
...
async def u_pdate_marketing_communication_for_user(
    userID: StrictInt = Path(..., description=""),
    ...
    x_bunq_client_request_id: Annotated[Optional[StrictStr], Field(description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer.")] = Header(None, description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer."),
    x_bunq_geolocation: Annotated[Optional[StrictStr], Field(description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued.")] = Header(None, description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued."),
) -> MarketingCommunicationUpdate:
    """Endpoint for creating a custom event and notification. apiBird:CREATE/UPDATE"""
    if not BaseMarketingCommunicationApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseMarketingCommunicationApi.subclasses[0]().u_pdate_marketing_communication_for_user(userID, itemId, user_agent, x_bunq_client_authentication, marketing_communication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
