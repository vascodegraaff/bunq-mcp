...
from openapi_server.apis.carbon_footprint_progress_api_base import BaseCarbonFootprintProgressApi
...
from openapi_server.models.carbon_offset_progress_listing import CarbonOffsetProgressListing
...
@router.get(
    "/user/{userID}/carbon-footprint-progress",
    responses={
        200: {"model": List[CarbonOffsetProgressListing], "description": "Get the current carbon offset of the user. apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["carbon-footprint-progress"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_carbon_footprint_progress_for_user(
...
) -> List[CarbonOffsetProgressListing]:
    """Get the current carbon offset of the user. apiApp:LISTING"""
    if not BaseCarbonFootprintProgressApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseCarbonFootprintProgressApi.subclasses[0]().list_all_carbon_footprint_progress_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
