...
from openapi_server.apis.incident_api_base import BaseIncidentApi
...
from openapi_server.models.incident_listing import IncidentListing
from openapi_server.models.incident_read import IncidentRead
...
@router.get(
    "/incident",
    responses={
        200: {"model": List[IncidentListing], "description": "Check for incidents from the apps. apiApp:LISTING/READ"},
...
) -> List[IncidentListing]:
    """Check for incidents from the apps. apiApp:LISTING/READ"""
    if not BaseIncidentApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseIncidentApi.subclasses[0]().list_all_incident(user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/incident/{itemId}",
    responses={
        200: {"model": IncidentRead, "description": "Check for incidents from the apps. apiApp:LISTING/READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["incident"],
    summary="",
...
async def r_ead_incident(
    ...
    if not BaseIncidentApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseIncidentApi.subclasses[0]().r_ead_incident(itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
