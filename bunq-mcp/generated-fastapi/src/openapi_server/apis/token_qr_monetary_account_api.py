...
from openapi_server.apis.token_qr_monetary_account_api_base import BaseTokenQrMonetaryAccountApi
...
@router.get(
    "/user/{userID}/monetary-account/{monetary-accountID}/token-qr-monetary-account",
    responses={
        200: {"model": List[object], "description": "Fetch the QR code in a printable PDF for the monetary account. The raw content is the base64, without any JSON wrapping. apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["token-qr-monetary-account"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_token_qr_monetary_account_for_user_monetary_account(
    ...
    """Fetch the QR code in a printable PDF for the monetary account. The raw content is the base64, without any JSON wrapping. apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING"""
    if not BaseTokenQrMonetaryAccountApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseTokenQrMonetaryAccountApi.subclasses[0]().list_all_token_qr_monetary_account_for_user_monetary_account(userID, monetary-accountID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
