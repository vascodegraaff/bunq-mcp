...
from openapi_server.apis.admin_assistant_conversation_escalation_expertise_status_api_base import BaseAdminAssistantConversationEscalationExpertiseStatusApi
...
from openapi_server.models.admin_assistant_conversation_escalation_expertise_status_listing import AdminAssistantConversationEscalationExpertiseStatusListing
...
@router.get(
    ...
    responses={
        200: {"model": List[AdminAssistantConversationEscalationExpertiseStatusListing], "description": "This call is used to count all expertise with active admin. apiAdmin:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["admin-assistant-conversation-escalation-expertise-status"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_admin_assistant_conversation_escalation_expertise_status(
...
) -> List[AdminAssistantConversationEscalationExpertiseStatusListing]:
    """This call is used to count all expertise with active admin. apiAdmin:LISTING"""
    if not BaseAdminAssistantConversationEscalationExpertiseStatusApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAdminAssistantConversationEscalationExpertiseStatusApi.subclasses[0]().list_all_admin_assistant_conversation_escalation_expertise_status(user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
