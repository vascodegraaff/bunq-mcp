...
from openapi_server.apis.sdd_debit_incoming_api_base import BaseSddDebitIncomingApi
...
from openapi_server.models.sdd_debit_incoming_read import SddDebitIncomingRead
...
@router.get(
    "/sdd-debit-incoming/{itemId}",
    responses={
        200: {"model": SddDebitIncomingRead, "description": "Endpoint used by the search API only to return a representation of SddDebitIncomingModel. This is then shown in the Admin. apiAdmin:READ apiServiceInternal:READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["sdd-debit-incoming"],
    summary="",
    response_model_by_alias=True,
)
async def r_ead_sdd_debit_incoming(
    ...
    """Endpoint used by the search API only to return a representation of SddDebitIncomingModel. This is then shown in the Admin. apiAdmin:READ apiServiceInternal:READ"""
    if not BaseSddDebitIncomingApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseSddDebitIncomingApi.subclasses[0]().r_ead_sdd_debit_incoming(itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
