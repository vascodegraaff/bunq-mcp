...
from openapi_server.apis.insights_merchant_api_base import BaseInsightsMerchantApi
...
from openapi_server.models.insight_merchant_listing import InsightMerchantListing
...
@router.get(
    "/user/{userID}/insights-merchant",
    responses={
        200: {"model": List[InsightMerchantListing], "description": "Used to get events based on time and insight category. apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["insights-merchant"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_insights_merchant_for_user(
...
) -> List[InsightMerchantListing]:
    """Used to get events based on time and insight category. apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING"""
    if not BaseInsightsMerchantApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseInsightsMerchantApi.subclasses[0]().list_all_insights_merchant_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
