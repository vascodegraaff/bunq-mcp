...
from openapi_server.apis.is_user_company_api_base import BaseIsUserCompanyApi
...
from openapi_server.models.is_user_company_listing import IsUserCompanyListing
...
@router.get(
    "/is-user-company",
    responses={
        200: {"model": List[IsUserCompanyListing], "description": "Endpoint to check existence of users company with the same CoC number. apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["is-user-company"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_is_user_company(
...
) -> List[IsUserCompanyListing]:
    """Endpoint to check existence of users company with the same CoC number. apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING"""
    if not BaseIsUserCompanyApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseIsUserCompanyApi.subclasses[0]().list_all_is_user_company(user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
