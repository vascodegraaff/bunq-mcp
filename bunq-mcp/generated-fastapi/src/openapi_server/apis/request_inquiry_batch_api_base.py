...
from openapi_server.models.request_inquiry_batch import RequestInquiryBatch
from openapi_server.models.request_inquiry_batch_create import RequestInquiryBatchCreate
from openapi_server.models.request_inquiry_batch_listing import RequestInquiryBatchListing
from openapi_server.models.request_inquiry_batch_read import RequestInquiryBatchRead
from openapi_server.models.request_inquiry_batch_update import RequestInquiryBatchUpdate


class BaseRequestInquiryBatchApi:
    subclasses: ClassVar[Tuple] = ()

    def __init_subclass__(cls, **kwargs):
        super().__init_subclass__(**kwargs)
        BaseRequestInquiryBatchApi.subclasses = BaseRequestInquiryBatchApi.subclasses + (cls,)
    async def c_reate_request_inquiry_batch_for_user_monetary_account(
        self,
        userID: StrictInt,
        ...
        request_inquiry_batch: RequestInquiryBatch,
    ...
    async def list_all_request_inquiry_batch_for_user_monetary_account(
        ...
        x_bunq_client_request_id: Annotated[Optional[StrictStr], Field(description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer.")],
        x_bunq_geolocation: Annotated[Optional[StrictStr], Field(description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued.")],
    ) -> List[RequestInquiryBatchListing]:
        """Return all the request batches for a monetary account."""
        ...


    async def r_ead_request_inquiry_batch_for_user_monetary_account(
        self,
        userID: StrictInt,
        monetary-accountID: StrictInt,
        itemId: StrictInt,
    ...
    async def u_pdate_request_inquiry_batch_for_user_monetary_account(
        self,
        userID: StrictInt,
        monetary-accountID: StrictInt,
        itemId: StrictInt,
