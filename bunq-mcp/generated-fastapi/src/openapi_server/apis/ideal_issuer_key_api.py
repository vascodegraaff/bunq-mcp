...
from openapi_server.apis.ideal_issuer_key_api_base import BaseIdealIssuerKeyApi
...
from openapi_server.models.ideal_issuer_key_listing import IdealIssuerKeyListing
...
@router.get(
    "/ideal-issuer-key",
    responses={
        200: {"model": List[IdealIssuerKeyListing], "description": "Output the available bunq iDEAL issuer keys in JSON Web Key Set format. apiIdealIssuerKey:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["ideal-issuer-key"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_ideal_issuer_key(
...
) -> List[IdealIssuerKeyListing]:
    """Output the available bunq iDEAL issuer keys in JSON Web Key Set format. apiIdealIssuerKey:LISTING"""
    if not BaseIdealIssuerKeyApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseIdealIssuerKeyApi.subclasses[0]().list_all_ideal_issuer_key(user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
