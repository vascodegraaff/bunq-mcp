...
from openapi_server.apis.scan_receipt_event_api_base import BaseScanReceiptEventApi
...
from openapi_server.models.scan_receipt_event_listing import ScanReceiptEventListing
...
@router.get(
    "/user/{userID}/scan-receipt-event",
    responses={
        200: {"model": List[ScanReceiptEventListing], "description": "Used to list event history for all the cards related to a user. apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["scan-receipt-event"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_scan_receipt_event_for_user(
...
) -> List[ScanReceiptEventListing]:
    """Used to list event history for all the cards related to a user. apiApp:LISTING"""
    if not BaseScanReceiptEventApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseScanReceiptEventApi.subclasses[0]().list_all_scan_receipt_event_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
