...
from openapi_server.apis.wsnot_transitoria_api_base import BaseWsnotTransitoriaApi
...
from openapi_server.models.bizum_notification_transient_create import BizumNotificationTransientCreate
...
@router.post(
    "/wsnot-transitoria",
    responses={
        200: {"model": BizumNotificationTransientCreate, "description": "Endpoint for receiving Bizum notifications. apiBizum:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["wsnot-transitoria"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_wsnot_transitoria(
...
) -> BizumNotificationTransientCreate:
    """Endpoint for receiving Bizum notifications. apiBizum:CREATE"""
    if not BaseWsnotTransitoriaApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseWsnotTransitoriaApi.subclasses[0]().c_reate_wsnot_transitoria(user_agent, x_bunq_client_authentication, body, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
