...
from openapi_server.apis.registry_synchronization_api_base import BaseRegistrySynchronizationApi
...
from openapi_server.models.registry_synchronization import RegistrySynchronization
from openapi_server.models.registry_synchronization_create import RegistrySynchronizationCreate
...
@router.post(
    "/user/{userID}/registry-synchronization",
    responses={
        200: {"model": RegistrySynchronizationCreate, "description": "This can be used to sync all local changes in the (Tricount) app to the backend in a single API call. Endpoint primarily exists for compatibility purposes. apiApp:CREATE"},
...
async def c_reate_registry_synchronization_for_user(
    ...
    registry_synchronization: RegistrySynchronization = Body(None, description=""),
...
) -> RegistrySynchronizationCreate:
    """This can be used to sync all local changes in the (Tricount) app to the backend in a single API call. Endpoint primarily exists for compatibility purposes. apiApp:CREATE"""
    if not BaseRegistrySynchronizationApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseRegistrySynchronizationApi.subclasses[0]().c_reate_registry_synchronization_for_user(userID, user_agent, x_bunq_client_authentication, registry_synchronization, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
