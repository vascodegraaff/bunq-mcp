...
from openapi_server.apis.admin_monetary_account_external_api_base import BaseAdminMonetaryAccountExternalApi
...
from openapi_server.models.admin_monetary_account_external_listing import AdminMonetaryAccountExternalListing
from openapi_server.models.admin_monetary_account_external_read import AdminMonetaryAccountExternalRead
...
@router.get(
    "/user/{userID}/admin-monetary-account-external",
    responses={
        200: {"model": List[AdminMonetaryAccountExternalListing], "description": "Endpoint for managing MonetaryAccountExternal from the Admin apiAdmin:READ/LISTING apiServiceInternal:READ/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["admin-monetary-account-external"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_admin_monetary_account_external_for_user(
...
) -> List[AdminMonetaryAccountExternalListing]:
    """Endpoint for managing MonetaryAccountExternal from the Admin apiAdmin:READ/LISTING apiServiceInternal:READ/LISTING"""
    if not BaseAdminMonetaryAccountExternalApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAdminMonetaryAccountExternalApi.subclasses[0]().list_all_admin_monetary_account_external_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/admin-monetary-account-external/{itemId}",
    responses={
        200: {"model": AdminMonetaryAccountExternalRead, "description": "Endpoint for managing MonetaryAccountExternal from the Admin apiAdmin:READ/LISTING apiServiceInternal:READ/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["admin-monetary-account-external"],
    summary="",
...
async def r_ead_admin_monetary_account_external_for_user(
    userID: StrictInt = Path(..., description=""),
    ...
    if not BaseAdminMonetaryAccountExternalApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAdminMonetaryAccountExternalApi.subclasses[0]().r_ead_admin_monetary_account_external_for_user(userID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
