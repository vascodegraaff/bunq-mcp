...
from openapi_server.apis.identity_verification_report_api_base import BaseIdentityVerificationReportApi
...
from openapi_server.models.identity_verification_report_read import IdentityVerificationReportRead
...
@router.get(
    "/user/{userID}/identity-verification-report/{itemId}",
    responses={
        200: {"model": IdentityVerificationReportRead, "description": "Endpoint to create and fetch an identity verification report. apiAdmin:READ apiServiceInternal:READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["identity-verification-report"],
    summary="",
    response_model_by_alias=True,
)
async def r_ead_identity_verification_report_for_user(
...
) -> IdentityVerificationReportRead:
    """Endpoint to create and fetch an identity verification report. apiAdmin:READ apiServiceInternal:READ"""
    if not BaseIdentityVerificationReportApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseIdentityVerificationReportApi.subclasses[0]().r_ead_identity_verification_report_for_user(userID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
