...
from openapi_server.apis.insight_preference_date_api_base import BaseInsightPreferenceDateApi
...
from openapi_server.models.insight_preference_date import InsightPreferenceDate
from openapi_server.models.insight_preference_date_create import InsightPreferenceDateCreate
from openapi_server.models.insight_preference_date_listing import InsightPreferenceDateListing
...
@router.post(
    ...
    responses={
        200: {"model": InsightPreferenceDateCreate, "description": "Used to allow users to set insight/budget preferences. apiApp:CREATE/LISTING apiPublic:LISTING"},
...
async def c_reate_insight_preference_date_for_user(
    ...
    insight_preference_date: InsightPreferenceDate = Body(None, description=""),
...
) -> InsightPreferenceDateCreate:
    """Used to allow users to set insight/budget preferences. apiApp:CREATE/LISTING apiPublic:LISTING"""
    if not BaseInsightPreferenceDateApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseInsightPreferenceDateApi.subclasses[0]().c_reate_insight_preference_date_for_user(userID, user_agent, x_bunq_client_authentication, insight_preference_date, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/insight-preference-date",
    responses={
        200: {"model": List[InsightPreferenceDateListing], "description": "Used to allow users to set insight/budget preferences. apiApp:CREATE/LISTING apiPublic:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["insight-preference-date"],
    summary="",
...
async def list_all_insight_preference_date_for_user(
    ...
    x_bunq_client_request_id: Annotated[Optional[StrictStr], Field(description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer.")] = Header(None, description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer."),
    x_bunq_geolocation: Annotated[Optional[StrictStr], Field(description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued.")] = Header(None, description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued."),
) -> List[InsightPreferenceDateListing]:
    """Used to allow users to set insight/budget preferences. apiApp:CREATE/LISTING apiPublic:LISTING"""
    if not BaseInsightPreferenceDateApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseInsightPreferenceDateApi.subclasses[0]().list_all_insight_preference_date_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
