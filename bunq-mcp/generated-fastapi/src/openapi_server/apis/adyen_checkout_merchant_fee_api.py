...
from openapi_server.apis.adyen_checkout_merchant_fee_api_base import BaseAdyenCheckoutMerchantFeeApi
...
from openapi_server.models.adyen_checkout_merchant_transaction_fee import AdyenCheckoutMerchantTransactionFee
from openapi_server.models.adyen_checkout_merchant_transaction_fee_create import AdyenCheckoutMerchantTransactionFeeCreate
...
@router.post(
    "/user/{userID}/adyen-checkout-merchant-fee",
    responses={
        200: {"model": AdyenCheckoutMerchantTransactionFeeCreate, "description": "Used to calculated the fee we will charge for the transaction. apiApp:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["adyen-checkout-merchant-fee"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_adyen_checkout_merchant_fee_for_user(
    ...
    adyen_checkout_merchant_transaction_fee: AdyenCheckoutMerchantTransactionFee = Body(None, description=""),
...
) -> AdyenCheckoutMerchantTransactionFeeCreate:
    """Used to calculated the fee we will charge for the transaction. apiApp:CREATE"""
    if not BaseAdyenCheckoutMerchantFeeApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAdyenCheckoutMerchantFeeApi.subclasses[0]().c_reate_adyen_checkout_merchant_fee_for_user(userID, user_agent, x_bunq_client_authentication, adyen_checkout_merchant_transaction_fee, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
