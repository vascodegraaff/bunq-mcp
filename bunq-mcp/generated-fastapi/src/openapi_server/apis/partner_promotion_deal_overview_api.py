...
from openapi_server.apis.partner_promotion_deal_overview_api_base import BasePartnerPromotionDealOverviewApi
...
from openapi_server.models.partner_promotion_deal_overview_listing import PartnerPromotionDealOverviewListing
...
@router.get(
    "/user/{userID}/partner-promotion-deal-overview",
    responses={
        200: {"model": List[PartnerPromotionDealOverviewListing], "description": "Endpoint to get an overview of partner promotion deals for a user. apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["partner-promotion-deal-overview"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_partner_promotion_deal_overview_for_user(
...
) -> List[PartnerPromotionDealOverviewListing]:
    """Endpoint to get an overview of partner promotion deals for a user. apiApp:LISTING"""
    if not BasePartnerPromotionDealOverviewApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BasePartnerPromotionDealOverviewApi.subclasses[0]().list_all_partner_promotion_deal_overview_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
