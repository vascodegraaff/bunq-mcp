...
from openapi_server.apis.iberpay_return_request_api_base import BaseIberpayReturnRequestApi
...
from openapi_server.models.iberpay_incoming_return_request_create import IberpayIncomingReturnRequestCreate
...
@router.post(
    "/iberpay-return-request",
    responses={
        200: {"model": IberpayIncomingReturnRequestCreate, "description": "Endpoint for Iberpay to send payment return requests to bunq. Iberpay service name: WS_RECEP_SOLICITUD. apiIberpay:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["iberpay-return-request"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_iberpay_return_request(
...
) -> IberpayIncomingReturnRequestCreate:
    """Endpoint for Iberpay to send payment return requests to bunq. Iberpay service name: WS_RECEP_SOLICITUD. apiIberpay:CREATE"""
    if not BaseIberpayReturnRequestApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseIberpayReturnRequestApi.subclasses[0]().c_reate_iberpay_return_request(user_agent, x_bunq_client_authentication, body, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
