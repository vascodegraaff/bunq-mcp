...
from openapi_server.apis.admin_user_api_base import BaseAdminUserApi
...
from openapi_server.models.admin_user_read import AdminUserRead
...
@router.get(
    "/admin-user/{itemId}",
    responses={
        200: {"model": AdminUserRead, "description": "Used for the user endpoint accessed from admin. apiAdmin:READ apiServiceInternal:READ apiProcess:READ"},
...
async def r_ead_admin_user(
    ...
    """Used for the user endpoint accessed from admin. apiAdmin:READ apiServiceInternal:READ apiProcess:READ"""
    if not BaseAdminUserApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAdminUserApi.subclasses[0]().r_ead_admin_user(itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
