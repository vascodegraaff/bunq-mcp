...
async def list_all_event_for_user(
    ...
    return await BaseEventApi.subclasses[0]().list_all_event_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/card/{cardID}/event",
    responses={
        200: {"model": List[CardEventListing], "description": "Used to list event history for a card. apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["event"],
    summary="",
...
async def list_all_event_for_user_card(
    ...
    if not BaseEventApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseEventApi.subclasses[0]().list_all_event_for_user_card(userID, cardID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/event/{itemId}",
    responses={
        200: {"model": EventRead, "description": "Used to view events. Events are automatically created and contain information about everything that happens to your bunq account. In the bunq app events are shown in your &#39;overview&#39;. Examples of when events are created or modified: payment sent, payment received, request for payment received or connect invite received. apiAdmin:READ/LISTING apiApp:READ/LISTING apiPublic:READ/LISTING apiServiceInternal:READ/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["event"],
    summary="",
...
async def r_ead_event_for_user(
    ...
    if not BaseEventApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseEventApi.subclasses[0]().r_ead_event_for_user(userID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
