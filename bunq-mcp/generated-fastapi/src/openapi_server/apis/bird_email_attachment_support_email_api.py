...
from openapi_server.apis.bird_email_attachment_support_email_api_base import BaseBirdEmailAttachmentSupportEmailApi
...
@router.get(
    "/bird-email-attachment-support-email/{itemId}",
    responses={
        200: {"model": object, "description": "Allow Bird to retrieve the support email attachment content. apiBird:READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["bird-email-attachment-support-email"],
    summary="",
    response_model_by_alias=True,
)
async def r_ead_bird_email_attachment_support_email(
    ...
    """Allow Bird to retrieve the support email attachment content. apiBird:READ"""
    if not BaseBirdEmailAttachmentSupportEmailApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseBirdEmailAttachmentSupportEmailApi.subclasses[0]().r_ead_bird_email_attachment_support_email(itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
