...
from openapi_server.apis.partner_promotion_deal_deeplink_api_base import BasePartnerPromotionDealDeeplinkApi
...
@router.get(
    "/partner-promotion-deal-deeplink/{itemId}",
    responses={
        200: {"model": object, "description": "Endpoint to read the partner promotion using it&#39;s human readable deeplink identifier. apiApp:READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["partner-promotion-deal-deeplink"],
    summary="",
    response_model_by_alias=True,
)
async def r_ead_partner_promotion_deal_deeplink(
    ...
    """Endpoint to read the partner promotion using it&#39;s human readable deeplink identifier. apiApp:READ"""
    if not BasePartnerPromotionDealDeeplinkApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BasePartnerPromotionDealDeeplinkApi.subclasses[0]().r_ead_partner_promotion_deal_deeplink(itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
