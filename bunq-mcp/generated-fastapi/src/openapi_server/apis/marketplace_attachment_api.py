...
from openapi_server.apis.marketplace_attachment_api_base import BaseMarketplaceAttachmentApi
...
from openapi_server.models.attachment_marketplace_create import AttachmentMarketplaceCreate
from openapi_server.models.attachment_marketplace_read import AttachmentMarketplaceRead
...
@router.post(
    ...
    responses={
        200: {"model": AttachmentMarketplaceCreate, "description": "This call is used to upload an attachment that is accessible by all members of a registry. apiAdmin:CREATE/READ apiApp:CREATE/READ apiServiceInternal:CREATE/READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["marketplace-attachment"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_marketplace_attachment(
    ...
    if not BaseMarketplaceAttachmentApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseMarketplaceAttachmentApi.subclasses[0]().c_reate_marketplace_attachment(user_agent, x_bunq_client_authentication, body, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/marketplace-attachment/{itemId}",
    responses={
        200: {"model": AttachmentMarketplaceRead, "description": "This call is used to upload an attachment that is accessible by all members of a registry. apiAdmin:CREATE/READ apiApp:CREATE/READ apiServiceInternal:CREATE/READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["marketplace-attachment"],
    summary="",
...
async def r_ead_marketplace_attachment(
    ...
    x_bunq_client_request_id: Annotated[Optional[StrictStr], Field(description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer.")] = Header(None, description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer."),
    x_bunq_geolocation: Annotated[Optional[StrictStr], Field(description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued.")] = Header(None, description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued."),
) -> AttachmentMarketplaceRead:
    """Get a specific attachment. The header of the response contains the content-type of the attachment."""
    if not BaseMarketplaceAttachmentApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseMarketplaceAttachmentApi.subclasses[0]().r_ead_marketplace_attachment(itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
