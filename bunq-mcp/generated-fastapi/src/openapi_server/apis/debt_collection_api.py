...
from openapi_server.apis.debt_collection_api_base import BaseDebtCollectionApi
...
from openapi_server.models.debt_collection_listing import DebtCollectionListing
from openapi_server.models.debt_collection_read import DebtCollectionRead
...
@router.get(
    "/user/{userID}/debt-collection",
    responses={
        200: {"model": List[DebtCollectionListing], "description": "View the debt collection row. apiAdmin:LISTING/READ apiApp:LISTING/READ apiServiceInternal:LISTING/READ"},
...
async def list_all_debt_collection_for_user(
...
) -> List[DebtCollectionListing]:
    """View the debt collection row. apiAdmin:LISTING/READ apiApp:LISTING/READ apiServiceInternal:LISTING/READ"""
    if not BaseDebtCollectionApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseDebtCollectionApi.subclasses[0]().list_all_debt_collection_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/debt-collection/{itemId}",
    responses={
        200: {"model": DebtCollectionRead, "description": "View the debt collection row. apiAdmin:LISTING/READ apiApp:LISTING/READ apiServiceInternal:LISTING/READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["debt-collection"],
    summary="",
...
async def r_ead_debt_collection_for_user(
    userID: StrictInt = Path(..., description=""),
    ...
    if not BaseDebtCollectionApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseDebtCollectionApi.subclasses[0]().r_ead_debt_collection_for_user(userID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
