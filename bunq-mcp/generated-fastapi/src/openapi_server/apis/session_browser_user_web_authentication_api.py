...
from openapi_server.apis.session_browser_user_web_authentication_api_base import BaseSessionBrowserUserWebAuthenticationApi
...
from openapi_server.models.session_browser_user_web_authentication import SessionBrowserUserWebAuthentication
from openapi_server.models.session_browser_user_web_authentication_create import SessionBrowserUserWebAuthenticationCreate
...
@router.post(
    "/session-browser-user-web-authentication",
    responses={
        200: {"model": SessionBrowserUserWebAuthenticationCreate, "description": "Create a new session for a browser with a UserWebAuthentication token. apiApp:CREATE apiDeveloperPortal:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["session-browser-user-web-authentication"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_session_browser_user_web_authentication(
    ...
    session_browser_user_web_authentication: SessionBrowserUserWebAuthentication = Body(None, description=""),
...
) -> SessionBrowserUserWebAuthenticationCreate:
    """Create a new session for a browser with a UserWebAuthentication token. apiApp:CREATE apiDeveloperPortal:CREATE"""
    if not BaseSessionBrowserUserWebAuthenticationApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseSessionBrowserUserWebAuthenticationApi.subclasses[0]().c_reate_session_browser_user_web_authentication(user_agent, x_bunq_client_authentication, session_browser_user_web_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
