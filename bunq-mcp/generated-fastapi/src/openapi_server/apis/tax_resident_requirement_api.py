...
from openapi_server.apis.tax_resident_requirement_api_base import BaseTaxResidentRequirementApi
...
from openapi_server.models.tax_resident_requirement_listing import TaxResidentRequirementListing
...
@router.get(
    "/user/{userID}/tax-resident-requirement",
    responses={
        200: {"model": List[TaxResidentRequirementListing], "description": "Endpoint for getting all the countries the user should supply a TIN for. apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["tax-resident-requirement"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_tax_resident_requirement_for_user(
...
) -> List[TaxResidentRequirementListing]:
    """Endpoint for getting all the countries the user should supply a TIN for. apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING"""
    if not BaseTaxResidentRequirementApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseTaxResidentRequirementApi.subclasses[0]().list_all_tax_resident_requirement_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
