...
from openapi_server.apis.user_point_challenge_promotion_api_base import BaseUserPointChallengePromotionApi
...
from openapi_server.models.user_point_challenge_promotion_listing import UserPointChallengePromotionListing
...
@router.get(
    "/user/{userID}/user-point-challenge-promotion",
    responses={
        200: {"model": List[UserPointChallengePromotionListing], "description": "Endpoint to retrieve user point challenge promotions. apiAdmin:LISTING apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["user-point-challenge-promotion"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_user_point_challenge_promotion_for_user(
...
) -> List[UserPointChallengePromotionListing]:
    """Endpoint to retrieve user point challenge promotions. apiAdmin:LISTING apiApp:LISTING"""
    if not BaseUserPointChallengePromotionApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseUserPointChallengePromotionApi.subclasses[0]().list_all_user_point_challenge_promotion_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
