...
from openapi_server.apis.admin_registry_gdpr_api_base import BaseAdminRegistryGdprApi
...
from openapi_server.models.admin_registry_gdpr_report_listing import AdminRegistryGdprReportListing
...
@router.get(
    "/user/{userID}/admin-registry-gdpr",
    responses={
        200: {"model": List[AdminRegistryGdprReportListing], "description": "Used to generate GDPR report of a user. apiAdmin:LISTING apiServiceInternal:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["admin-registry-gdpr"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_admin_registry_gdpr_for_user(
...
) -> List[AdminRegistryGdprReportListing]:
    """Used to generate GDPR report of a user. apiAdmin:LISTING apiServiceInternal:LISTING"""
    if not BaseAdminRegistryGdprApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAdminRegistryGdprApi.subclasses[0]().list_all_admin_registry_gdpr_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
