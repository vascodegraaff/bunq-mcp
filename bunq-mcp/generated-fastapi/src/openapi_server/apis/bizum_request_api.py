...
from openapi_server.apis.bizum_request_api_base import BaseBizumRequestApi
...
from openapi_server.models.bizum_request_listing import BizumRequestListing
from openapi_server.models.bizum_request_read import BizumRequestRead
...
@router.get(
    "/user/{userID}/bizum-request",
    responses={
        200: {"model": List[BizumRequestListing], "description": "Endpoint for retrieving your Bizum requests. apiApp:READ/LISTING"},
...
async def list_all_bizum_request_for_user(
...
) -> List[BizumRequestListing]:
    """Endpoint for retrieving your Bizum requests. apiApp:READ/LISTING"""
    if not BaseBizumRequestApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseBizumRequestApi.subclasses[0]().list_all_bizum_request_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/bizum-request/{itemId}",
    responses={
        200: {"model": BizumRequestRead, "description": "Endpoint for retrieving your Bizum requests. apiApp:READ/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["bizum-request"],
    summary="",
...
async def r_ead_bizum_request_for_user(
    userID: StrictInt = Path(..., description=""),
    ...
    if not BaseBizumRequestApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseBizumRequestApi.subclasses[0]().r_ead_bizum_request_for_user(userID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
