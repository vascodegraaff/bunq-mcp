...
from openapi_server.apis.mastercard_action_api_base import BaseMastercardActionApi
...
from openapi_server.models.master_card_action_listing import MasterCardActionListing
from openapi_server.models.master_card_action_read import MasterCardActionRead
from openapi_server.models.master_card_action_user_listing import MasterCardActionUserListing
...
@router.get(
    "/user/{userID}/mastercard-action",
    responses={
        200: {"model": List[MasterCardActionUserListing], "description": "MasterCard transaction view for a specific user. apiApp:LISTING"},
...
async def list_all_mastercard_action_for_user(
...
) -> List[MasterCardActionUserListing]:
    """MasterCard transaction view for a specific user. apiApp:LISTING"""
    if not BaseMastercardActionApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseMastercardActionApi.subclasses[0]().list_all_mastercard_action_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/monetary-account/{monetary-accountID}/mastercard-action",
    responses={
        200: {"model": List[MasterCardActionListing], "description": "MasterCard transaction view. apiAdmin:READ/LISTING apiApp:READ/LISTING apiPublic:READ/LISTING apiServiceInternal:READ/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["mastercard-action"],
    summary="",
...
async def list_all_mastercard_action_for_user_monetary_account(
    ...
    x_bunq_client_request_id: Annotated[Optional[StrictStr], Field(description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer.")] = Header(None, description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer."),
    x_bunq_geolocation: Annotated[Optional[StrictStr], Field(description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued.")] = Header(None, description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued."),
) -> List[MasterCardActionListing]:
    """MasterCard transaction view. apiAdmin:READ/LISTING apiApp:READ/LISTING apiPublic:READ/LISTING apiServiceInternal:READ/LISTING"""
    if not BaseMastercardActionApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseMastercardActionApi.subclasses[0]().list_all_mastercard_action_for_user_monetary_account(userID, monetary-accountID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/monetary-account/{monetary-accountID}/mastercard-action/{itemId}",
    responses={
        200: {"model": MasterCardActionRead, "description": "MasterCard transaction view. apiAdmin:READ/LISTING apiApp:READ/LISTING apiPublic:READ/LISTING apiServiceInternal:READ/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["mastercard-action"],
    summary="",
...
async def r_ead_mastercard_action_for_user_monetary_account(
    userID: StrictInt = Path(..., description=""),
    monetary-accountID: StrictInt = Path(..., description=""),
    ...
    if not BaseMastercardActionApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseMastercardActionApi.subclasses[0]().r_ead_mastercard_action_for_user_monetary_account(userID, monetary-accountID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
