...
from openapi_server.apis.confirmation_retry_api_base import BaseConfirmationRetryApi
...
from openapi_server.models.admin_confirmation_alias_retry_create import AdminConfirmationAliasRetryCreate
...
@router.post(
    "/user/{userID}/alias-user/{alias-userID}/confirmation-retry",
    responses={
        200: {"model": AdminConfirmationAliasRetryCreate, "description": "Used to resend confirmation alias. apiAdmin:CREATE apiServiceInternal:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["confirmation-retry"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_confirmation_retry_for_user_alias_user(
    userID: StrictInt = Path(..., description=""),
    alias-userID: StrictInt = Path(..., description=""),
...
) -> AdminConfirmationAliasRetryCreate:
    """Used to resend confirmation alias. apiAdmin:CREATE apiServiceInternal:CREATE"""
    if not BaseConfirmationRetryApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseConfirmationRetryApi.subclasses[0]().c_reate_confirmation_retry_for_user_alias_user(userID, alias-userID, user_agent, x_bunq_client_authentication, body, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
