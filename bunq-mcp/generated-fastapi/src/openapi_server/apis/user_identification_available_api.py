...
from openapi_server.apis.user_identification_available_api_base import BaseUserIdentificationAvailableApi
...
from openapi_server.models.user_identification_available_listing import UserIdentificationAvailableListing
...
@router.get(
    "/user/{userID}/user-identification-available",
    responses={
        200: {"model": List[UserIdentificationAvailableListing], "description": "Endpoint for listing all available identification methods for a specific user apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["user-identification-available"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_user_identification_available_for_user(
...
) -> List[UserIdentificationAvailableListing]:
    """Endpoint for listing all available identification methods for a specific user apiApp:LISTING"""
    if not BaseUserIdentificationAvailableApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseUserIdentificationAvailableApi.subclasses[0]().list_all_user_identification_available_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
