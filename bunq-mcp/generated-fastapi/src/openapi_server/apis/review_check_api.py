...
from openapi_server.apis.review_check_api_base import BaseReviewCheckApi
...
from openapi_server.models.user_recovery_expiry_review_check_listing import UserRecoveryExpiryReviewCheckListing
...
@router.get(
    "/user/{userID}/user-recovery/{user-recoveryID}/review-check",
    responses={
        200: {"model": List[UserRecoveryExpiryReviewCheckListing], "description": "View the review checks associated with the user recovery expiry. apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING"},
...
async def list_all_review_check_for_user_user_recovery(
    userID: StrictInt = Path(..., description=""),
    user-recoveryID: StrictInt = Path(..., description=""),
...
) -> List[UserRecoveryExpiryReviewCheckListing]:
    """View the review checks associated with the user recovery expiry. apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING"""
    if not BaseReviewCheckApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseReviewCheckApi.subclasses[0]().list_all_review_check_for_user_user_recovery(userID, user-recoveryID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
