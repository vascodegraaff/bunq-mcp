...
from openapi_server.apis.name_api_base import BaseNameApi
...
from openapi_server.models.user_company_name_listing import UserCompanyNameListing
...
@router.get(
    ...
    responses={
        200: {"model": List[UserCompanyNameListing], "description": "Endpoint for getting all the known (trade) names for a user company. This is needed for updating the user name, as we only accept legal or trade names. apiAdmin:LISTING apiApp:LISTING apiPublic:LISTING apiServiceInternal:LISTING"},
...
) -> List[UserCompanyNameListing]:
    """Return all the known (trade) names for a specific user company."""
    if not BaseNameApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseNameApi.subclasses[0]().list_all_name_for_user_company(user-companyID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
