...
from openapi_server.apis.schedule_payment_pocket_money_overview_api_base import BaseSchedulePaymentPocketMoneyOverviewApi
...
from openapi_server.models.schedule_payment_pocket_money_overview_listing import SchedulePaymentPocketMoneyOverviewListing
...
@router.get(
    "/user/{userID}/monetary-account/{monetary-accountID}/schedule-payment-pocket-money-overview",
    responses={
        200: {"model": List[SchedulePaymentPocketMoneyOverviewListing], "description": "Endpoint to retrieve suggested pocket money amount. apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["schedule-payment-pocket-money-overview"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_schedule_payment_pocket_money_overview_for_user_monetary_account(
...
) -> List[SchedulePaymentPocketMoneyOverviewListing]:
    """Endpoint to retrieve suggested pocket money amount. apiApp:LISTING"""
    if not BaseSchedulePaymentPocketMoneyOverviewApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseSchedulePaymentPocketMoneyOverviewApi.subclasses[0]().list_all_schedule_payment_pocket_money_overview_for_user_monetary_account(userID, monetary-accountID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
