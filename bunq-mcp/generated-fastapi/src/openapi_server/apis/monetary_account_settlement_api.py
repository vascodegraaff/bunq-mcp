...
from openapi_server.apis.monetary_account_settlement_api_base import BaseMonetaryAccountSettlementApi
...
from openapi_server.models.monetary_account_settlement_read import MonetaryAccountSettlementRead
...
@router.get(
    "/monetary-account-settlement/{itemId}",
    responses={
        200: {"model": MonetaryAccountSettlementRead, "description": "View for the settlement monetary accounts apiAdmin:READ apiServiceInternal:READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["monetary-account-settlement"],
    summary="",
    response_model_by_alias=True,
)
async def r_ead_monetary_account_settlement(
...
) -> MonetaryAccountSettlementRead:
    """View for the settlement monetary accounts apiAdmin:READ apiServiceInternal:READ"""
    if not BaseMonetaryAccountSettlementApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseMonetaryAccountSettlementApi.subclasses[0]().r_ead_monetary_account_settlement(itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
