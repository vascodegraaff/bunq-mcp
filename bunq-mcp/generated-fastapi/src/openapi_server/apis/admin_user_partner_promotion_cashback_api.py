...
from openapi_server.apis.admin_user_partner_promotion_cashback_api_base import BaseAdminUserPartnerPromotionCashbackApi
...
from openapi_server.models.admin_user_partner_promotion_cashback import AdminUserPartnerPromotionCashback
from openapi_server.models.admin_user_partner_promotion_cashback_create import AdminUserPartnerPromotionCashbackCreate
...
@router.post(
    "/user/{userID}/admin-user-partner-promotion-cashback",
    responses={
        200: {"model": AdminUserPartnerPromotionCashbackCreate, "description": "Manually link a user to a partner promotion. apiAdmin:CREATE/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["admin-user-partner-promotion-cashback"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_admin_user_partner_promotion_cashback_for_user(
    ...
    admin_user_partner_promotion_cashback: AdminUserPartnerPromotionCashback = Body(None, description=""),
...
) -> AdminUserPartnerPromotionCashbackCreate:
    """Manually link a user to a partner promotion. apiAdmin:CREATE/LISTING"""
    if not BaseAdminUserPartnerPromotionCashbackApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAdminUserPartnerPromotionCashbackApi.subclasses[0]().c_reate_admin_user_partner_promotion_cashback_for_user(userID, user_agent, x_bunq_client_authentication, admin_user_partner_promotion_cashback, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/admin-user-partner-promotion-cashback",
    responses={
        200: {"model": List[object], "description": "Manually link a user to a partner promotion. apiAdmin:CREATE/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["admin-user-partner-promotion-cashback"],
    summary="",
...
async def list_all_admin_user_partner_promotion_cashback_for_user(
    userID: StrictInt = Path(..., description=""),
    user_agent: Annotated[StrictStr, Field(description="The User-Agent header field should contain information about the user agent originating the request. There are no restrictions on the value of this header.")] = Header(None, description="The User-Agent header field should contain information about the user agent originating the request. There are no restrictions on the value of this header."),
    x_bunq_client_authentication: Annotated[StrictStr, Field(description="The authentication token is used to authenticate the source of the API call. It is required by all API calls except for POST /v1/installation. It is important to note that the device and session calls are using the token from the response of the installation call, while all the other calls use the token from the response of the session-server call")] = Header(None, description="The authentication token is used to authenticate the source of the API call. It is required by all API calls except for POST /v1/installation. It is important to note that the device and session calls are using the token from the response of the installation call, while all the other calls use the token from the response of the session-server call"),
    cache_control: Annotated[Optional[StrictStr], Field(description="The standard HTTP Cache-Control header is required for all signed requests.")] = Header(None, description="The standard HTTP Cache-Control header is required for all signed requests."),
    ...
    if not BaseAdminUserPartnerPromotionCashbackApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAdminUserPartnerPromotionCashbackApi.subclasses[0]().list_all_admin_user_partner_promotion_cashback_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
