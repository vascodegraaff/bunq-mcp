...
from openapi_server.apis.wsnot_defintiva_api_base import BaseWsnotDefintivaApi
...
from openapi_server.models.bizum_notification_definitive_create import BizumNotificationDefinitiveCreate
...
@router.post(
    "/wsnot-defintiva",
    responses={
        200: {"model": BizumNotificationDefinitiveCreate, "description": "Endpoint for receiving Bizum notifications. apiBizum:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["wsnot-defintiva"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_wsnot_defintiva(
...
) -> BizumNotificationDefinitiveCreate:
    """Endpoint for receiving Bizum notifications. apiBizum:CREATE"""
    if not BaseWsnotDefintivaApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseWsnotDefintivaApi.subclasses[0]().c_reate_wsnot_defintiva(user_agent, x_bunq_client_authentication, body, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
