...
from openapi_server.apis.iberpay_liquidity_information_api_base import BaseIberpayLiquidityInformationApi
...
from openapi_server.models.iberpay_incoming_liquidity_information_create import IberpayIncomingLiquidityInformationCreate
...
@router.post(
    "/iberpay-liquidity-information",
    responses={
        200: {"model": IberpayIncomingLiquidityInformationCreate, "description": "Endpoint for Iberpay to send reconciliation messages to bunq. Iberpay service name: WS_INFOLIQ. apiIberpay:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["iberpay-liquidity-information"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_iberpay_liquidity_information(
...
) -> IberpayIncomingLiquidityInformationCreate:
    """Endpoint for Iberpay to send reconciliation messages to bunq. Iberpay service name: WS_INFOLIQ. apiIberpay:CREATE"""
    if not BaseIberpayLiquidityInformationApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseIberpayLiquidityInformationApi.subclasses[0]().c_reate_iberpay_liquidity_information(user_agent, x_bunq_client_authentication, body, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
