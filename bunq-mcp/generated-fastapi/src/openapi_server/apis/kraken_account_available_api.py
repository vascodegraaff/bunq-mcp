...
from openapi_server.apis.kraken_account_available_api_base import BaseKrakenAccountAvailableApi
...
from openapi_server.models.kraken_account_available_listing import KrakenAccountAvailableListing
...
@router.get(
    "/user/{userID}/kraken-account-available",
    responses={
        200: {"model": List[KrakenAccountAvailableListing], "description": "To determine whether signing up for a Kraken account is available for you. apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["kraken-account-available"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_kraken_account_available_for_user(
...
) -> List[KrakenAccountAvailableListing]:
    """To determine whether signing up for a Kraken account is available for you. apiApp:LISTING"""
    if not BaseKrakenAccountAvailableApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseKrakenAccountAvailableApi.subclasses[0]().list_all_kraken_account_available_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
