...
from openapi_server.apis.admin_user_approval_identification_document_api_base import BaseAdminUserApprovalIdentificationDocumentApi
...
from openapi_server.models.admin_user_approval_identification_document_listing import AdminUserApprovalIdentificationDocumentListing
...
@router.get(
    "/admin-user-approval-identification-document",
    responses={
        200: {"model": List[AdminUserApprovalIdentificationDocumentListing], "description": "Get the identification documents from auto-approved users. apiAdmin:LISTING apiServiceInternal:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["admin-user-approval-identification-document"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_admin_user_approval_identification_document(
...
) -> List[AdminUserApprovalIdentificationDocumentListing]:
    """Get the identification documents from auto-approved users. apiAdmin:LISTING apiServiceInternal:LISTING"""
    if not BaseAdminUserApprovalIdentificationDocumentApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAdminUserApprovalIdentificationDocumentApi.subclasses[0]().list_all_admin_user_approval_identification_document(user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
