...
from openapi_server.apis.approval_queue_stats_api_base import BaseApprovalQueueStatsApi
...
from openapi_server.models.user_approval_queue_stats_listing import UserApprovalQueueStatsListing
...
@router.get(
    "/user/{userID}/approval-queue-stats",
    responses={
        200: {"model": List[UserApprovalQueueStatsListing], "description": "Gets information about the approval queue. apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["approval-queue-stats"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_approval_queue_stats_for_user(
...
) -> List[UserApprovalQueueStatsListing]:
    """Gets information about the approval queue. apiApp:LISTING"""
    if not BaseApprovalQueueStatsApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseApprovalQueueStatsApi.subclasses[0]().list_all_approval_queue_stats_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
