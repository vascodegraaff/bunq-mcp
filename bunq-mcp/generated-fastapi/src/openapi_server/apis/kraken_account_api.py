...
from openapi_server.apis.kraken_account_api_base import BaseKrakenAccountApi
...
from openapi_server.models.kraken_account_create import KrakenAccountCreate
from openapi_server.models.kraken_account_read import KrakenAccountRead
from openapi_server.models.kraken_account_update import KrakenAccountUpdate
...
@router.post(
    "/user/{userID}/kraken-account",
    responses={
        200: {"model": KrakenAccountCreate, "description": "Kraken accounts. apiAdmin:READ apiApp:CREATE/UPDATE/READ"},
...
async def c_reate_kraken_account_for_user(
    ...
    """Kraken accounts. apiAdmin:READ apiApp:CREATE/UPDATE/READ"""
    if not BaseKrakenAccountApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseKrakenAccountApi.subclasses[0]().c_reate_kraken_account_for_user(userID, user_agent, x_bunq_client_authentication, body, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/kraken-account/{itemId}",
    responses={
        200: {"model": KrakenAccountRead, "description": "Kraken accounts. apiAdmin:READ apiApp:CREATE/UPDATE/READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["kraken-account"],
    summary="",
...
async def r_ead_kraken_account_for_user(
    userID: StrictInt = Path(..., description=""),
    ...
    if not BaseKrakenAccountApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseKrakenAccountApi.subclasses[0]().r_ead_kraken_account_for_user(userID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.put(
    "/user/{userID}/kraken-account/{itemId}",
    responses={
        200: {"model": KrakenAccountUpdate, "description": "Kraken accounts. apiAdmin:READ apiApp:CREATE/UPDATE/READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["kraken-account"],
    summary="",
...
async def u_pdate_kraken_account_for_user(
    userID: StrictInt = Path(..., description=""),
    itemId: StrictInt = Path(..., description=""),
    user_agent: Annotated[StrictStr, Field(description="The User-Agent header field should contain information about the user agent originating the request. There are no restrictions on the value of this header.")] = Header(None, description="The User-Agent header field should contain information about the user agent originating the request. There are no restrictions on the value of this header."),
    x_bunq_client_authentication: Annotated[StrictStr, Field(description="The authentication token is used to authenticate the source of the API call. It is required by all API calls except for POST /v1/installation. It is important to note that the device and session calls are using the token from the response of the installation call, while all the other calls use the token from the response of the session-server call")] = Header(None, description="The authentication token is used to authenticate the source of the API call. It is required by all API calls except for POST /v1/installation. It is important to note that the device and session calls are using the token from the response of the installation call, while all the other calls use the token from the response of the session-server call"),
    ...
    if not BaseKrakenAccountApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseKrakenAccountApi.subclasses[0]().u_pdate_kraken_account_for_user(userID, itemId, user_agent, x_bunq_client_authentication, body, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
