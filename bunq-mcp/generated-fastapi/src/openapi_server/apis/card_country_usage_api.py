...
from openapi_server.apis.card_country_usage_api_base import BaseCardCountryUsageApi
...
from openapi_server.models.card_country_usage_listing import CardCountryUsageListing
...
@router.get(
    "/card-country-usage",
    responses={
        200: {"model": List[CardCountryUsageListing], "description": "Card usages per country. apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["card-country-usage"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_card_country_usage(
...
) -> List[CardCountryUsageListing]:
    """Card usages per country. apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING"""
    if not BaseCardCountryUsageApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseCardCountryUsageApi.subclasses[0]().list_all_card_country_usage(user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
