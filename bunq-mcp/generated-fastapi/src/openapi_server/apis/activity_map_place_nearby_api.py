...
from openapi_server.apis.activity_map_place_nearby_api_base import BaseActivityMapPlaceNearbyApi
...
from openapi_server.models.activity_map_place_suggestion_nearby_listing import ActivityMapPlaceSuggestionNearbyListing
...
@router.get(
    "/user/{userID}/activity-map-place-nearby",
    responses={
        200: {"model": List[ActivityMapPlaceSuggestionNearbyListing], "description": "Returns generic info about nearby places. apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["activity-map-place-nearby"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_activity_map_place_nearby_for_user(
...
) -> List[ActivityMapPlaceSuggestionNearbyListing]:
    """Returns generic info about nearby places. apiApp:LISTING"""
    if not BaseActivityMapPlaceNearbyApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseActivityMapPlaceNearbyApi.subclasses[0]().list_all_activity_map_place_nearby_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
