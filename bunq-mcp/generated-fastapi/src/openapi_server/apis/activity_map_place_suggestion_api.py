...
from openapi_server.apis.activity_map_place_suggestion_api_base import BaseActivityMapPlaceSuggestionApi
...
@router.get(
    "/user/{userID}/activity-map-place-suggestion",
    responses={
        200: {"model": List[List[str]], "description": "Endpoint to get activity map places suggested by finn or by bunq users. apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["activity-map-place-suggestion"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_activity_map_place_suggestion_for_user(
    ...
    """Endpoint to get activity map places suggested by finn or by bunq users. apiApp:LISTING"""
    if not BaseActivityMapPlaceSuggestionApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseActivityMapPlaceSuggestionApi.subclasses[0]().list_all_activity_map_place_suggestion_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
