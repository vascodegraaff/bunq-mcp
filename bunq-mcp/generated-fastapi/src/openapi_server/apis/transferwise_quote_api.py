...
from openapi_server.apis.transferwise_quote_api_base import BaseTransferwiseQuoteApi
...
from openapi_server.models.transferwise_quote import TransferwiseQuote
from openapi_server.models.transferwise_quote_create import TransferwiseQuoteCreate
from openapi_server.models.transferwise_quote_read import TransferwiseQuoteRead
...
@router.post(
    ...
    responses={
        200: {"model": TransferwiseQuoteCreate, "description": "Used to get quotes from Transferwise. These can be used to initiate payments. apiApp:CREATE/READ apiPublic:CREATE/READ"},
...
async def c_reate_transferwise_quote_for_user(
    ...
    transferwise_quote: TransferwiseQuote = Body(None, description=""),
    ...
    """Used to get quotes from Transferwise. These can be used to initiate payments. apiApp:CREATE/READ apiPublic:CREATE/READ"""
    if not BaseTransferwiseQuoteApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseTransferwiseQuoteApi.subclasses[0]().c_reate_transferwise_quote_for_user(userID, user_agent, x_bunq_client_authentication, transferwise_quote, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/transferwise-quote/{itemId}",
    responses={
        200: {"model": TransferwiseQuoteRead, "description": "Used to get quotes from Transferwise. These can be used to initiate payments. apiApp:CREATE/READ apiPublic:CREATE/READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["transferwise-quote"],
    summary="",
...
async def r_ead_transferwise_quote_for_user(
    userID: StrictInt = Path(..., description=""),
    ...
    if not BaseTransferwiseQuoteApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseTransferwiseQuoteApi.subclasses[0]().r_ead_transferwise_quote_for_user(userID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
