...
from openapi_server.apis.allowed_srp_parameters_api_base import BaseAllowedSrpParametersApi
...
from openapi_server.models.allowed_srp_parameters_listing import AllowedSrpParametersListing
...
@router.get(
    "/user/{userID}/allowed-srp-parameters",
    responses={
        200: {"model": List[AllowedSrpParametersListing], "description": "View to display to the allowed SRP parameters to the user. apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING apiDeveloperPortal:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["allowed-srp-parameters"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_allowed_srp_parameters_for_user(
...
) -> List[AllowedSrpParametersListing]:
    """View to display to the allowed SRP parameters to the user. apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING apiDeveloperPortal:LISTING"""
    if not BaseAllowedSrpParametersApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAllowedSrpParametersApi.subclasses[0]().list_all_allowed_srp_parameters_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
