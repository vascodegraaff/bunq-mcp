...
from openapi_server.apis.mastercard_token_connect_api_base import BaseMastercardTokenConnectApi
...
from openapi_server.models.master_card_token_connect_listing import MasterCardTokenConnectListing
...
@router.get(
    "/user/{userID}/mastercard-token-connect",
    responses={
        200: {"model": List[MasterCardTokenConnectListing], "description": "Get a list of all token requestors enabled for token connect. apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["mastercard-token-connect"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_mastercard_token_connect_for_user(
...
) -> List[MasterCardTokenConnectListing]:
    """Get a list of all token requestors enabled for token connect. apiApp:LISTING"""
    if not BaseMastercardTokenConnectApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseMastercardTokenConnectApi.subclasses[0]().list_all_mastercard_token_connect_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
