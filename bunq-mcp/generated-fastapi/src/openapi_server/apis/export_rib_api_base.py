...
from openapi_server.models.export_rib_create import ExportRibCreate
from openapi_server.models.export_rib_listing import ExportRibListing
from openapi_server.models.export_rib_read import ExportRibRead


class BaseExportRibApi:
    subclasses: ClassVar[Tuple] = ()

    def __init_subclass__(cls, **kwargs):
        super().__init_subclass__(**kwargs)
        BaseExportRibApi.subclasses = BaseExportRibApi.subclasses + (cls,)
    async def c_reate_export_rib_for_user_monetary_account(
        self,
        userID: StrictInt,
    ...
    async def d_elete_export_rib_for_user_monetary_account(
        self,
        userID: StrictInt,
        monetary-accountID: StrictInt,
        itemId: StrictInt,
        ...
        """Used to create new and read existing RIBs of a monetary account apiAdmin:CREATE/READ/LISTING apiApp:CREATE/READ/DELETE/LISTING apiPublic:CREATE/READ/DELETE/LISTING apiServiceInternal:CREATE/READ/LISTING"""
        ...
    ...
    async def list_all_export_rib_for_user_monetary_account(
        ...
        x_bunq_client_request_id: Annotated[Optional[StrictStr], Field(description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer.")],
        x_bunq_geolocation: Annotated[Optional[StrictStr], Field(description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued.")],
    ) -> List[ExportRibListing]:
        """List all the RIBs for a monetary account."""
        ...


    async def r_ead_export_rib_for_user_monetary_account(
        self,
        userID: StrictInt,
        monetary-accountID: StrictInt,
        itemId: StrictInt,
