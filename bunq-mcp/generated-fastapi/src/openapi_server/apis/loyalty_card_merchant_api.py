...
from openapi_server.apis.loyalty_card_merchant_api_base import BaseLoyaltyCardMerchantApi
...
from openapi_server.models.loyalty_card_merchant_listing import LoyaltyCardMerchantListing
from openapi_server.models.loyalty_card_merchant_read import LoyaltyCardMerchantRead
...
@router.get(
    "/user/{userID}/loyalty-card-merchant",
    responses={
        200: {"model": List[LoyaltyCardMerchantListing], "description": "Endpoint to list Loyalty Card merchants. apiApp:LISTING/READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["loyalty-card-merchant"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_loyalty_card_merchant_for_user(
...
) -> List[LoyaltyCardMerchantListing]:
    """Endpoint to list Loyalty Card merchants. apiApp:LISTING/READ"""
    if not BaseLoyaltyCardMerchantApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseLoyaltyCardMerchantApi.subclasses[0]().list_all_loyalty_card_merchant_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/loyalty-card-merchant/{itemId}",
    responses={
        200: {"model": LoyaltyCardMerchantRead, "description": "Endpoint to list Loyalty Card merchants. apiApp:LISTING/READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["loyalty-card-merchant"],
    summary="",
...
async def r_ead_loyalty_card_merchant_for_user(
    userID: StrictInt = Path(..., description=""),
    ...
    if not BaseLoyaltyCardMerchantApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseLoyaltyCardMerchantApi.subclasses[0]().r_ead_loyalty_card_merchant_for_user(userID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
