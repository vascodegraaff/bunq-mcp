...
from openapi_server.apis.iberpay_liquidity_adjustment_api_base import BaseIberpayLiquidityAdjustmentApi
...
from openapi_server.models.iberpay_incoming_liquidity_adjustment_create import IberpayIncomingLiquidityAdjustmentCreate
...
@router.post(
    "/iberpay-liquidity-adjustment",
    responses={
        200: {"model": IberpayIncomingLiquidityAdjustmentCreate, "description": "Endpoint for Iberpay to send liquidity adjustment messages to bunq. Iberpay service name: WS_AJUSTEEXTRAORDINARIO. apiIberpay:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["iberpay-liquidity-adjustment"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_iberpay_liquidity_adjustment(
...
) -> IberpayIncomingLiquidityAdjustmentCreate:
    """Endpoint for Iberpay to send liquidity adjustment messages to bunq. Iberpay service name: WS_AJUSTEEXTRAORDINARIO. apiIberpay:CREATE"""
    if not BaseIberpayLiquidityAdjustmentApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseIberpayLiquidityAdjustmentApi.subclasses[0]().c_reate_iberpay_liquidity_adjustment(user_agent, x_bunq_client_authentication, body, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
