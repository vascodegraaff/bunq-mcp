...
from openapi_server.apis.alias_installation_api_base import BaseAliasInstallationApi
...
async def c_reate_alias_installation(
    ...
    alias_installation: AliasInstallation = Body(None, description=""),
    ...
    if not BaseAliasInstallationApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAliasInstallationApi.subclasses[0]().c_reate_alias_installation(user_agent, x_bunq_client_authentication, alias_installation, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/alias-installation",
    responses={
        200: {"model": List[AliasInstallationListing], "description": "Used to register an alias for the current installation. Needed to register a device or a user. Possible response codes are 200 (OK), 400 (invalid json), 429 (rate limit), 460 (alias needs confirmation) or 470 (invalid alias). apiAdmin:CREATE/READ/LISTING apiApp:CREATE/READ/LISTING apiServiceInternal:CREATE/READ/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["alias-installation"],
    summary="",
...
async def list_all_alias_installation(
    ...
    if not BaseAliasInstallationApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAliasInstallationApi.subclasses[0]().list_all_alias_installation(user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/alias-installation/{itemId}",
    responses={
        200: {"model": AliasInstallationRead, "description": "Used to register an alias for the current installation. Needed to register a device or a user. Possible response codes are 200 (OK), 400 (invalid json), 429 (rate limit), 460 (alias needs confirmation) or 470 (invalid alias). apiAdmin:CREATE/READ/LISTING apiApp:CREATE/READ/LISTING apiServiceInternal:CREATE/READ/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["alias-installation"],
    summary="",
...
async def r_ead_alias_installation(
    ...
    if not BaseAliasInstallationApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAliasInstallationApi.subclasses[0]().r_ead_alias_installation(itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
