...
from openapi_server.apis.value_api_base import BaseValueApi
...
from openapi_server.models.user_review_value_listing import UserReviewValueListing
...
@router.get(
    "/user/{userID}/review/{reviewID}/check/{checkID}/value",
    responses={
        200: {"model": List[UserReviewValueListing], "description": "Endpoint for listing UserReviewValue. apiApp:LISTING"},
...
async def list_all_value_for_user_review_check(
...
) -> List[UserReviewValueListing]:
    """Endpoint for listing UserReviewValue. apiApp:LISTING"""
    if not BaseValueApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseValueApi.subclasses[0]().list_all_value_for_user_review_check(userID, reviewID, checkID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
