...
from openapi_server.apis.open_banking_merchant_transaction_api_base import BaseOpenBankingMerchantTransactionApi
...
from openapi_server.models.open_banking_merchant_transaction import OpenBankingMerchantTransaction
from openapi_server.models.open_banking_merchant_transaction_create import OpenBankingMerchantTransactionCreate
from openapi_server.models.open_banking_merchant_transaction_read import OpenBankingMerchantTransactionRead
...
@router.post(
    "/user/{userID}/monetary-account/{monetary-accountID}/open-banking-merchant-transaction",
    responses={
        200: {"model": OpenBankingMerchantTransactionCreate, "description": "Create Open Banking Aiia payment. apiApp:CREATE/READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["open-banking-merchant-transaction"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_open_banking_merchant_transaction_for_user_monetary_account(
    ...
    open_banking_merchant_transaction: OpenBankingMerchantTransaction = Body(None, description=""),
...
) -> OpenBankingMerchantTransactionCreate:
    """Create Open Banking Aiia payment. apiApp:CREATE/READ"""
    if not BaseOpenBankingMerchantTransactionApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseOpenBankingMerchantTransactionApi.subclasses[0]().c_reate_open_banking_merchant_transaction_for_user_monetary_account(userID, monetary-accountID, user_agent, x_bunq_client_authentication, open_banking_merchant_transaction, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/monetary-account/{monetary-accountID}/open-banking-merchant-transaction/{itemId}",
    responses={
        200: {"model": OpenBankingMerchantTransactionRead, "description": "Create Open Banking Aiia payment. apiApp:CREATE/READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["open-banking-merchant-transaction"],
    summary="",
...
async def r_ead_open_banking_merchant_transaction_for_user_monetary_account(
    userID: StrictInt = Path(..., description=""),
    monetary-accountID: StrictInt = Path(..., description=""),
    ...
    x_bunq_client_request_id: Annotated[Optional[StrictStr], Field(description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer.")] = Header(None, description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer."),
    x_bunq_geolocation: Annotated[Optional[StrictStr], Field(description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued.")] = Header(None, description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued."),
) -> OpenBankingMerchantTransactionRead:
    """Create Open Banking Aiia payment. apiApp:CREATE/READ"""
    if not BaseOpenBankingMerchantTransactionApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseOpenBankingMerchantTransactionApi.subclasses[0]().r_ead_open_banking_merchant_transaction_for_user_monetary_account(userID, monetary-accountID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
