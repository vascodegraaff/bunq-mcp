...
from openapi_server.apis.confirmation_alias_confirm_api_base import BaseConfirmationAliasConfirmApi
...
from openapi_server.models.confirmation_alias_confirm import ConfirmationAliasConfirm
from openapi_server.models.confirmation_alias_confirm_create import ConfirmationAliasConfirmCreate
from openapi_server.models.confirmation_alias_confirm_read import ConfirmationAliasConfirmRead
...
@router.post(
    ...
    responses={
        200: {"model": ConfirmationAliasConfirmCreate, "description": "Used to confirm an alias for the current user. Returns 200 (ok), 400 (invalid input), 401 (invalid confirmation code). apiConfirmationAlias:CREATE/READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["confirmation-alias-confirm"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_confirmation_alias_confirm(
    ...
    confirmation_alias_confirm: ConfirmationAliasConfirm = Body(None, description=""),
...
) -> ConfirmationAliasConfirmCreate:
    """Used to confirm an alias for the current user. Returns 200 (ok), 400 (invalid input), 401 (invalid confirmation code). apiConfirmationAlias:CREATE/READ"""
    if not BaseConfirmationAliasConfirmApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseConfirmationAliasConfirmApi.subclasses[0]().c_reate_confirmation_alias_confirm(user_agent, x_bunq_client_authentication, confirmation_alias_confirm, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/confirmation-alias-confirm/{itemId}",
    responses={
        200: {"model": ConfirmationAliasConfirmRead, "description": "Used to confirm an alias for the current user. Returns 200 (ok), 400 (invalid input), 401 (invalid confirmation code). apiConfirmationAlias:CREATE/READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["confirmation-alias-confirm"],
    summary="",
...
async def r_ead_confirmation_alias_confirm(
    ...
    x_bunq_client_request_id: Annotated[Optional[StrictStr], Field(description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer.")] = Header(None, description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer."),
    x_bunq_geolocation: Annotated[Optional[StrictStr], Field(description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued.")] = Header(None, description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued."),
) -> ConfirmationAliasConfirmRead:
    """Used to confirm an alias for the current user. Returns 200 (ok), 400 (invalid input), 401 (invalid confirmation code). apiConfirmationAlias:CREATE/READ"""
    if not BaseConfirmationAliasConfirmApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseConfirmationAliasConfirmApi.subclasses[0]().r_ead_confirmation_alias_confirm(itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
