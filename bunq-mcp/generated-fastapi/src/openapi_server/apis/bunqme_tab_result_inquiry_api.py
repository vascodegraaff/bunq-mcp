...
from openapi_server.apis.bunqme_tab_result_inquiry_api_base import BaseBunqmeTabResultInquiryApi
...
from openapi_server.models.bunq_me_tab_result_inquiry_read import BunqMeTabResultInquiryRead
...
@router.get(
    "/user/{userID}/monetary-account/{monetary-accountID}/bunqme-tab/{bunqme-tabID}/bunqme-tab-result-inquiry/{itemId}",
    responses={
        200: {"model": BunqMeTabResultInquiryRead, "description": "Used to view bunq.me TabResultInquiry objects belonging to a tab. A TabResultInquiry is an object that holds details on both the tab and a single payment made for that tab. apiApp:READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["bunqme-tab-result-inquiry"],
    summary="",
    response_model_by_alias=True,
)
async def r_ead_bunqme_tab_result_inquiry_for_user_monetary_account_bunqme_tab(
    userID: StrictInt = Path(..., description=""),
    monetary-accountID: StrictInt = Path(..., description=""),
    bunqme-tabID: StrictInt = Path(..., description=""),
...
) -> BunqMeTabResultInquiryRead:
    """Used to view bunq.me TabResultInquiry objects belonging to a tab. A TabResultInquiry is an object that holds details on both the tab and a single payment made for that tab. apiApp:READ"""
    if not BaseBunqmeTabResultInquiryApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseBunqmeTabResultInquiryApi.subclasses[0]().r_ead_bunqme_tab_result_inquiry_for_user_monetary_account_bunqme_tab(userID, monetary-accountID, bunqme-tabID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
