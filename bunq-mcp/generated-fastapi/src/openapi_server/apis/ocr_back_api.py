...
from openapi_server.apis.ocr_back_api_base import BaseOcrBackApi
...
from openapi_server.models.document_identification_ocr_back_create import DocumentIdentificationOcrBackCreate
from openapi_server.models.user_reactivation_document_identification_ocr_back_create import UserReactivationDocumentIdentificationOcrBackCreate
...
@router.post(
    "/device/{deviceID}/user-reactivation/{user-reactivationID}/ocr-back",
    responses={
        200: {"model": UserReactivationDocumentIdentificationOcrBackCreate, "description": "Used to upload or update the back side of a document for the purpose of regaining access to a user account. apiApp:CREATE"},
...
async def c_reate_ocr_back_for_device_user_reactivation(
    deviceID: StrictInt = Path(..., description=""),
    user-reactivationID: StrictInt = Path(..., description=""),
...
) -> UserReactivationDocumentIdentificationOcrBackCreate:
    """Used to upload or update the back side of a document for the purpose of regaining access to a user account. apiApp:CREATE"""
    if not BaseOcrBackApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseOcrBackApi.subclasses[0]().c_reate_ocr_back_for_device_user_reactivation(deviceID, user-reactivationID, user_agent, x_bunq_client_authentication, body, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.post(
    "/user/{userID}/ocr-back",
    responses={
        200: {"model": DocumentIdentificationOcrBackCreate, "description": "Used to upload or update the back side of a document. apiApp:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["ocr-back"],
    summary="",
...
async def c_reate_ocr_back_for_user(
    ...
    x_bunq_client_request_id: Annotated[Optional[StrictStr], Field(description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer.")] = Header(None, description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer."),
    x_bunq_geolocation: Annotated[Optional[StrictStr], Field(description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued.")] = Header(None, description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued."),
) -> DocumentIdentificationOcrBackCreate:
    """Used to upload or update the back side of a document. apiApp:CREATE"""
    if not BaseOcrBackApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseOcrBackApi.subclasses[0]().c_reate_ocr_back_for_user(userID, user_agent, x_bunq_client_authentication, body, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
