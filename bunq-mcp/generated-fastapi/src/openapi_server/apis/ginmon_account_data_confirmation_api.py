...
from openapi_server.apis.ginmon_account_data_confirmation_api_base import BaseGinmonAccountDataConfirmationApi
...
from openapi_server.models.ginmon_account_data_confirmation_create import GinmonAccountDataConfirmationCreate
...
@router.post(
    "/user/{userID}/ginmon-account-data-confirmation",
    responses={
        200: {"model": GinmonAccountDataConfirmationCreate, "description": "Used to retrieve the requirements for opening a Ginmon Account. apiApp:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["ginmon-account-data-confirmation"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_ginmon_account_data_confirmation_for_user(
...
) -> GinmonAccountDataConfirmationCreate:
    """Used to retrieve the requirements for opening a Ginmon Account. apiApp:CREATE"""
    if not BaseGinmonAccountDataConfirmationApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseGinmonAccountDataConfirmationApi.subclasses[0]().c_reate_ginmon_account_data_confirmation_for_user(userID, user_agent, x_bunq_client_authentication, body, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
