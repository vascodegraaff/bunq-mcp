...
from openapi_server.apis.1brn_api_base import Base1brnApi
import openapi_server.impl
from openapi_server.bunq_proxy import forward_to_bunq
...
from openapi_server.models.government_referral_portal_external_verify_bank_account import GovernmentReferralPortalExternalVerifyBankAccount
from openapi_server.models.government_referral_portal_external_verify_bank_account_create import GovernmentReferralPortalExternalVerifyBankAccountCreate
...
@router.post(
    "/1brn",
    responses={
        200: {"model": GovernmentReferralPortalExternalVerifyBankAccountCreate, "description": "View for retrieving user information. Should only be used by the government. apiGovernment:CREATE"},
...
async def c_reate1brn(
    ...
    government_referral_portal_external_verify_bank_account: GovernmentReferralPortalExternalVerifyBankAccount = Body(None, description=""),
...
) -> GovernmentReferralPortalExternalVerifyBankAccountCreate:
    """View for retrieving user information. Should only be used by the government. apiGovernment:CREATE"""
    # Forward request to bunq API
    headers = {
        "User-Agent": user_agent,
        "Cache-Control": cache_control,
        "X-Bunq-Language": x_bunq_language,
        "X-Bunq-Region": x_bunq_region,
        "X-Bunq-Client-Request-Id": x_bunq_client_request_id,
        "X-Bunq-Geolocation": x_bunq_geolocation,
    }
    # Remove None values
    headers = {k: v for k, v in headers.items() if v is not None}
    response = await forward_to_bunq(
        method="POST",
        endpoint="/1brn",
        headers=headers,
        json=government_referral_portal_external_verify_bank_account.dict() if government_referral_portal_external_verify_bank_account else None,
    )
