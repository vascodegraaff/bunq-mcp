...
from openapi_server.apis.ginmon_transaction_dividend_api_base import BaseGinmonTransactionDividendApi
...
from openapi_server.models.ginmon_transaction_dividend_listing import GinmonTransactionDividendListing
...
@router.get(
    "/user/{userID}/ginmon-account/{ginmon-accountID}/ginmon-transaction-dividend",
    responses={
        200: {"model": List[GinmonTransactionDividendListing], "description": "Endpoint for reading Ginmon transactions. apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["ginmon-transaction-dividend"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_ginmon_transaction_dividend_for_user_ginmon_account(
...
) -> List[GinmonTransactionDividendListing]:
    """Endpoint for reading Ginmon transactions. apiApp:LISTING"""
    if not BaseGinmonTransactionDividendApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseGinmonTransactionDividendApi.subclasses[0]().list_all_ginmon_transaction_dividend_for_user_ginmon_account(userID, ginmon-accountID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
