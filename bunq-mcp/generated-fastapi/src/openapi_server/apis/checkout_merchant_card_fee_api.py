...
from openapi_server.apis.checkout_merchant_card_fee_api_base import BaseCheckoutMerchantCardFeeApi
...
from openapi_server.models.checkout_merchant_card_fee import CheckoutMerchantCardFee
from openapi_server.models.checkout_merchant_card_fee_create import CheckoutMerchantCardFeeCreate
...
@router.post(
    "/user/{userID}/checkout-merchant-card-fee",
    responses={
        200: {"model": CheckoutMerchantCardFeeCreate, "description": "Used to calculated the fee we will charge for the transaction. apiApp:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["checkout-merchant-card-fee"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_checkout_merchant_card_fee_for_user(
    ...
    checkout_merchant_card_fee: CheckoutMerchantCardFee = Body(None, description=""),
...
) -> CheckoutMerchantCardFeeCreate:
    """Used to calculated the fee we will charge for the transaction."""
    if not BaseCheckoutMerchantCardFeeApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseCheckoutMerchantCardFeeApi.subclasses[0]().c_reate_checkout_merchant_card_fee_for_user(userID, user_agent, x_bunq_client_authentication, checkout_merchant_card_fee, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
