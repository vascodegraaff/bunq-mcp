...
from openapi_server.apis.open_banking_account_api_base import BaseOpenBankingAccountApi
...
from openapi_server.models.open_banking_account_read import OpenBankingAccountRead
...
@router.get(
    "/open-banking-account/{itemId}",
    responses={
        200: {"model": OpenBankingAccountRead, "description": "Post processor for open banking account to be returned in the monetary account external post processor. apiApp:READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["open-banking-account"],
    summary="",
    response_model_by_alias=True,
)
async def r_ead_open_banking_account(
    ...
    """Post processor for open banking account to be returned in the monetary account external post processor. apiApp:READ"""
    if not BaseOpenBankingAccountApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseOpenBankingAccountApi.subclasses[0]().r_ead_open_banking_account(itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
