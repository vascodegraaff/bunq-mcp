...
from openapi_server.apis.card_debit_api_base import BaseCardDebitApi
...
from openapi_server.models.card_debit import CardDebit
from openapi_server.models.card_debit_create import CardDebitCreate
...
@router.post(
    ...
    responses={
        200: {"model": CardDebitCreate, "description": "With bunq it is possible to order debit cards that can then be connected with each one of the monetary accounts the user has access to (including connected accounts). apiApp:CREATE apiPublic:CREATE"},
...
async def c_reate_card_debit_for_user(
    ...
    card_debit: CardDebit = Body(None, description=""),
    ...
    if not BaseCardDebitApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseCardDebitApi.subclasses[0]().c_reate_card_debit_for_user(userID, user_agent, x_bunq_client_authentication, card_debit, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
