...
from openapi_server.apis.transcription_api_base import BaseTranscriptionApi
...
from openapi_server.models.user_recording_transcription_read import UserRecordingTranscriptionRead
...
@router.get(
    "/user/{userID}/recording/{recordingID}/transcription/{itemId}",
    responses={
        200: {"model": UserRecordingTranscriptionRead, "description": "A recording with audio will be automatically transcribed to text. apiAdmin:READ apiApp:READ apiServiceInternal:READ"},
...
async def r_ead_transcription_for_user_recording(
    userID: StrictInt = Path(..., description=""),
    recordingID: StrictInt = Path(..., description=""),
...
) -> UserRecordingTranscriptionRead:
    """A recording with audio will be automatically transcribed to text. apiAdmin:READ apiApp:READ apiServiceInternal:READ"""
    if not BaseTranscriptionApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseTranscriptionApi.subclasses[0]().r_ead_transcription_for_user_recording(userID, recordingID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
