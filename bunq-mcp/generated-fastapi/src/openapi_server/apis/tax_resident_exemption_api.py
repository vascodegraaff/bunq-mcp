...
from openapi_server.apis.tax_resident_exemption_api_base import BaseTaxResidentExemptionApi
...
from openapi_server.models.tax_resident_exemption_listing import TaxResidentExemptionListing
...
@router.get(
    "/user/{userID}/tax-resident-exemption",
    responses={
        200: {"model": List[TaxResidentExemptionListing], "description": "Manage all countries the user is exempted from providing a TIN for. apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["tax-resident-exemption"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_tax_resident_exemption_for_user(
...
) -> List[TaxResidentExemptionListing]:
    """Manage all countries the user is exempted from providing a TIN for. apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING"""
    if not BaseTaxResidentExemptionApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseTaxResidentExemptionApi.subclasses[0]().list_all_tax_resident_exemption_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
