...
from openapi_server.apis.admin_access_allowlist_api_base import BaseAdminAccessAllowlistApi
...
from openapi_server.models.user_admin_access_allowlist_listing import UserAdminAccessAllowlistListing
...
@router.get(
    "/user/{userID}/admin-access-allowlist",
    responses={
        200: {"model": List[UserAdminAccessAllowlistListing], "description": "Creates a temporary access to a specific user’s resources for an admin user. apiAdmin:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["admin-access-allowlist"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_admin_access_allowlist_for_user(
...
) -> List[UserAdminAccessAllowlistListing]:
    """Creates a temporary access to a specific user’s resources for an admin user. apiAdmin:LISTING"""
    if not BaseAdminAccessAllowlistApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAdminAccessAllowlistApi.subclasses[0]().list_all_admin_access_allowlist_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
