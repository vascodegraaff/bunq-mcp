...
from openapi_server.apis.running_cost_api_base import BaseRunningCostApi
...
from openapi_server.models.running_cost_listing import RunningCostListing
...
@router.get(
    "/user/{userID}/running-cost",
    responses={
        200: {"model": List[RunningCostListing], "description": "Used to view the running costs of a monetary account. apiApp:LISTING"},
...
async def list_all_running_cost_for_user(
...
) -> List[RunningCostListing]:
    """Return the running costs of a monetary account. Examples include transaction costs and withdrawal costs"""
    if not BaseRunningCostApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseRunningCostApi.subclasses[0]().list_all_running_cost_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
