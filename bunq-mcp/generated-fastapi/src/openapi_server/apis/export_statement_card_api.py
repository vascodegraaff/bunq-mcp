...
from openapi_server.apis.export_statement_card_api_base import BaseExportStatementCardApi
...
from openapi_server.models.export_statement_card_listing import ExportStatementCardListing
from openapi_server.models.export_statement_card_read import ExportStatementCardRead
...
@router.get(
    ...
    responses={
        200: {"model": List[ExportStatementCardListing], "description": "Used to create new and read existing card statement exports. Statement exports can be created in either CSV or PDF file format. apiAdmin:READ/LISTING apiApp:READ/LISTING apiPublic:READ/LISTING apiServiceInternal:READ/LISTING"},
...
) -> List[ExportStatementCardListing]:
    """Used to create new and read existing card statement exports. Statement exports can be created in either CSV or PDF file format. apiAdmin:READ/LISTING apiApp:READ/LISTING apiPublic:READ/LISTING apiServiceInternal:READ/LISTING"""
    if not BaseExportStatementCardApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseExportStatementCardApi.subclasses[0]().list_all_export_statement_card_for_user_card(userID, cardID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/card/{cardID}/export-statement-card/{itemId}",
    responses={
        200: {"model": ExportStatementCardRead, "description": "Used to create new and read existing card statement exports. Statement exports can be created in either CSV or PDF file format. apiAdmin:READ/LISTING apiApp:READ/LISTING apiPublic:READ/LISTING apiServiceInternal:READ/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["export-statement-card"],
    summary="",
...
async def r_ead_export_statement_card_for_user_card(
    userID: StrictInt = Path(..., description=""),
    cardID: StrictInt = Path(..., description=""),
    ...
    if not BaseExportStatementCardApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseExportStatementCardApi.subclasses[0]().r_ead_export_statement_card_for_user_card(userID, cardID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
