...
from openapi_server.apis.mastercard_mastercom_queue_api_base import BaseMastercardMastercomQueueApi
...
from openapi_server.models.master_card_mastercom_queue_listing import MasterCardMastercomQueueListing
...
@router.get(
    "/mastercard-mastercom-queue",
    responses={
        200: {"model": List[MasterCardMastercomQueueListing], "description": "Endpoint to list all results from MasterCom for a specific queue. apiAdmin:LISTING apiServiceInternal:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["mastercard-mastercom-queue"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_mastercard_mastercom_queue(
...
) -> List[MasterCardMastercomQueueListing]:
    """Endpoint to list all results from MasterCom for a specific queue. apiAdmin:LISTING apiServiceInternal:LISTING"""
    if not BaseMastercardMastercomQueueApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseMastercardMastercomQueueApi.subclasses[0]().list_all_mastercard_mastercom_queue(user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
