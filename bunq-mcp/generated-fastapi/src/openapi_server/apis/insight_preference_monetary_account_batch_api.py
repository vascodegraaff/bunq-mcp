...
from openapi_server.apis.insight_preference_monetary_account_batch_api_base import BaseInsightPreferenceMonetaryAccountBatchApi
...
from openapi_server.models.insight_preference_monetary_account_batch import InsightPreferenceMonetaryAccountBatch
from openapi_server.models.insight_preference_monetary_account_batch_create import InsightPreferenceMonetaryAccountBatchCreate
...
@router.post(
    "/user/{userID}/insight-preference-monetary-account-batch",
    responses={
        200: {"model": InsightPreferenceMonetaryAccountBatchCreate, "description": "Used to allow users to set insight/budget preferences. apiApp:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["insight-preference-monetary-account-batch"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_insight_preference_monetary_account_batch_for_user(
    ...
    insight_preference_monetary_account_batch: InsightPreferenceMonetaryAccountBatch = Body(None, description=""),
...
) -> InsightPreferenceMonetaryAccountBatchCreate:
    """Used to allow users to set insight/budget preferences. apiApp:CREATE"""
    if not BaseInsightPreferenceMonetaryAccountBatchApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseInsightPreferenceMonetaryAccountBatchApi.subclasses[0]().c_reate_insight_preference_monetary_account_batch_for_user(userID, user_agent, x_bunq_client_authentication, insight_preference_monetary_account_batch, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
