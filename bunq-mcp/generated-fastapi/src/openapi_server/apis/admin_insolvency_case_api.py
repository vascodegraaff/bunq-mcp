...
from openapi_server.apis.admin_insolvency_case_api_base import BaseAdminInsolvencyCaseApi
...
from openapi_server.models.insolvency_case_listing import InsolvencyCaseListing
from openapi_server.models.insolvency_case_read import InsolvencyCaseRead
...
@router.get(
    "/admin-insolvency-case",
    responses={
        200: {"model": List[InsolvencyCaseListing], "description": "View for the insolvency cases. apiAdmin:READ/LISTING apiServiceInternal:READ/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["admin-insolvency-case"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_admin_insolvency_case(
...
) -> List[InsolvencyCaseListing]:
    """View for the insolvency cases. apiAdmin:READ/LISTING apiServiceInternal:READ/LISTING"""
    if not BaseAdminInsolvencyCaseApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAdminInsolvencyCaseApi.subclasses[0]().list_all_admin_insolvency_case(user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/admin-insolvency-case/{itemId}",
    responses={
        200: {"model": InsolvencyCaseRead, "description": "View for the insolvency cases. apiAdmin:READ/LISTING apiServiceInternal:READ/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["admin-insolvency-case"],
    summary="",
...
async def r_ead_admin_insolvency_case(
    ...
    if not BaseAdminInsolvencyCaseApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAdminInsolvencyCaseApi.subclasses[0]().r_ead_admin_insolvency_case(itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
