...
from openapi_server.apis.value_company_person_api_base import BaseValueCompanyPersonApi
...
from openapi_server.models.user_review_value_company_person import UserReviewValueCompanyPerson
from openapi_server.models.user_review_value_company_person_create import UserReviewValueCompanyPersonCreate
from openapi_server.models.user_review_value_company_person_update import UserReviewValueCompanyPersonUpdate
...
@router.post(
    "/user/{userID}/review/{reviewID}/check/{checkID}/value-company-person",
    responses={
        200: {"model": UserReviewValueCompanyPersonCreate, "description": "Endpoint for managing UserReviewValueCompanyPerson. apiAdmin:CREATE/UPDATE apiApp:UPDATE apiServiceInternal:CREATE/UPDATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["value-company-person"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_value_company_person_for_user_review_check(
    ...
    user_review_value_company_person: UserReviewValueCompanyPerson = Body(None, description=""),
...
) -> UserReviewValueCompanyPersonCreate:
    """Endpoint for managing UserReviewValueCompanyPerson. apiAdmin:CREATE/UPDATE apiApp:UPDATE apiServiceInternal:CREATE/UPDATE"""
    if not BaseValueCompanyPersonApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseValueCompanyPersonApi.subclasses[0]().c_reate_value_company_person_for_user_review_check(userID, reviewID, checkID, user_agent, x_bunq_client_authentication, user_review_value_company_person, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.put(
    "/user/{userID}/review/{reviewID}/check/{checkID}/value-company-person/{itemId}",
    responses={
        200: {"model": UserReviewValueCompanyPersonUpdate, "description": "Endpoint for managing UserReviewValueCompanyPerson. apiAdmin:CREATE/UPDATE apiApp:UPDATE apiServiceInternal:CREATE/UPDATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["value-company-person"],
    summary="",
...
async def u_pdate_value_company_person_for_user_review_check(
    userID: StrictInt = Path(..., description=""),
    reviewID: StrictInt = Path(..., description=""),
    checkID: StrictInt = Path(..., description=""),
    ...
    x_bunq_client_request_id: Annotated[Optional[StrictStr], Field(description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer.")] = Header(None, description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer."),
    x_bunq_geolocation: Annotated[Optional[StrictStr], Field(description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued.")] = Header(None, description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued."),
) -> UserReviewValueCompanyPersonUpdate:
    """Endpoint for managing UserReviewValueCompanyPerson. apiAdmin:CREATE/UPDATE apiApp:UPDATE apiServiceInternal:CREATE/UPDATE"""
    if not BaseValueCompanyPersonApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseValueCompanyPersonApi.subclasses[0]().u_pdate_value_company_person_for_user_review_check(userID, reviewID, checkID, itemId, user_agent, x_bunq_client_authentication, user_review_value_company_person, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
