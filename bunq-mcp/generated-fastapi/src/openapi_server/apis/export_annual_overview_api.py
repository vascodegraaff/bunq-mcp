...
from openapi_server.apis.export_annual_overview_api_base import BaseExportAnnualOverviewApi
...
from openapi_server.models.export_annual_overview import ExportAnnualOverview
from openapi_server.models.export_annual_overview_create import ExportAnnualOverviewCreate
from openapi_server.models.export_annual_overview_listing import ExportAnnualOverviewListing
from openapi_server.models.export_annual_overview_read import ExportAnnualOverviewRead
...
@router.post(
    ...
    responses={
        200: {"model": ExportAnnualOverviewCreate, "description": "Used to create new and read existing annual overviews of all the user&#39;s monetary accounts. Once created, annual overviews can be downloaded in PDF format via the &#39;export-annual-overview/{id}/content&#39; endpoint. apiAdmin:CREATE/READ/LISTING apiApp:CREATE/READ/DELETE/LISTING apiPublic:CREATE/READ/DELETE/LISTING apiServiceInternal:CREATE/READ/LISTING"},
...
async def c_reate_export_annual_overview_for_user(
    ...
    export_annual_overview: ExportAnnualOverview = Body(None, description=""),
    ...
    if not BaseExportAnnualOverviewApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseExportAnnualOverviewApi.subclasses[0]().c_reate_export_annual_overview_for_user(userID, user_agent, x_bunq_client_authentication, export_annual_overview, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.delete(
    "/user/{userID}/export-annual-overview/{itemId}",
    responses={
        200: {"model": object, "description": "Used to create new and read existing annual overviews of all the user&#39;s monetary accounts. Once created, annual overviews can be downloaded in PDF format via the &#39;export-annual-overview/{id}/content&#39; endpoint. apiAdmin:CREATE/READ/LISTING apiApp:CREATE/READ/DELETE/LISTING apiPublic:CREATE/READ/DELETE/LISTING apiServiceInternal:CREATE/READ/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["export-annual-overview"],
    summary="",
...
async def d_elete_export_annual_overview_for_user(
    userID: StrictInt = Path(..., description=""),
    ...
    if not BaseExportAnnualOverviewApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseExportAnnualOverviewApi.subclasses[0]().d_elete_export_annual_overview_for_user(userID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/export-annual-overview",
    responses={
        200: {"model": List[ExportAnnualOverviewListing], "description": "Used to create new and read existing annual overviews of all the user&#39;s monetary accounts. Once created, annual overviews can be downloaded in PDF format via the &#39;export-annual-overview/{id}/content&#39; endpoint. apiAdmin:CREATE/READ/LISTING apiApp:CREATE/READ/DELETE/LISTING apiPublic:CREATE/READ/DELETE/LISTING apiServiceInternal:CREATE/READ/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["export-annual-overview"],
    summary="",
...
async def list_all_export_annual_overview_for_user(
    ...
    x_bunq_client_request_id: Annotated[Optional[StrictStr], Field(description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer.")] = Header(None, description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer."),
    x_bunq_geolocation: Annotated[Optional[StrictStr], Field(description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued.")] = Header(None, description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued."),
) -> List[ExportAnnualOverviewListing]:
    """List all the annual overviews for a user."""
    if not BaseExportAnnualOverviewApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseExportAnnualOverviewApi.subclasses[0]().list_all_export_annual_overview_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/export-annual-overview/{itemId}",
    responses={
        200: {"model": ExportAnnualOverviewRead, "description": "Used to create new and read existing annual overviews of all the user&#39;s monetary accounts. Once created, annual overviews can be downloaded in PDF format via the &#39;export-annual-overview/{id}/content&#39; endpoint. apiAdmin:CREATE/READ/LISTING apiApp:CREATE/READ/DELETE/LISTING apiPublic:CREATE/READ/DELETE/LISTING apiServiceInternal:CREATE/READ/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["export-annual-overview"],
    summary="",
...
async def r_ead_export_annual_overview_for_user(
    userID: StrictInt = Path(..., description=""),
    itemId: StrictInt = Path(..., description=""),
    user_agent: Annotated[StrictStr, Field(description="The User-Agent header field should contain information about the user agent originating the request. There are no restrictions on the value of this header.")] = Header(None, description="The User-Agent header field should contain information about the user agent originating the request. There are no restrictions on the value of this header."),
    x_bunq_client_authentication: Annotated[StrictStr, Field(description="The authentication token is used to authenticate the source of the API call. It is required by all API calls except for POST /v1/installation. It is important to note that the device and session calls are using the token from the response of the installation call, while all the other calls use the token from the response of the session-server call")] = Header(None, description="The authentication token is used to authenticate the source of the API call. It is required by all API calls except for POST /v1/installation. It is important to note that the device and session calls are using the token from the response of the installation call, while all the other calls use the token from the response of the session-server call"),
    ...
    if not BaseExportAnnualOverviewApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseExportAnnualOverviewApi.subclasses[0]().r_ead_export_annual_overview_for_user(userID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
