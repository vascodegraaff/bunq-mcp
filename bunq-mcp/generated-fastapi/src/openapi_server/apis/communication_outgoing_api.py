...
from openapi_server.apis.communication_outgoing_api_base import BaseCommunicationOutgoingApi
...
from openapi_server.models.communication_outgoing_user_listing import CommunicationOutgoingUserListing
...
@router.get(
    "/user/{userID}/communication-outgoing",
    responses={
        200: {"model": List[CommunicationOutgoingUserListing], "description": "Get the communication history for a user. apiAdmin:LISTING apiRetool:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["communication-outgoing"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_communication_outgoing_for_user(
...
) -> List[CommunicationOutgoingUserListing]:
    """Get the communication history for a user. apiAdmin:LISTING apiRetool:LISTING"""
    if not BaseCommunicationOutgoingApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseCommunicationOutgoingApi.subclasses[0]().list_all_communication_outgoing_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
