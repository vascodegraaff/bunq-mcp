...
from openapi_server.apis.server_public_key_api_base import BaseServerPublicKeyApi
...
from openapi_server.models.installation_server_public_key_listing import InstallationServerPublicKeyListing
...
@router.get(
    ...
    responses={
        200: {"model": List[InstallationServerPublicKeyListing], "description": "Using /installation/_/server-public-key you can request the ServerPublicKey again. This is done by referring to the id of the Installation. apiAdmin:LISTING apiApp:LISTING apiPublic:LISTING apiServiceInternal:LISTING apiTranslink:LISTING"},
...
async def list_all_server_public_key_for_installation(
    installationID: StrictInt = Path(..., description=""),
...
) -> List[InstallationServerPublicKeyListing]:
    """Show the ServerPublicKey for this Installation."""
    if not BaseServerPublicKeyApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseServerPublicKeyApi.subclasses[0]().list_all_server_public_key_for_installation(installationID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
