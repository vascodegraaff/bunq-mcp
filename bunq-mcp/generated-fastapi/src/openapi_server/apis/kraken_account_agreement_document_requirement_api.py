...
from openapi_server.apis.kraken_account_agreement_document_requirement_api_base import BaseKrakenAccountAgreementDocumentRequirementApi
...
from openapi_server.models.kraken_account_document_requirement_listing import KrakenAccountDocumentRequirementListing
...
@router.get(
    "/user/{userID}/kraken-account-agreement-document-requirement",
    responses={
        200: {"model": List[KrakenAccountDocumentRequirementListing], "description": "Used to retrieve the requirements for opening a Kraken Account. apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["kraken-account-agreement-document-requirement"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_kraken_account_agreement_document_requirement_for_user(
...
) -> List[KrakenAccountDocumentRequirementListing]:
    """Used to retrieve the requirements for opening a Kraken Account. apiApp:LISTING"""
    if not BaseKrakenAccountAgreementDocumentRequirementApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseKrakenAccountAgreementDocumentRequirementApi.subclasses[0]().list_all_kraken_account_agreement_document_requirement_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
