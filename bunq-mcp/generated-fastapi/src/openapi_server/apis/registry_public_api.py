...
from openapi_server.apis.registry_public_api_base import BaseRegistryPublicApi
...
from openapi_server.models.registry_public_listing import RegistryPublicListing
...
@router.get(
    ...
    responses={
        200: {"model": List[RegistryPublicListing], "description": "Use to import Tricounts(Registry) by using public identifier token apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["registry-public"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_registry_public_for_user(
...
) -> List[RegistryPublicListing]:
    """Use to import Tricounts(Registry) by using public identifier token apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING"""
    if not BaseRegistryPublicApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseRegistryPublicApi.subclasses[0]().list_all_registry_public_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
