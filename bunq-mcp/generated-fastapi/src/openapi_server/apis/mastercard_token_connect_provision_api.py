...
from openapi_server.apis.mastercard_token_connect_provision_api_base import BaseMastercardTokenConnectProvisionApi
...
from openapi_server.models.master_card_token_connect_provision import MasterCardTokenConnectProvision
from openapi_server.models.master_card_token_connect_provision_create import MasterCardTokenConnectProvisionCreate
from openapi_server.models.master_card_token_connect_provision_read import MasterCardTokenConnectProvisionRead
...
@router.post(
    "/user/{userID}/card/{cardID}/mastercard-token-connect-provision",
    responses={
        200: {"model": MasterCardTokenConnectProvisionCreate, "description": "Create a provision for Token Connect for a specific card. apiApp:CREATE/READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["mastercard-token-connect-provision"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_mastercard_token_connect_provision_for_user_card(
    ...
    master_card_token_connect_provision: MasterCardTokenConnectProvision = Body(None, description=""),
...
) -> MasterCardTokenConnectProvisionCreate:
    """Create a provision for Token Connect for a specific card. apiApp:CREATE/READ"""
    if not BaseMastercardTokenConnectProvisionApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseMastercardTokenConnectProvisionApi.subclasses[0]().c_reate_mastercard_token_connect_provision_for_user_card(userID, cardID, user_agent, x_bunq_client_authentication, master_card_token_connect_provision, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/card/{cardID}/mastercard-token-connect-provision/{itemId}",
    responses={
        200: {"model": MasterCardTokenConnectProvisionRead, "description": "Create a provision for Token Connect for a specific card. apiApp:CREATE/READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["mastercard-token-connect-provision"],
    summary="",
...
async def r_ead_mastercard_token_connect_provision_for_user_card(
    userID: StrictInt = Path(..., description=""),
    cardID: StrictInt = Path(..., description=""),
    ...
    x_bunq_client_request_id: Annotated[Optional[StrictStr], Field(description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer.")] = Header(None, description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer."),
    x_bunq_geolocation: Annotated[Optional[StrictStr], Field(description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued.")] = Header(None, description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued."),
) -> MasterCardTokenConnectProvisionRead:
    """Create a provision for Token Connect for a specific card. apiApp:CREATE/READ"""
    if not BaseMastercardTokenConnectProvisionApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseMastercardTokenConnectProvisionApi.subclasses[0]().r_ead_mastercard_token_connect_provision_for_user_card(userID, cardID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
