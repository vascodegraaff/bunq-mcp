...
from openapi_server.apis.deposit_daily_api_base import BaseDepositDailyApi
...
from openapi_server.models.deposit_daily_listing import DepositDailyListing
from openapi_server.models.deposit_daily_read import DepositDailyRead
...
@router.get(
    "/user/{userID}/deposit-daily",
    responses={
        200: {"model": List[DepositDailyListing], "description": "Used to view DepositDaily models for a user. apiAdmin:READ/LISTING apiServiceInternal:READ/LISTING"},
...
async def list_all_deposit_daily_for_user(
...
) -> List[DepositDailyListing]:
    """Used to view DepositDaily models for a user. apiAdmin:READ/LISTING apiServiceInternal:READ/LISTING"""
    if not BaseDepositDailyApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseDepositDailyApi.subclasses[0]().list_all_deposit_daily_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/deposit-daily/{itemId}",
    responses={
        200: {"model": DepositDailyRead, "description": "Used to view DepositDaily models for a user. apiAdmin:READ/LISTING apiServiceInternal:READ/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["deposit-daily"],
    summary="",
...
async def r_ead_deposit_daily_for_user(
    userID: StrictInt = Path(..., description=""),
    ...
    if not BaseDepositDailyApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseDepositDailyApi.subclasses[0]().r_ead_deposit_daily_for_user(userID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
