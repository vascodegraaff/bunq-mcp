...
from openapi_server.apis.admin_user_web_authentication_api_base import BaseAdminUserWebAuthenticationApi
...
from openapi_server.models.admin_user_web_authentication_create import AdminUserWebAuthenticationCreate
...
@router.post(
    "/admin-user-web-authentication",
    responses={
        200: {"model": AdminUserWebAuthenticationCreate, "description": "The endpoint for creating and updating a web authentication token for admin users apiAdmin:CREATE apiServiceInternal:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["admin-user-web-authentication"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_admin_user_web_authentication(
...
) -> AdminUserWebAuthenticationCreate:
    """The endpoint for creating and updating a web authentication token for admin users apiAdmin:CREATE apiServiceInternal:CREATE"""
    if not BaseAdminUserWebAuthenticationApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAdminUserWebAuthenticationApi.subclasses[0]().c_reate_admin_user_web_authentication(user_agent, x_bunq_client_authentication, body, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
