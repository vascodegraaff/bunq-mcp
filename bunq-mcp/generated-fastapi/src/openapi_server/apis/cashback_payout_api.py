...
from openapi_server.apis.cashback_payout_api_base import BaseCashbackPayoutApi
...
from openapi_server.models.cashback_payout_listing import CashbackPayoutListing
from openapi_server.models.cashback_payout_read import CashbackPayoutRead
...
@router.get(
    "/user/{userID}/cashback-payout",
    responses={
        200: {"model": List[CashbackPayoutListing], "description": "Cashback payout details. apiAdmin:LISTING/READ apiApp:LISTING/READ apiServiceInternal:LISTING/READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["cashback-payout"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_cashback_payout_for_user(
...
) -> List[CashbackPayoutListing]:
    """Cashback payout details. apiAdmin:LISTING/READ apiApp:LISTING/READ apiServiceInternal:LISTING/READ"""
    if not BaseCashbackPayoutApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseCashbackPayoutApi.subclasses[0]().list_all_cashback_payout_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/cashback-payout/{itemId}",
    responses={
        200: {"model": CashbackPayoutRead, "description": "Cashback payout details. apiAdmin:LISTING/READ apiApp:LISTING/READ apiServiceInternal:LISTING/READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["cashback-payout"],
    summary="",
...
async def r_ead_cashback_payout_for_user(
    userID: StrictInt = Path(..., description=""),
    ...
    if not BaseCashbackPayoutApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseCashbackPayoutApi.subclasses[0]().r_ead_cashback_payout_for_user(userID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
