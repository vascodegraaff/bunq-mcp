...
from openapi_server.apis.notification_filter_email_api_base import BaseNotificationFilterEmailApi
...
from openapi_server.models.notification_filter_email import NotificationFilterEmail
from openapi_server.models.notification_filter_email_create import NotificationFilterEmailCreate
from openapi_server.models.notification_filter_email_listing import NotificationFilterEmailListing
...
@router.post(
    ...
    responses={
        200: {"model": NotificationFilterEmailCreate, "description": "Manage the email notification filters for a user. apiApp:CREATE/LISTING apiPublic:CREATE/LISTING"},
...
async def c_reate_notification_filter_email_for_user(
    ...
    notification_filter_email: NotificationFilterEmail = Body(None, description=""),
    ...
    """Manage the email notification filters for a user. apiApp:CREATE/LISTING apiPublic:CREATE/LISTING"""
    if not BaseNotificationFilterEmailApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseNotificationFilterEmailApi.subclasses[0]().c_reate_notification_filter_email_for_user(userID, user_agent, x_bunq_client_authentication, notification_filter_email, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/notification-filter-email",
    responses={
        200: {"model": List[NotificationFilterEmailListing], "description": "Manage the email notification filters for a user. apiApp:CREATE/LISTING apiPublic:CREATE/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["notification-filter-email"],
    summary="",
...
async def list_all_notification_filter_email_for_user(
    ...
    x_bunq_client_request_id: Annotated[Optional[StrictStr], Field(description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer.")] = Header(None, description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer."),
    x_bunq_geolocation: Annotated[Optional[StrictStr], Field(description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued.")] = Header(None, description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued."),
) -> List[NotificationFilterEmailListing]:
    """Manage the email notification filters for a user. apiApp:CREATE/LISTING apiPublic:CREATE/LISTING"""
    if not BaseNotificationFilterEmailApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseNotificationFilterEmailApi.subclasses[0]().list_all_notification_filter_email_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
