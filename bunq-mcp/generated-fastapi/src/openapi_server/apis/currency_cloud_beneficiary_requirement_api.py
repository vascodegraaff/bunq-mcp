...
from openapi_server.apis.currency_cloud_beneficiary_requirement_api_base import BaseCurrencyCloudBeneficiaryRequirementApi
...
from openapi_server.models.currency_cloud_beneficiary_requirement_bunq_to_listing import CurrencyCloudBeneficiaryRequirementBunqToListing
from openapi_server.models.currency_cloud_beneficiary_requirement_listing import CurrencyCloudBeneficiaryRequirementListing
...
@router.get(
    ...
    responses={
        200: {"model": List[CurrencyCloudBeneficiaryRequirementBunqToListing], "description": "Endpoint to list requirements for CurrencyCloud beneficiaries for bunq.to. apiBunqTo:LISTING"},
...
async def list_all_currency_cloud_beneficiary_requirement(
...
) -> List[CurrencyCloudBeneficiaryRequirementBunqToListing]:
    """Endpoint to list requirements for CurrencyCloud beneficiaries for bunq.to. apiBunqTo:LISTING"""
    if not BaseCurrencyCloudBeneficiaryRequirementApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseCurrencyCloudBeneficiaryRequirementApi.subclasses[0]().list_all_currency_cloud_beneficiary_requirement(user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/currency-cloud-beneficiary-requirement",
    responses={
        200: {"model": List[CurrencyCloudBeneficiaryRequirementListing], "description": "Endpoint to list requirements for CurrencyCloud beneficiaries. apiAdmin:LISTING apiApp:LISTING apiPublic:LISTING apiServiceInternal:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["currency-cloud-beneficiary-requirement"],
    summary="",
...
async def list_all_currency_cloud_beneficiary_requirement_for_user(
    ...
    x_bunq_client_request_id: Annotated[Optional[StrictStr], Field(description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer.")] = Header(None, description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer."),
    x_bunq_geolocation: Annotated[Optional[StrictStr], Field(description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued.")] = Header(None, description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued."),
) -> List[CurrencyCloudBeneficiaryRequirementListing]:
    """Endpoint to list requirements for CurrencyCloud beneficiaries. apiAdmin:LISTING apiApp:LISTING apiPublic:LISTING apiServiceInternal:LISTING"""
    if not BaseCurrencyCloudBeneficiaryRequirementApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseCurrencyCloudBeneficiaryRequirementApi.subclasses[0]().list_all_currency_cloud_beneficiary_requirement_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
