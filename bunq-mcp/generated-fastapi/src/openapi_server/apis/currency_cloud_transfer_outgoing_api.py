...
from openapi_server.apis.currency_cloud_transfer_outgoing_api_base import BaseCurrencyCloudTransferOutgoingApi
...
from openapi_server.models.currency_cloud_transfer_outgoing_listing import CurrencyCloudTransferOutgoingListing
from openapi_server.models.currency_cloud_transfer_outgoing_read import CurrencyCloudTransferOutgoingRead
...
@router.get(
    "/currency-cloud-transfer-outgoing",
    responses={
        200: {"model": List[CurrencyCloudTransferOutgoingListing], "description": "Used only for serialization purposes (search endpoint) apiAdmin:READ/LISTING apiServiceInternal:READ/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["currency-cloud-transfer-outgoing"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_currency_cloud_transfer_outgoing(
...
) -> List[CurrencyCloudTransferOutgoingListing]:
    """Used only for serialization purposes (search endpoint) apiAdmin:READ/LISTING apiServiceInternal:READ/LISTING"""
    if not BaseCurrencyCloudTransferOutgoingApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseCurrencyCloudTransferOutgoingApi.subclasses[0]().list_all_currency_cloud_transfer_outgoing(user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/currency-cloud-transfer-outgoing/{itemId}",
    responses={
        200: {"model": CurrencyCloudTransferOutgoingRead, "description": "Used only for serialization purposes (search endpoint) apiAdmin:READ/LISTING apiServiceInternal:READ/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["currency-cloud-transfer-outgoing"],
    summary="",
...
async def r_ead_currency_cloud_transfer_outgoing(
    ...
    x_bunq_client_request_id: Annotated[Optional[StrictStr], Field(description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer.")] = Header(None, description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer."),
    x_bunq_geolocation: Annotated[Optional[StrictStr], Field(description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued.")] = Header(None, description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued."),
) -> CurrencyCloudTransferOutgoingRead:
    """Used only for serialization purposes (search endpoint) apiAdmin:READ/LISTING apiServiceInternal:READ/LISTING"""
    if not BaseCurrencyCloudTransferOutgoingApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseCurrencyCloudTransferOutgoingApi.subclasses[0]().r_ead_currency_cloud_transfer_outgoing(itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
