...
from openapi_server.apis.wheel_of_fortune_point_api_base import BaseWheelOfFortunePointApi
...
from openapi_server.models.wheel_of_fortune_point import WheelOfFortunePoint
from openapi_server.models.wheel_of_fortune_point_create import WheelOfFortunePointCreate
from openapi_server.models.wheel_of_fortune_point_read import WheelOfFortunePointRead
...
@router.post(
    "/user/{userID}/wheel-of-fortune-point",
    responses={
        200: {"model": WheelOfFortunePointCreate, "description": "Awards points to a user in the Wheel of Fortune. apiApp:CREATE/READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["wheel-of-fortune-point"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_wheel_of_fortune_point_for_user(
    ...
    wheel_of_fortune_point: WheelOfFortunePoint = Body(None, description=""),
...
) -> WheelOfFortunePointCreate:
    """Awards points to a user in the Wheel of Fortune. apiApp:CREATE/READ"""
    if not BaseWheelOfFortunePointApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseWheelOfFortunePointApi.subclasses[0]().c_reate_wheel_of_fortune_point_for_user(userID, user_agent, x_bunq_client_authentication, wheel_of_fortune_point, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/wheel-of-fortune-point/{itemId}",
    responses={
        200: {"model": WheelOfFortunePointRead, "description": "Awards points to a user in the Wheel of Fortune. apiApp:CREATE/READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["wheel-of-fortune-point"],
    summary="",
...
async def r_ead_wheel_of_fortune_point_for_user(
    userID: StrictInt = Path(..., description=""),
    ...
    x_bunq_client_request_id: Annotated[Optional[StrictStr], Field(description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer.")] = Header(None, description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer."),
    x_bunq_geolocation: Annotated[Optional[StrictStr], Field(description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued.")] = Header(None, description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued."),
) -> WheelOfFortunePointRead:
    """Awards points to a user in the Wheel of Fortune. apiApp:CREATE/READ"""
    if not BaseWheelOfFortunePointApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseWheelOfFortunePointApi.subclasses[0]().r_ead_wheel_of_fortune_point_for_user(userID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
