...
from openapi_server.apis.location_event_zero_fx_api_base import BaseLocationEventZeroFxApi
...
from openapi_server.models.user_location_event_zero_fx_listing import UserLocationEventZeroFxListing
...
@router.get(
    "/user/{userID}/location-event-zero-fx",
    responses={
        200: {"model": List[UserLocationEventZeroFxListing], "description": "ZeroFx location events of the user apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["location-event-zero-fx"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_location_event_zero_fx_for_user(
...
) -> List[UserLocationEventZeroFxListing]:
    """ZeroFx location events of the user apiApp:LISTING"""
    if not BaseLocationEventZeroFxApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseLocationEventZeroFxApi.subclasses[0]().list_all_location_event_zero_fx_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
