...
from openapi_server.apis.carbon_footprint_progress_stats_api_base import BaseCarbonFootprintProgressStatsApi
...
from openapi_server.models.carbon_footprint_progress_stats_listing import CarbonFootprintProgressStatsListing
...
@router.get(
    "/user/{userID}/carbon-footprint-progress-stats",
    responses={
        200: {"model": List[CarbonFootprintProgressStatsListing], "description": "Get monthly carbon emission and offset statistics for a user. apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["carbon-footprint-progress-stats"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_carbon_footprint_progress_stats_for_user(
...
) -> List[CarbonFootprintProgressStatsListing]:
    """Get monthly carbon emission and offset statistics for a user. apiApp:LISTING"""
    if not BaseCarbonFootprintProgressStatsApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseCarbonFootprintProgressStatsApi.subclasses[0]().list_all_carbon_footprint_progress_stats_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
