...
from openapi_server.apis.admin_group_api_base import BaseAdminGroupApi
...
from openapi_server.models.admin_group_listing import AdminGroupListing
from openapi_server.models.admin_group_read import AdminGroupRead
...
@router.get(
    "/user/{userID}/admin-group",
    responses={
        200: {"model": List[AdminGroupListing], "description": "View for the admin group. apiAdmin:LISTING/READ apiServiceInternal:LISTING/READ"},
...
async def list_all_admin_group_for_user(
...
) -> List[AdminGroupListing]:
    """View for the admin group. apiAdmin:LISTING/READ apiServiceInternal:LISTING/READ"""
    if not BaseAdminGroupApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAdminGroupApi.subclasses[0]().list_all_admin_group_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/admin-group/{itemId}",
    responses={
        200: {"model": AdminGroupRead, "description": "View for the admin group. apiAdmin:LISTING/READ apiServiceInternal:LISTING/READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["admin-group"],
    summary="",
...
async def r_ead_admin_group_for_user(
    userID: StrictInt = Path(..., description=""),
    ...
    if not BaseAdminGroupApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAdminGroupApi.subclasses[0]().r_ead_admin_group_for_user(userID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
