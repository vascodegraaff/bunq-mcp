...
from openapi_server.apis.savings_auto_save_entry_api_base import BaseSavingsAutoSaveEntryApi
...
from openapi_server.models.savings_auto_save_entry_read import SavingsAutoSaveEntryRead
...
@router.get(
    "/user/{userID}/monetary-account/{monetary-accountID}/savings-auto-save-entry/{itemId}",
    responses={
        200: {"model": SavingsAutoSaveEntryRead, "description": "Auto Saving entry containing all payments. apiAdmin:READ apiApp:READ apiServiceInternal:READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["savings-auto-save-entry"],
    summary="",
    response_model_by_alias=True,
)
async def r_ead_savings_auto_save_entry_for_user_monetary_account(
...
) -> SavingsAutoSaveEntryRead:
    """Auto Saving entry containing all payments. apiAdmin:READ apiApp:READ apiServiceInternal:READ"""
    if not BaseSavingsAutoSaveEntryApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseSavingsAutoSaveEntryApi.subclasses[0]().r_ead_savings_auto_save_entry_for_user_monetary_account(userID, monetary-accountID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
