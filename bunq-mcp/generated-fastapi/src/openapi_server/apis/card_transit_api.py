...
from openapi_server.apis.card_transit_api_base import BaseCardTransitApi
...
from openapi_server.models.card_transit import CardTransit
from openapi_server.models.card_transit_create import CardTransitCreate
...
@router.post(
    "/user/{userID}/card-transit",
    responses={
        200: {"model": CardTransitCreate, "description": "With bunq it is possible to order transit cards that can then be connected with each one of the monetary accounts the user has access to (including connected accounts). apiTranslink:CREATE"},
...
async def c_reate_card_transit_for_user(
    ...
    card_transit: CardTransit = Body(None, description=""),
    ...
    """Create a new transit card request."""
    if not BaseCardTransitApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseCardTransitApi.subclasses[0]().c_reate_card_transit_for_user(userID, user_agent, x_bunq_client_authentication, card_transit, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
