...
from openapi_server.apis.whitelist_sdd_api_base import BaseWhitelistSddApi
...
from openapi_server.models.whitelist_sdd_listing import WhitelistSddListing
from openapi_server.models.whitelist_sdd_monetary_account_paying_listing import WhitelistSddMonetaryAccountPayingListing
from openapi_server.models.whitelist_sdd_monetary_account_paying_read import WhitelistSddMonetaryAccountPayingRead
from openapi_server.models.whitelist_sdd_read import WhitelistSddRead
...
@router.get(
    ...
    responses={
        200: {"model": List[WhitelistSddListing], "description": "Depreciated route, replaced with whitelist-sdd-recurring apiAdmin:READ/LISTING apiApp:READ/LISTING apiPublic:READ/LISTING apiServiceInternal:READ/LISTING"},
...
) -> List[WhitelistSddListing]:
    """Get a listing of all recurring SDD whitelist entries for a target monetary account."""
    if not BaseWhitelistSddApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseWhitelistSddApi.subclasses[0]().list_all_whitelist_sdd_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/monetary-account/{monetary-accountID}/whitelist-sdd",
    responses={
        200: {"model": List[WhitelistSddMonetaryAccountPayingListing], "description": "Whitelist an SDD so that when one comes in, it is automatically accepted. apiAdmin:READ/LISTING apiApp:READ/LISTING apiPublic:READ/LISTING apiServiceInternal:READ/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["whitelist-sdd"],
    summary="",
...
async def list_all_whitelist_sdd_for_user_monetary_account(
    ...
    x_bunq_client_request_id: Annotated[Optional[StrictStr], Field(description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer.")] = Header(None, description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer."),
    x_bunq_geolocation: Annotated[Optional[StrictStr], Field(description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued.")] = Header(None, description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued."),
) -> List[WhitelistSddMonetaryAccountPayingListing]:
    """Get a listing of all SDD whitelist entries for a target monetary account."""
    if not BaseWhitelistSddApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseWhitelistSddApi.subclasses[0]().list_all_whitelist_sdd_for_user_monetary_account(userID, monetary-accountID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/whitelist-sdd/{itemId}",
    responses={
        200: {"model": WhitelistSddRead, "description": "Depreciated route, replaced with whitelist-sdd-recurring apiAdmin:READ/LISTING apiApp:READ/LISTING apiPublic:READ/LISTING apiServiceInternal:READ/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["whitelist-sdd"],
    summary="",
...
async def r_ead_whitelist_sdd_for_user(
    userID: StrictInt = Path(..., description=""),
    ...
    if not BaseWhitelistSddApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseWhitelistSddApi.subclasses[0]().r_ead_whitelist_sdd_for_user(userID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/monetary-account/{monetary-accountID}/whitelist-sdd/{itemId}",
    responses={
        200: {"model": WhitelistSddMonetaryAccountPayingRead, "description": "Whitelist an SDD so that when one comes in, it is automatically accepted. apiAdmin:READ/LISTING apiApp:READ/LISTING apiPublic:READ/LISTING apiServiceInternal:READ/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["whitelist-sdd"],
    summary="",
...
async def r_ead_whitelist_sdd_for_user_monetary_account(
    userID: StrictInt = Path(..., description=""),
    monetary-accountID: StrictInt = Path(..., description=""),
    itemId: StrictInt = Path(..., description=""),
    user_agent: Annotated[StrictStr, Field(description="The User-Agent header field should contain information about the user agent originating the request. There are no restrictions on the value of this header.")] = Header(None, description="The User-Agent header field should contain information about the user agent originating the request. There are no restrictions on the value of this header."),
    ...
    if not BaseWhitelistSddApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseWhitelistSddApi.subclasses[0]().r_ead_whitelist_sdd_for_user_monetary_account(userID, monetary-accountID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
