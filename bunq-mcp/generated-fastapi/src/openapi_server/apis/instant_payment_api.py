...
from openapi_server.apis.instant_payment_api_base import BaseInstantPaymentApi
...
from openapi_server.models.instant_payment_user_listing import InstantPaymentUserListing
...
@router.get(
    "/user/{userID}/instant-payment",
    responses={
        200: {"model": List[InstantPaymentUserListing], "description": "List instant SEPA payments for a user.. apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["instant-payment"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_instant_payment_for_user(
...
) -> List[InstantPaymentUserListing]:
    """List instant SEPA payments for a user.. apiApp:LISTING"""
    if not BaseInstantPaymentApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseInstantPaymentApi.subclasses[0]().list_all_instant_payment_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
