...
from openapi_server.apis.payment_investigation_incoming_api_base import BasePaymentInvestigationIncomingApi
...
from openapi_server.models.payment_investigation_incoming import PaymentInvestigationIncoming
from openapi_server.models.payment_investigation_incoming_create import PaymentInvestigationIncomingCreate
...
@router.post(
    "/user/{userID}/payment-investigation-incoming",
    responses={
        200: {"model": PaymentInvestigationIncomingCreate, "description": "Create incoming payment investigations initiated by a user. apiApp:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["payment-investigation-incoming"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_payment_investigation_incoming_for_user(
    ...
    payment_investigation_incoming: PaymentInvestigationIncoming = Body(None, description=""),
...
) -> PaymentInvestigationIncomingCreate:
    """Create a new incoming payment investigation using the fields specified in the request body."""
    if not BasePaymentInvestigationIncomingApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BasePaymentInvestigationIncomingApi.subclasses[0]().c_reate_payment_investigation_incoming_for_user(userID, user_agent, x_bunq_client_authentication, payment_investigation_incoming, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
