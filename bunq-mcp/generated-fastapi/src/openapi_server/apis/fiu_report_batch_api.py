...
from openapi_server.apis.fiu_report_batch_api_base import BaseFiuReportBatchApi
...
from openapi_server.models.fiu_report_batch_listing import FiuReportBatchListing
from openapi_server.models.fiu_report_batch_read import FiuReportBatchRead
...
@router.get(
    "/user/{userID}/fiu-report-batch",
    responses={
        200: {"model": List[FiuReportBatchListing], "description": "View for the fiu reporting. apiAdmin:LISTING/READ apiServiceInternal:LISTING/READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["fiu-report-batch"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_fiu_report_batch_for_user(
...
) -> List[FiuReportBatchListing]:
    """View for the fiu reporting. apiAdmin:LISTING/READ apiServiceInternal:LISTING/READ"""
    if not BaseFiuReportBatchApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseFiuReportBatchApi.subclasses[0]().list_all_fiu_report_batch_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/fiu-report-batch/{itemId}",
    responses={
        200: {"model": FiuReportBatchRead, "description": "View for the fiu reporting. apiAdmin:LISTING/READ apiServiceInternal:LISTING/READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["fiu-report-batch"],
    summary="",
...
async def r_ead_fiu_report_batch_for_user(
    userID: StrictInt = Path(..., description=""),
    ...
    if not BaseFiuReportBatchApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseFiuReportBatchApi.subclasses[0]().r_ead_fiu_report_batch_for_user(userID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
