...
from openapi_server.apis.admin_user_video_onboarding_api_base import BaseAdminUserVideoOnboardingApi
...
from openapi_server.models.admin_user_video_onboarding_listing import AdminUserVideoOnboardingListing
from openapi_server.models.admin_user_video_onboarding_read import AdminUserVideoOnboardingRead
...
@router.get(
    "/user/{userID}/admin-user-video-onboarding",
    responses={
        200: {"model": List[AdminUserVideoOnboardingListing], "description": "Admin endpoint for reading and updating video onboarding sessions. apiAdmin:READ/LISTING apiServiceInternal:READ/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["admin-user-video-onboarding"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_admin_user_video_onboarding_for_user(
...
) -> List[AdminUserVideoOnboardingListing]:
    """Admin endpoint for reading and updating video onboarding sessions. apiAdmin:READ/LISTING apiServiceInternal:READ/LISTING"""
    if not BaseAdminUserVideoOnboardingApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAdminUserVideoOnboardingApi.subclasses[0]().list_all_admin_user_video_onboarding_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/admin-user-video-onboarding/{itemId}",
    responses={
        200: {"model": AdminUserVideoOnboardingRead, "description": "Admin endpoint for reading and updating video onboarding sessions. apiAdmin:READ/LISTING apiServiceInternal:READ/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["admin-user-video-onboarding"],
    summary="",
...
async def r_ead_admin_user_video_onboarding_for_user(
    userID: StrictInt = Path(..., description=""),
    ...
    x_bunq_client_request_id: Annotated[Optional[StrictStr], Field(description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer.")] = Header(None, description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer."),
    x_bunq_geolocation: Annotated[Optional[StrictStr], Field(description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued.")] = Header(None, description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued."),
) -> AdminUserVideoOnboardingRead:
    """Admin endpoint for reading and updating video onboarding sessions. apiAdmin:READ/LISTING apiServiceInternal:READ/LISTING"""
    if not BaseAdminUserVideoOnboardingApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAdminUserVideoOnboardingApi.subclasses[0]().r_ead_admin_user_video_onboarding_for_user(userID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
