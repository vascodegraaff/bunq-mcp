...
from openapi_server.apis.traded_ginmon_instrument_api_base import BaseTradedGinmonInstrumentApi
...
from openapi_server.models.user_traded_ginmon_instrument_listing import UserTradedGinmonInstrumentListing
...
@router.get(
    "/user/{userID}/traded-ginmon-instrument",
    responses={
        200: {"model": List[UserTradedGinmonInstrumentListing], "description": "Get the trades me and my friends have made recently. apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["traded-ginmon-instrument"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_traded_ginmon_instrument_for_user(
...
) -> List[UserTradedGinmonInstrumentListing]:
    """Get the trades me and my friends have made recently. apiApp:LISTING"""
    if not BaseTradedGinmonInstrumentApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseTradedGinmonInstrumentApi.subclasses[0]().list_all_traded_ginmon_instrument_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
