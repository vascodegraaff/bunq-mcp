...
from openapi_server.apis.device_browser_api_base import BaseDeviceBrowserApi
...
from openapi_server.models.device_browser import DeviceBrowser
from openapi_server.models.device_browser_create import DeviceBrowserCreate
from openapi_server.models.device_browser_read import DeviceBrowserRead
...
@router.post(
    "/device-browser",
    responses={
        200: {"model": DeviceBrowserCreate, "description": "Used to register a browser. This is an unsigned/verified request. apiApp:CREATE/READ/DELETE apiDeveloperPortal:CREATE/READ/DELETE"},
...
async def c_reate_device_browser(
    ...
    device_browser: DeviceBrowser = Body(None, description=""),
    ...
    """Register a new DeviceBrowser."""
    if not BaseDeviceBrowserApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseDeviceBrowserApi.subclasses[0]().c_reate_device_browser(user_agent, x_bunq_client_authentication, device_browser, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.delete(
    "/device-browser/{itemId}",
    responses={
        200: {"model": object, "description": "Used to register a browser. This is an unsigned/verified request. apiApp:CREATE/READ/DELETE apiDeveloperPortal:CREATE/READ/DELETE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["device-browser"],
    summary="",
...
async def d_elete_device_browser(
    ...
    """Used to register a browser. This is an unsigned/verified request. apiApp:CREATE/READ/DELETE apiDeveloperPortal:CREATE/READ/DELETE"""
    if not BaseDeviceBrowserApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseDeviceBrowserApi.subclasses[0]().d_elete_device_browser(itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/device-browser/{itemId}",
    responses={
        200: {"model": DeviceBrowserRead, "description": "Used to register a browser. This is an unsigned/verified request. apiApp:CREATE/READ/DELETE apiDeveloperPortal:CREATE/READ/DELETE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["device-browser"],
    summary="",
...
async def r_ead_device_browser(
    itemId: StrictInt = Path(..., description=""),
    user_agent: Annotated[StrictStr, Field(description="The User-Agent header field should contain information about the user agent originating the request. There are no restrictions on the value of this header.")] = Header(None, description="The User-Agent header field should contain information about the user agent originating the request. There are no restrictions on the value of this header."),
    x_bunq_client_authentication: Annotated[StrictStr, Field(description="The authentication token is used to authenticate the source of the API call. It is required by all API calls except for POST /v1/installation. It is important to note that the device and session calls are using the token from the response of the installation call, while all the other calls use the token from the response of the session-server call")] = Header(None, description="The authentication token is used to authenticate the source of the API call. It is required by all API calls except for POST /v1/installation. It is important to note that the device and session calls are using the token from the response of the installation call, while all the other calls use the token from the response of the session-server call"),
    cache_control: Annotated[Optional[StrictStr], Field(description="The standard HTTP Cache-Control header is required for all signed requests.")] = Header(None, description="The standard HTTP Cache-Control header is required for all signed requests."),
    ...
    """Get an existing DeviceBrowser."""
    if not BaseDeviceBrowserApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseDeviceBrowserApi.subclasses[0]().r_ead_device_browser(itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
