...
from openapi_server.apis.fraud_registry_netherlands_report_batch_api_base import BaseFraudRegistryNetherlandsReportBatchApi
...
from openapi_server.models.fraud_registry_netherlands_report_batch_listing import FraudRegistryNetherlandsReportBatchListing
...
@router.get(
    "/fraud-registry-netherlands-report-batch",
    responses={
        200: {"model": List[FraudRegistryNetherlandsReportBatchListing], "description": "View to get all EVR report batches. apiAdmin:LISTING apiServiceInternal:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["fraud-registry-netherlands-report-batch"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_fraud_registry_netherlands_report_batch(
...
) -> List[FraudRegistryNetherlandsReportBatchListing]:
    """View to get all EVR report batches. apiAdmin:LISTING apiServiceInternal:LISTING"""
    if not BaseFraudRegistryNetherlandsReportBatchApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseFraudRegistryNetherlandsReportBatchApi.subclasses[0]().list_all_fraud_registry_netherlands_report_batch(user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
