...
from openapi_server.apis.session_reset_identification_verification_api_base import BaseSessionResetIdentificationVerificationApi
...
from openapi_server.models.session_reset_identification_verification import SessionResetIdentificationVerification
from openapi_server.models.session_reset_identification_verification_create import SessionResetIdentificationVerificationCreate
...
@router.post(
    "/session-reset-identification-verification",
    responses={
        200: {"model": SessionResetIdentificationVerificationCreate, "description": "Used by the user to confirm the pin reset request with . apiAdmin:CREATE apiApp:CREATE apiServiceInternal:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["session-reset-identification-verification"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_session_reset_identification_verification(
    ...
    session_reset_identification_verification: SessionResetIdentificationVerification = Body(None, description=""),
...
) -> SessionResetIdentificationVerificationCreate:
    """Used by the user to confirm the pin reset request with . apiAdmin:CREATE apiApp:CREATE apiServiceInternal:CREATE"""
    if not BaseSessionResetIdentificationVerificationApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseSessionResetIdentificationVerificationApi.subclasses[0]().c_reate_session_reset_identification_verification(user_agent, x_bunq_client_authentication, session_reset_identification_verification, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
