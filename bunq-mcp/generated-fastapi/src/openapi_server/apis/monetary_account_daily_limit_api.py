...
from openapi_server.apis.monetary_account_daily_limit_api_base import BaseMonetaryAccountDailyLimitApi
...
from openapi_server.models.monetary_account_daily_limit_read import MonetaryAccountDailyLimitRead
...
@router.get(
    "/monetary-account-daily-limit/{itemId}",
    responses={
        200: {"model": MonetaryAccountDailyLimitRead, "description": "Endpoint used by the search API. apiAdmin:READ apiServiceInternal:READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["monetary-account-daily-limit"],
    summary="",
    response_model_by_alias=True,
)
async def r_ead_monetary_account_daily_limit(
    ...
    if not BaseMonetaryAccountDailyLimitApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseMonetaryAccountDailyLimitApi.subclasses[0]().r_ead_monetary_account_daily_limit(itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
