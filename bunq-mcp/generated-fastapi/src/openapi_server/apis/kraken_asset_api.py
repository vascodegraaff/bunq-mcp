...
from openapi_server.apis.kraken_asset_api_base import BaseKrakenAssetApi
...
from openapi_server.models.kraken_asset_listing import KrakenAssetListing
from openapi_server.models.kraken_asset_read import KrakenAssetRead
...
@router.get(
    "/user/{userID}/kraken-asset",
    responses={
        200: {"model": List[KrakenAssetListing], "description": "Endpoint for read/listing assets of Kraken. apiApp:READ/LISTING"},
...
async def list_all_kraken_asset_for_user(
...
) -> List[KrakenAssetListing]:
    """Endpoint for read/listing assets of Kraken. apiApp:READ/LISTING"""
    if not BaseKrakenAssetApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseKrakenAssetApi.subclasses[0]().list_all_kraken_asset_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/kraken-asset/{itemId}",
    responses={
        200: {"model": KrakenAssetRead, "description": "Endpoint for read/listing assets of Kraken. apiApp:READ/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["kraken-asset"],
    summary="",
...
async def r_ead_kraken_asset_for_user(
    userID: StrictInt = Path(..., description=""),
    ...
    if not BaseKrakenAssetApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseKrakenAssetApi.subclasses[0]().r_ead_kraken_asset_for_user(userID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
