...
from openapi_server.apis.ticket_queue_stats_api_base import BaseTicketQueueStatsApi
...
from openapi_server.models.user_ticket_queue_stats_listing import UserTicketQueueStatsListing
...
@router.get(
    "/user/{userID}/ticket-queue-stats",
    responses={
        200: {"model": List[UserTicketQueueStatsListing], "description": "Get the estimated waiting time for tickets. apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["ticket-queue-stats"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_ticket_queue_stats_for_user(
...
) -> List[UserTicketQueueStatsListing]:
    """Get the estimated waiting time for tickets. apiApp:LISTING"""
    if not BaseTicketQueueStatsApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseTicketQueueStatsApi.subclasses[0]().list_all_ticket_queue_stats_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
