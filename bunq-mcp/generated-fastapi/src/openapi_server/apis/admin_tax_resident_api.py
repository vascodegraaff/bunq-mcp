...
from openapi_server.apis.admin_tax_resident_api_base import BaseAdminTaxResidentApi
...
from openapi_server.models.admin_tax_resident import AdminTaxResident
from openapi_server.models.admin_tax_resident_create import AdminTaxResidentCreate
...
@router.post(
    "/user/{userID}/admin-tax-resident",
    responses={
        200: {"model": AdminTaxResidentCreate, "description": "Manage a user&#39;s TINs apiAdmin:CREATE/DELETE apiServiceInternal:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["admin-tax-resident"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_admin_tax_resident_for_user(
    ...
    admin_tax_resident: AdminTaxResident = Body(None, description=""),
...
) -> AdminTaxResidentCreate:
    """Manage a user&#39;s TINs apiAdmin:CREATE/DELETE apiServiceInternal:CREATE"""
    if not BaseAdminTaxResidentApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAdminTaxResidentApi.subclasses[0]().c_reate_admin_tax_resident_for_user(userID, user_agent, x_bunq_client_authentication, admin_tax_resident, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.delete(
    "/user/{userID}/admin-tax-resident/{itemId}",
    responses={
        200: {"model": object, "description": "Manage a user&#39;s TINs apiAdmin:CREATE/DELETE apiServiceInternal:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["admin-tax-resident"],
    summary="",
...
async def d_elete_admin_tax_resident_for_user(
    userID: StrictInt = Path(..., description=""),
    ...
    if not BaseAdminTaxResidentApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAdminTaxResidentApi.subclasses[0]().d_elete_admin_tax_resident_for_user(userID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
