...
from openapi_server.apis.replace_api_base import BaseReplaceApi
...
@router.post(
    ...
    responses={
        200: {"model": CardReplaceCreate, "description": "It is possible to order a card replacement with the bunq API.&lt;br/&gt;&lt;br/&gt;You can order up to one free card replacement per year. Additional replacement requests will be billed.&lt;br/&gt;&lt;br/&gt;The card replacement will have the same expiry date and the same pricing as the old card, but it will have a new card number. You can change the description and optional the PIN through the card replacement endpoint. apiApp:CREATE apiPublic:CREATE"},
...
async def c_reate_replace_for_user_card(
    ...
    card_replace: CardReplace = Body(None, description=""),
    ...
    if not BaseReplaceApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseReplaceApi.subclasses[0]().c_reate_replace_for_user_card(userID, cardID, user_agent, x_bunq_client_authentication, card_replace, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
