...
from openapi_server.apis.adyen_merchant_stored_payment_method_api_base import BaseAdyenMerchantStoredPaymentMethodApi
...
@router.delete(
    "/user/{userID}/adyen-merchant-stored-payment-method/{itemId}",
    responses={
        200: {"model": object, "description": "Delete stored payment methods. apiAdmin:DELETE apiApp:DELETE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["adyen-merchant-stored-payment-method"],
    summary="",
    response_model_by_alias=True,
)
async def d_elete_adyen_merchant_stored_payment_method_for_user(
    ...
    """Delete stored payment methods. apiAdmin:DELETE apiApp:DELETE"""
    if not BaseAdyenMerchantStoredPaymentMethodApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAdyenMerchantStoredPaymentMethodApi.subclasses[0]().d_elete_adyen_merchant_stored_payment_method_for_user(userID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
