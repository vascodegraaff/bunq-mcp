...
from openapi_server.apis.admin_attachment_comment_api_base import BaseAdminAttachmentCommentApi
...
from openapi_server.models.admin_attachment_comment_create import AdminAttachmentCommentCreate
from openapi_server.models.admin_attachment_comment_read import AdminAttachmentCommentRead
...
@router.post(
    "/user/{userID}/admin-attachment-comment",
    responses={
        200: {"model": AdminAttachmentCommentCreate, "description": "This call is used to upload a comment attachment. Attachments are submitted base64 encoded. apiAdmin:CREATE/READ apiServiceInternal:CREATE/READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["admin-attachment-comment"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_admin_attachment_comment_for_user(
...
) -> AdminAttachmentCommentCreate:
    """Create a new comment attachment. Create a POST request with a body that is the raw base64 content of the image. Make sure you define the MIME type (i.e. image/jpeg) in the header."""
    if not BaseAdminAttachmentCommentApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAdminAttachmentCommentApi.subclasses[0]().c_reate_admin_attachment_comment_for_user(userID, user_agent, x_bunq_client_authentication, body, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/admin-attachment-comment/{itemId}",
    responses={
        200: {"model": AdminAttachmentCommentRead, "description": "This call is used to upload a comment attachment. Attachments are submitted base64 encoded. apiAdmin:CREATE/READ apiServiceInternal:CREATE/READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["admin-attachment-comment"],
    summary="",
...
async def r_ead_admin_attachment_comment_for_user(
    userID: StrictInt = Path(..., description=""),
    ...
    x_bunq_client_request_id: Annotated[Optional[StrictStr], Field(description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer.")] = Header(None, description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer."),
    x_bunq_geolocation: Annotated[Optional[StrictStr], Field(description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued.")] = Header(None, description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued."),
) -> AdminAttachmentCommentRead:
    """This call is used to upload a comment attachment. Attachments are submitted base64 encoded. apiAdmin:CREATE/READ apiServiceInternal:CREATE/READ"""
    if not BaseAdminAttachmentCommentApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAdminAttachmentCommentApi.subclasses[0]().r_ead_admin_attachment_comment_for_user(userID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
