...
from openapi_server.apis.ginmon_account_valuation_weekly_api_base import BaseGinmonAccountValuationWeeklyApi
...
from openapi_server.models.ginmon_account_valuation_weekly_listing import GinmonAccountValuationWeeklyListing
from openapi_server.models.ginmon_account_valuation_weekly_read import GinmonAccountValuationWeeklyRead
...
@router.get(
    "/user/{userID}/ginmon-account-valuation-weekly",
    responses={
        200: {"model": List[GinmonAccountValuationWeeklyListing], "description": "Endpoint to fetch the details from a User&#39;s account valuation weekly. apiApp:LISTING/READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["ginmon-account-valuation-weekly"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_ginmon_account_valuation_weekly_for_user(
...
) -> List[GinmonAccountValuationWeeklyListing]:
    """Endpoint to fetch the details from a User&#39;s account valuation weekly. apiApp:LISTING/READ"""
    if not BaseGinmonAccountValuationWeeklyApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseGinmonAccountValuationWeeklyApi.subclasses[0]().list_all_ginmon_account_valuation_weekly_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/ginmon-account-valuation-weekly/{itemId}",
    responses={
        200: {"model": GinmonAccountValuationWeeklyRead, "description": "Endpoint to fetch the details from a User&#39;s account valuation weekly. apiApp:LISTING/READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["ginmon-account-valuation-weekly"],
    summary="",
...
async def r_ead_ginmon_account_valuation_weekly_for_user(
    userID: StrictInt = Path(..., description=""),
    ...
    x_bunq_client_request_id: Annotated[Optional[StrictStr], Field(description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer.")] = Header(None, description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer."),
    x_bunq_geolocation: Annotated[Optional[StrictStr], Field(description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued.")] = Header(None, description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued."),
) -> GinmonAccountValuationWeeklyRead:
    """Endpoint to fetch the details from a User&#39;s account valuation weekly. apiApp:LISTING/READ"""
    if not BaseGinmonAccountValuationWeeklyApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseGinmonAccountValuationWeeklyApi.subclasses[0]().r_ead_ginmon_account_valuation_weekly_for_user(userID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
