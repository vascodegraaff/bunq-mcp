...
from openapi_server.apis.currency_cloud_purpose_code_api_base import BaseCurrencyCloudPurposeCodeApi
...
from openapi_server.models.currency_cloud_purpose_code_listing import CurrencyCloudPurposeCodeListing
...
@router.get(
    "/currency-cloud-purpose-code",
    responses={
        200: {"model": List[CurrencyCloudPurposeCodeListing], "description": "Endpoint to get a list of purpose codes available for CurrencyCloud payments. apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["currency-cloud-purpose-code"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_currency_cloud_purpose_code(
...
) -> List[CurrencyCloudPurposeCodeListing]:
    """Endpoint to get a list of purpose codes available for CurrencyCloud payments. apiApp:LISTING"""
    if not BaseCurrencyCloudPurposeCodeApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseCurrencyCloudPurposeCodeApi.subclasses[0]().list_all_currency_cloud_purpose_code(user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
