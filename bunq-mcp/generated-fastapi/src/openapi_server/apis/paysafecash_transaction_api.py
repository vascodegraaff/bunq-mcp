...
from openapi_server.apis.paysafecash_transaction_api_base import BasePaysafecashTransactionApi
...
from openapi_server.models.paysafecash_transaction_user_listing import PaysafecashTransactionUserListing
...
@router.get(
    "/user/{userID}/paysafecash-transaction",
    responses={
        200: {"model": List[PaysafecashTransactionUserListing], "description": "Used to get a list of transactions initiated with Paysafecash. apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["paysafecash-transaction"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_paysafecash_transaction_for_user(
...
) -> List[PaysafecashTransactionUserListing]:
    """Used to get a list of transactions initiated with Paysafecash. apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING"""
    if not BasePaysafecashTransactionApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BasePaysafecashTransactionApi.subclasses[0]().list_all_paysafecash_transaction_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
