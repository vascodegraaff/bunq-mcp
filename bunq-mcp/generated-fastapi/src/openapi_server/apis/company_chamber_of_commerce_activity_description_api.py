...
from openapi_server.apis.company_chamber_of_commerce_activity_description_api_base import BaseCompanyChamberOfCommerceActivityDescriptionApi
...
from openapi_server.models.company_chamber_of_commerce_activity_description_listing import CompanyChamberOfCommerceActivityDescriptionListing
...
@router.get(
    "/company-chamber-of-commerce-activity-description",
    responses={
        200: {"model": List[CompanyChamberOfCommerceActivityDescriptionListing], "description": "Get all the available company categories. apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["company-chamber-of-commerce-activity-description"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_company_chamber_of_commerce_activity_description(
...
) -> List[CompanyChamberOfCommerceActivityDescriptionListing]:
    """Get all the available company categories. apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING"""
    if not BaseCompanyChamberOfCommerceActivityDescriptionApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseCompanyChamberOfCommerceActivityDescriptionApi.subclasses[0]().list_all_company_chamber_of_commerce_activity_description(user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
