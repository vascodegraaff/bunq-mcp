...
from openapi_server.apis.currency_cloud_transfer_incoming_api_base import BaseCurrencyCloudTransferIncomingApi
...
from openapi_server.models.currency_cloud_transfer_incoming_listing import CurrencyCloudTransferIncomingListing
from openapi_server.models.currency_cloud_transfer_incoming_read import CurrencyCloudTransferIncomingRead
...
@router.get(
    "/currency-cloud-transfer-incoming",
    responses={
        200: {"model": List[CurrencyCloudTransferIncomingListing], "description": "Used only for serialization purposes (search endpoint) apiAdmin:READ/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["currency-cloud-transfer-incoming"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_currency_cloud_transfer_incoming(
...
) -> List[CurrencyCloudTransferIncomingListing]:
    """Used only for serialization purposes (search endpoint) apiAdmin:READ/LISTING"""
    if not BaseCurrencyCloudTransferIncomingApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseCurrencyCloudTransferIncomingApi.subclasses[0]().list_all_currency_cloud_transfer_incoming(user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/currency-cloud-transfer-incoming/{itemId}",
    responses={
        200: {"model": CurrencyCloudTransferIncomingRead, "description": "Used only for serialization purposes (search endpoint) apiAdmin:READ/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["currency-cloud-transfer-incoming"],
    summary="",
...
async def r_ead_currency_cloud_transfer_incoming(
    ...
    x_bunq_client_request_id: Annotated[Optional[StrictStr], Field(description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer.")] = Header(None, description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer."),
    x_bunq_geolocation: Annotated[Optional[StrictStr], Field(description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued.")] = Header(None, description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued."),
) -> CurrencyCloudTransferIncomingRead:
    """Used only for serialization purposes (search endpoint) apiAdmin:READ/LISTING"""
    if not BaseCurrencyCloudTransferIncomingApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseCurrencyCloudTransferIncomingApi.subclasses[0]().r_ead_currency_cloud_transfer_incoming(itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
