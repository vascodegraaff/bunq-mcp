...
from openapi_server.apis.ticket_membership_api_base import BaseTicketMembershipApi
...
from openapi_server.models.ticket_membership_for_user_listing import TicketMembershipForUserListing
...
@router.get(
    "/user/{userID}/ticket-membership",
    responses={
        200: {"model": List[TicketMembershipForUserListing], "description": "Get all the tickets for the user. apiAdmin:LISTING apiServiceInternal:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["ticket-membership"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_ticket_membership_for_user(
...
) -> List[TicketMembershipForUserListing]:
    """Get all the tickets for the user. apiAdmin:LISTING apiServiceInternal:LISTING"""
    if not BaseTicketMembershipApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseTicketMembershipApi.subclasses[0]().list_all_ticket_membership_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
