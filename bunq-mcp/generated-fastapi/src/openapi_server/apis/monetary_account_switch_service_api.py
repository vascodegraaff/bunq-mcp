...
from openapi_server.apis.monetary_account_switch_service_api_base import BaseMonetaryAccountSwitchServiceApi
...
from openapi_server.models.monetary_account_switch_service_read import MonetaryAccountSwitchServiceRead
...
@router.get(
    "/user/{userID}/monetary-account-switch-service/{itemId}",
    responses={
        200: {"model": MonetaryAccountSwitchServiceRead, "description": "View for MonetaryAccountSwitchService apiAdmin:READ apiServiceInternal:READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["monetary-account-switch-service"],
    summary="",
    response_model_by_alias=True,
)
async def r_ead_monetary_account_switch_service_for_user(
...
) -> MonetaryAccountSwitchServiceRead:
    """View for MonetaryAccountSwitchService apiAdmin:READ apiServiceInternal:READ"""
    if not BaseMonetaryAccountSwitchServiceApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseMonetaryAccountSwitchServiceApi.subclasses[0]().r_ead_monetary_account_switch_service_for_user(userID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
