...
from openapi_server.apis.bunqme_merchant_request_checkout_fee_api_base import BaseBunqmeMerchantRequestCheckoutFeeApi
...
from openapi_server.models.bunq_me_merchant_request_checkout_fee import BunqMeMerchantRequestCheckoutFee
from openapi_server.models.bunq_me_merchant_request_checkout_fee_create import BunqMeMerchantRequestCheckoutFeeCreate
...
@router.post(
    "/bunqme-merchant-request-checkout-fee",
    responses={
        200: {"model": BunqMeMerchantRequestCheckoutFeeCreate, "description": "Used to calculated the fee we will charge for the transaction. apiBunqMe:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["bunqme-merchant-request-checkout-fee"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_bunqme_merchant_request_checkout_fee(
    ...
    bunq_me_merchant_request_checkout_fee: BunqMeMerchantRequestCheckoutFee = Body(None, description=""),
...
) -> BunqMeMerchantRequestCheckoutFeeCreate:
    """Used to calculated the fee we will charge for the transaction."""
    if not BaseBunqmeMerchantRequestCheckoutFeeApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseBunqmeMerchantRequestCheckoutFeeApi.subclasses[0]().c_reate_bunqme_merchant_request_checkout_fee(user_agent, x_bunq_client_authentication, bunq_me_merchant_request_checkout_fee, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
