...
from openapi_server.apis.tricount_bancontact_signature_api_base import BaseTricountBancontactSignatureApi
...
from openapi_server.models.tricount_bancontact_signature import TricountBancontactSignature
from openapi_server.models.tricount_bancontact_signature_create import TricountBancontactSignatureCreate
...
@router.post(
    ...
    responses={
        200: {"model": TricountBancontactSignatureCreate, "description": "Called by the Tricount app to sign their Bancontact requests to allow peer-to-peer payments. apiApp:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["tricount-bancontact-signature"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_tricount_bancontact_signature(
    ...
    tricount_bancontact_signature: TricountBancontactSignature = Body(None, description=""),
...
) -> TricountBancontactSignatureCreate:
    """Called by the Tricount app to sign their Bancontact requests to allow peer-to-peer payments. apiApp:CREATE"""
    if not BaseTricountBancontactSignatureApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseTricountBancontactSignatureApi.subclasses[0]().c_reate_tricount_bancontact_signature(user_agent, x_bunq_client_authentication, tricount_bancontact_signature, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
