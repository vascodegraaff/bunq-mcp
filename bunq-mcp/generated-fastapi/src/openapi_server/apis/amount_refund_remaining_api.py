...
from openapi_server.apis.amount_refund_remaining_api_base import BaseAmountRefundRemainingApi
...
from openapi_server.models.adyen_card_transaction_amount_refund_remaining_listing import AdyenCardTransactionAmountRefundRemainingListing
...
@router.get(
    "/user/{userID}/monetary-account/{monetary-accountID}/adyen-card-transaction/{adyen-card-transactionID}/amount-refund-remaining",
    responses={
        200: {"model": List[AdyenCardTransactionAmountRefundRemainingListing], "description": "Get the amount that can still be refunded for a Tap to Pay payment apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["amount-refund-remaining"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_amount_refund_remaining_for_user_monetary_account_adyen_card_transaction(
...
) -> List[AdyenCardTransactionAmountRefundRemainingListing]:
    """Get the amount that can still be refunded for a Tap to Pay payment apiApp:LISTING"""
    if not BaseAmountRefundRemainingApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAmountRefundRemainingApi.subclasses[0]().list_all_amount_refund_remaining_for_user_monetary_account_adyen_card_transaction(userID, monetary-accountID, adyen-card-transactionID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
