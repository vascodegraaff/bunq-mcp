...
from openapi_server.apis.confirmation_alias_api_base import BaseConfirmationAliasApi
...
from openapi_server.models.confirmation_alias_listing import ConfirmationAliasListing
...
@router.get(
    "/confirmation-alias",
    responses={
        200: {"model": List[ConfirmationAliasListing], "description": "Used to view confirmation aliases in Retool through Search. apiAdmin:LISTING apiServiceInternal:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["confirmation-alias"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_confirmation_alias(
...
) -> List[ConfirmationAliasListing]:
    """Used to view confirmation aliases in Retool through Search. apiAdmin:LISTING apiServiceInternal:LISTING"""
    if not BaseConfirmationAliasApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseConfirmationAliasApi.subclasses[0]().list_all_confirmation_alias(user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
