...
from openapi_server.apis.card_limited_edition_remaining_public_api_base import BaseCardLimitedEditionRemainingPublicApi
...
from openapi_server.models.card_limited_edition_remaining_public_listing import CardLimitedEditionRemainingPublicListing
...
@router.get(
    "/card-limited-edition-remaining-public",
    responses={
        200: {"model": List[CardLimitedEditionRemainingPublicListing], "description": "Lists number of remaining limited edition cards. apiWebsite:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["card-limited-edition-remaining-public"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_card_limited_edition_remaining_public(
...
) -> List[CardLimitedEditionRemainingPublicListing]:
    """Lists number of remaining limited edition cards. apiWebsite:LISTING"""
    if not BaseCardLimitedEditionRemainingPublicApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseCardLimitedEditionRemainingPublicApi.subclasses[0]().list_all_card_limited_edition_remaining_public(user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
