...
from openapi_server.apis.savings_auto_save_api_base import BaseSavingsAutoSaveApi
...
from openapi_server.models.savings_auto_save_listing import SavingsAutoSaveListing
from openapi_server.models.savings_auto_save_read import SavingsAutoSaveRead
...
@router.get(
    "/user/{userID}/savings-auto-save",
    responses={
        200: {"model": List[SavingsAutoSaveListing], "description": "Auto Saving hold auto save settings. apiAdmin:READ apiApp:READ/LISTING apiServiceInternal:READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["savings-auto-save"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_savings_auto_save_for_user(
...
) -> List[SavingsAutoSaveListing]:
    """Auto Saving hold auto save settings. apiAdmin:READ apiApp:READ/LISTING apiServiceInternal:READ"""
    if not BaseSavingsAutoSaveApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseSavingsAutoSaveApi.subclasses[0]().list_all_savings_auto_save_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/savings-auto-save/{itemId}",
    responses={
        200: {"model": SavingsAutoSaveRead, "description": "Auto Saving hold auto save settings. apiAdmin:READ apiApp:READ/LISTING apiServiceInternal:READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["savings-auto-save"],
    summary="",
...
async def r_ead_savings_auto_save_for_user(
    userID: StrictInt = Path(..., description=""),
    ...
    if not BaseSavingsAutoSaveApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseSavingsAutoSaveApi.subclasses[0]().r_ead_savings_auto_save_for_user(userID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
