...
from openapi_server.apis.user_audit_entry_api_base import BaseUserAuditEntryApi
...
from openapi_server.models.user_audit_entry_listing import UserAuditEntryListing
from openapi_server.models.user_audit_entry_read import UserAuditEntryRead
...
@router.get(
    "/user/{userID}/user-audit-entry",
    responses={
        200: {"model": List[UserAuditEntryListing], "description": "View that provides all models necessary to investigate a user. apiAdmin:READ/LISTING apiServiceInternal:READ/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["user-audit-entry"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_user_audit_entry_for_user(
...
) -> List[UserAuditEntryListing]:
    """View that provides all models necessary to investigate a user. apiAdmin:READ/LISTING apiServiceInternal:READ/LISTING"""
    if not BaseUserAuditEntryApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseUserAuditEntryApi.subclasses[0]().list_all_user_audit_entry_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/user-audit-entry/{itemId}",
    responses={
        200: {"model": UserAuditEntryRead, "description": "View that provides all models necessary to investigate a user. apiAdmin:READ/LISTING apiServiceInternal:READ/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["user-audit-entry"],
    summary="",
...
async def r_ead_user_audit_entry_for_user(
    userID: StrictInt = Path(..., description=""),
    ...
    if not BaseUserAuditEntryApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseUserAuditEntryApi.subclasses[0]().r_ead_user_audit_entry_for_user(userID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
