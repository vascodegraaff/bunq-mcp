...
from openapi_server.apis.insight_auto_allocate_api_base import BaseInsightAutoAllocateApi
...
from openapi_server.models.insight_auto_allocate_listing import InsightAutoAllocateListing
...
@router.get(
    "/user/{userID}/insight-auto-allocate",
    responses={
        200: {"model": List[InsightAutoAllocateListing], "description": "The insight auto allocate endpoint apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["insight-auto-allocate"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_insight_auto_allocate_for_user(
...
) -> List[InsightAutoAllocateListing]:
    """The insight auto allocate endpoint apiApp:LISTING"""
    if not BaseInsightAutoAllocateApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseInsightAutoAllocateApi.subclasses[0]().list_all_insight_auto_allocate_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
