...
from openapi_server.apis.invite_user_available_api_base import BaseInviteUserAvailableApi
...
from openapi_server.models.invite_user_available_listing import InviteUserAvailableListing
...
@router.get(
    "/user/{userID}/invite-user-available",
    responses={
        200: {"model": List[InviteUserAvailableListing], "description": "Get the types of invites that the user can send. apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["invite-user-available"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_invite_user_available_for_user(
...
) -> List[InviteUserAvailableListing]:
    """Get the types of invites that the user can send. apiApp:LISTING"""
    if not BaseInviteUserAvailableApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseInviteUserAvailableApi.subclasses[0]().list_all_invite_user_available_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
