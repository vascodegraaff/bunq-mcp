...
from openapi_server.apis.ginmon_account_identification_attachment_api_base import BaseGinmonAccountIdentificationAttachmentApi
...
from openapi_server.models.ginmon_account_identification_attachment_listing import GinmonAccountIdentificationAttachmentListing
...
@router.get(
    "/user/{userID}/ginmon-account/{ginmon-accountID}/ginmon-account-identification-attachment",
    responses={
        200: {"model": List[GinmonAccountIdentificationAttachmentListing], "description": "Proof of identity used for the Ginmon account. apiAdmin:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["ginmon-account-identification-attachment"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_ginmon_account_identification_attachment_for_user_ginmon_account(
...
) -> List[GinmonAccountIdentificationAttachmentListing]:
    """Proof of identity used for the Ginmon account. apiAdmin:LISTING"""
    if not BaseGinmonAccountIdentificationAttachmentApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseGinmonAccountIdentificationAttachmentApi.subclasses[0]().list_all_ginmon_account_identification_attachment_for_user_ginmon_account(userID, ginmon-accountID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
