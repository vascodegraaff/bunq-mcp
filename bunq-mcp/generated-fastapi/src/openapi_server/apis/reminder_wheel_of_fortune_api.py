...
from openapi_server.apis.reminder_wheel_of_fortune_api_base import BaseReminderWheelOfFortuneApi
...
from openapi_server.models.reminder_wheel_of_fortune_listing import ReminderWheelOfFortuneListing
...
@router.get(
    "/user/{userID}/reminder-wheel-of-fortune",
    responses={
        200: {"model": List[ReminderWheelOfFortuneListing], "description": "Reminders for wheel of fortune, Only for serialization. apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["reminder-wheel-of-fortune"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_reminder_wheel_of_fortune_for_user(
...
) -> List[ReminderWheelOfFortuneListing]:
    """Reminders for wheel of fortune, Only for serialization. apiApp:LISTING"""
    if not BaseReminderWheelOfFortuneApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseReminderWheelOfFortuneApi.subclasses[0]().list_all_reminder_wheel_of_fortune_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
