...
from openapi_server.apis.token_confirmation_login_token_api_base import BaseTokenConfirmationLoginTokenApi
...
from openapi_server.models.token_confirmation_login_token import TokenConfirmationLoginToken
from openapi_server.models.token_confirmation_login_token_create import TokenConfirmationLoginTokenCreate
...
@router.post(
    "/token-confirmation-login-token",
    responses={
        200: {"model": TokenConfirmationLoginTokenCreate, "description": "Used to authenticate using a token. apiAdmin:CREATE apiApp:CREATE apiServiceInternal:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["token-confirmation-login-token"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_token_confirmation_login_token(
    ...
    token_confirmation_login_token: TokenConfirmationLoginToken = Body(None, description=""),
...
) -> TokenConfirmationLoginTokenCreate:
    """Used to authenticate using a token. apiAdmin:CREATE apiApp:CREATE apiServiceInternal:CREATE"""
    if not BaseTokenConfirmationLoginTokenApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseTokenConfirmationLoginTokenApi.subclasses[0]().c_reate_token_confirmation_login_token(user_agent, x_bunq_client_authentication, token_confirmation_login_token, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
