...
from openapi_server.apis.birdee_investment_portfolio_fee_api_base import BaseBirdeeInvestmentPortfolioFeeApi
...
from openapi_server.models.birdee_investment_portfolio_fee_read import BirdeeInvestmentPortfolioFeeRead
...
@router.get(
    "/user/{userID}/monetary-account-investment/{monetary-account-investmentID}/birdee-investment-portfolio-fee/{itemId}",
    responses={
        200: {"model": BirdeeInvestmentPortfolioFeeRead, "description": "Endpoint for fetching the montlhy fee charged to the user.. apiApp:READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["birdee-investment-portfolio-fee"],
    summary="",
    response_model_by_alias=True,
)
async def r_ead_birdee_investment_portfolio_fee_for_user_monetary_account_investment(
    ...
    """Endpoint for fetching the montlhy fee charged to the user.. apiApp:READ"""
    if not BaseBirdeeInvestmentPortfolioFeeApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseBirdeeInvestmentPortfolioFeeApi.subclasses[0]().r_ead_birdee_investment_portfolio_fee_for_user_monetary_account_investment(userID, monetary-account-investmentID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
