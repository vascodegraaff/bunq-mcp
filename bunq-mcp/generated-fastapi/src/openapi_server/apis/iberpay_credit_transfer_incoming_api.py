...
from openapi_server.apis.iberpay_credit_transfer_incoming_api_base import BaseIberpayCreditTransferIncomingApi
...
@router.get(
    "/iberpay-credit-transfer-incoming/{itemId}",
    responses={
        200: {"model": object, "description": "Endpoint used by the search API. apiAdmin:READ apiServiceInternal:READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["iberpay-credit-transfer-incoming"],
    summary="",
    response_model_by_alias=True,
)
async def r_ead_iberpay_credit_transfer_incoming(
    ...
    if not BaseIberpayCreditTransferIncomingApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseIberpayCreditTransferIncomingApi.subclasses[0]().r_ead_iberpay_credit_transfer_incoming(itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
