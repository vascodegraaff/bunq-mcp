...
from openapi_server.apis.point_prize_api_base import BasePointPrizeApi
...
from openapi_server.models.point_prize_listing import PointPrizeListing
...
@router.get(
    "/user/{userID}/point-prize",
    responses={
        200: {"model": List[PointPrizeListing], "description": "Get available prizes for Points. apiAdmin:LISTING apiApp:LISTING"},
...
async def list_all_point_prize_for_user(
...
) -> List[PointPrizeListing]:
    """Get available prizes for Points. apiAdmin:LISTING apiApp:LISTING"""
    if not BasePointPrizeApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BasePointPrizeApi.subclasses[0]().list_all_point_prize_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
