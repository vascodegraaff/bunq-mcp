...
from openapi_server.apis.attachment_report_notification_api_base import BaseAttachmentReportNotificationApi
...
from openapi_server.models.attachment_report_notification_read import AttachmentReportNotificationRead
...
@router.get(
    "/attachment-report-notification/{itemId}",
    responses={
        200: {"model": AttachmentReportNotificationRead, "description": "Endpoint used to retrieve notification model for the Attachment Report. apiAdmin:READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["attachment-report-notification"],
    summary="",
    response_model_by_alias=True,
)
async def r_ead_attachment_report_notification(
...
) -> AttachmentReportNotificationRead:
    """Endpoint used to retrieve notification model for the Attachment Report. apiAdmin:READ"""
    if not BaseAttachmentReportNotificationApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAttachmentReportNotificationApi.subclasses[0]().r_ead_attachment_report_notification(itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
