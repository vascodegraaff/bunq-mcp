...
from openapi_server.apis.interest_payout_api_base import BaseInterestPayoutApi
...
from openapi_server.models.interest_payout_listing import InterestPayoutListing
from openapi_server.models.interest_payout_read import InterestPayoutRead
from openapi_server.models.interest_payout_user_listing import InterestPayoutUserListing
...
@router.get(
    "/user/{userID}/interest-payout",
    responses={
        200: {"model": List[InterestPayoutUserListing], "description": "Interest payout details. apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["interest-payout"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_interest_payout_for_user(
...
) -> List[InterestPayoutUserListing]:
    """Interest payout details. apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING"""
    if not BaseInterestPayoutApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseInterestPayoutApi.subclasses[0]().list_all_interest_payout_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/monetary-account/{monetary-accountID}/interest-payout",
    responses={
        200: {"model": List[InterestPayoutListing], "description": "Interest payout details. apiAdmin:READ/LISTING apiApp:READ/LISTING apiServiceInternal:READ/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["interest-payout"],
    summary="",
...
async def list_all_interest_payout_for_user_monetary_account(
    userID: StrictInt = Path(..., description=""),
    ...
    x_bunq_client_request_id: Annotated[Optional[StrictStr], Field(description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer.")] = Header(None, description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer."),
    x_bunq_geolocation: Annotated[Optional[StrictStr], Field(description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued.")] = Header(None, description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued."),
) -> List[InterestPayoutListing]:
    """Interest payout details. apiAdmin:READ/LISTING apiApp:READ/LISTING apiServiceInternal:READ/LISTING"""
    if not BaseInterestPayoutApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseInterestPayoutApi.subclasses[0]().list_all_interest_payout_for_user_monetary_account(userID, monetary-accountID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/monetary-account/{monetary-accountID}/interest-payout/{itemId}",
    responses={
        200: {"model": InterestPayoutRead, "description": "Interest payout details. apiAdmin:READ/LISTING apiApp:READ/LISTING apiServiceInternal:READ/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["interest-payout"],
    summary="",
...
async def r_ead_interest_payout_for_user_monetary_account(
    userID: StrictInt = Path(..., description=""),
    monetary-accountID: StrictInt = Path(..., description=""),
    ...
    if not BaseInterestPayoutApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseInterestPayoutApi.subclasses[0]().r_ead_interest_payout_for_user_monetary_account(userID, monetary-accountID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
