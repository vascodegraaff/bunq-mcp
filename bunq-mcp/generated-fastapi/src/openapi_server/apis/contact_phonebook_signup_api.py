...
from openapi_server.apis.contact_phonebook_signup_api_base import BaseContactPhonebookSignupApi
...
from openapi_server.models.contact_phonebook_signup_read import ContactPhonebookSignupRead
...
@router.get(
    "/user/{userID}/contact-phonebook-signup/{itemId}",
    responses={
        200: {"model": ContactPhonebookSignupRead, "description": "View to manage contacts in your phonebook that have signed up to bunq. apiApp:READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["contact-phonebook-signup"],
    summary="",
    response_model_by_alias=True,
)
async def r_ead_contact_phonebook_signup_for_user(
...
) -> ContactPhonebookSignupRead:
    """View to manage contacts in your phonebook that have signed up to bunq. apiApp:READ"""
    if not BaseContactPhonebookSignupApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseContactPhonebookSignupApi.subclasses[0]().r_ead_contact_phonebook_signup_for_user(userID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
