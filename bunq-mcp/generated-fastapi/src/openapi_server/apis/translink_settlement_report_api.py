...
from openapi_server.apis.translink_settlement_report_api_base import BaseTranslinkSettlementReportApi
...
from openapi_server.models.translink_settlement_report_listing import TranslinkSettlementReportListing
from openapi_server.models.translink_settlement_report_read import TranslinkSettlementReportRead
...
@router.get(
    "/user/{userID}/translink-settlement-report",
    responses={
        200: {"model": List[TranslinkSettlementReportListing], "description": "Get daily generated settlement reports for a specific user. apiTranslink:READ/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["translink-settlement-report"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_translink_settlement_report_for_user(
...
) -> List[TranslinkSettlementReportListing]:
    """Get daily generated settlement reports for a specific user. apiTranslink:READ/LISTING"""
    if not BaseTranslinkSettlementReportApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseTranslinkSettlementReportApi.subclasses[0]().list_all_translink_settlement_report_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/translink-settlement-report/{itemId}",
    responses={
        200: {"model": TranslinkSettlementReportRead, "description": "Get daily generated settlement reports for a specific user. apiTranslink:READ/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["translink-settlement-report"],
    summary="",
...
async def r_ead_translink_settlement_report_for_user(
    userID: StrictInt = Path(..., description=""),
    ...
    if not BaseTranslinkSettlementReportApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseTranslinkSettlementReportApi.subclasses[0]().r_ead_translink_settlement_report_for_user(userID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
