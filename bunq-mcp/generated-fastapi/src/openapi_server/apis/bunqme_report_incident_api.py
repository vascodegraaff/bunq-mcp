...
from openapi_server.apis.bunqme_report_incident_api_base import BaseBunqmeReportIncidentApi
...
from openapi_server.models.bunq_me_report_incident_read import BunqMeReportIncidentRead
...
@router.get(
    "/user/{userID}/bunqme-report-incident/{itemId}",
    responses={
        200: {"model": BunqMeReportIncidentRead, "description": "Required only for event post processor and not for any API apiApp:READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["bunqme-report-incident"],
    summary="",
    response_model_by_alias=True,
)
async def r_ead_bunqme_report_incident_for_user(
...
) -> BunqMeReportIncidentRead:
    """Required only for event post processor and not for any API apiApp:READ"""
    if not BaseBunqmeReportIncidentApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseBunqmeReportIncidentApi.subclasses[0]().r_ead_bunqme_report_incident_for_user(userID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
