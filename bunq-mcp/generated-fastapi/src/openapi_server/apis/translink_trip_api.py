...
from openapi_server.apis.translink_trip_api_base import BaseTranslinkTripApi
...
@router.get(
    "/card/{cardID}/translink-trip",
    responses={
        200: {"model": List[TranslinkTripListing], "description": "Used to retrieve translink trips for a specific card apiApp:READ/LISTING"},
...
async def list_all_translink_trip_for_card(
    ...
    if not BaseTranslinkTripApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseTranslinkTripApi.subclasses[0]().list_all_translink_trip_for_card(cardID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/card/{cardID}/translink-trip/{itemId}",
    responses={
        200: {"model": TranslinkTripRead, "description": "Used to retrieve translink trips for a specific card apiApp:READ/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["translink-trip"],
    summary="",
...
async def r_ead_translink_trip_for_card(
    ...
    if not BaseTranslinkTripApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseTranslinkTripApi.subclasses[0]().r_ead_translink_trip_for_card(cardID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
