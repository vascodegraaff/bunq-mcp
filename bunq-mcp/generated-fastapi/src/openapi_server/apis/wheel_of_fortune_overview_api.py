...
from openapi_server.apis.wheel_of_fortune_overview_api_base import BaseWheelOfFortuneOverviewApi
...
from openapi_server.models.wheel_of_fortune_overview_listing import WheelOfFortuneOverviewListing
...
@router.get(
    "/user/{userID}/wheel-of-fortune-overview",
    responses={
        200: {"model": List[WheelOfFortuneOverviewListing], "description": "Endpoint to retrieve an overview to be used to enrich the Wheel of Fortune screen. apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["wheel-of-fortune-overview"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_wheel_of_fortune_overview_for_user(
...
) -> List[WheelOfFortuneOverviewListing]:
    """Endpoint to retrieve an overview to be used to enrich the Wheel of Fortune screen. apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING"""
    if not BaseWheelOfFortuneOverviewApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseWheelOfFortuneOverviewApi.subclasses[0]().list_all_wheel_of_fortune_overview_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
