...
from openapi_server.apis.notification_filter_push_api_base import BaseNotificationFilterPushApi
...
from openapi_server.models.notification_filter_push import NotificationFilterPush
from openapi_server.models.notification_filter_push_create import NotificationFilterPushCreate
from openapi_server.models.notification_filter_push_listing import NotificationFilterPushListing
...
@router.post(
    ...
    responses={
        200: {"model": NotificationFilterPushCreate, "description": "Manage the push notification filters for a user. apiApp:CREATE/LISTING apiPublic:CREATE/LISTING"},
...
async def c_reate_notification_filter_push_for_user(
    ...
    notification_filter_push: NotificationFilterPush = Body(None, description=""),
    ...
    """Manage the push notification filters for a user. apiApp:CREATE/LISTING apiPublic:CREATE/LISTING"""
    if not BaseNotificationFilterPushApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseNotificationFilterPushApi.subclasses[0]().c_reate_notification_filter_push_for_user(userID, user_agent, x_bunq_client_authentication, notification_filter_push, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/notification-filter-push",
    responses={
        200: {"model": List[NotificationFilterPushListing], "description": "Manage the push notification filters for a user. apiApp:CREATE/LISTING apiPublic:CREATE/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["notification-filter-push"],
    summary="",
...
async def list_all_notification_filter_push_for_user(
    ...
    x_bunq_client_request_id: Annotated[Optional[StrictStr], Field(description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer.")] = Header(None, description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer."),
    x_bunq_geolocation: Annotated[Optional[StrictStr], Field(description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued.")] = Header(None, description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued."),
) -> List[NotificationFilterPushListing]:
    """Manage the push notification filters for a user. apiApp:CREATE/LISTING apiPublic:CREATE/LISTING"""
    if not BaseNotificationFilterPushApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseNotificationFilterPushApi.subclasses[0]().list_all_notification_filter_push_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
