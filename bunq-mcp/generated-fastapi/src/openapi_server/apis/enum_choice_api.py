...
from openapi_server.apis.enum_choice_api_base import BaseEnumChoiceApi
...
from openapi_server.models.enum_choice_listing import EnumChoiceListing
...
@router.get(
    "/enum-choice",
    responses={
        200: {"model": List[EnumChoiceListing], "description": "Get all values for a given enum. apiAdmin:LISTING apiServiceInternal:LISTING"},
...
async def list_all_enum_choice(
...
) -> List[EnumChoiceListing]:
    """Get all values for a given enum. apiAdmin:LISTING apiServiceInternal:LISTING"""
    if not BaseEnumChoiceApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseEnumChoiceApi.subclasses[0]().list_all_enum_choice(user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
