...
from openapi_server.apis.company_person_director_api_base import BaseCompanyPersonDirectorApi
...
from openapi_server.models.company_person_director_listing import CompanyPersonDirectorListing
from openapi_server.models.company_person_director_read import CompanyPersonDirectorRead
...
@router.get(
    "/user-company/{user-companyID}/company-person-director",
    responses={
        200: {"model": List[CompanyPersonDirectorListing], "description": "Manage the company person director details. apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING apiProcess:READ/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["company-person-director"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_company_person_director_for_user_company(
...
) -> List[CompanyPersonDirectorListing]:
    """Manage the company person director details. apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING apiProcess:READ/LISTING"""
    if not BaseCompanyPersonDirectorApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseCompanyPersonDirectorApi.subclasses[0]().list_all_company_person_director_for_user_company(user-companyID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user-company/{user-companyID}/company-person-director/{itemId}",
    responses={
        200: {"model": CompanyPersonDirectorRead, "description": "Manage the company person director details. apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING apiProcess:READ/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["company-person-director"],
    summary="",
...
async def r_ead_company_person_director_for_user_company(
    user-companyID: StrictInt = Path(..., description=""),
    ...
    x_bunq_client_request_id: Annotated[Optional[StrictStr], Field(description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer.")] = Header(None, description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer."),
    x_bunq_geolocation: Annotated[Optional[StrictStr], Field(description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued.")] = Header(None, description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued."),
) -> CompanyPersonDirectorRead:
    """Manage the company person director details. apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING apiProcess:READ/LISTING"""
    if not BaseCompanyPersonDirectorApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseCompanyPersonDirectorApi.subclasses[0]().r_ead_company_person_director_for_user_company(user-companyID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
