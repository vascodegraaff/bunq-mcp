...
from openapi_server.apis.bunqme_tab_settlement_api_base import BaseBunqmeTabSettlementApi
...
from openapi_server.models.bunq_me_tab_settlement_listing import BunqMeTabSettlementListing
...
@router.get(
    "/bunqme-tab-settlement",
    responses={
        200: {"model": List[BunqMeTabSettlementListing], "description": " apiAdmin:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["bunqme-tab-settlement"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_bunqme_tab_settlement(
...
) -> List[BunqMeTabSettlementListing]:
    """ apiAdmin:LISTING"""
    if not BaseBunqmeTabSettlementApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseBunqmeTabSettlementApi.subclasses[0]().list_all_bunqme_tab_settlement(user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
