...
from openapi_server.apis.ginmon_overview_api_base import BaseGinmonOverviewApi
...
from openapi_server.models.ginmon_overview_listing import GinmonOverviewListing
...
@router.get(
    "/user/{userID}/ginmon-overview",
    responses={
        200: {"model": List[GinmonOverviewListing], "description": "Ginmon overview. apiAdmin:LISTING apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["ginmon-overview"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_ginmon_overview_for_user(
...
) -> List[GinmonOverviewListing]:
    """Ginmon overview. apiAdmin:LISTING apiApp:LISTING"""
    if not BaseGinmonOverviewApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseGinmonOverviewApi.subclasses[0]().list_all_ginmon_overview_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
