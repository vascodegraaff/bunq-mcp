...
from openapi_server.apis.ginmon_instrument_entry_api_base import BaseGinmonInstrumentEntryApi
...
from openapi_server.models.ginmon_instrument_entry_listing import GinmonInstrumentEntryListing
...
@router.get(
    "/user/{userID}/ginmon-account/{ginmon-accountID}/ginmon-instrument-entry",
    responses={
        200: {"model": List[GinmonInstrumentEntryListing], "description": "Endpoint for read/listing instruments entries. apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["ginmon-instrument-entry"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_ginmon_instrument_entry_for_user_ginmon_account(
...
) -> List[GinmonInstrumentEntryListing]:
    """Endpoint for read/listing instruments entries. apiApp:LISTING"""
    if not BaseGinmonInstrumentEntryApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseGinmonInstrumentEntryApi.subclasses[0]().list_all_ginmon_instrument_entry_for_user_ginmon_account(userID, ginmon-accountID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
