...
from openapi_server.apis.admin_user_payment_service_provider_api_base import BaseAdminUserPaymentServiceProviderApi
...
from openapi_server.models.admin_user_payment_service_provider_read import AdminUserPaymentServiceProviderRead
...
@router.get(
    "/admin-user-payment-service-provider/{itemId}",
    responses={
        200: {"model": AdminUserPaymentServiceProviderRead, "description": "Admin-only endpoint to manage PSD2 users. apiAdmin:READ apiServiceInternal:READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["admin-user-payment-service-provider"],
    summary="",
    response_model_by_alias=True,
)
async def r_ead_admin_user_payment_service_provider(
...
) -> AdminUserPaymentServiceProviderRead:
    """Admin-only endpoint to manage PSD2 users. apiAdmin:READ apiServiceInternal:READ"""
    if not BaseAdminUserPaymentServiceProviderApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAdminUserPaymentServiceProviderApi.subclasses[0]().r_ead_admin_user_payment_service_provider(itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
