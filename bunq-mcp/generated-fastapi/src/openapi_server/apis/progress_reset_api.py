...
from openapi_server.apis.progress_reset_api_base import BaseProgressResetApi
...
from openapi_server.models.monetary_account_budget_progress_reset_create import MonetaryAccountBudgetProgressResetCreate
...
@router.post(
    "/user/{userID}/monetary-account/{monetary-accountID}/budget/{budgetID}/progress-reset",
    responses={
        200: {"model": MonetaryAccountBudgetProgressResetCreate, "description": "Endpoint to reset the budget progress for a monetary account apiAdmin:CREATE apiApp:CREATE apiServiceInternal:CREATE"},
...
async def c_reate_progress_reset_for_user_monetary_account_budget(
    userID: StrictInt = Path(..., description=""),
    monetary-accountID: StrictInt = Path(..., description=""),
    budgetID: StrictInt = Path(..., description=""),
...
) -> MonetaryAccountBudgetProgressResetCreate:
    """Endpoint to reset the budget progress for a monetary account apiAdmin:CREATE apiApp:CREATE apiServiceInternal:CREATE"""
    if not BaseProgressResetApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseProgressResetApi.subclasses[0]().c_reate_progress_reset_for_user_monetary_account_budget(userID, monetary-accountID, budgetID, user_agent, x_bunq_client_authentication, body, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
