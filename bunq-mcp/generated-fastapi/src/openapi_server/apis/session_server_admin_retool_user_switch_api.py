...
from openapi_server.apis.session_server_admin_retool_user_switch_api_base import BaseSessionServerAdminRetoolUserSwitchApi
...
from openapi_server.models.session_server_admin_retool_user_switch import SessionServerAdminRetoolUserSwitch
from openapi_server.models.session_server_admin_retool_user_switch_create import SessionServerAdminRetoolUserSwitchCreate
...
@router.post(
    "/session-server-admin-retool-user-switch",
    responses={
        200: {"model": SessionServerAdminRetoolUserSwitchCreate, "description": "Authenticate as a different user in a Retool session. Only available in test mode. apiAdmin:CREATE apiServiceInternal:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["session-server-admin-retool-user-switch"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_session_server_admin_retool_user_switch(
    ...
    session_server_admin_retool_user_switch: SessionServerAdminRetoolUserSwitch = Body(None, description=""),
...
) -> SessionServerAdminRetoolUserSwitchCreate:
    """Authenticate as a different user in a Retool session. Only available in test mode. apiAdmin:CREATE apiServiceInternal:CREATE"""
    if not BaseSessionServerAdminRetoolUserSwitchApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseSessionServerAdminRetoolUserSwitchApi.subclasses[0]().c_reate_session_server_admin_retool_user_switch(user_agent, x_bunq_client_authentication, session_server_admin_retool_user_switch, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
