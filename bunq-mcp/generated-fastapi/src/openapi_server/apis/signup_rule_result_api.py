...
from openapi_server.apis.signup_rule_result_api_base import BaseSignupRuleResultApi
...
from openapi_server.models.user_signup_rule_result_listing import UserSignupRuleResultListing
...
@router.get(
    "/user/{userID}/signup-rule-result",
    responses={
        200: {"model": List[UserSignupRuleResultListing], "description": "The view endpoint for listing user failure reasons in 5 minute promise flow. apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["signup-rule-result"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_signup_rule_result_for_user(
...
) -> List[UserSignupRuleResultListing]:
    """The view endpoint for listing user failure reasons in 5 minute promise flow. apiApp:LISTING"""
    if not BaseSignupRuleResultApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseSignupRuleResultApi.subclasses[0]().list_all_signup_rule_result_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
