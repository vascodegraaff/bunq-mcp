...
from openapi_server.apis.tree_certificate_api_base import BaseTreeCertificateApi
...
from openapi_server.models.tree_certificate_listing import TreeCertificateListing
from openapi_server.models.tree_certificate_read import TreeCertificateRead
...
@router.get(
    "/tree-certificate",
    responses={
        200: {"model": List[TreeCertificateListing], "description": "Lists the user&#39;s tree certificates. apiApp:READ/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["tree-certificate"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_tree_certificate(
...
) -> List[TreeCertificateListing]:
    """Lists the user&#39;s tree certificates. apiApp:READ/LISTING"""
    if not BaseTreeCertificateApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseTreeCertificateApi.subclasses[0]().list_all_tree_certificate(user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/tree-certificate/{itemId}",
    responses={
        200: {"model": TreeCertificateRead, "description": "Lists the user&#39;s tree certificates. apiApp:READ/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["tree-certificate"],
    summary="",
...
async def r_ead_tree_certificate(
    ...
    if not BaseTreeCertificateApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseTreeCertificateApi.subclasses[0]().r_ead_tree_certificate(itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
