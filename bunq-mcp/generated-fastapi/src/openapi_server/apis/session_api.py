...
from openapi_server.apis.session_api_base import BaseSessionApi
...
@router.delete(
    "/session/{itemId}",
    responses={
        200: {"model": object, "description": "Endpoint for operations over the current session. apiAdmin:READ/DELETE apiApp:READ/DELETE apiPublic:DELETE apiServiceInternal:READ/DELETE apiDeveloperPortal:READ/DELETE"},
...
async def d_elete_session(
    ...
    return await BaseSessionApi.subclasses[0]().d_elete_session(itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/session",
    responses={
        200: {"model": List[object], "description": "Load in the login response for a specific user. apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["session"],
    summary="",
...
async def list_all_session_for_user(
    ...
    """Load in the login response for a specific user. apiApp:LISTING"""
    if not BaseSessionApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseSessionApi.subclasses[0]().list_all_session_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/session/{itemId}",
    responses={
        200: {"model": SessionRead, "description": "Endpoint for operations over the current session. apiAdmin:READ/DELETE apiApp:READ/DELETE apiPublic:DELETE apiServiceInternal:READ/DELETE apiDeveloperPortal:READ/DELETE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["session"],
    summary="",
...
async def r_ead_session(
    ...
    if not BaseSessionApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseSessionApi.subclasses[0]().r_ead_session(itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
