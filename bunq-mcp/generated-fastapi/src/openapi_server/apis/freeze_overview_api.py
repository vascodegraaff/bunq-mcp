...
from openapi_server.apis.freeze_overview_api_base import BaseFreezeOverviewApi
...
from openapi_server.models.freeze_user_overview_listing import FreezeUserOverviewListing
...
@router.get(
    "/user/{userID}/freeze-overview",
    responses={
        200: {"model": List[FreezeUserOverviewListing], "description": "Used to get an overview of user account on user attempt to freeze their account. apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["freeze-overview"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_freeze_overview_for_user(
...
) -> List[FreezeUserOverviewListing]:
    """Used to get an overview of user account on user attempt to freeze their account. apiApp:LISTING"""
    if not BaseFreezeOverviewApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseFreezeOverviewApi.subclasses[0]().list_all_freeze_overview_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
