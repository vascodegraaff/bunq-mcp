...
from openapi_server.apis.watchlist_check_api_base import BaseWatchlistCheckApi
...
from openapi_server.models.watchlist_check_read import WatchlistCheckRead
...
@router.get(
    "/watchlist-check/{itemId}",
    responses={
        200: {"model": WatchlistCheckRead, "description": "The endpoint to manage watchlist checks. apiAdmin:READ apiServiceInternal:READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["watchlist-check"],
    summary="",
    response_model_by_alias=True,
)
async def r_ead_watchlist_check(
    ...
    """The endpoint to manage watchlist checks. apiAdmin:READ apiServiceInternal:READ"""
    if not BaseWatchlistCheckApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseWatchlistCheckApi.subclasses[0]().r_ead_watchlist_check(itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
