...
from openapi_server.apis.investigation_draft_api_base import BaseInvestigationDraftApi
...
from openapi_server.models.user_investigation_draft_listing import UserInvestigationDraftListing
...
@router.get(
    "/investigation/{investigationID}/investigation-draft",
    responses={
        200: {"model": List[UserInvestigationDraftListing], "description": "Endpoint to list user investigation drafts. apiAdmin:LISTING apiServiceInternal:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["investigation-draft"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_investigation_draft_for_investigation(
    investigationID: StrictInt = Path(..., description=""),
...
) -> List[UserInvestigationDraftListing]:
    """Endpoint to list user investigation drafts. apiAdmin:LISTING apiServiceInternal:LISTING"""
    if not BaseInvestigationDraftApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseInvestigationDraftApi.subclasses[0]().list_all_investigation_draft_for_investigation(investigationID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
