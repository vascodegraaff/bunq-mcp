...
from openapi_server.apis.opvragen_product_api_base import BaseOpvragenProductApi
...
from openapi_server.models.government_referral_portal_external_search_by_product import GovernmentReferralPortalExternalSearchByProduct
from openapi_server.models.government_referral_portal_external_search_by_product_create import GovernmentReferralPortalExternalSearchByProductCreate
...
@router.post(
    "/opvragen-product",
    responses={
        200: {"model": GovernmentReferralPortalExternalSearchByProductCreate, "description": "View for retrieving user information. Should only be used by the government. apiGovernment:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["opvragen-product"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_opvragen_product(
    ...
    government_referral_portal_external_search_by_product: GovernmentReferralPortalExternalSearchByProduct = Body(None, description=""),
...
) -> GovernmentReferralPortalExternalSearchByProductCreate:
    """View for retrieving user information. Should only be used by the government. apiGovernment:CREATE"""
    if not BaseOpvragenProductApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseOpvragenProductApi.subclasses[0]().c_reate_opvragen_product(user_agent, x_bunq_client_authentication, government_referral_portal_external_search_by_product, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
