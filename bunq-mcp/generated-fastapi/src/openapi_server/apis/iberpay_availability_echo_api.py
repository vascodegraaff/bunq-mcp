...
from openapi_server.apis.iberpay_availability_echo_api_base import BaseIberpayAvailabilityEchoApi
...
from openapi_server.models.iberpay_incoming_availability_echo_create import IberpayIncomingAvailabilityEchoCreate
...
@router.post(
    "/iberpay-availability-echo",
    responses={
        200: {"model": IberpayIncomingAvailabilityEchoCreate, "description": "Endpoint for Iberpay to send availability checks to bunq. Iberpay service name: WS_DISP_ECO. apiIberpay:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["iberpay-availability-echo"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_iberpay_availability_echo(
...
) -> IberpayIncomingAvailabilityEchoCreate:
    """Endpoint for Iberpay to send availability checks to bunq. Iberpay service name: WS_DISP_ECO. apiIberpay:CREATE"""
    if not BaseIberpayAvailabilityEchoApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseIberpayAvailabilityEchoApi.subclasses[0]().c_reate_iberpay_availability_echo(user_agent, x_bunq_client_authentication, body, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
