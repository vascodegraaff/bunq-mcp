...
from openapi_server.apis.iberpay_investigation_api_base import BaseIberpayInvestigationApi
...
from openapi_server.models.iberpay_incoming_investigation_create import IberpayIncomingInvestigationCreate
...
@router.post(
    "/iberpay-investigation",
    responses={
        200: {"model": IberpayIncomingInvestigationCreate, "description": "Endpoint for Iberpay to send investigation messages to bunq. Iberpay service name: WS_RECEP_INVESTIGACION. apiIberpay:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["iberpay-investigation"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_iberpay_investigation(
...
) -> IberpayIncomingInvestigationCreate:
    """Endpoint for Iberpay to send investigation messages to bunq. Iberpay service name: WS_RECEP_INVESTIGACION. apiIberpay:CREATE"""
    if not BaseIberpayInvestigationApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseIberpayInvestigationApi.subclasses[0]().c_reate_iberpay_investigation(user_agent, x_bunq_client_authentication, body, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
