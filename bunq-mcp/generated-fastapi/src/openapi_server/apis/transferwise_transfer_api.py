...
from openapi_server.apis.transferwise_transfer_api_base import BaseTransferwiseTransferApi
...
from openapi_server.models.transferwise_transfer import TransferwiseTransfer
from openapi_server.models.transferwise_transfer_create import TransferwiseTransferCreate
from openapi_server.models.transferwise_transfer_listing import TransferwiseTransferListing
from openapi_server.models.transferwise_transfer_read import TransferwiseTransferRead
...
@router.post(
    ...
    responses={
        200: {"model": TransferwiseTransferCreate, "description": "Used to create Transferwise payments. apiApp:CREATE/READ/LISTING apiPublic:CREATE/READ/LISTING"},
...
async def c_reate_transferwise_transfer_for_user_transferwise_quote(
    ...
    transferwise_transfer: TransferwiseTransfer = Body(None, description=""),
    ...
    """Used to create Transferwise payments. apiApp:CREATE/READ/LISTING apiPublic:CREATE/READ/LISTING"""
    if not BaseTransferwiseTransferApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseTransferwiseTransferApi.subclasses[0]().c_reate_transferwise_transfer_for_user_transferwise_quote(userID, transferwise-quoteID, user_agent, x_bunq_client_authentication, transferwise_transfer, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/transferwise-quote/{transferwise-quoteID}/transferwise-transfer",
    responses={
        200: {"model": List[TransferwiseTransferListing], "description": "Used to create Transferwise payments. apiApp:CREATE/READ/LISTING apiPublic:CREATE/READ/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["transferwise-transfer"],
    summary="",
...
async def list_all_transferwise_transfer_for_user_transferwise_quote(
    ...
    x_bunq_client_request_id: Annotated[Optional[StrictStr], Field(description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer.")] = Header(None, description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer."),
    x_bunq_geolocation: Annotated[Optional[StrictStr], Field(description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued.")] = Header(None, description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued."),
) -> List[TransferwiseTransferListing]:
    """Used to create Transferwise payments. apiApp:CREATE/READ/LISTING apiPublic:CREATE/READ/LISTING"""
    if not BaseTransferwiseTransferApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseTransferwiseTransferApi.subclasses[0]().list_all_transferwise_transfer_for_user_transferwise_quote(userID, transferwise-quoteID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/transferwise-quote/{transferwise-quoteID}/transferwise-transfer/{itemId}",
    responses={
        200: {"model": TransferwiseTransferRead, "description": "Used to create Transferwise payments. apiApp:CREATE/READ/LISTING apiPublic:CREATE/READ/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["transferwise-transfer"],
    summary="",
...
async def r_ead_transferwise_transfer_for_user_transferwise_quote(
    userID: StrictInt = Path(..., description=""),
    transferwise-quoteID: StrictInt = Path(..., description=""),
    ...
    if not BaseTransferwiseTransferApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseTransferwiseTransferApi.subclasses[0]().r_ead_transferwise_transfer_for_user_transferwise_quote(userID, transferwise-quoteID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
