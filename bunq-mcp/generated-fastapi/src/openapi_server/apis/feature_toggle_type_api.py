...
from openapi_server.apis.feature_toggle_type_api_base import BaseFeatureToggleTypeApi
...
from openapi_server.models.feature_toggle_type_listing import FeatureToggleTypeListing
...
@router.get(
    "/feature-toggle-type",
    responses={
        200: {"model": List[FeatureToggleTypeListing], "description": "Fetch all supported feature toggle types. apiAdmin:LISTING apiServiceInternal:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["feature-toggle-type"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_feature_toggle_type(
...
) -> List[FeatureToggleTypeListing]:
    """Fetch all supported feature toggle types. apiAdmin:LISTING apiServiceInternal:LISTING"""
    if not BaseFeatureToggleTypeApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseFeatureToggleTypeApi.subclasses[0]().list_all_feature_toggle_type(user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
