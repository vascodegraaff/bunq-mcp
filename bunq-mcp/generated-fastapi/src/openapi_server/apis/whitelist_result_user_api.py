...
from openapi_server.apis.whitelist_result_user_api_base import BaseWhitelistResultUserApi
...
from openapi_server.models.whitelist_result_user_listing import WhitelistResultUserListing
...
@router.get(
    "/user/{userID}/whitelist-result-user",
    responses={
        200: {"model": List[WhitelistResultUserListing], "description": "List circle of trust results. apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["whitelist-result-user"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_whitelist_result_user_for_user(
...
) -> List[WhitelistResultUserListing]:
    """List circle of trust results. apiApp:LISTING"""
    if not BaseWhitelistResultUserApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseWhitelistResultUserApi.subclasses[0]().list_all_whitelist_result_user_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
