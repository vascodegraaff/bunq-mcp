...
from openapi_server.apis.support_user_rating_average_api_base import BaseSupportUserRatingAverageApi
...
from openapi_server.models.support_user_rating_user_average_listing import SupportUserRatingUserAverageListing
...
@router.get(
    "/user/{userID}/support-user-rating-average",
    responses={
        200: {"model": List[SupportUserRatingUserAverageListing], "description": "Endpoint to get average support rating for a user. apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["support-user-rating-average"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_support_user_rating_average_for_user(
...
) -> List[SupportUserRatingUserAverageListing]:
    """Endpoint to get average support rating for a user. apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING"""
    if not BaseSupportUserRatingAverageApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseSupportUserRatingAverageApi.subclasses[0]().list_all_support_user_rating_average_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
