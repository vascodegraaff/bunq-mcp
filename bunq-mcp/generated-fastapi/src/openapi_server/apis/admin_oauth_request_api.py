...
from openapi_server.apis.admin_oauth_request_api_base import BaseAdminOauthRequestApi
...
from openapi_server.models.admin_oauth_request_user_listing import AdminOauthRequestUserListing
...
@router.get(
    "/user/{userID}/admin-oauth-request",
    responses={
        200: {"model": List[AdminOauthRequestUserListing], "description": "Used for viewing OAuth connections. apiAdmin:LISTING apiServiceInternal:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["admin-oauth-request"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_admin_oauth_request_for_user(
...
) -> List[AdminOauthRequestUserListing]:
    """Used for viewing OAuth connections. apiAdmin:LISTING apiServiceInternal:LISTING"""
    if not BaseAdminOauthRequestApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAdminOauthRequestApi.subclasses[0]().list_all_admin_oauth_request_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
