...
from openapi_server.apis.session_phone_verify_api_base import BaseSessionPhoneVerifyApi
...
from openapi_server.models.session_phone_authentication_token_user import SessionPhoneAuthenticationTokenUser
from openapi_server.models.session_phone_authentication_token_user_create import SessionPhoneAuthenticationTokenUserCreate
...
async def c_reate_session_phone_verify(
    ...
    session_phone_authentication_token_user: SessionPhoneAuthenticationTokenUser = Body(None, description=""),
...
) -> SessionPhoneAuthenticationTokenUserCreate:
    """Used to verify an existing user session by sending a previously created authentication token for the user. Like other session endpoints this needs to be called using the device token, but with the existing user token as the argument. If the user token exists and belongs to this device, the normal session response is returned to the client. apiAdmin:CREATE apiApp:CREATE apiServiceInternal:CREATE"""
    if not BaseSessionPhoneVerifyApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseSessionPhoneVerifyApi.subclasses[0]().c_reate_session_phone_verify(user_agent, x_bunq_client_authentication, session_phone_authentication_token_user, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
