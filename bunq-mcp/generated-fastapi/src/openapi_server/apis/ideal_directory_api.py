...
from openapi_server.apis.ideal_directory_api_base import BaseIdealDirectoryApi
...
from openapi_server.models.ideal_directory_listing import IdealDirectoryListing
...
@router.get(
    "/ideal-directory",
    responses={
        200: {"model": List[IdealDirectoryListing], "description": "View for displaying the iDEAL issuer directory to customers apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["ideal-directory"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_ideal_directory(
...
) -> List[IdealDirectoryListing]:
    """View for displaying the iDEAL issuer directory to customers apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING"""
    if not BaseIdealDirectoryApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseIdealDirectoryApi.subclasses[0]().list_all_ideal_directory(user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
