...
from openapi_server.apis.ginmon_account_agreement_document_requirement_api_base import BaseGinmonAccountAgreementDocumentRequirementApi
...
from openapi_server.models.ginmon_account_agreement_document_requirement_listing import GinmonAccountAgreementDocumentRequirementListing
...
@router.get(
    "/user/{userID}/ginmon-account-agreement-document-requirement",
    responses={
        200: {"model": List[GinmonAccountAgreementDocumentRequirementListing], "description": "Used to retrieve the requirements for opening a Ginmon Account. apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["ginmon-account-agreement-document-requirement"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_ginmon_account_agreement_document_requirement_for_user(
...
) -> List[GinmonAccountAgreementDocumentRequirementListing]:
    """Used to retrieve the requirements for opening a Ginmon Account. apiApp:LISTING"""
    if not BaseGinmonAccountAgreementDocumentRequirementApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseGinmonAccountAgreementDocumentRequirementApi.subclasses[0]().list_all_ginmon_account_agreement_document_requirement_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
