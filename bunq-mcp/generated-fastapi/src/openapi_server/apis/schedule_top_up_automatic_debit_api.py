...
from openapi_server.apis.schedule_top_up_automatic_debit_api_base import BaseScheduleTopUpAutomaticDebitApi
...
from openapi_server.models.schedule_top_up_automatic_debit_listing import ScheduleTopUpAutomaticDebitListing
from openapi_server.models.schedule_top_up_automatic_debit_read import ScheduleTopUpAutomaticDebitRead
...
@router.get(
    "/user/{userID}/monetary-account/{monetary-accountID}/schedule-top-up-automatic-debit",
    responses={
        200: {"model": List[ScheduleTopUpAutomaticDebitListing], "description": "Endpoint for scheduled automatic debit top-up. apiApp:READ/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["schedule-top-up-automatic-debit"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_schedule_top_up_automatic_debit_for_user_monetary_account(
...
) -> List[ScheduleTopUpAutomaticDebitListing]:
    """Endpoint for scheduled automatic debit top-up. apiApp:READ/LISTING"""
    if not BaseScheduleTopUpAutomaticDebitApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseScheduleTopUpAutomaticDebitApi.subclasses[0]().list_all_schedule_top_up_automatic_debit_for_user_monetary_account(userID, monetary-accountID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/monetary-account/{monetary-accountID}/schedule-top-up-automatic-debit/{itemId}",
    responses={
        200: {"model": ScheduleTopUpAutomaticDebitRead, "description": "Endpoint for scheduled automatic debit top-up. apiApp:READ/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["schedule-top-up-automatic-debit"],
    summary="",
...
async def r_ead_schedule_top_up_automatic_debit_for_user_monetary_account(
    userID: StrictInt = Path(..., description=""),
    monetary-accountID: StrictInt = Path(..., description=""),
    ...
    x_bunq_client_request_id: Annotated[Optional[StrictStr], Field(description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer.")] = Header(None, description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer."),
    x_bunq_geolocation: Annotated[Optional[StrictStr], Field(description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued.")] = Header(None, description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued."),
) -> ScheduleTopUpAutomaticDebitRead:
    """Endpoint for scheduled automatic debit top-up. apiApp:READ/LISTING"""
    if not BaseScheduleTopUpAutomaticDebitApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseScheduleTopUpAutomaticDebitApi.subclasses[0]().r_ead_schedule_top_up_automatic_debit_for_user_monetary_account(userID, monetary-accountID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
