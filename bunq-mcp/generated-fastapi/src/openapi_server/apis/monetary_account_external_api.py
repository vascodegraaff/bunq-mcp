...
from openapi_server.apis.monetary_account_external_api_base import BaseMonetaryAccountExternalApi
...
from openapi_server.models.monetary_account_external import MonetaryAccountExternal
from openapi_server.models.monetary_account_external_create import MonetaryAccountExternalCreate
from openapi_server.models.monetary_account_external_listing import MonetaryAccountExternalListing
from openapi_server.models.monetary_account_external_read import MonetaryAccountExternalRead
from openapi_server.models.monetary_account_external_update import MonetaryAccountExternalUpdate
...
async def c_reate_monetary_account_external_for_user(
    ...
    monetary_account_external: MonetaryAccountExternal = Body(None, description=""),
    ...
    """Endpoint for managing monetary accounts which are connected to external services. apiAdmin:CREATE/READ/UPDATE/LISTING apiApp:CREATE/READ/UPDATE/LISTING apiPublic:CREATE/READ/UPDATE/LISTING apiServiceInternal:CREATE/READ/UPDATE/LISTING apiQualityAssurance:CREATE/READ/UPDATE/LISTING"""
    if not BaseMonetaryAccountExternalApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseMonetaryAccountExternalApi.subclasses[0]().c_reate_monetary_account_external_for_user(userID, user_agent, x_bunq_client_authentication, monetary_account_external, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/monetary-account-external",
    responses={
        200: {"model": List[MonetaryAccountExternalListing], "description": "Endpoint for managing monetary accounts which are connected to external services. apiAdmin:CREATE/READ/UPDATE/LISTING apiApp:CREATE/READ/UPDATE/LISTING apiPublic:CREATE/READ/UPDATE/LISTING apiServiceInternal:CREATE/READ/UPDATE/LISTING apiQualityAssurance:CREATE/READ/UPDATE/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["monetary-account-external"],
    summary="",
...
async def list_all_monetary_account_external_for_user(
    ...
    x_bunq_client_request_id: Annotated[Optional[StrictStr], Field(description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer.")] = Header(None, description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer."),
    x_bunq_geolocation: Annotated[Optional[StrictStr], Field(description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued.")] = Header(None, description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued."),
) -> List[MonetaryAccountExternalListing]:
    """Endpoint for managing monetary accounts which are connected to external services. apiAdmin:CREATE/READ/UPDATE/LISTING apiApp:CREATE/READ/UPDATE/LISTING apiPublic:CREATE/READ/UPDATE/LISTING apiServiceInternal:CREATE/READ/UPDATE/LISTING apiQualityAssurance:CREATE/READ/UPDATE/LISTING"""
    if not BaseMonetaryAccountExternalApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseMonetaryAccountExternalApi.subclasses[0]().list_all_monetary_account_external_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/monetary-account-external/{itemId}",
    responses={
        200: {"model": MonetaryAccountExternalRead, "description": "Endpoint for managing monetary accounts which are connected to external services. apiAdmin:CREATE/READ/UPDATE/LISTING apiApp:CREATE/READ/UPDATE/LISTING apiPublic:CREATE/READ/UPDATE/LISTING apiServiceInternal:CREATE/READ/UPDATE/LISTING apiQualityAssurance:CREATE/READ/UPDATE/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["monetary-account-external"],
    summary="",
...
async def r_ead_monetary_account_external_for_user(
    userID: StrictInt = Path(..., description=""),
    ...
    if not BaseMonetaryAccountExternalApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseMonetaryAccountExternalApi.subclasses[0]().r_ead_monetary_account_external_for_user(userID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.put(
    "/user/{userID}/monetary-account-external/{itemId}",
    responses={
        200: {"model": MonetaryAccountExternalUpdate, "description": "Endpoint for managing monetary accounts which are connected to external services. apiAdmin:CREATE/READ/UPDATE/LISTING apiApp:CREATE/READ/UPDATE/LISTING apiPublic:CREATE/READ/UPDATE/LISTING apiServiceInternal:CREATE/READ/UPDATE/LISTING apiQualityAssurance:CREATE/READ/UPDATE/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["monetary-account-external"],
    summary="",
...
async def u_pdate_monetary_account_external_for_user(
    userID: StrictInt = Path(..., description=""),
    itemId: StrictInt = Path(..., description=""),
    user_agent: Annotated[StrictStr, Field(description="The User-Agent header field should contain information about the user agent originating the request. There are no restrictions on the value of this header.")] = Header(None, description="The User-Agent header field should contain information about the user agent originating the request. There are no restrictions on the value of this header."),
    x_bunq_client_authentication: Annotated[StrictStr, Field(description="The authentication token is used to authenticate the source of the API call. It is required by all API calls except for POST /v1/installation. It is important to note that the device and session calls are using the token from the response of the installation call, while all the other calls use the token from the response of the session-server call")] = Header(None, description="The authentication token is used to authenticate the source of the API call. It is required by all API calls except for POST /v1/installation. It is important to note that the device and session calls are using the token from the response of the installation call, while all the other calls use the token from the response of the session-server call"),
    ...
    if not BaseMonetaryAccountExternalApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseMonetaryAccountExternalApi.subclasses[0]().u_pdate_monetary_account_external_for_user(userID, itemId, user_agent, x_bunq_client_authentication, monetary_account_external, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
