...
from openapi_server.apis.account_closure_check_api_base import BaseAccountClosureCheckApi
...
from openapi_server.models.user_account_closure_check_listing import UserAccountClosureCheckListing
...
@router.get(
    "/user/{userID}/account-closure-check",
    responses={
        200: {"model": List[UserAccountClosureCheckListing], "description": "Used to get get the results of the checks that are performed to verify a user account can be closed. apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["account-closure-check"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_account_closure_check_for_user(
...
) -> List[UserAccountClosureCheckListing]:
    """Used to get get the results of the checks that are performed to verify a user account can be closed. apiApp:LISTING"""
    if not BaseAccountClosureCheckApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAccountClosureCheckApi.subclasses[0]().list_all_account_closure_check_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
