...
from openapi_server.apis.attachment_installation_api_base import BaseAttachmentInstallationApi
...
from openapi_server.models.attachment_installation_create import AttachmentInstallationCreate
from openapi_server.models.attachment_installation_read import AttachmentInstallationRead
...
@router.post(
    ...
    responses={
        200: {"model": AttachmentInstallationCreate, "description": "Used to upload a new attachment with an installation as the owner, and to read its metadata. apiAdmin:CREATE/READ apiApp:CREATE/READ apiServiceInternal:CREATE/READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["attachment-installation"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_attachment_installation(
...
) -> AttachmentInstallationCreate:
    """Used to upload a new attachment with an installation as the owner, and to read its metadata. apiAdmin:CREATE/READ apiApp:CREATE/READ apiServiceInternal:CREATE/READ"""
    if not BaseAttachmentInstallationApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAttachmentInstallationApi.subclasses[0]().c_reate_attachment_installation(user_agent, x_bunq_client_authentication, body, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/attachment-installation/{itemId}",
    responses={
        200: {"model": AttachmentInstallationRead, "description": "Used to upload a new attachment with an installation as the owner, and to read its metadata. apiAdmin:CREATE/READ apiApp:CREATE/READ apiServiceInternal:CREATE/READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["attachment-installation"],
    summary="",
...
async def r_ead_attachment_installation(
    ...
    x_bunq_client_request_id: Annotated[Optional[StrictStr], Field(description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer.")] = Header(None, description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer."),
    x_bunq_geolocation: Annotated[Optional[StrictStr], Field(description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued.")] = Header(None, description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued."),
) -> AttachmentInstallationRead:
    """Used to upload a new attachment with an installation as the owner, and to read its metadata. apiAdmin:CREATE/READ apiApp:CREATE/READ apiServiceInternal:CREATE/READ"""
    if not BaseAttachmentInstallationApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAttachmentInstallationApi.subclasses[0]().r_ead_attachment_installation(itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
