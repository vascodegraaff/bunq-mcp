...
from openapi_server.apis.alias_user_check_api_base import BaseAliasUserCheckApi
...
from openapi_server.models.alias_device_check_create import AliasDeviceCheckCreate
...
@router.post(
    "/alias-device/{alias-deviceID}/alias-user-check",
    responses={
        200: {"model": AliasDeviceCheckCreate, "description": "Used to check if there is any alias user with same pointer of the confirmed alias device. apiApp:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["alias-user-check"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_alias_user_check_for_alias_device(
    alias-deviceID: StrictInt = Path(..., description=""),
...
) -> AliasDeviceCheckCreate:
    """Used to check if there is any alias user with same pointer of the confirmed alias device. apiApp:CREATE"""
    if not BaseAliasUserCheckApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAliasUserCheckApi.subclasses[0]().c_reate_alias_user_check_for_alias_device(alias-deviceID, user_agent, x_bunq_client_authentication, body, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
