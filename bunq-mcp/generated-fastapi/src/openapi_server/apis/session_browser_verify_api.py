...
from openapi_server.apis.session_browser_verify_api_base import BaseSessionBrowserVerifyApi
...
from openapi_server.models.session_browser_authentication_token_user import SessionBrowserAuthenticationTokenUser
from openapi_server.models.session_browser_authentication_token_user_create import SessionBrowserAuthenticationTokenUserCreate
...
async def c_reate_session_browser_verify(
    ...
    session_browser_authentication_token_user: SessionBrowserAuthenticationTokenUser = Body(None, description=""),
...
) -> SessionBrowserAuthenticationTokenUserCreate:
    """Used to verify an existing user session by sending a previously created authentication token for the user. Like other session endpoints this needs to be called using the device token, but with the existing user token as the argument. If the user token exists and belongs to this device, the normal session response is returned to the client. apiApp:CREATE apiDeveloperPortal:CREATE"""
    if not BaseSessionBrowserVerifyApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseSessionBrowserVerifyApi.subclasses[0]().c_reate_session_browser_verify(user_agent, x_bunq_client_authentication, session_browser_authentication_token_user, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
