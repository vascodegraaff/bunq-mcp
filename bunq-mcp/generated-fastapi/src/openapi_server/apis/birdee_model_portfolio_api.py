...
from openapi_server.apis.birdee_model_portfolio_api_base import BaseBirdeeModelPortfolioApi
...
from openapi_server.models.birdee_model_portfolio_listing import BirdeeModelPortfolioListing
...
@router.get(
    "/birdee-model-portfolio",
    responses={
        200: {"model": List[BirdeeModelPortfolioListing], "description": "Endpoint for viewing the model portfolios Birdee offers. apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["birdee-model-portfolio"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_birdee_model_portfolio(
...
) -> List[BirdeeModelPortfolioListing]:
    """Endpoint for viewing the model portfolios Birdee offers. apiApp:LISTING"""
    if not BaseBirdeeModelPortfolioApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseBirdeeModelPortfolioApi.subclasses[0]().list_all_birdee_model_portfolio(user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
