...
from openapi_server.apis.fiu_report_threshold_api_base import BaseFiuReportThresholdApi
...
from openapi_server.models.fiu_report_threshold_listing import FiuReportThresholdListing
...
@router.get(
    "/user/{userID}/fiu-report-threshold",
    responses={
        200: {"model": List[FiuReportThresholdListing], "description": "View for the fiu reporting of transactions exceeding thresholds. apiAdmin:LISTING apiServiceInternal:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["fiu-report-threshold"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_fiu_report_threshold_for_user(
...
) -> List[FiuReportThresholdListing]:
    """View for the fiu reporting of transactions exceeding thresholds. apiAdmin:LISTING apiServiceInternal:LISTING"""
    if not BaseFiuReportThresholdApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseFiuReportThresholdApi.subclasses[0]().list_all_fiu_report_threshold_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
