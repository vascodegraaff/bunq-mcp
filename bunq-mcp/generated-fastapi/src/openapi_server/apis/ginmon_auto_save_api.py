...
from openapi_server.apis.ginmon_auto_save_api_base import BaseGinmonAutoSaveApi
...
from openapi_server.models.ginmon_auto_save_listing import GinmonAutoSaveListing
...
@router.get(
    "/user/{userID}/ginmon-auto-save",
    responses={
        200: {"model": List[GinmonAutoSaveListing], "description": "Endpoint for read how much is auto saved towards. apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["ginmon-auto-save"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_ginmon_auto_save_for_user(
...
) -> List[GinmonAutoSaveListing]:
    """Endpoint for read how much is auto saved towards. apiApp:LISTING"""
    if not BaseGinmonAutoSaveApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseGinmonAutoSaveApi.subclasses[0]().list_all_ginmon_auto_save_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
