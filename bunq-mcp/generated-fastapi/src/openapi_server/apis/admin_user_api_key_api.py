...
from openapi_server.apis.admin_user_api_key_api_base import BaseAdminUserApiKeyApi
...
from openapi_server.models.admin_user_api_key_read import AdminUserApiKeyRead
...
@router.get(
    "/admin-user-api-key/{itemId}",
    responses={
        200: {"model": AdminUserApiKeyRead, "description": "Admin-only endpoint to manage OAuth users. apiAdmin:READ apiServiceInternal:READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["admin-user-api-key"],
    summary="",
    response_model_by_alias=True,
)
async def r_ead_admin_user_api_key(
    ...
    """Admin-only endpoint to manage OAuth users. apiAdmin:READ apiServiceInternal:READ"""
    if not BaseAdminUserApiKeyApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAdminUserApiKeyApi.subclasses[0]().r_ead_admin_user_api_key(itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
