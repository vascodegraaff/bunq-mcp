...
from openapi_server.apis.ginmon_account_available_api_base import BaseGinmonAccountAvailableApi
...
from openapi_server.models.ginmon_account_available_listing import GinmonAccountAvailableListing
...
@router.get(
    "/user/{userID}/ginmon-account-available",
    responses={
        200: {"model": List[GinmonAccountAvailableListing], "description": "To determine whether signing up for a Ginmon account is available for you. apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["ginmon-account-available"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_ginmon_account_available_for_user(
...
) -> List[GinmonAccountAvailableListing]:
    """To determine whether signing up for a Ginmon account is available for you. apiApp:LISTING"""
    if not BaseGinmonAccountAvailableApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseGinmonAccountAvailableApi.subclasses[0]().list_all_ginmon_account_available_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
