...
from openapi_server.apis.bizum_prizum_debit_api_base import BaseBizumPrizumDebitApi
...
from openapi_server.models.bizum_prizum_debit_create import BizumPrizumDebitCreate
...
@router.post(
    "/bizum-prizum-debit",
    responses={
        200: {"model": BizumPrizumDebitCreate, "description": "Endpoint for receiving Bizum Payment Requests via the Prizum protocol. apiBizum:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["bizum-prizum-debit"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_bizum_prizum_debit(
...
) -> BizumPrizumDebitCreate:
    """Endpoint for receiving Bizum Payment Requests via the Prizum protocol. apiBizum:CREATE"""
    if not BaseBizumPrizumDebitApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseBizumPrizumDebitApi.subclasses[0]().c_reate_bizum_prizum_debit(user_agent, x_bunq_client_authentication, body, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
