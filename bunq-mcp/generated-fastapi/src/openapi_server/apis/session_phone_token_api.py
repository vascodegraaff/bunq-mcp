...
from openapi_server.apis.session_phone_token_api_base import BaseSessionPhoneTokenApi
...
from openapi_server.models.session_phone_login_token import SessionPhoneLoginToken
from openapi_server.models.session_phone_login_token_create import SessionPhoneLoginTokenCreate
...
@router.post(
    "/session-phone-token",
    responses={
        200: {"model": SessionPhoneLoginTokenCreate, "description": "Used to log in a user with a token. apiAdmin:CREATE apiApp:CREATE apiServiceInternal:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["session-phone-token"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_session_phone_token(
    ...
    session_phone_login_token: SessionPhoneLoginToken = Body(None, description=""),
...
) -> SessionPhoneLoginTokenCreate:
    """Used to log in a user with a token. apiAdmin:CREATE apiApp:CREATE apiServiceInternal:CREATE"""
    if not BaseSessionPhoneTokenApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseSessionPhoneTokenApi.subclasses[0]().c_reate_session_phone_token(user_agent, x_bunq_client_authentication, session_phone_login_token, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
