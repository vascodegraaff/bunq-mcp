...
from openapi_server.apis.bizum_notification_profile_unregister_api_base import BaseBizumNotificationProfileUnregisterApi
...
from openapi_server.models.bizum_notification_profile_unregister_create import BizumNotificationProfileUnregisterCreate
...
@router.post(
    "/bizum-notification-profile-unregister",
    responses={
        200: {"model": BizumNotificationProfileUnregisterCreate, "description": "Endpoint for receiving notifications that a bunq user has been unregistered by the Bizum system (more likely because they registered with another bank). apiBizum:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["bizum-notification-profile-unregister"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_bizum_notification_profile_unregister(
...
) -> BizumNotificationProfileUnregisterCreate:
    """Endpoint for receiving notifications that a bunq user has been unregistered by the Bizum system (more likely because they registered with another bank). apiBizum:CREATE"""
    if not BaseBizumNotificationProfileUnregisterApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseBizumNotificationProfileUnregisterApi.subclasses[0]().c_reate_bizum_notification_profile_unregister(user_agent, x_bunq_client_authentication, body, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
