...
from openapi_server.apis.support_promotion_type_api_base import BaseSupportPromotionTypeApi
...
from openapi_server.models.support_promotion_type_listing import SupportPromotionTypeListing
...
@router.get(
    "/user/{userID}/support-promotion-type",
    responses={
        200: {"model": List[SupportPromotionTypeListing], "description": "The view endpoint for reading all available promotion type. apiAdmin:LISTING apiServiceInternal:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["support-promotion-type"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_support_promotion_type_for_user(
...
) -> List[SupportPromotionTypeListing]:
    """The view endpoint for reading all available promotion type. apiAdmin:LISTING apiServiceInternal:LISTING"""
    if not BaseSupportPromotionTypeApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseSupportPromotionTypeApi.subclasses[0]().list_all_support_promotion_type_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
