...
from openapi_server.apis.instance_api_base import BaseInstanceApi
...
from openapi_server.models.payment_auto_allocate_instance_listing import PaymentAutoAllocateInstanceListing
from openapi_server.models.payment_auto_allocate_instance_read import PaymentAutoAllocateInstanceRead
...
@router.get(
    ...
    responses={
        200: {"model": List[PaymentAutoAllocateInstanceListing], "description": "List all the times a users payment was automatically allocated. apiApp:LISTING/READ apiPublic:LISTING/READ"},
...
async def list_all_instance_for_user_monetary_account_payment_auto_allocate(
    ...
    payment-auto-allocateID: StrictInt = Path(..., description=""),
...
) -> List[PaymentAutoAllocateInstanceListing]:
    """List all the times a users payment was automatically allocated. apiApp:LISTING/READ apiPublic:LISTING/READ"""
    if not BaseInstanceApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseInstanceApi.subclasses[0]().list_all_instance_for_user_monetary_account_payment_auto_allocate(userID, monetary-accountID, payment-auto-allocateID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/monetary-account/{monetary-accountID}/payment-auto-allocate/{payment-auto-allocateID}/instance/{itemId}",
    responses={
        200: {"model": PaymentAutoAllocateInstanceRead, "description": "List all the times a users payment was automatically allocated. apiApp:LISTING/READ apiPublic:LISTING/READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["instance"],
    summary="",
...
async def r_ead_instance_for_user_monetary_account_payment_auto_allocate(
    userID: StrictInt = Path(..., description=""),
    monetary-accountID: StrictInt = Path(..., description=""),
    payment-auto-allocateID: StrictInt = Path(..., description=""),
    ...
    if not BaseInstanceApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseInstanceApi.subclasses[0]().r_ead_instance_for_user_monetary_account_payment_auto_allocate(userID, monetary-accountID, payment-auto-allocateID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
