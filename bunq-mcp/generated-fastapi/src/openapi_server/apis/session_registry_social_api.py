...
from openapi_server.apis.session_registry_social_api_base import BaseSessionRegistrySocialApi
...
from openapi_server.models.session_registry_social import SessionRegistrySocial
from openapi_server.models.session_registry_social_create import SessionRegistrySocialCreate
...
@router.post(
    "/session-registry-social",
    responses={
        200: {"model": SessionRegistrySocialCreate, "description": "Used to verify a login attempt of the user on a social platform (Facebook, Google, Apple). apiApp:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["session-registry-social"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_session_registry_social(
    ...
    session_registry_social: SessionRegistrySocial = Body(None, description=""),
...
) -> SessionRegistrySocialCreate:
    """Used to verify a login attempt of the user on a social platform (Facebook, Google, Apple). apiApp:CREATE"""
    if not BaseSessionRegistrySocialApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseSessionRegistrySocialApi.subclasses[0]().c_reate_session_registry_social(user_agent, x_bunq_client_authentication, session_registry_social, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
