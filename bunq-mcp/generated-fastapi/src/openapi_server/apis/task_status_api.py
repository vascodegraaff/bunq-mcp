...
from openapi_server.apis.task_status_api_base import BaseTaskStatusApi
...
from openapi_server.models.task_status_listing import TaskStatusListing
...
@router.get(
    "/user/{userID}/task-status",
    responses={
        200: {"model": List[TaskStatusListing], "description": "Gets the TaskStatus of the given employee. apiAdmin:LISTING apiServiceInternal:LISTING"},
...
async def list_all_task_status_for_user(
...
) -> List[TaskStatusListing]:
    """Gets the TaskStatus of the given employee. apiAdmin:LISTING apiServiceInternal:LISTING"""
    if not BaseTaskStatusApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseTaskStatusApi.subclasses[0]().list_all_task_status_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
