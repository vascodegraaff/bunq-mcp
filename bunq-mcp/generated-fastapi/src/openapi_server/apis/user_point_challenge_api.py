...
from openapi_server.apis.user_point_challenge_api_base import BaseUserPointChallengeApi
...
from openapi_server.models.user_point_challenge_listing import UserPointChallengeListing
...
@router.get(
    "/user/{userID}/user-point-challenge",
    responses={
        200: {"model": List[UserPointChallengeListing], "description": "Endpoint to retrieve point challenges. apiAdmin:LISTING apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["user-point-challenge"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_user_point_challenge_for_user(
...
) -> List[UserPointChallengeListing]:
    """Endpoint to retrieve point challenges. apiAdmin:LISTING apiApp:LISTING"""
    if not BaseUserPointChallengeApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseUserPointChallengeApi.subclasses[0]().list_all_user_point_challenge_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
