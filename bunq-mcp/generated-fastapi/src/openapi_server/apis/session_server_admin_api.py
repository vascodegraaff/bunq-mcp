...
from openapi_server.apis.session_server_admin_api_base import BaseSessionServerAdminApi
...
from openapi_server.models.session_server_admin_listing import SessionServerAdminListing
...
@router.get(
    ...
    responses={
        200: {"model": List[SessionServerAdminListing], "description": "Authenticate a session for a server device. apiAdmin:LISTING apiServiceInternal:LISTING apiProcess:LISTING apiRetool:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["session-server-admin"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_session_server_admin(
...
) -> List[SessionServerAdminListing]:
    """Authenticate a session for a server device. apiAdmin:LISTING apiServiceInternal:LISTING apiProcess:LISTING apiRetool:LISTING"""
    if not BaseSessionServerAdminApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseSessionServerAdminApi.subclasses[0]().list_all_session_server_admin(user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
