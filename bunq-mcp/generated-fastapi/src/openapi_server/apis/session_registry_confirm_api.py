...
from openapi_server.apis.session_registry_confirm_api_base import BaseSessionRegistryConfirmApi
...
from openapi_server.models.session_registry_confirm import SessionRegistryConfirm
from openapi_server.models.session_registry_confirm_create import SessionRegistryConfirmCreate
...
@router.post(
    "/session-registry-confirm",
    responses={
        200: {"model": SessionRegistryConfirmCreate, "description": "Used to confirm a login attempt for Registry access. apiApp:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["session-registry-confirm"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_session_registry_confirm(
    ...
    session_registry_confirm: SessionRegistryConfirm = Body(None, description=""),
...
) -> SessionRegistryConfirmCreate:
    """Used to confirm a login attempt for Registry access. apiApp:CREATE"""
    if not BaseSessionRegistryConfirmApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseSessionRegistryConfirmApi.subclasses[0]().c_reate_session_registry_confirm(user_agent, x_bunq_client_authentication, session_registry_confirm, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
