...
from openapi_server.apis.legal_name_api_base import BaseLegalNameApi
...
from openapi_server.models.user_legal_name_listing import UserLegalNameListing
...
@router.get(
    ...
    responses={
        200: {"model": List[UserLegalNameListing], "description": "Endpoint for getting available legal names that can be used by the user. apiApp:LISTING apiPublic:LISTING"},
...
) -> List[UserLegalNameListing]:
    """Endpoint for getting available legal names that can be used by the user. apiApp:LISTING apiPublic:LISTING"""
    if not BaseLegalNameApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseLegalNameApi.subclasses[0]().list_all_legal_name_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
