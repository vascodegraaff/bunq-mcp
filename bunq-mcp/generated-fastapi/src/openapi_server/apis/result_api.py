...
from openapi_server.apis.result_api_base import BaseResultApi
...
from openapi_server.models.watchlist_result_read import WatchlistResultRead
...
@router.get(
    "/watchlist-check/{watchlist-checkID}/result/{itemId}",
    responses={
        200: {"model": WatchlistResultRead, "description": "The endpoint to get the result of a watchlist check. apiAdmin:READ apiServiceInternal:READ"},
...
async def r_ead_result_for_watchlist_check(
    watchlist-checkID: StrictInt = Path(..., description=""),
    ...
    """The endpoint to get the result of a watchlist check. apiAdmin:READ apiServiceInternal:READ"""
    if not BaseResultApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseResultApi.subclasses[0]().r_ead_result_for_watchlist_check(watchlist-checkID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
