...
from openapi_server.apis.partner_promotion_deal_api_base import BasePartnerPromotionDealApi
...
from openapi_server.models.partner_promotion_deal_listing import PartnerPromotionDealListing
from openapi_server.models.partner_promotion_deal_read import PartnerPromotionDealRead
...
@router.get(
    "/user/{userID}/partner-promotion-deal",
    responses={
        200: {"model": List[PartnerPromotionDealListing], "description": "Endpoint to list available partner promotion deals for a user. apiApp:READ/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["partner-promotion-deal"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_partner_promotion_deal_for_user(
...
) -> List[PartnerPromotionDealListing]:
    """Endpoint to list available partner promotion deals for a user. apiApp:READ/LISTING"""
    if not BasePartnerPromotionDealApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BasePartnerPromotionDealApi.subclasses[0]().list_all_partner_promotion_deal_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/partner-promotion-deal/{itemId}",
    responses={
        200: {"model": PartnerPromotionDealRead, "description": "Endpoint to list available partner promotion deals for a user. apiApp:READ/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["partner-promotion-deal"],
    summary="",
...
async def r_ead_partner_promotion_deal_for_user(
    userID: StrictInt = Path(..., description=""),
    ...
    if not BasePartnerPromotionDealApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BasePartnerPromotionDealApi.subclasses[0]().r_ead_partner_promotion_deal_for_user(userID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
