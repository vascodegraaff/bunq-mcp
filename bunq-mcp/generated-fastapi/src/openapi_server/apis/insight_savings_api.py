...
from openapi_server.apis.insight_savings_api_base import BaseInsightSavingsApi
...
from openapi_server.models.insight_savings_listing import InsightSavingsListing
...
@router.get(
    "/user/{userID}/insight-savings",
    responses={
        200: {"model": List[InsightSavingsListing], "description": "The endpoint used to manage insight savings apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["insight-savings"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_insight_savings_for_user(
...
) -> List[InsightSavingsListing]:
    """The endpoint used to manage insight savings apiApp:LISTING"""
    if not BaseInsightSavingsApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseInsightSavingsApi.subclasses[0]().list_all_insight_savings_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
