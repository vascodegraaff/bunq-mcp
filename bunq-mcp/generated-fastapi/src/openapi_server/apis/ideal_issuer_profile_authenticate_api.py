...
from openapi_server.apis.ideal_issuer_profile_authenticate_api_base import BaseIdealIssuerProfileAuthenticateApi
...
from openapi_server.models.ideal_issuer_profile_authentication import IdealIssuerProfileAuthentication
from openapi_server.models.ideal_issuer_profile_authentication_create import IdealIssuerProfileAuthenticationCreate
...
@router.post(
    "/user/{userID}/ideal-issuer-profile-authenticate",
    responses={
        200: {"model": IdealIssuerProfileAuthenticationCreate, "description": "The endpoint to authenticate user to iDEAL profile using the payload URI provided by iDEAL. apiApp:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["ideal-issuer-profile-authenticate"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_ideal_issuer_profile_authenticate_for_user(
    ...
    ideal_issuer_profile_authentication: IdealIssuerProfileAuthentication = Body(None, description=""),
...
) -> IdealIssuerProfileAuthenticationCreate:
    """The endpoint to authenticate user to iDEAL profile using the payload URI provided by iDEAL. apiApp:CREATE"""
    if not BaseIdealIssuerProfileAuthenticateApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseIdealIssuerProfileAuthenticateApi.subclasses[0]().c_reate_ideal_issuer_profile_authenticate_for_user(userID, user_agent, x_bunq_client_authentication, ideal_issuer_profile_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
