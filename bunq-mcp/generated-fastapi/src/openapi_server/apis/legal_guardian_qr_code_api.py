...
from openapi_server.apis.legal_guardian_qr_code_api_base import BaseLegalGuardianQrCodeApi
...
from openapi_server.models.user_legal_guardian_qr_code_create import UserLegalGuardianQrCodeCreate
...
@router.post(
    "/user/{userID}/legal-guardian-qr-code",
    responses={
        200: {"model": UserLegalGuardianQrCodeCreate, "description": "Using this call a minor can create a QR code his legal guardian can use to approve him. apiApp:CREATE/READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["legal-guardian-qr-code"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_legal_guardian_qr_code_for_user(
...
) -> UserLegalGuardianQrCodeCreate:
    """Create a QR code to approve a minor."""
    if not BaseLegalGuardianQrCodeApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseLegalGuardianQrCodeApi.subclasses[0]().c_reate_legal_guardian_qr_code_for_user(userID, user_agent, x_bunq_client_authentication, body, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/legal-guardian-qr-code/{itemId}",
    responses={
        200: {"model": object, "description": "Using this call a minor can create a QR code his legal guardian can use to approve him. apiApp:CREATE/READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["legal-guardian-qr-code"],
    summary="",
...
async def r_ead_legal_guardian_qr_code_for_user(
    userID: StrictInt = Path(..., description=""),
    ...
    """Using this call a minor can create a QR code his legal guardian can use to approve him. apiApp:CREATE/READ"""
    if not BaseLegalGuardianQrCodeApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseLegalGuardianQrCodeApi.subclasses[0]().r_ead_legal_guardian_qr_code_for_user(userID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
