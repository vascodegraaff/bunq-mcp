...
from openapi_server.apis.voucher_api_base import BaseVoucherApi
...
from openapi_server.models.partner_promotion_deal_voucher_create import PartnerPromotionDealVoucherCreate
...
@router.post(
    "/user/{userID}/partner-promotion-deal/{partner-promotion-dealID}/voucher",
    responses={
        200: {"model": PartnerPromotionDealVoucherCreate, "description": "Endpoint to claim a voucher for a partner promotion deal. apiApp:CREATE"},
...
async def c_reate_voucher_for_user_partner_promotion_deal(
    userID: StrictInt = Path(..., description=""),
    partner-promotion-dealID: StrictInt = Path(..., description=""),
...
) -> PartnerPromotionDealVoucherCreate:
    """Endpoint to claim a voucher for a partner promotion deal. apiApp:CREATE"""
    if not BaseVoucherApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseVoucherApi.subclasses[0]().c_reate_voucher_for_user_partner_promotion_deal(userID, partner-promotion-dealID, user_agent, x_bunq_client_authentication, body, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
