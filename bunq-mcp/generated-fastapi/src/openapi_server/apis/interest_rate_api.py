...
from openapi_server.apis.interest_rate_api_base import BaseInterestRateApi
...
from openapi_server.models.user_interest_rate_listing import UserInterestRateListing
...
@router.get(
    "/user/{userID}/interest-rate",
    responses={
        200: {"model": List[UserInterestRateListing], "description": "Get the currently applicable interest rate for the user. apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING"},
...
async def list_all_interest_rate_for_user(
...
) -> List[UserInterestRateListing]:
    """Get the currently applicable interest rate for the user. apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING"""
    if not BaseInterestRateApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseInterestRateApi.subclasses[0]().list_all_interest_rate_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
