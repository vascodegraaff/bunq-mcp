...
from openapi_server.apis.instant_payment_credit_transfer_incoming_api_base import BaseInstantPaymentCreditTransferIncomingApi
...
from openapi_server.models.instant_payment_credit_transfer_incoming_read import InstantPaymentCreditTransferIncomingRead
...
@router.get(
    "/instant-payment-credit-transfer-incoming/{itemId}",
    responses={
        200: {"model": InstantPaymentCreditTransferIncomingRead, "description": "Endpoint used by the search API. apiAdmin:READ apiServiceInternal:READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["instant-payment-credit-transfer-incoming"],
    summary="",
    response_model_by_alias=True,
)
async def r_ead_instant_payment_credit_transfer_incoming(
...
) -> InstantPaymentCreditTransferIncomingRead:
    """Endpoint used by the search API. apiAdmin:READ apiServiceInternal:READ"""
    if not BaseInstantPaymentCreditTransferIncomingApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseInstantPaymentCreditTransferIncomingApi.subclasses[0]().r_ead_instant_payment_credit_transfer_incoming(itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
