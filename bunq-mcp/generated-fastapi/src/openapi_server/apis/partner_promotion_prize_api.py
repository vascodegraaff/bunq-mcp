...
from openapi_server.apis.partner_promotion_prize_api_base import BasePartnerPromotionPrizeApi
...
from openapi_server.models.partner_promotion_prize_listing import PartnerPromotionPrizeListing
...
@router.get(
    "/user/{userID}/partner-promotion-prize",
    responses={
        200: {"model": List[PartnerPromotionPrizeListing], "description": "Get available prizes for Points. apiAdmin:LISTING apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["partner-promotion-prize"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_partner_promotion_prize_for_user(
...
) -> List[PartnerPromotionPrizeListing]:
    """Get available prizes for Points. apiAdmin:LISTING apiApp:LISTING"""
    if not BasePartnerPromotionPrizeApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BasePartnerPromotionPrizeApi.subclasses[0]().list_all_partner_promotion_prize_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
