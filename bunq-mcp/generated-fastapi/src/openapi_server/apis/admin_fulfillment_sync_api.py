...
from openapi_server.apis.admin_fulfillment_sync_api_base import BaseAdminFulfillmentSyncApi
...
from openapi_server.models.admin_fulfillment_sync_create import AdminFulfillmentSyncCreate
...
@router.post(
    "/user/{userID}/admin-fulfillment-sync",
    responses={
        200: {"model": AdminFulfillmentSyncCreate, "description": "Endpoint to trigger the fulfillment sync apiAdmin:CREATE apiServiceInternal:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["admin-fulfillment-sync"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_admin_fulfillment_sync_for_user(
...
) -> AdminFulfillmentSyncCreate:
    """Endpoint to trigger the fulfillment sync apiAdmin:CREATE apiServiceInternal:CREATE"""
    if not BaseAdminFulfillmentSyncApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAdminFulfillmentSyncApi.subclasses[0]().c_reate_admin_fulfillment_sync_for_user(userID, user_agent, x_bunq_client_authentication, body, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
