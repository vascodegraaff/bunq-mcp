...
from openapi_server.apis.admin_credential_srp_api_base import BaseAdminCredentialSrpApi
...
from openapi_server.models.admin_user_credential_srp import AdminUserCredentialSrp
from openapi_server.models.admin_user_credential_srp_create import AdminUserCredentialSrpCreate
...
@router.post(
    "/user/{userID}/admin-credential-srp",
    responses={
        200: {"model": AdminUserCredentialSrpCreate, "description": "Used to create a new SRP credential for the admin. apiAdmin:CREATE apiServiceInternal:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["admin-credential-srp"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_admin_credential_srp_for_user(
    ...
    admin_user_credential_srp: AdminUserCredentialSrp = Body(None, description=""),
...
) -> AdminUserCredentialSrpCreate:
    """Used to create a new SRP credential for the admin. apiAdmin:CREATE apiServiceInternal:CREATE"""
    if not BaseAdminCredentialSrpApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAdminCredentialSrpApi.subclasses[0]().c_reate_admin_credential_srp_for_user(userID, user_agent, x_bunq_client_authentication, admin_user_credential_srp, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
