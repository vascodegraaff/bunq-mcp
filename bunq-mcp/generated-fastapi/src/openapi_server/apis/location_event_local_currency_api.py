...
from openapi_server.apis.location_event_local_currency_api_base import BaseLocationEventLocalCurrencyApi
...
from openapi_server.models.user_location_event_local_currency_listing import UserLocationEventLocalCurrencyListing
...
@router.get(
    "/user/{userID}/location-event-local-currency",
    responses={
        200: {"model": List[UserLocationEventLocalCurrencyListing], "description": "Local Currency location events of the user apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["location-event-local-currency"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_location_event_local_currency_for_user(
...
) -> List[UserLocationEventLocalCurrencyListing]:
    """Local Currency location events of the user apiApp:LISTING"""
    if not BaseLocationEventLocalCurrencyApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseLocationEventLocalCurrencyApi.subclasses[0]().list_all_location_event_local_currency_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
