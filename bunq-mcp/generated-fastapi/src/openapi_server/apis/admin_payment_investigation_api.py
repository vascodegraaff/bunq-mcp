...
from openapi_server.apis.admin_payment_investigation_api_base import BaseAdminPaymentInvestigationApi
...
from openapi_server.models.admin_payment_investigation_listing import AdminPaymentInvestigationListing
...
@router.get(
    "/admin-payment-investigation",
    responses={
        200: {"model": List[AdminPaymentInvestigationListing], "description": "The the payment investigations initiated by users. apiAdmin:LISTING apiServiceInternal:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["admin-payment-investigation"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_admin_payment_investigation(
...
) -> List[AdminPaymentInvestigationListing]:
    """List and filter on all payment investigations initiated by users."""
    if not BaseAdminPaymentInvestigationApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAdminPaymentInvestigationApi.subclasses[0]().list_all_admin_payment_investigation(user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
