...
from openapi_server.apis.transferwise_recipient_requirement_api_base import BaseTransferwiseRecipientRequirementApi
...
from openapi_server.models.transferwise_account_requirement import TransferwiseAccountRequirement
from openapi_server.models.transferwise_account_requirement_create import TransferwiseAccountRequirementCreate
from openapi_server.models.transferwise_account_requirement_listing import TransferwiseAccountRequirementListing
...
@router.post(
    ...
    responses={
        200: {"model": TransferwiseAccountRequirementCreate, "description": "Used to determine the recipient account requirements for Transferwise transfers. apiApp:CREATE/LISTING apiPublic:CREATE/LISTING"},
...
async def c_reate_transferwise_recipient_requirement_for_user_transferwise_quote(
    ...
    transferwise_account_requirement: TransferwiseAccountRequirement = Body(None, description=""),
    ...
    """Used to determine the recipient account requirements for Transferwise transfers. apiApp:CREATE/LISTING apiPublic:CREATE/LISTING"""
    if not BaseTransferwiseRecipientRequirementApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseTransferwiseRecipientRequirementApi.subclasses[0]().c_reate_transferwise_recipient_requirement_for_user_transferwise_quote(userID, transferwise-quoteID, user_agent, x_bunq_client_authentication, transferwise_account_requirement, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/transferwise-quote/{transferwise-quoteID}/transferwise-recipient-requirement",
    responses={
        200: {"model": List[TransferwiseAccountRequirementListing], "description": "Used to determine the recipient account requirements for Transferwise transfers. apiApp:CREATE/LISTING apiPublic:CREATE/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["transferwise-recipient-requirement"],
    summary="",
...
async def list_all_transferwise_recipient_requirement_for_user_transferwise_quote(
    ...
    x_bunq_client_request_id: Annotated[Optional[StrictStr], Field(description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer.")] = Header(None, description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer."),
    x_bunq_geolocation: Annotated[Optional[StrictStr], Field(description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued.")] = Header(None, description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued."),
) -> List[TransferwiseAccountRequirementListing]:
    """Used to determine the recipient account requirements for Transferwise transfers. apiApp:CREATE/LISTING apiPublic:CREATE/LISTING"""
    if not BaseTransferwiseRecipientRequirementApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseTransferwiseRecipientRequirementApi.subclasses[0]().list_all_transferwise_recipient_requirement_for_user_transferwise_quote(userID, transferwise-quoteID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
