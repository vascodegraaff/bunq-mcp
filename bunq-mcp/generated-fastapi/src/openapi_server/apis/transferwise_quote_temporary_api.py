...
from openapi_server.apis.transferwise_quote_temporary_api_base import BaseTransferwiseQuoteTemporaryApi
...
from openapi_server.models.transferwise_quote_temporary import TransferwiseQuoteTemporary
from openapi_server.models.transferwise_quote_temporary_create import TransferwiseQuoteTemporaryCreate
from openapi_server.models.transferwise_quote_temporary_read import TransferwiseQuoteTemporaryRead
...
@router.post(
    ...
    responses={
        200: {"model": TransferwiseQuoteTemporaryCreate, "description": "Used to get temporary quotes from Transferwise. These cannot be used to initiate payments apiApp:CREATE/READ apiPublic:CREATE/READ"},
...
async def c_reate_transferwise_quote_temporary_for_user(
    ...
    transferwise_quote_temporary: TransferwiseQuoteTemporary = Body(None, description=""),
    ...
    """Used to get temporary quotes from Transferwise. These cannot be used to initiate payments apiApp:CREATE/READ apiPublic:CREATE/READ"""
    if not BaseTransferwiseQuoteTemporaryApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseTransferwiseQuoteTemporaryApi.subclasses[0]().c_reate_transferwise_quote_temporary_for_user(userID, user_agent, x_bunq_client_authentication, transferwise_quote_temporary, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/transferwise-quote-temporary/{itemId}",
    responses={
        200: {"model": TransferwiseQuoteTemporaryRead, "description": "Used to get temporary quotes from Transferwise. These cannot be used to initiate payments apiApp:CREATE/READ apiPublic:CREATE/READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["transferwise-quote-temporary"],
    summary="",
...
async def r_ead_transferwise_quote_temporary_for_user(
    userID: StrictInt = Path(..., description=""),
    ...
    if not BaseTransferwiseQuoteTemporaryApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseTransferwiseQuoteTemporaryApi.subclasses[0]().r_ead_transferwise_quote_temporary_for_user(userID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
