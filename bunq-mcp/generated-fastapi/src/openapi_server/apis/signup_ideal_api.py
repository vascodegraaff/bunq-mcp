...
from openapi_server.apis.signup_ideal_api_base import BaseSignupIdealApi
...
from openapi_server.models.ideal_signup_listing import IdealSignupListing
from openapi_server.models.ideal_signup_read import IdealSignupRead
...
@router.get(
    "/user/{userID}/signup-ideal",
    responses={
        200: {"model": List[IdealSignupListing], "description": "Endpoint to initiate (and poll) the status of the ideal step during signup. apiAdmin:READ/LISTING apiApp:READ/LISTING apiServiceInternal:READ/LISTING"},
...
async def list_all_signup_ideal_for_user(
...
) -> List[IdealSignupListing]:
    """Endpoint to initiate (and poll) the status of the ideal step during signup. apiAdmin:READ/LISTING apiApp:READ/LISTING apiServiceInternal:READ/LISTING"""
    if not BaseSignupIdealApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseSignupIdealApi.subclasses[0]().list_all_signup_ideal_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/signup-ideal/{itemId}",
    responses={
        200: {"model": IdealSignupRead, "description": "Endpoint to initiate (and poll) the status of the ideal step during signup. apiAdmin:READ/LISTING apiApp:READ/LISTING apiServiceInternal:READ/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["signup-ideal"],
    summary="",
...
async def r_ead_signup_ideal_for_user(
    userID: StrictInt = Path(..., description=""),
    ...
    if not BaseSignupIdealApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseSignupIdealApi.subclasses[0]().r_ead_signup_ideal_for_user(userID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
