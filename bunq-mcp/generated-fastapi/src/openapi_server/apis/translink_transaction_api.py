...
from openapi_server.apis.translink_transaction_api_base import BaseTranslinkTransactionApi
...
from openapi_server.models.translink_transaction import TranslinkTransaction
from openapi_server.models.translink_transaction_create import TranslinkTransactionCreate
from openapi_server.models.translink_transaction_listing import TranslinkTransactionListing
from openapi_server.models.translink_transaction_read import TranslinkTransactionRead
...
@router.post(
    ...
    responses={
        200: {"model": TranslinkTransactionCreate, "description": "Used to create translink transactions. apiTranslink:CREATE/READ/LISTING"},
...
async def c_reate_translink_transaction_for_user_monetary_account(
    ...
    translink_transaction: TranslinkTransaction = Body(None, description=""),
    ...
    """Used to create translink transactions. apiTranslink:CREATE/READ/LISTING"""
    if not BaseTranslinkTransactionApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseTranslinkTransactionApi.subclasses[0]().c_reate_translink_transaction_for_user_monetary_account(userID, monetary-accountID, user_agent, x_bunq_client_authentication, translink_transaction, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/monetary-account/{monetary-accountID}/translink-transaction",
    responses={
        200: {"model": List[TranslinkTransactionListing], "description": "Used to create translink transactions. apiTranslink:CREATE/READ/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["translink-transaction"],
    summary="",
...
async def list_all_translink_transaction_for_user_monetary_account(
    ...
    x_bunq_client_request_id: Annotated[Optional[StrictStr], Field(description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer.")] = Header(None, description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer."),
    x_bunq_geolocation: Annotated[Optional[StrictStr], Field(description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued.")] = Header(None, description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued."),
) -> List[TranslinkTransactionListing]:
    """Used to create translink transactions. apiTranslink:CREATE/READ/LISTING"""
    if not BaseTranslinkTransactionApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseTranslinkTransactionApi.subclasses[0]().list_all_translink_transaction_for_user_monetary_account(userID, monetary-accountID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/monetary-account/{monetary-accountID}/translink-transaction/{itemId}",
    responses={
        200: {"model": TranslinkTransactionRead, "description": "Used to create translink transactions. apiTranslink:CREATE/READ/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["translink-transaction"],
    summary="",
...
async def r_ead_translink_transaction_for_user_monetary_account(
    userID: StrictInt = Path(..., description=""),
    monetary-accountID: StrictInt = Path(..., description=""),
    ...
    if not BaseTranslinkTransactionApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseTranslinkTransactionApi.subclasses[0]().r_ead_translink_transaction_for_user_monetary_account(userID, monetary-accountID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
