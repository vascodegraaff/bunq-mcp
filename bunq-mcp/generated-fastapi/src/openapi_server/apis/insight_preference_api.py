...
from openapi_server.apis.insight_preference_api_base import BaseInsightPreferenceApi
...
from openapi_server.models.insight_preference_listing import InsightPreferenceListing
...
@router.get(
    "/user/{userID}/insight-preference",
    responses={
        200: {"model": List[InsightPreferenceListing], "description": "Used to allow users to set insight/budget preferences. apiApp:LISTING"},
...
async def list_all_insight_preference_for_user(
...
) -> List[InsightPreferenceListing]:
    """Used to allow users to set insight/budget preferences. apiApp:LISTING"""
    if not BaseInsightPreferenceApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseInsightPreferenceApi.subclasses[0]().list_all_insight_preference_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
