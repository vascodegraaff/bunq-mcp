...
from openapi_server.models.admin_user_review_announcement_listing import AdminUserReviewAnnouncementListing
...
@router.get(
    "/user/{userID}/review/{reviewID}/announcement",
    responses={
        200: {"model": List[AdminUserReviewAnnouncementListing], "description": "Announcement for user review. Used for listing all announcements of a given user review. apiAdmin:LISTING apiServiceInternal:LISTING"},
...
async def list_all_announcement_for_user_review(
...
) -> List[AdminUserReviewAnnouncementListing]:
    """Announcement for user review. Used for listing all announcements of a given user review. apiAdmin:LISTING apiServiceInternal:LISTING"""
    if not BaseAnnouncementApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAnnouncementApi.subclasses[0]().list_all_announcement_for_user_review(userID, reviewID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
