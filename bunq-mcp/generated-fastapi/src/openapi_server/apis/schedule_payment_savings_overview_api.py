...
from openapi_server.apis.schedule_payment_savings_overview_api_base import BaseSchedulePaymentSavingsOverviewApi
...
from openapi_server.models.schedule_payment_savings_overview_listing import SchedulePaymentSavingsOverviewListing
...
@router.get(
    "/user/{userID}/monetary-account/{monetary-accountID}/schedule-payment-savings-overview",
    responses={
        200: {"model": List[SchedulePaymentSavingsOverviewListing], "description": "Endpoint to retrieve suggested schedule savings amount. apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["schedule-payment-savings-overview"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_schedule_payment_savings_overview_for_user_monetary_account(
...
) -> List[SchedulePaymentSavingsOverviewListing]:
    """Endpoint to retrieve suggested schedule savings amount. apiApp:LISTING"""
    if not BaseSchedulePaymentSavingsOverviewApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseSchedulePaymentSavingsOverviewApi.subclasses[0]().list_all_schedule_payment_savings_overview_for_user_monetary_account(userID, monetary-accountID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
