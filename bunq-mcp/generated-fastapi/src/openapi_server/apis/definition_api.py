...
from openapi_server.apis.definition_api_base import BaseDefinitionApi
...
from openapi_server.models.payment_auto_allocate_definition_listing import PaymentAutoAllocateDefinitionListing
...
@router.get(
    ...
    responses={
        200: {"model": List[PaymentAutoAllocateDefinitionListing], "description": "List all the definitions in a payment auto allocate. apiApp:LISTING apiPublic:LISTING"},
...
) -> List[PaymentAutoAllocateDefinitionListing]:
    """List all the definitions in a payment auto allocate. apiApp:LISTING apiPublic:LISTING"""
    if not BaseDefinitionApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseDefinitionApi.subclasses[0]().list_all_definition_for_user_monetary_account_payment_auto_allocate(userID, monetary-accountID, payment-auto-allocateID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
