...
from openapi_server.apis.savings_auto_save_result_api_base import BaseSavingsAutoSaveResultApi
...
from openapi_server.models.savings_auto_save_result_read import SavingsAutoSaveResultRead
from openapi_server.models.savings_auto_save_result_user_listing import SavingsAutoSaveResultUserListing
...
@router.get(
    "/user/{userID}/savings-auto-save-result",
    responses={
        200: {"model": List[SavingsAutoSaveResultUserListing], "description": "Auto Saving result containing all payments. apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["savings-auto-save-result"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_savings_auto_save_result_for_user(
...
) -> List[SavingsAutoSaveResultUserListing]:
    """Auto Saving result containing all payments. apiApp:LISTING"""
    if not BaseSavingsAutoSaveResultApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseSavingsAutoSaveResultApi.subclasses[0]().list_all_savings_auto_save_result_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/monetary-account/{monetary-accountID}/savings-auto-save-result/{itemId}",
    responses={
        200: {"model": SavingsAutoSaveResultRead, "description": "Auto Saving result containing all payments. apiAdmin:READ apiApp:READ apiServiceInternal:READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["savings-auto-save-result"],
    summary="",
...
async def r_ead_savings_auto_save_result_for_user_monetary_account(
    userID: StrictInt = Path(..., description=""),
    ...
    x_bunq_client_request_id: Annotated[Optional[StrictStr], Field(description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer.")] = Header(None, description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer."),
    x_bunq_geolocation: Annotated[Optional[StrictStr], Field(description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued.")] = Header(None, description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued."),
) -> SavingsAutoSaveResultRead:
    """Auto Saving result containing all payments. apiAdmin:READ apiApp:READ apiServiceInternal:READ"""
    if not BaseSavingsAutoSaveResultApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseSavingsAutoSaveResultApi.subclasses[0]().r_ead_savings_auto_save_result_for_user_monetary_account(userID, monetary-accountID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
