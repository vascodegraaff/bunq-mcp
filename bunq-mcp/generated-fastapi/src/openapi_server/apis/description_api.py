...
from openapi_server.apis.description_api_base import BaseDescriptionApi
...
from openapi_server.models.assessment_description_listing import AssessmentDescriptionListing
from openapi_server.models.assessment_description_read import AssessmentDescriptionRead
...
@router.get(
    ...
    responses={
        200: {"model": List[AssessmentDescriptionListing], "description": "This endpoint handles all descriptions on assessments apiAdmin:READ/LISTING apiServiceInternal:READ/LISTING"},
...
async def list_all_description_for_assessment(
    assessmentID: StrictInt = Path(..., description=""),
...
) -> List[AssessmentDescriptionListing]:
    """This endpoint handles all descriptions on assessments apiAdmin:READ/LISTING apiServiceInternal:READ/LISTING"""
    if not BaseDescriptionApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseDescriptionApi.subclasses[0]().list_all_description_for_assessment(assessmentID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/assessment/{assessmentID}/description/{itemId}",
    responses={
        200: {"model": AssessmentDescriptionRead, "description": "This endpoint handles all descriptions on assessments apiAdmin:READ/LISTING apiServiceInternal:READ/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["description"],
    summary="",
...
async def r_ead_description_for_assessment(
    assessmentID: StrictInt = Path(..., description=""),
    ...
    x_bunq_client_request_id: Annotated[Optional[StrictStr], Field(description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer.")] = Header(None, description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer."),
    x_bunq_geolocation: Annotated[Optional[StrictStr], Field(description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued.")] = Header(None, description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued."),
) -> AssessmentDescriptionRead:
    """This endpoint handles all descriptions on assessments apiAdmin:READ/LISTING apiServiceInternal:READ/LISTING"""
    if not BaseDescriptionApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseDescriptionApi.subclasses[0]().r_ead_description_for_assessment(assessmentID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
