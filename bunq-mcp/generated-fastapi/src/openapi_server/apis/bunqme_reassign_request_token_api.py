...
from openapi_server.apis.bunqme_reassign_request_token_api_base import BaseBunqmeReassignRequestTokenApi
...
from openapi_server.models.bunq_me_reassign_request_token_create import BunqMeReassignRequestTokenCreate
from openapi_server.models.bunq_me_reassign_request_token_read import BunqMeReassignRequestTokenRead
...
@router.post(
    "/bunqme-request/{bunqme-requestUUID}/bunqme-reassign-request-token",
    responses={
        200: {"model": BunqMeReassignRequestTokenCreate, "description": "Create a bunq.me QR token used for transferring the request for payment. apiBunqMe:CREATE/READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["bunqme-reassign-request-token"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_bunqme_reassign_request_token_for_bunqme_request(
    bunqme-requestUUID: StrictStr = Path(..., description=""),
...
) -> BunqMeReassignRequestTokenCreate:
    """Create a bunq.me QR token used for transferring the request for payment. apiBunqMe:CREATE/READ"""
    if not BaseBunqmeReassignRequestTokenApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseBunqmeReassignRequestTokenApi.subclasses[0]().c_reate_bunqme_reassign_request_token_for_bunqme_request(bunqme-requestUUID, user_agent, x_bunq_client_authentication, body, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/bunqme-request/{bunqme-requestUUID}/bunqme-reassign-request-token/{itemId}",
    responses={
        200: {"model": BunqMeReassignRequestTokenRead, "description": "Create a bunq.me QR token used for transferring the request for payment. apiBunqMe:CREATE/READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["bunqme-reassign-request-token"],
    summary="",
...
async def r_ead_bunqme_reassign_request_token_for_bunqme_request(
    bunqme-requestUUID: StrictStr = Path(..., description=""),
    ...
    x_bunq_client_request_id: Annotated[Optional[StrictStr], Field(description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer.")] = Header(None, description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer."),
    x_bunq_geolocation: Annotated[Optional[StrictStr], Field(description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued.")] = Header(None, description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued."),
) -> BunqMeReassignRequestTokenRead:
    """Create a bunq.me QR token used for transferring the request for payment. apiBunqMe:CREATE/READ"""
    if not BaseBunqmeReassignRequestTokenApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseBunqmeReassignRequestTokenApi.subclasses[0]().r_ead_bunqme_reassign_request_token_for_bunqme_request(bunqme-requestUUID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
