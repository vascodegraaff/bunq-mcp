...
from openapi_server.apis.bunqme_merchant_directory_ideal_api_base import BaseBunqmeMerchantDirectoryIdealApi
...
from openapi_server.models.bunq_me_merchant_directory_ideal_listing import BunqMeMerchantDirectoryIdealListing
...
@router.get(
    ...
    responses={
        200: {"model": List[BunqMeMerchantDirectoryIdealListing], "description": "View for displaying the merchant issuer directory to customers apiBunqMe:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["bunqme-merchant-directory-ideal"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_bunqme_merchant_directory_ideal(
...
) -> List[BunqMeMerchantDirectoryIdealListing]:
    """View for displaying the merchant issuer directory to customers apiBunqMe:LISTING"""
    if not BaseBunqmeMerchantDirectoryIdealApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseBunqmeMerchantDirectoryIdealApi.subclasses[0]().list_all_bunqme_merchant_directory_ideal(user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
