...
from openapi_server.apis.point_mutation_api_base import BasePointMutationApi
...
from openapi_server.models.point_mutation_listing import PointMutationListing
...
@router.get(
    "/user/{userID}/point-mutation",
    responses={
        200: {"model": List[PointMutationListing], "description": "Endpoint to retrieve point mutation. apiAdmin:LISTING apiApp:LISTING"},
...
async def list_all_point_mutation_for_user(
...
) -> List[PointMutationListing]:
    """Endpoint to retrieve point mutation. apiAdmin:LISTING apiApp:LISTING"""
    if not BasePointMutationApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BasePointMutationApi.subclasses[0]().list_all_point_mutation_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
