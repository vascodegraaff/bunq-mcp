...
from openapi_server.apis.card_limited_edition_remaining_api_base import BaseCardLimitedEditionRemainingApi
...
from openapi_server.models.card_limited_edition_remaining_listing import CardLimitedEditionRemainingListing
...
@router.get(
    "/user/{userID}/card-limited-edition-remaining",
    responses={
        200: {"model": List[CardLimitedEditionRemainingListing], "description": "Lists number of remaining limited edition cards. apiAdmin:LISTING apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["card-limited-edition-remaining"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_card_limited_edition_remaining_for_user(
...
) -> List[CardLimitedEditionRemainingListing]:
    """Lists number of remaining limited edition cards. apiAdmin:LISTING apiApp:LISTING"""
    if not BaseCardLimitedEditionRemainingApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseCardLimitedEditionRemainingApi.subclasses[0]().list_all_card_limited_edition_remaining_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
