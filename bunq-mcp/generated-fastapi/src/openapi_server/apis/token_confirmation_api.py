...
from openapi_server.apis.token_confirmation_api_base import BaseTokenConfirmationApi
...
from openapi_server.models.token_confirmation_create import TokenConfirmationCreate
...
@router.post(
    "/user/{userID}/token-confirmation",
    responses={
        200: {"model": TokenConfirmationCreate, "description": "This call is used to confirm a future API call. When getting error code 461 as a response you need to confirm your call by calling token-confirmation and then doing your original call again. apiAdmin:CREATE apiApp:CREATE apiServiceInternal:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["token-confirmation"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_token_confirmation_for_user(
...
) -> TokenConfirmationCreate:
    """The body of this request is empty. The response contains the Token you are using in your current Session."""
    if not BaseTokenConfirmationApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseTokenConfirmationApi.subclasses[0]().c_reate_token_confirmation_for_user(userID, user_agent, x_bunq_client_authentication, body, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
