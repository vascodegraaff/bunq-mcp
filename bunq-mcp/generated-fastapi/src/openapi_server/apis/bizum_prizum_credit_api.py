...
from openapi_server.apis.bizum_prizum_credit_api_base import BaseBizumPrizumCreditApi
...
from openapi_server.models.bizum_prizum_credit_create import BizumPrizumCreditCreate
...
@router.post(
    "/bizum-prizum-credit",
    responses={
        200: {"model": BizumPrizumCreditCreate, "description": "Endpoint for receiving Bizum Payments via the Prizum protocol. apiBizum:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["bizum-prizum-credit"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_bizum_prizum_credit(
...
) -> BizumPrizumCreditCreate:
    """Endpoint for receiving Bizum Payments via the Prizum protocol. apiBizum:CREATE"""
    if not BaseBizumPrizumCreditApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseBizumPrizumCreditApi.subclasses[0]().c_reate_bizum_prizum_credit(user_agent, x_bunq_client_authentication, body, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
