...
from openapi_server.apis.attachment_report_user_api_base import BaseAttachmentReportUserApi
...
from openapi_server.models.attachment_report_user import AttachmentReportUser
from openapi_server.models.attachment_report_user_create import AttachmentReportUserCreate
...
@router.post(
    "/user/{userID}/attachment-report-user",
    responses={
        200: {"model": AttachmentReportUserCreate, "description": "Endpoint used to report abusive material in the form of attachments by users. apiAdmin:CREATE apiApp:CREATE apiServiceInternal:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["attachment-report-user"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_attachment_report_user_for_user(
    ...
    attachment_report_user: AttachmentReportUser = Body(None, description=""),
...
) -> AttachmentReportUserCreate:
    """Endpoint used to report abusive material in the form of attachments by users. apiAdmin:CREATE apiApp:CREATE apiServiceInternal:CREATE"""
    if not BaseAttachmentReportUserApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAttachmentReportUserApi.subclasses[0]().c_reate_attachment_report_user_for_user(userID, user_agent, x_bunq_client_authentication, attachment_report_user, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
