...
from openapi_server.apis.ginmon_custodian_document_api_base import BaseGinmonCustodianDocumentApi
...
from openapi_server.models.ginmon_custodian_document_listing import GinmonCustodianDocumentListing
...
@router.get(
    "/user/{userID}/ginmon-custodian-document",
    responses={
        200: {"model": List[GinmonCustodianDocumentListing], "description": "Get custodial documents for a Ginmon account. apiAdmin:LISTING apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["ginmon-custodian-document"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_ginmon_custodian_document_for_user(
...
) -> List[GinmonCustodianDocumentListing]:
    """Get custodial documents for a Ginmon account. apiAdmin:LISTING apiApp:LISTING"""
    if not BaseGinmonCustodianDocumentApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseGinmonCustodianDocumentApi.subclasses[0]().list_all_ginmon_custodian_document_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
