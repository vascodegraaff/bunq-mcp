...
from openapi_server.apis.note_attachment_batch_api_base import BaseNoteAttachmentBatchApi
...
from openapi_server.models.note_attachment_batch import NoteAttachmentBatch
from openapi_server.models.note_attachment_batch_create import NoteAttachmentBatchCreate
...
@router.post(
    "/user/{userID}/note-attachment-batch",
    responses={
        200: {"model": NoteAttachmentBatchCreate, "description": "Endpoint to add attachments as notes to events in batches. apiApp:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["note-attachment-batch"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_note_attachment_batch_for_user(
    ...
    note_attachment_batch: NoteAttachmentBatch = Body(None, description=""),
...
) -> NoteAttachmentBatchCreate:
    """Endpoint to add attachments as notes to events in batches. apiApp:CREATE"""
    if not BaseNoteAttachmentBatchApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseNoteAttachmentBatchApi.subclasses[0]().c_reate_note_attachment_batch_for_user(userID, user_agent, x_bunq_client_authentication, note_attachment_batch, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
