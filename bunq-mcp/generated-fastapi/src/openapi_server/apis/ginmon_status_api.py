...
from openapi_server.apis.ginmon_status_api_base import BaseGinmonStatusApi
...
from openapi_server.models.ginmon_status_listing import GinmonStatusListing
...
@router.get(
    "/ginmon-status",
    responses={
        200: {"model": List[GinmonStatusListing], "description": "Listing for Ginmon status. apiApp:LISTING"},
...
async def list_all_ginmon_status(
...
) -> List[GinmonStatusListing]:
    """Listing for Ginmon status. apiApp:LISTING"""
    if not BaseGinmonStatusApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseGinmonStatusApi.subclasses[0]().list_all_ginmon_status(user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
