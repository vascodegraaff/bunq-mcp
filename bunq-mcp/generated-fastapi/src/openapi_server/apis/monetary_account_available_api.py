...
from openapi_server.apis.monetary_account_available_api_base import BaseMonetaryAccountAvailableApi
...
from openapi_server.models.monetary_account_available_listing import MonetaryAccountAvailableListing
...
@router.get(
    "/user/{userID}/monetary-account-available",
    responses={
        200: {"model": List[MonetaryAccountAvailableListing], "description": "Endpoint for listing availability of monetary accounts for a user. apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["monetary-account-available"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_monetary_account_available_for_user(
...
) -> List[MonetaryAccountAvailableListing]:
    """Endpoint for listing availability of monetary accounts for a user. apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING"""
    if not BaseMonetaryAccountAvailableApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseMonetaryAccountAvailableApi.subclasses[0]().list_all_monetary_account_available_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
