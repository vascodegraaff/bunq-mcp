...
from openapi_server.apis.mastercard_benefit_api_base import BaseMastercardBenefitApi
...
from openapi_server.models.master_card_benefit_card_listing import MasterCardBenefitCardListing
from openapi_server.models.master_card_benefit_purchase_listing import MasterCardBenefitPurchaseListing
...
@router.get(
    "/user/{userID}/card/{cardID}/mastercard-benefit",
    responses={
        200: {"model": List[MasterCardBenefitCardListing], "description": "MasterCard benefit by Card view. apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["mastercard-benefit"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_mastercard_benefit_for_user_card(
...
) -> List[MasterCardBenefitCardListing]:
    """MasterCard benefit by Card view. apiApp:LISTING"""
    if not BaseMastercardBenefitApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseMastercardBenefitApi.subclasses[0]().list_all_mastercard_benefit_for_user_card(userID, cardID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/monetary-account/{monetary-accountID}/mastercard-action/{mastercard-actionID}/mastercard-benefit",
    responses={
        200: {"model": List[MasterCardBenefitPurchaseListing], "description": "MasterCard benefit by MasterCardAction view. apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["mastercard-benefit"],
    summary="",
...
async def list_all_mastercard_benefit_for_user_monetary_account_mastercard_action(
    userID: StrictInt = Path(..., description=""),
    ...
    x_bunq_client_request_id: Annotated[Optional[StrictStr], Field(description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer.")] = Header(None, description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer."),
    x_bunq_geolocation: Annotated[Optional[StrictStr], Field(description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued.")] = Header(None, description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued."),
) -> List[MasterCardBenefitPurchaseListing]:
    """MasterCard benefit by MasterCardAction view. apiApp:LISTING"""
    if not BaseMastercardBenefitApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseMastercardBenefitApi.subclasses[0]().list_all_mastercard_benefit_for_user_monetary_account_mastercard_action(userID, monetary-accountID, mastercard-actionID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
