...
from openapi_server.apis.link_universal_api_base import BaseLinkUniversalApi
...
from openapi_server.models.link_universal_listing import LinkUniversalListing
...
@router.get(
    "/link-universal",
    responses={
        200: {"model": List[LinkUniversalListing], "description": "Get the details for a UUID from a link, so we can have simple and consistent links and apps can ask the backend how they should be handled. apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING"},
...
async def list_all_link_universal(
...
) -> List[LinkUniversalListing]:
    """Get the details for a UUID from a link."""
    if not BaseLinkUniversalApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseLinkUniversalApi.subclasses[0]().list_all_link_universal(user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
