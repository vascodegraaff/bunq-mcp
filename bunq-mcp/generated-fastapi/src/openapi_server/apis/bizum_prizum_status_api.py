...
from openapi_server.apis.bizum_prizum_status_api_base import BaseBizumPrizumStatusApi
...
from openapi_server.models.bizum_prizum_status_create import BizumPrizumStatusCreate
...
@router.post(
    "/bizum-prizum-status",
    responses={
        200: {"model": BizumPrizumStatusCreate, "description": "Endpoint for receiving status requets for Bizum transactions via the Prizum protocol. apiBizum:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["bizum-prizum-status"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_bizum_prizum_status(
...
) -> BizumPrizumStatusCreate:
    """Endpoint for receiving status requets for Bizum transactions via the Prizum protocol. apiBizum:CREATE"""
    if not BaseBizumPrizumStatusApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseBizumPrizumStatusApi.subclasses[0]().c_reate_bizum_prizum_status(user_agent, x_bunq_client_authentication, body, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
