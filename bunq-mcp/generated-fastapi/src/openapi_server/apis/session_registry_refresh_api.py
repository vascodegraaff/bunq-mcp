...
from openapi_server.apis.session_registry_refresh_api_base import BaseSessionRegistryRefreshApi
...
from openapi_server.models.session_registry_refresh import SessionRegistryRefresh
from openapi_server.models.session_registry_refresh_create import SessionRegistryRefreshCreate
...
@router.post(
    ...
    responses={
        200: {"model": SessionRegistryRefreshCreate, "description": "Used to resume a user session using the previously received refresh token. apiApp:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["session-registry-refresh"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_session_registry_refresh(
    ...
    session_registry_refresh: SessionRegistryRefresh = Body(None, description=""),
...
) -> SessionRegistryRefreshCreate:
    """Used to resume a user session using the previously received refresh token. apiApp:CREATE"""
    if not BaseSessionRegistryRefreshApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseSessionRegistryRefreshApi.subclasses[0]().c_reate_session_registry_refresh(user_agent, x_bunq_client_authentication, session_registry_refresh, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
