...
from openapi_server.apis.fulfillment_api_base import BaseFulfillmentApi
...
from openapi_server.models.fulfillment_listing import FulfillmentListing
...
@router.get(
    "/user/{userID}/fulfillment",
    responses={
        200: {"model": List[FulfillmentListing], "description": "Endpoint for getting information fulfillments for a user. apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING"},
...
async def list_all_fulfillment_for_user(
...
) -> List[FulfillmentListing]:
    """Endpoint for getting information fulfillments for a user. apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING"""
    if not BaseFulfillmentApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseFulfillmentApi.subclasses[0]().list_all_fulfillment_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
