...
from openapi_server.apis.partner_promotion_voucher_api_base import BasePartnerPromotionVoucherApi
...
from openapi_server.models.partner_promotion_voucher_listing import PartnerPromotionVoucherListing
...
@router.get(
    "/user/{userID}/partner-promotion-voucher",
    responses={
        200: {"model": List[PartnerPromotionVoucherListing], "description": "Endpoint to retrieve partner promotion voucher. apiAdmin:LISTING apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["partner-promotion-voucher"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_partner_promotion_voucher_for_user(
...
) -> List[PartnerPromotionVoucherListing]:
    """Endpoint to retrieve partner promotion voucher. apiAdmin:LISTING apiApp:LISTING"""
    if not BasePartnerPromotionVoucherApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BasePartnerPromotionVoucherApi.subclasses[0]().list_all_partner_promotion_voucher_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
