...
from openapi_server.apis.top_up_method_configuration_api_base import BaseTopUpMethodConfigurationApi
...
from openapi_server.models.top_up_method_configuration_listing import TopUpMethodConfigurationListing
...
@router.get(
    "/user/{userID}/top-up-method-configuration",
    responses={
        200: {"model": List[TopUpMethodConfigurationListing], "description": "List all the available top-up method configurations. apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["top-up-method-configuration"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_top_up_method_configuration_for_user(
...
) -> List[TopUpMethodConfigurationListing]:
    """List all the available top-up method configurations. apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING"""
    if not BaseTopUpMethodConfigurationApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseTopUpMethodConfigurationApi.subclasses[0]().list_all_top_up_method_configuration_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
