...
from openapi_server.apis.token_qr_credential_password_ip_request_api_base import BaseTokenQrCredentialPasswordIpRequestApi
...
from openapi_server.models.token_qr_user_credential_password_ip_request import TokenQrUserCredentialPasswordIpRequest
from openapi_server.models.token_qr_user_credential_password_ip_request_create import TokenQrUserCredentialPasswordIpRequestCreate
...
@router.post(
    "/user/{userID}/token-qr-credential-password-ip-request",
    responses={
        200: {"model": TokenQrUserCredentialPasswordIpRequestCreate, "description": "Usage of this endpoint (by scanning a QR code) results in a UserCredentialPasswordIpRequest. apiApp:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["token-qr-credential-password-ip-request"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_token_qr_credential_password_ip_request_for_user(
    ...
    token_qr_user_credential_password_ip_request: TokenQrUserCredentialPasswordIpRequest = Body(None, description=""),
...
) -> TokenQrUserCredentialPasswordIpRequestCreate:
    """Usage of this endpoint (by scanning a QR code) results in a UserCredentialPasswordIpRequest. apiApp:CREATE"""
    if not BaseTokenQrCredentialPasswordIpRequestApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseTokenQrCredentialPasswordIpRequestApi.subclasses[0]().c_reate_token_qr_credential_password_ip_request_for_user(userID, user_agent, x_bunq_client_authentication, token_qr_user_credential_password_ip_request, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
