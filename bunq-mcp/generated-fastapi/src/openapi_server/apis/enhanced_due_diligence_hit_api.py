...
from openapi_server.apis.enhanced_due_diligence_hit_api_base import BaseEnhancedDueDiligenceHitApi
...
from openapi_server.models.enhanced_due_diligence_hit_read import EnhancedDueDiligenceHitRead
...
@router.get(
    "/enhanced-due-diligence-hit/{itemId}",
    responses={
        200: {"model": EnhancedDueDiligenceHitRead, "description": "The endpoint to manage all EDD checks for a user. apiAdmin:READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["enhanced-due-diligence-hit"],
    summary="",
    response_model_by_alias=True,
)
async def r_ead_enhanced_due_diligence_hit(
...
) -> EnhancedDueDiligenceHitRead:
    """The endpoint to manage all EDD checks for a user. apiAdmin:READ"""
    if not BaseEnhancedDueDiligenceHitApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseEnhancedDueDiligenceHitApi.subclasses[0]().r_ead_enhanced_due_diligence_hit(itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
