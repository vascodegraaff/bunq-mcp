...
from openapi_server.apis.session_tracking_reset_api_base import BaseSessionTrackingResetApi
...
from openapi_server.models.session_tracking_reset_create import SessionTrackingResetCreate
...
@router.post(
    "/device/{deviceID}/session-tracking-reset",
    responses={
        200: {"model": SessionTrackingResetCreate, "description": "The endpoint to reset a session tracking uuid. apiApp:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["session-tracking-reset"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_session_tracking_reset_for_device(
...
) -> SessionTrackingResetCreate:
    """The endpoint to reset a session tracking uuid. apiApp:CREATE"""
    if not BaseSessionTrackingResetApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseSessionTrackingResetApi.subclasses[0]().c_reate_session_tracking_reset_for_device(deviceID, user_agent, x_bunq_client_authentication, body, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
