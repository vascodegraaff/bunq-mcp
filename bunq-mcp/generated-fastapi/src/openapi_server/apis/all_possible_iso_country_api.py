...
from openapi_server.apis.all_possible_iso_country_api_base import BaseAllPossibleIsoCountryApi
...
from openapi_server.models.all_possible_iso_country_listing import AllPossibleIsoCountryListing
...
@router.get(
    "/all-possible-iso-country",
    responses={
        200: {"model": List[AllPossibleIsoCountryListing], "description": "Used to retrieve the possible iso countries. apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["all-possible-iso-country"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_all_possible_iso_country(
...
) -> List[AllPossibleIsoCountryListing]:
    """Used to retrieve the possible iso countries. apiApp:LISTING"""
    if not BaseAllPossibleIsoCountryApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAllPossibleIsoCountryApi.subclasses[0]().list_all_all_possible_iso_country(user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
