...
from openapi_server.apis.cashback_payout_item_api_base import BaseCashbackPayoutItemApi
...
from openapi_server.models.cashback_payout_item_listing import CashbackPayoutItemListing
...
@router.get(
    "/user/{userID}/cashback-payout/{cashback-payoutID}/cashback-payout-item",
    responses={
        200: {"model": List[CashbackPayoutItemListing], "description": "Cashback payout item details. apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["cashback-payout-item"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_cashback_payout_item_for_user_cashback_payout(
    userID: StrictInt = Path(..., description=""),
    cashback-payoutID: StrictInt = Path(..., description=""),
...
) -> List[CashbackPayoutItemListing]:
    """Cashback payout item details. apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING"""
    if not BaseCashbackPayoutItemApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseCashbackPayoutItemApi.subclasses[0]().list_all_cashback_payout_item_for_user_cashback_payout(userID, cashback-payoutID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
