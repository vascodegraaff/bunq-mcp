...
from openapi_server.apis.mixpanel_user_setting_communication_api_base import BaseMixpanelUserSettingCommunicationApi
...
from openapi_server.models.mixpanel_user_setting_communication import MixpanelUserSettingCommunication
from openapi_server.models.mixpanel_user_setting_communication_update import MixpanelUserSettingCommunicationUpdate
...
@router.put(
    "/mixpanel-user-setting-communication/{itemId}",
    responses={
        200: {"model": MixpanelUserSettingCommunicationUpdate, "description": "Let user update marketing communication preference through the website. apiMixpanel:UPDATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["mixpanel-user-setting-communication"],
    summary="",
    response_model_by_alias=True,
)
async def u_pdate_mixpanel_user_setting_communication(
    ...
    mixpanel_user_setting_communication: MixpanelUserSettingCommunication = Body(None, description=""),
...
) -> MixpanelUserSettingCommunicationUpdate:
    """Let user update marketing communication preference through the website. apiMixpanel:UPDATE"""
    if not BaseMixpanelUserSettingCommunicationApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseMixpanelUserSettingCommunicationApi.subclasses[0]().u_pdate_mixpanel_user_setting_communication(itemId, user_agent, x_bunq_client_authentication, mixpanel_user_setting_communication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
