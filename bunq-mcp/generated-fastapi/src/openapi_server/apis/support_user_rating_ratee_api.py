...
from openapi_server.apis.support_user_rating_ratee_api_base import BaseSupportUserRatingRateeApi
...
@router.get(
    "/user/{userID}/support-user-rating-ratee",
    responses={
        200: {"model": List[object], "description": "Endpoint for ratings in which the user is the one being rated. apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["support-user-rating-ratee"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_support_user_rating_ratee_for_user(
    ...
    """Endpoint for ratings in which the user is the one being rated. apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING"""
    if not BaseSupportUserRatingRateeApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseSupportUserRatingRateeApi.subclasses[0]().list_all_support_user_rating_ratee_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/ticket/{ticketID}/support-user-rating-ratee",
    responses={
        200: {"model": List[object], "description": "Endpoint for ratings in which the user is the one being rated. apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
...
async def list_all_support_user_rating_ratee_for_user_ticket(
    userID: StrictInt = Path(..., description=""),
    ...
    if not BaseSupportUserRatingRateeApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseSupportUserRatingRateeApi.subclasses[0]().list_all_support_user_rating_ratee_for_user_ticket(userID, ticketID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
