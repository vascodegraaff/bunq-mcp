...
from openapi_server.apis.admin_swift_order_api_base import BaseAdminSwiftOrderApi
...
from openapi_server.models.admin_swift_order_listing import AdminSwiftOrderListing
from openapi_server.models.admin_swift_order_read import AdminSwiftOrderRead
...
@router.get(
    "/admin-swift-order",
    responses={
        200: {"model": List[AdminSwiftOrderListing], "description": "Used for viewing the SwiftOrders. Admin-only. apiAdmin:LISTING/READ apiServiceInternal:LISTING/READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["admin-swift-order"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_admin_swift_order(
...
) -> List[AdminSwiftOrderListing]:
    """Used for viewing the SwiftOrders. Admin-only. apiAdmin:LISTING/READ apiServiceInternal:LISTING/READ"""
    if not BaseAdminSwiftOrderApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAdminSwiftOrderApi.subclasses[0]().list_all_admin_swift_order(user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/admin-swift-order/{itemId}",
    responses={
        200: {"model": AdminSwiftOrderRead, "description": "Used for viewing the SwiftOrders. Admin-only. apiAdmin:LISTING/READ apiServiceInternal:LISTING/READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["admin-swift-order"],
    summary="",
...
async def r_ead_admin_swift_order(
    ...
    if not BaseAdminSwiftOrderApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAdminSwiftOrderApi.subclasses[0]().r_ead_admin_swift_order(itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
