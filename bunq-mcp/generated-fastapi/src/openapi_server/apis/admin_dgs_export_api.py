...
from openapi_server.apis.admin_dgs_export_api_base import BaseAdminDgsExportApi
...
from openapi_server.models.admin_dgs_export_read import AdminDgsExportRead
...
@router.get(
    "/user/{userID}/admin-dgs-export/{itemId}",
    responses={
        200: {"model": AdminDgsExportRead, "description": "View endpoint for dgs export. apiAdmin:READ apiServiceInternal:READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["admin-dgs-export"],
    summary="",
    response_model_by_alias=True,
)
async def r_ead_admin_dgs_export_for_user(
    ...
    """View endpoint for dgs export. apiAdmin:READ apiServiceInternal:READ"""
    if not BaseAdminDgsExportApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAdminDgsExportApi.subclasses[0]().r_ead_admin_dgs_export_for_user(userID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
