...
from openapi_server.apis.identification_verification_applicant_token_api_base import BaseIdentificationVerificationApplicantTokenApi
...
from openapi_server.models.user_identification_verification_applicant_token import UserIdentificationVerificationApplicantToken
from openapi_server.models.user_identification_verification_applicant_token_create import UserIdentificationVerificationApplicantTokenCreate
...
@router.post(
    "/user/{userID}/identification-verification-applicant-token",
    responses={
        200: {"model": UserIdentificationVerificationApplicantTokenCreate, "description": "Endpoint for creating a new token (after the first one is expired) required for the Onfido SDK V3. apiApp:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["identification-verification-applicant-token"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_identification_verification_applicant_token_for_user(
    ...
    user_identification_verification_applicant_token: UserIdentificationVerificationApplicantToken = Body(None, description=""),
...
) -> UserIdentificationVerificationApplicantTokenCreate:
    """Endpoint for creating a new token (after the first one is expired) required for the Onfido SDK V3. apiApp:CREATE"""
    if not BaseIdentificationVerificationApplicantTokenApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseIdentificationVerificationApplicantTokenApi.subclasses[0]().c_reate_identification_verification_applicant_token_for_user(userID, user_agent, x_bunq_client_authentication, user_identification_verification_applicant_token, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
