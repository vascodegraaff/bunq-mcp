...
from openapi_server.apis.scan_invoice_ocr_api_base import BaseScanInvoiceOcrApi
...
from openapi_server.models.scan_invoice_create import ScanInvoiceCreate
...
@router.post(
    "/user/{userID}/scan-invoice-ocr",
    responses={
        200: {"model": ScanInvoiceCreate, "description": "This endpoint fetches payment details from invoices. apiApp:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["scan-invoice-ocr"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_scan_invoice_ocr_for_user(
    ...
    """This endpoint fetches payment details from invoices. apiApp:CREATE"""
    if not BaseScanInvoiceOcrApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseScanInvoiceOcrApi.subclasses[0]().c_reate_scan_invoice_ocr_for_user(userID, user_agent, x_bunq_client_authentication, body, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
