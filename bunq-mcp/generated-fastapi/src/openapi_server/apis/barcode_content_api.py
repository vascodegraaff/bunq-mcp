...
from openapi_server.apis.barcode_content_api_base import BaseBarcodeContentApi
...
@router.get(
    "/user/{userID}/monetary-account/{monetary-accountID}/barzahlen-transaction/{barzahlen-transactionID}/barcode-content",
    responses={
        200: {"model": List[object], "description": "Used to get a Barzahlen barcode image. apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["barcode-content"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_barcode_content_for_user_monetary_account_barzahlen_transaction(
    userID: StrictInt = Path(..., description=""),
    monetary-accountID: StrictInt = Path(..., description=""),
    barzahlen-transactionID: StrictInt = Path(..., description=""),
    ...
    """Used to get a Barzahlen barcode image. apiApp:LISTING"""
    if not BaseBarcodeContentApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseBarcodeContentApi.subclasses[0]().list_all_barcode_content_for_user_monetary_account_barzahlen_transaction(userID, monetary-accountID, barzahlen-transactionID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
