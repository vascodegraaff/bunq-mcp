...
from openapi_server.apis.attachment_content_api_base import BaseAttachmentContentApi
...
from openapi_server.models.oauth_attachment_content_read import OauthAttachmentContentRead
...
@router.get(
    "/oauth-request/{oauth-requestUUID}/attachment-content/{itemId}",
    responses={
        200: {"model": OauthAttachmentContentRead, "description": "Gets publicly available attachment content for bunqme requests and tabs. apiOauth:READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["attachment-content"],
    summary="",
    response_model_by_alias=True,
)
async def r_ead_attachment_content_for_oauth_request(
...
) -> OauthAttachmentContentRead:
    """Gets publicly available attachment content for bunqme requests and tabs. apiOauth:READ"""
    if not BaseAttachmentContentApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAttachmentContentApi.subclasses[0]().r_ead_attachment_content_for_oauth_request(oauth-requestUUID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
