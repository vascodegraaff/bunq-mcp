...
from openapi_server.apis.log_entry_api_base import BaseLogEntryApi
...
from openapi_server.models.admin_dgs_log_entry_listing import AdminDgsLogEntryListing
...
@router.get(
    "/user/{userID}/admin-dgs-export/{admin-dgs-exportID}/log-entry",
    responses={
        200: {"model": List[AdminDgsLogEntryListing], "description": "View endpoint for the log entries of a Dgs export. apiAdmin:LISTING apiServiceInternal:LISTING"},
...
async def list_all_log_entry_for_user_admin_dgs_export(
    userID: StrictInt = Path(..., description=""),
    admin-dgs-exportID: StrictInt = Path(..., description=""),
...
) -> List[AdminDgsLogEntryListing]:
    """View endpoint for the log entries of a Dgs export. apiAdmin:LISTING apiServiceInternal:LISTING"""
    if not BaseLogEntryApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseLogEntryApi.subclasses[0]().list_all_log_entry_for_user_admin_dgs_export(userID, admin-dgs-exportID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
