...
from openapi_server.apis.point_challenge_api_base import BasePointChallengeApi
...
from openapi_server.models.point_challenge_listing import PointChallengeListing
...
@router.get(
    "/user/{userID}/point-challenge",
    responses={
        200: {"model": List[PointChallengeListing], "description": "Endpoint to retrieve point challenges. apiAdmin:LISTING apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["point-challenge"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_point_challenge_for_user(
...
) -> List[PointChallengeListing]:
    """Endpoint to retrieve point challenges. apiAdmin:LISTING apiApp:LISTING"""
    if not BasePointChallengeApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BasePointChallengeApi.subclasses[0]().list_all_point_challenge_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
