...
from openapi_server.apis.scan_invoice_batch_api_base import BaseScanInvoiceBatchApi
...
from openapi_server.models.scan_invoice_batch import ScanInvoiceBatch
from openapi_server.models.scan_invoice_batch_create import ScanInvoiceBatchCreate
...
@router.post(
    "/user/{userID}/scan-invoice-batch",
    responses={
        200: {"model": ScanInvoiceBatchCreate, "description": "Scan invoice batch endpoint use to prefill payment information from invoices for users. apiApp:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["scan-invoice-batch"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_scan_invoice_batch_for_user(
    ...
    scan_invoice_batch: ScanInvoiceBatch = Body(None, description=""),
...
) -> ScanInvoiceBatchCreate:
    """Scan invoice batch endpoint use to prefill payment information from invoices for users. apiApp:CREATE"""
    if not BaseScanInvoiceBatchApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseScanInvoiceBatchApi.subclasses[0]().c_reate_scan_invoice_batch_for_user(userID, user_agent, x_bunq_client_authentication, scan_invoice_batch, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
