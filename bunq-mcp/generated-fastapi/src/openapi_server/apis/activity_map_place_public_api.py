...
from openapi_server.apis.activity_map_place_public_api_base import BaseActivityMapPlacePublicApi
...
from openapi_server.models.activity_map_place_public_read import ActivityMapPlacePublicRead
...
@router.get(
    ...
    responses={
        200: {"model": ActivityMapPlacePublicRead, "description": "Public endpoint for getting the place info. apiWebsite:READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["activity-map-place-public"],
    summary="",
    response_model_by_alias=True,
)
async def r_ead_activity_map_place_public(
    ...
    """Public endpoint for getting the place info. apiWebsite:READ"""
    if not BaseActivityMapPlacePublicApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseActivityMapPlacePublicApi.subclasses[0]().r_ead_activity_map_place_public(itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
