...
from openapi_server.apis.token_confirmation_login_api_base import BaseTokenConfirmationLoginApi
...
from openapi_server.models.token_confirmation_login import TokenConfirmationLogin
from openapi_server.models.token_confirmation_login_create import TokenConfirmationLoginCreate
...
@router.post(
    "/token-confirmation-login",
    responses={
        200: {"model": TokenConfirmationLoginCreate, "description": "Used to authenticate using a challenge and response. apiAdmin:CREATE apiApp:CREATE apiServiceInternal:CREATE apiDeveloperPortal:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["token-confirmation-login"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_token_confirmation_login(
    ...
    token_confirmation_login: TokenConfirmationLogin = Body(None, description=""),
...
) -> TokenConfirmationLoginCreate:
    """Used to authenticate using a challenge and response. apiAdmin:CREATE apiApp:CREATE apiServiceInternal:CREATE apiDeveloperPortal:CREATE"""
    if not BaseTokenConfirmationLoginApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseTokenConfirmationLoginApi.subclasses[0]().c_reate_token_confirmation_login(user_agent, x_bunq_client_authentication, token_confirmation_login, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
