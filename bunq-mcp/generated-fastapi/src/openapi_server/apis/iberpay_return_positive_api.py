...
from openapi_server.apis.iberpay_return_positive_api_base import BaseIberpayReturnPositiveApi
...
from openapi_server.models.iberpay_incoming_return_positive_create import IberpayIncomingReturnPositiveCreate
...
@router.post(
    "/iberpay-return-positive",
    responses={
        200: {"model": IberpayIncomingReturnPositiveCreate, "description": "Endpoint for Iberpay to send positive replies concerning payment return requests to bunq. Iberpay service name: WS_RECEP_DEVOLUCION. apiIberpay:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["iberpay-return-positive"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_iberpay_return_positive(
...
) -> IberpayIncomingReturnPositiveCreate:
    """Endpoint for Iberpay to send positive replies concerning payment return requests to bunq. Iberpay service name: WS_RECEP_DEVOLUCION. apiIberpay:CREATE"""
    if not BaseIberpayReturnPositiveApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseIberpayReturnPositiveApi.subclasses[0]().c_reate_iberpay_return_positive(user_agent, x_bunq_client_authentication, body, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
