...
from openapi_server.apis.attachment_public_api_base import BaseAttachmentPublicApi
...
from openapi_server.models.attachment_public_create import AttachmentPublicCreate
from openapi_server.models.attachment_public_read import AttachmentPublicRead
...
@router.post(
    "/attachment-public",
    responses={
        200: {"model": AttachmentPublicCreate, "description": "This call is used to upload an attachment that can be referenced to as an avatar (through the Avatar endpoint) or in a tab sent. Attachments supported are png, jpg and gif. apiAdmin:CREATE/READ apiBunqMe:READ apiBunqTo:READ apiApp:CREATE/READ apiPublic:CREATE/READ apiUserAuthenticationWeb:READ apiServiceInternal:CREATE/READ apiOauth:READ apiProcess:READ apiIdealIssuerWeb:READ apiIdinIssuerWeb:READ apiQr:READ apiWidget:READ"},
...
async def c_reate_attachment_public(
    ...
    attachment_public: AttachmentPublic = Body(None, description=""),
    ...
    if not BaseAttachmentPublicApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAttachmentPublicApi.subclasses[0]().c_reate_attachment_public(user_agent, x_bunq_client_authentication, attachment_public, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/attachment-public/{itemId}",
    responses={
        200: {"model": AttachmentPublicRead, "description": "This call is used to upload an attachment that can be referenced to as an avatar (through the Avatar endpoint) or in a tab sent. Attachments supported are png, jpg and gif. apiAdmin:CREATE/READ apiBunqMe:READ apiBunqTo:READ apiApp:CREATE/READ apiPublic:CREATE/READ apiUserAuthenticationWeb:READ apiServiceInternal:CREATE/READ apiOauth:READ apiProcess:READ apiIdealIssuerWeb:READ apiIdinIssuerWeb:READ apiQr:READ apiWidget:READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["attachment-public"],
    summary="",
...
async def r_ead_attachment_public(
    ...
    if not BaseAttachmentPublicApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAttachmentPublicApi.subclasses[0]().r_ead_attachment_public(itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
