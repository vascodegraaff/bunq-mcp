...
from openapi_server.apis.risk_score_api_base import BaseRiskScoreApi
...
from openapi_server.models.risk_score_listing import RiskScoreListing
...
@router.get(
    "/user/{userID}/risk-score",
    responses={
        200: {"model": List[RiskScoreListing], "description": "view for listing risk scores for users. apiAdmin:LISTING apiServiceInternal:LISTING"},
...
async def list_all_risk_score_for_user(
...
) -> List[RiskScoreListing]:
    """view for listing risk scores for users. apiAdmin:LISTING apiServiceInternal:LISTING"""
    if not BaseRiskScoreApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseRiskScoreApi.subclasses[0]().list_all_risk_score_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
