...
from openapi_server.apis.card_result_api_base import BaseCardResultApi
...
from openapi_server.models.card_result_listing import CardResultListing
from openapi_server.models.card_result_read import CardResultRead
...
@router.get(
    "/user/{userID}/monetary-account/{monetary-accountID}/card-result",
    responses={
        200: {"model": List[CardResultListing], "description": "Endpoint for Card result requests (failed and successful transactions). apiAdmin:READ/LISTING apiApp:READ/LISTING apiServiceInternal:READ/LISTING"},
...
async def list_all_card_result_for_user_monetary_account(
...
) -> List[CardResultListing]:
    """Endpoint for Card result requests (failed and successful transactions). apiAdmin:READ/LISTING apiApp:READ/LISTING apiServiceInternal:READ/LISTING"""
    if not BaseCardResultApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseCardResultApi.subclasses[0]().list_all_card_result_for_user_monetary_account(userID, monetary-accountID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/monetary-account/{monetary-accountID}/card-result/{itemId}",
    responses={
        200: {"model": CardResultRead, "description": "Endpoint for Card result requests (failed and successful transactions). apiAdmin:READ/LISTING apiApp:READ/LISTING apiServiceInternal:READ/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["card-result"],
    summary="",
...
async def r_ead_card_result_for_user_monetary_account(
    userID: StrictInt = Path(..., description=""),
    monetary-accountID: StrictInt = Path(..., description=""),
    ...
    if not BaseCardResultApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseCardResultApi.subclasses[0]().r_ead_card_result_for_user_monetary_account(userID, monetary-accountID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
