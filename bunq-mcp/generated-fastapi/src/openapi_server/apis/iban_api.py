...
from openapi_server.apis.iban_api_base import BaseIbanApi
...
from openapi_server.models.registry_membership_iban_listing import RegistryMembershipIbanListing
...
@router.get(
    "/user/{userID}/registry/{registryID}/registry-membership/{registry-membershipID}/iban",
    responses={
        200: {"model": List[RegistryMembershipIbanListing], "description": "Fetch the IBAN with the bank information associated with a RegistryMembership which the app can use or display for manual Registry settlement. apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING"},
...
async def list_all_iban_for_user_registry_registry_membership(
    userID: StrictInt = Path(..., description=""),
    registryID: StrictInt = Path(..., description=""),
    registry-membershipID: StrictInt = Path(..., description=""),
...
) -> List[RegistryMembershipIbanListing]:
    """Fetch the IBAN with the bank information associated with a RegistryMembership which the app can use or display for manual Registry settlement. apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING"""
    if not BaseIbanApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseIbanApi.subclasses[0]().list_all_iban_for_user_registry_registry_membership(userID, registryID, registry-membershipID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
