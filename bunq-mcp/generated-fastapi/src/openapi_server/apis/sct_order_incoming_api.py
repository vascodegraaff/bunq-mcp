...
from openapi_server.apis.sct_order_incoming_api_base import BaseSctOrderIncomingApi
...
from openapi_server.models.sct_order_incoming_read import SctOrderIncomingRead
...
@router.get(
    "/sct-order-incoming/{itemId}",
    responses={
        200: {"model": SctOrderIncomingRead, "description": "Endpoint used by the search API. apiAdmin:READ apiServiceInternal:READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["sct-order-incoming"],
    summary="",
    response_model_by_alias=True,
)
async def r_ead_sct_order_incoming(
...
) -> SctOrderIncomingRead:
    """Endpoint used by the search API. apiAdmin:READ apiServiceInternal:READ"""
    if not BaseSctOrderIncomingApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseSctOrderIncomingApi.subclasses[0]().r_ead_sct_order_incoming(itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
