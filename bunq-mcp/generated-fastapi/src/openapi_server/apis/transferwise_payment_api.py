...
from openapi_server.apis.transferwise_payment_api_base import BaseTransferwisePaymentApi
...
@router.get(
    "/user/{userID}/transferwise-payment",
    responses={
        200: {"model": List[object], "description": "Used to list all transferwise payments for a user. apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["transferwise-payment"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_transferwise_payment_for_user(
    ...
    """Used to list all transferwise payments for a user. apiApp:LISTING"""
    if not BaseTransferwisePaymentApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseTransferwisePaymentApi.subclasses[0]().list_all_transferwise_payment_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
