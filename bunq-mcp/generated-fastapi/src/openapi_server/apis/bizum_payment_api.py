...
from openapi_server.apis.bizum_payment_api_base import BaseBizumPaymentApi
...
from openapi_server.models.bizum_payment_listing import BizumPaymentListing
from openapi_server.models.bizum_payment_read import BizumPaymentRead
...
@router.get(
    "/user/{userID}/bizum-payment",
    responses={
        200: {"model": List[BizumPaymentListing], "description": "Endpoint for retrieving your Bizum payments. apiApp:READ/LISTING"},
...
async def list_all_bizum_payment_for_user(
...
) -> List[BizumPaymentListing]:
    """Endpoint for retrieving your Bizum payments. apiApp:READ/LISTING"""
    if not BaseBizumPaymentApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseBizumPaymentApi.subclasses[0]().list_all_bizum_payment_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/bizum-payment/{itemId}",
    responses={
        200: {"model": BizumPaymentRead, "description": "Endpoint for retrieving your Bizum payments. apiApp:READ/LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["bizum-payment"],
    summary="",
...
async def r_ead_bizum_payment_for_user(
    userID: StrictInt = Path(..., description=""),
    ...
    if not BaseBizumPaymentApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseBizumPaymentApi.subclasses[0]().r_ead_bizum_payment_for_user(userID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
