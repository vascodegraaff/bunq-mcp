...
from openapi_server.apis.company_director_api_base import BaseCompanyDirectorApi
...
@router.get(
    "/user/{userID}/company-director",
    responses={
        200: {"model": List[object], "description": "Endpoint for listing all company directors associated with a user apiApp:LISTING"},
...
async def list_all_company_director_for_user(
    ...
    """Endpoint for listing all company directors associated with a user apiApp:LISTING"""
    if not BaseCompanyDirectorApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseCompanyDirectorApi.subclasses[0]().list_all_company_director_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
