...
from openapi_server.apis.export_csv_api_base import BaseExportCsvApi
...
@router.get(
    "/user/{userID}/registry/{registryID}/export-csv",
    responses={
        200: {"model": List[object], "description": "Retrieve a CSV export of a registry apiAdmin:LISTING apiApp:LISTING"},
...
async def list_all_export_csv_for_user_registry(
    ...
    """Retrieve a CSV export of a registry apiAdmin:LISTING apiApp:LISTING"""
    if not BaseExportCsvApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseExportCsvApi.subclasses[0]().list_all_export_csv_for_user_registry(userID, registryID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
