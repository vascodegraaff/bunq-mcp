...
from openapi_server.apis.review_announcement_api_base import BaseReviewAnnouncementApi
...
from openapi_server.models.admin_user_review_announcement_user_listing import AdminUserReviewAnnouncementUserListing
...
@router.get(
    "/user/{userID}/review-announcement",
    responses={
        200: {"model": List[AdminUserReviewAnnouncementUserListing], "description": "Announcement for user review. Used for listing all announcements across all user reviews. apiAdmin:LISTING apiServiceInternal:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["review-announcement"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_review_announcement_for_user(
...
) -> List[AdminUserReviewAnnouncementUserListing]:
    """Announcement for user review. Used for listing all announcements across all user reviews. apiAdmin:LISTING apiServiceInternal:LISTING"""
    if not BaseReviewAnnouncementApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseReviewAnnouncementApi.subclasses[0]().list_all_review_announcement_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
