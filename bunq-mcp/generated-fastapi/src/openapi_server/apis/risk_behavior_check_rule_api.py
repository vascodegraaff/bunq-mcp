...
from openapi_server.apis.risk_behavior_check_rule_api_base import BaseRiskBehaviorCheckRuleApi
...
from openapi_server.models.risk_behavior_check_rule_listing import RiskBehaviorCheckRuleListing
...
@router.get(
    "/user/{userID}/risk-behavior-check/{risk-behavior-checkID}/risk-behavior-check-rule",
    responses={
        200: {"model": List[RiskBehaviorCheckRuleListing], "description": "Fetches the rules in risk behavior check. apiAdmin:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["risk-behavior-check-rule"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_risk_behavior_check_rule_for_user_risk_behavior_check(
    userID: StrictInt = Path(..., description=""),
    risk-behavior-checkID: StrictInt = Path(..., description=""),
...
) -> List[RiskBehaviorCheckRuleListing]:
    """Fetches the rules in risk behavior check. apiAdmin:LISTING"""
    if not BaseRiskBehaviorCheckRuleApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseRiskBehaviorCheckRuleApi.subclasses[0]().list_all_risk_behavior_check_rule_for_user_risk_behavior_check(userID, risk-behavior-checkID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
