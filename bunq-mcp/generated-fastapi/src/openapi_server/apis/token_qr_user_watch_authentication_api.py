...
from openapi_server.apis.token_qr_user_watch_authentication_api_base import BaseTokenQrUserWatchAuthenticationApi
...
from openapi_server.models.token_qr_user_watch_authentication import TokenQrUserWatchAuthentication
from openapi_server.models.token_qr_user_watch_authentication_create import TokenQrUserWatchAuthenticationCreate
...
@router.post(
    "/user/{userID}/token-qr-user-watch-authentication",
    responses={
        200: {"model": TokenQrUserWatchAuthenticationCreate, "description": "Usage of this endpoint (by scanning a QR code) results in creating a token for the user. apiApp:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["token-qr-user-watch-authentication"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_token_qr_user_watch_authentication_for_user(
    ...
    token_qr_user_watch_authentication: TokenQrUserWatchAuthentication = Body(None, description=""),
...
) -> TokenQrUserWatchAuthenticationCreate:
    """Usage of this endpoint (by scanning a QR code) results in creating a token for the user. apiApp:CREATE"""
    if not BaseTokenQrUserWatchAuthenticationApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseTokenQrUserWatchAuthenticationApi.subclasses[0]().c_reate_token_qr_user_watch_authentication_for_user(userID, user_agent, x_bunq_client_authentication, token_qr_user_watch_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
