...
from openapi_server.apis.balance_aggregation_api_base import BaseBalanceAggregationApi
...
from openapi_server.models.monetary_account_balance_aggregation_listing import MonetaryAccountBalanceAggregationListing
from openapi_server.models.monetary_account_balance_aggregation_user_listing import MonetaryAccountBalanceAggregationUserListing
...
@router.get(
    "/user/{userID}/balance-aggregation",
    responses={
        200: {"model": List[MonetaryAccountBalanceAggregationUserListing], "description": "Get information to display a graph of the user&#39;s past, current and (predicted) future balance. apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["balance-aggregation"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_balance_aggregation_for_user(
...
) -> List[MonetaryAccountBalanceAggregationUserListing]:
    """Get information to display a graph of the user&#39;s past, current and (predicted) future balance. apiApp:LISTING"""
    if not BaseBalanceAggregationApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseBalanceAggregationApi.subclasses[0]().list_all_balance_aggregation_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/monetary-account/{monetary-accountID}/balance-aggregation",
    responses={
        200: {"model": List[MonetaryAccountBalanceAggregationListing], "description": "Get information to display a graph of the user&#39;s past, current and (predicted) future balance. apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["balance-aggregation"],
    summary="",
...
async def list_all_balance_aggregation_for_user_monetary_account(
    userID: StrictInt = Path(..., description=""),
    ...
    x_bunq_client_request_id: Annotated[Optional[StrictStr], Field(description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer.")] = Header(None, description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer."),
    x_bunq_geolocation: Annotated[Optional[StrictStr], Field(description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued.")] = Header(None, description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued."),
) -> List[MonetaryAccountBalanceAggregationListing]:
    """Get information to display a graph of the user&#39;s past, current and (predicted) future balance. apiApp:LISTING"""
    if not BaseBalanceAggregationApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseBalanceAggregationApi.subclasses[0]().list_all_balance_aggregation_for_user_monetary_account(userID, monetary-accountID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
