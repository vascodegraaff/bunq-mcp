...
from openapi_server.apis.avatar_api_base import BaseAvatarApi
...
from openapi_server.models.avatar_create import AvatarCreate
from openapi_server.models.avatar_read import AvatarRead
...
@router.post(
    "/avatar",
    responses={
        200: {"model": AvatarCreate, "description": "Avatars are public images used to represent you or your company. Avatars are used to represent users, monetary accounts and cash registers. Avatars cannot be deleted, only replaced. Avatars can be updated after uploading the image you would like to use through AttachmentPublic. Using the attachment_public_uuid which is returned you can update your Avatar. Avatars used for cash registers and company accounts will be reviewed by bunq. apiAdmin:CREATE/READ apiApp:CREATE/READ apiPublic:CREATE/READ apiServiceInternal:CREATE/READ"},
...
async def c_reate_avatar(
    ...
    avatar: Avatar = Body(None, description=""),
    ...
    """Avatars are public images used to represent you or your company. Avatars are used to represent users, monetary accounts and cash registers. Avatars cannot be deleted, only replaced. Avatars can be updated after uploading the image you would like to use through AttachmentPublic. Using the attachment_public_uuid which is returned you can update your Avatar. Avatars used for cash registers and company accounts will be reviewed by bunq. apiAdmin:CREATE/READ apiApp:CREATE/READ apiPublic:CREATE/READ apiServiceInternal:CREATE/READ"""
    if not BaseAvatarApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAvatarApi.subclasses[0]().c_reate_avatar(user_agent, x_bunq_client_authentication, avatar, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/avatar/{itemId}",
    responses={
        200: {"model": AvatarRead, "description": "Avatars are public images used to represent you or your company. Avatars are used to represent users, monetary accounts and cash registers. Avatars cannot be deleted, only replaced. Avatars can be updated after uploading the image you would like to use through AttachmentPublic. Using the attachment_public_uuid which is returned you can update your Avatar. Avatars used for cash registers and company accounts will be reviewed by bunq. apiAdmin:CREATE/READ apiApp:CREATE/READ apiPublic:CREATE/READ apiServiceInternal:CREATE/READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["avatar"],
    summary="",
...
async def r_ead_avatar(
    ...
    if not BaseAvatarApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAvatarApi.subclasses[0]().r_ead_avatar(itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
