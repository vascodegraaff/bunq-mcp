...
from openapi_server.apis.admin_monetary_account_switch_service_api_base import BaseAdminMonetaryAccountSwitchServiceApi
...
from openapi_server.models.admin_monetary_account_switch_service_read import AdminMonetaryAccountSwitchServiceRead
...
@router.get(
    "/admin-monetary-account-switch-service/{itemId}",
    responses={
        200: {"model": AdminMonetaryAccountSwitchServiceRead, "description": "View for MonetaryAccountSwitchService. (only for serialization) apiAdmin:READ apiServiceInternal:READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["admin-monetary-account-switch-service"],
    summary="",
    response_model_by_alias=True,
)
async def r_ead_admin_monetary_account_switch_service(
...
) -> AdminMonetaryAccountSwitchServiceRead:
    """View for MonetaryAccountSwitchService. (only for serialization) apiAdmin:READ apiServiceInternal:READ"""
    if not BaseAdminMonetaryAccountSwitchServiceApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAdminMonetaryAccountSwitchServiceApi.subclasses[0]().r_ead_admin_monetary_account_switch_service(itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
