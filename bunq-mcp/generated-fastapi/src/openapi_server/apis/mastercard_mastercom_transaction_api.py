...
from openapi_server.apis.mastercard_mastercom_transaction_api_base import BaseMastercardMastercomTransactionApi
...
from openapi_server.models.master_card_mastercom_transaction_listing import MasterCardMastercomTransactionListing
...
@router.get(
    "/card/{cardID}/mastercard-mastercom-transaction",
    responses={
        200: {"model": List[MasterCardMastercomTransactionListing], "description": "Endpoint to fetch authorisation and clearing information from Mastercom. apiAdmin:LISTING apiServiceInternal:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["mastercard-mastercom-transaction"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_mastercard_mastercom_transaction_for_card(
...
) -> List[MasterCardMastercomTransactionListing]:
    """Endpoint to fetch authorisation and clearing information from Mastercom. apiAdmin:LISTING apiServiceInternal:LISTING"""
    if not BaseMastercardMastercomTransactionApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseMastercardMastercomTransactionApi.subclasses[0]().list_all_mastercard_mastercom_transaction_for_card(cardID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
