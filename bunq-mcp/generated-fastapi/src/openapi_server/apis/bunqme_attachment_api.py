...
from openapi_server.apis.bunqme_attachment_api_base import BaseBunqmeAttachmentApi
...
from openapi_server.models.bunq_me_attachment_read import BunqMeAttachmentRead
...
@router.get(
    "/bunqme-attachment/{itemId}",
    responses={
        200: {"model": BunqMeAttachmentRead, "description": "Gets publicly available attachments for bunq.me requests and tabs. apiBunqMe:READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["bunqme-attachment"],
    summary="",
    response_model_by_alias=True,
)
async def r_ead_bunqme_attachment(
...
) -> BunqMeAttachmentRead:
    """Gets publicly available attachments for bunq.me requests and tabs. apiBunqMe:READ"""
    if not BaseBunqmeAttachmentApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseBunqmeAttachmentApi.subclasses[0]().r_ead_bunqme_attachment(itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
