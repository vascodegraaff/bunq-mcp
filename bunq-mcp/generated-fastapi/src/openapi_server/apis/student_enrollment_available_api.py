...
from openapi_server.apis.student_enrollment_available_api_base import BaseStudentEnrollmentAvailableApi
...
from openapi_server.models.user_person_student_enrollment_available_listing import UserPersonStudentEnrollmentAvailableListing
...
@router.get(
    "/user/{userID}/student-enrollment-available",
    responses={
        200: {"model": List[UserPersonStudentEnrollmentAvailableListing], "description": "The endpoint that shows the availability for a student enrollment for a user. apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["student-enrollment-available"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_student_enrollment_available_for_user(
...
) -> List[UserPersonStudentEnrollmentAvailableListing]:
    """The endpoint that shows the availability for a student enrollment for a user. apiApp:LISTING"""
    if not BaseStudentEnrollmentAvailableApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseStudentEnrollmentAvailableApi.subclasses[0]().list_all_student_enrollment_available_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
