...
from openapi_server.apis.profiles_notify_api_base import BaseProfilesNotifyApi
...
from openapi_server.models.ideal_issuer_provider_profile_notify import IdealIssuerProviderProfileNotify
from openapi_server.models.ideal_issuer_provider_profile_notify_create import IdealIssuerProviderProfileNotifyCreate
...
@router.post(
    "/profiles/notify",
    responses={
        200: {"model": IdealIssuerProviderProfileNotifyCreate, "description": "Endpoint used by iDEAL Access Point to inform us about iDEAL profile registration status. apiIdealIssuerProvider:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["profiles/notify"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_profiles_notify(
    ...
    ideal_issuer_provider_profile_notify: IdealIssuerProviderProfileNotify = Body(None, description=""),
...
) -> IdealIssuerProviderProfileNotifyCreate:
    """Endpoint used by iDEAL Access Point to inform us about iDEAL profile registration status. apiIdealIssuerProvider:CREATE"""
    if not BaseProfilesNotifyApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseProfilesNotifyApi.subclasses[0]().c_reate_profiles_notify(user_agent, x_bunq_client_authentication, ideal_issuer_provider_profile_notify, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
