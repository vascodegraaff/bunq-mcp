...
from openapi_server.apis.payment_auto_allocate_instance_api_base import BasePaymentAutoAllocateInstanceApi
...
from openapi_server.models.payment_auto_allocate_instance_user_listing import PaymentAutoAllocateInstanceUserListing
...
@router.get(
    "/user/{userID}/payment-auto-allocate-instance",
    responses={
        200: {"model": List[PaymentAutoAllocateInstanceUserListing], "description": "List all the times a users payment was automatically allocated. apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["payment-auto-allocate-instance"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_payment_auto_allocate_instance_for_user(
...
) -> List[PaymentAutoAllocateInstanceUserListing]:
    """List all the times a users payment was automatically allocated. apiApp:LISTING"""
    if not BasePaymentAutoAllocateInstanceApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BasePaymentAutoAllocateInstanceApi.subclasses[0]().list_all_payment_auto_allocate_instance_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
