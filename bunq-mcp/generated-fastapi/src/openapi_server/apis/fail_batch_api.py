...
from openapi_server.apis.fail_batch_api_base import BaseFailBatchApi
...
from openapi_server.models.identity_verification_report_fail_batch import IdentityVerificationReportFailBatch
from openapi_server.models.identity_verification_report_fail_batch_create import IdentityVerificationReportFailBatchCreate
...
@router.post(
    "/user/{userID}/identity-verification-report/{identity-verification-reportID}/fail-batch",
    responses={
        200: {"model": IdentityVerificationReportFailBatchCreate, "description": "Endpoint to manage the fail reason of an Identity Verification apiAdmin:CREATE apiServiceInternal:CREATE"},
...
async def c_reate_fail_batch_for_user_identity_verification_report(
    userID: StrictInt = Path(..., description=""),
    identity-verification-reportID: StrictInt = Path(..., description=""),
    ...
    identity_verification_report_fail_batch: IdentityVerificationReportFailBatch = Body(None, description=""),
...
) -> IdentityVerificationReportFailBatchCreate:
    """Endpoint to manage the fail reason of an Identity Verification apiAdmin:CREATE apiServiceInternal:CREATE"""
    if not BaseFailBatchApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseFailBatchApi.subclasses[0]().c_reate_fail_batch_for_user_identity_verification_report(userID, identity-verification-reportID, user_agent, x_bunq_client_authentication, identity_verification_report_fail_batch, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
