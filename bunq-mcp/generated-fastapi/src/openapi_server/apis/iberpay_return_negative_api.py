...
from openapi_server.apis.iberpay_return_negative_api_base import BaseIberpayReturnNegativeApi
...
from openapi_server.models.iberpay_incoming_return_negative_create import IberpayIncomingReturnNegativeCreate
...
@router.post(
    ...
    responses={
        200: {"model": IberpayIncomingReturnNegativeCreate, "description": "Endpoint for Iberpay to send negative replies concerning payment return requests to bunq. Iberpay service name: WS_RECEP_NEGACION. apiIberpay:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["iberpay-return-negative"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_iberpay_return_negative(
...
) -> IberpayIncomingReturnNegativeCreate:
    """Endpoint for Iberpay to send negative replies concerning payment return requests to bunq. Iberpay service name: WS_RECEP_NEGACION. apiIberpay:CREATE"""
    if not BaseIberpayReturnNegativeApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseIberpayReturnNegativeApi.subclasses[0]().c_reate_iberpay_return_negative(user_agent, x_bunq_client_authentication, body, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
