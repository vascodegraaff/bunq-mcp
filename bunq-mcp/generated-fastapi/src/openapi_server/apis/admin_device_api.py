...
from openapi_server.apis.admin_device_api_base import BaseAdminDeviceApi
...
from openapi_server.models.admin_device_listing import AdminDeviceListing
...
@router.get(
    "/user/{userID}/admin-device",
    responses={
        200: {"model": List[AdminDeviceListing], "description": "Used for managing the devices a user visited. Admin-only. apiAdmin:LISTING apiServiceInternal:LISTING"},
...
async def list_all_admin_device_for_user(
...
) -> List[AdminDeviceListing]:
    """Used for managing the devices a user visited. Admin-only. apiAdmin:LISTING apiServiceInternal:LISTING"""
    if not BaseAdminDeviceApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAdminDeviceApi.subclasses[0]().list_all_admin_device_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
