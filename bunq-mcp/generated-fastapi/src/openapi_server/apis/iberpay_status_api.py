...
from openapi_server.apis.iberpay_status_api_base import BaseIberpayStatusApi
...
from openapi_server.models.iberpay_incoming_status_confirmation_create import IberpayIncomingStatusConfirmationCreate
...
@router.post(
    "/iberpay-status",
    responses={
        200: {"model": IberpayIncomingStatusConfirmationCreate, "description": "Endpoint for Iberpay to send status messages to bunq. Iberpay service name: WS_RECEP_CONF_ENVIO. apiIberpay:CREATE"},
...
async def c_reate_iberpay_status(
...
) -> IberpayIncomingStatusConfirmationCreate:
    """Endpoint for Iberpay to send status messages to bunq. Iberpay service name: WS_RECEP_CONF_ENVIO. apiIberpay:CREATE"""
    if not BaseIberpayStatusApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseIberpayStatusApi.subclasses[0]().c_reate_iberpay_status(user_agent, x_bunq_client_authentication, body, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
