...
from openapi_server.apis.exchange_rate_api_base import BaseExchangeRateApi
...
from openapi_server.models.exchange_rate_listing import ExchangeRateListing
...
@router.get(
    "/user/{userID}/exchange-rate",
    responses={
        200: {"model": List[ExchangeRateListing], "description": "Endpoint to list exchange rates. These are indicative and don&#39;t represent real-time data. apiApp:LISTING"},
...
async def list_all_exchange_rate_for_user(
...
) -> List[ExchangeRateListing]:
    """Endpoint to list exchange rates. These are indicative and don&#39;t represent real-time data. apiApp:LISTING"""
    if not BaseExchangeRateApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseExchangeRateApi.subclasses[0]().list_all_exchange_rate_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
