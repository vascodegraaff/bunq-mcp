...
from openapi_server.apis.admin_adyen_checkout_message_api_base import BaseAdminAdyenCheckoutMessageApi
...
from openapi_server.models.admin_adyen_checkout_message import AdminAdyenCheckoutMessage
from openapi_server.models.admin_adyen_checkout_message_update import AdminAdyenCheckoutMessageUpdate
...
@router.put(
    "/admin-adyen-checkout-message/{itemId}",
    responses={
        200: {"model": AdminAdyenCheckoutMessageUpdate, "description": "Manage Adyen Checkout Messages (Webhooks). apiAdmin:UPDATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["admin-adyen-checkout-message"],
    summary="",
    response_model_by_alias=True,
)
async def u_pdate_admin_adyen_checkout_message(
    ...
    admin_adyen_checkout_message: AdminAdyenCheckoutMessage = Body(None, description=""),
...
) -> AdminAdyenCheckoutMessageUpdate:
    """Manage Adyen Checkout Messages (Webhooks). apiAdmin:UPDATE"""
    if not BaseAdminAdyenCheckoutMessageApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAdminAdyenCheckoutMessageApi.subclasses[0]().u_pdate_admin_adyen_checkout_message(itemId, user_agent, x_bunq_client_authentication, admin_adyen_checkout_message, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
