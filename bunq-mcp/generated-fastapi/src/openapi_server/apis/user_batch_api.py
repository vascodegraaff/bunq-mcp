...
from openapi_server.apis.user_batch_api_base import BaseUserBatchApi
...
@router.get(
    "/user-batch",
    responses={
        200: {"model": List[object], "description": "Fetch multiple users at once. apiAdmin:LISTING apiServiceInternal:LISTING"},
...
async def list_all_user_batch(
    ...
    """Fetch multiple users at once. apiAdmin:LISTING apiServiceInternal:LISTING"""
    if not BaseUserBatchApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseUserBatchApi.subclasses[0]().list_all_user_batch(user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
