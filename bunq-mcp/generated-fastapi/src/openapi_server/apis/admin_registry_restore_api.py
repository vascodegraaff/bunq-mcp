...
from openapi_server.apis.admin_registry_restore_api_base import BaseAdminRegistryRestoreApi
...
from openapi_server.models.admin_registry_restore import AdminRegistryRestore
from openapi_server.models.admin_registry_restore_create import AdminRegistryRestoreCreate
...
@router.post(
    "/user/{userID}/registry/{registryID}/admin-registry-restore",
    responses={
        200: {"model": AdminRegistryRestoreCreate, "description": "Used to view and restore deleted registries for user. apiAdmin:LISTING/CREATE apiServiceInternal:LISTING/CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["admin-registry-restore"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_admin_registry_restore_for_user_registry(
    ...
    admin_registry_restore: AdminRegistryRestore = Body(None, description=""),
...
) -> AdminRegistryRestoreCreate:
    """Used to view and restore deleted registries for user. apiAdmin:LISTING/CREATE apiServiceInternal:LISTING/CREATE"""
    if not BaseAdminRegistryRestoreApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAdminRegistryRestoreApi.subclasses[0]().c_reate_admin_registry_restore_for_user_registry(userID, registryID, user_agent, x_bunq_client_authentication, admin_registry_restore, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/registry/{registryID}/admin-registry-restore",
    responses={
        200: {"model": List[object], "description": "Used to view and restore deleted registries for user. apiAdmin:LISTING/CREATE apiServiceInternal:LISTING/CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["admin-registry-restore"],
    summary="",
...
async def list_all_admin_registry_restore_for_user_registry(
    userID: StrictInt = Path(..., description=""),
    registryID: StrictInt = Path(..., description=""),
    user_agent: Annotated[StrictStr, Field(description="The User-Agent header field should contain information about the user agent originating the request. There are no restrictions on the value of this header.")] = Header(None, description="The User-Agent header field should contain information about the user agent originating the request. There are no restrictions on the value of this header."),
    x_bunq_client_authentication: Annotated[StrictStr, Field(description="The authentication token is used to authenticate the source of the API call. It is required by all API calls except for POST /v1/installation. It is important to note that the device and session calls are using the token from the response of the installation call, while all the other calls use the token from the response of the session-server call")] = Header(None, description="The authentication token is used to authenticate the source of the API call. It is required by all API calls except for POST /v1/installation. It is important to note that the device and session calls are using the token from the response of the installation call, while all the other calls use the token from the response of the session-server call"),
    ...
    if not BaseAdminRegistryRestoreApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAdminRegistryRestoreApi.subclasses[0]().list_all_admin_registry_restore_for_user_registry(userID, registryID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
