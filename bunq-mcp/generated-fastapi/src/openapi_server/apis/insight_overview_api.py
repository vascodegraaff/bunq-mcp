...
from openapi_server.apis.insight_overview_api_base import BaseInsightOverviewApi
...
from openapi_server.models.insight_overview_listing import InsightOverviewListing
...
@router.get(
    "/user/{userID}/insight-overview",
    responses={
        200: {"model": List[InsightOverviewListing], "description": "The endpoint to list insight overviews over a given period apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["insight-overview"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_insight_overview_for_user(
...
) -> List[InsightOverviewListing]:
    """The endpoint to list insight overviews over a given period apiApp:LISTING"""
    if not BaseInsightOverviewApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseInsightOverviewApi.subclasses[0]().list_all_insight_overview_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
