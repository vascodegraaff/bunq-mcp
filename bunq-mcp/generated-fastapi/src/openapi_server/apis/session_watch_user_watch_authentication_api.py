...
from openapi_server.apis.session_watch_user_watch_authentication_api_base import BaseSessionWatchUserWatchAuthenticationApi
...
from openapi_server.models.session_watch_user_watch_authentication import SessionWatchUserWatchAuthentication
from openapi_server.models.session_watch_user_watch_authentication_create import SessionWatchUserWatchAuthenticationCreate
...
@router.post(
    "/session-watch-user-watch-authentication",
    responses={
        200: {"model": SessionWatchUserWatchAuthenticationCreate, "description": "Create a new session for a watch with a UserWatchAuthentication token. apiApp:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["session-watch-user-watch-authentication"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_session_watch_user_watch_authentication(
    ...
    session_watch_user_watch_authentication: SessionWatchUserWatchAuthentication = Body(None, description=""),
...
) -> SessionWatchUserWatchAuthenticationCreate:
    """Create a new session for a watch with a UserWatchAuthentication token. apiApp:CREATE"""
    if not BaseSessionWatchUserWatchAuthenticationApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseSessionWatchUserWatchAuthenticationApi.subclasses[0]().c_reate_session_watch_user_watch_authentication(user_agent, x_bunq_client_authentication, session_watch_user_watch_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
