...
from openapi_server.apis.comment_api_base import BaseCommentApi
...
from openapi_server.models.admin_comment_feature_access_listing import AdminCommentFeatureAccessListing
from openapi_server.models.admin_comment_read import AdminCommentRead
...
@router.get(
    "/feature-access/{feature-accessID}/comment",
    responses={
        200: {"model": List[AdminCommentFeatureAccessListing], "description": "Used to manage admin comments regarding feature accesss. apiAdmin:LISTING apiServiceInternal:LISTING"},
...
async def list_all_comment_for_feature_access(
    feature-accessID: StrictInt = Path(..., description=""),
...
) -> List[AdminCommentFeatureAccessListing]:
    """Used to manage admin comments regarding feature accesss. apiAdmin:LISTING apiServiceInternal:LISTING"""
    if not BaseCommentApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseCommentApi.subclasses[0]().list_all_comment_for_feature_access(feature-accessID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/comment/{itemId}",
    responses={
        200: {"model": AdminCommentRead, "description": "Used to read comments. apiAdmin:READ apiServiceInternal:READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["comment"],
    summary="",
...
async def r_ead_comment(
    ...
    """Used to read comments. apiAdmin:READ apiServiceInternal:READ"""
    if not BaseCommentApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseCommentApi.subclasses[0]().r_ead_comment(itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
