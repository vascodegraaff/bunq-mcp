...
from openapi_server.apis.legal_guardian_api_base import BaseLegalGuardianApi
...
@router.delete(
    "/user-person/{user-personID}/legal-guardian/{itemId}",
    responses={
        200: {"model": object, "description": "Admin only endpoint that manages the legal guardian on a UserPerson apiAdmin:DELETE apiServiceInternal:DELETE"},
...
async def d_elete_legal_guardian_for_user_person(
    ...
    """Unsets the legal guardian on a UserPerson."""
    if not BaseLegalGuardianApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseLegalGuardianApi.subclasses[0]().d_elete_legal_guardian_for_user_person(user-personID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
