...
from openapi_server.apis.notification_badge_category_api_base import BaseNotificationBadgeCategoryApi
...
from openapi_server.models.notification_badge_category_listing import NotificationBadgeCategoryListing
...
@router.get(
    "/user/{userID}/notification-badge-category",
    responses={
        200: {"model": List[NotificationBadgeCategoryListing], "description": "To get the notification badge counts per category for the given user. apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["notification-badge-category"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_notification_badge_category_for_user(
...
) -> List[NotificationBadgeCategoryListing]:
    """To get the notification badge counts per category for the given user. apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING"""
    if not BaseNotificationBadgeCategoryApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseNotificationBadgeCategoryApi.subclasses[0]().list_all_notification_badge_category_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
