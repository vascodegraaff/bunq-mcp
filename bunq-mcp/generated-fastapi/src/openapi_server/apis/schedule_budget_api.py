...
from openapi_server.apis.schedule_budget_api_base import BaseScheduleBudgetApi
...
from openapi_server.models.schedule_budget_read import ScheduleBudgetRead
...
@router.get(
    "/user/{userID}/monetary-account/{monetary-accountID}/schedule-budget/{itemId}",
    responses={
        200: {"model": ScheduleBudgetRead, "description": "Endpoint to view the schedule definition of a budget apiAdmin:READ apiApp:READ apiServiceInternal:READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["schedule-budget"],
    summary="",
    response_model_by_alias=True,
)
async def r_ead_schedule_budget_for_user_monetary_account(
    ...
    """Endpoint to view the schedule definition of a budget apiAdmin:READ apiApp:READ apiServiceInternal:READ"""
    if not BaseScheduleBudgetApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseScheduleBudgetApi.subclasses[0]().r_ead_schedule_budget_for_user_monetary_account(userID, monetary-accountID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
