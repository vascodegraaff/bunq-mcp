...
from openapi_server.apis.vonage_token_api_base import BaseVonageTokenApi
...
from openapi_server.models.vonage_token_create import VonageTokenCreate
...
@router.post(
    "/user/{userID}/vonage-token",
    responses={
        200: {"model": VonageTokenCreate, "description": "Create jwt token and vonage user if needed for user calling Vonage hotline. apiAdmin:CREATE apiApp:CREATE"},
...
async def c_reate_vonage_token_for_user(
    ...
    """Create jwt token and vonage user if needed for user calling Vonage hotline. apiAdmin:CREATE apiApp:CREATE"""
    if not BaseVonageTokenApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseVonageTokenApi.subclasses[0]().c_reate_vonage_token_for_user(userID, user_agent, x_bunq_client_authentication, body, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
