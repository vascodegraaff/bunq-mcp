...
from openapi_server.apis.export_statement_card_csv_api_base import BaseExportStatementCardCsvApi
...
from openapi_server.models.export_statement_card_csv import ExportStatementCardCsv
from openapi_server.models.export_statement_card_csv_create import ExportStatementCardCsvCreate
from openapi_server.models.export_statement_card_csv_listing import ExportStatementCardCsvListing
from openapi_server.models.export_statement_card_csv_read import ExportStatementCardCsvRead
...
@router.post(
    ...
    responses={
        200: {"model": ExportStatementCardCsvCreate, "description": "Used to serialize ExportStatementCardCsv apiAdmin:CREATE/READ/LISTING/DELETE apiApp:CREATE/READ/LISTING/DELETE apiPublic:CREATE/READ/LISTING/DELETE apiServiceInternal:CREATE/READ/LISTING/DELETE"},
...
async def c_reate_export_statement_card_csv_for_user_card(
    ...
    export_statement_card_csv: ExportStatementCardCsv = Body(None, description=""),
    ...
    """Used to serialize ExportStatementCardCsv apiAdmin:CREATE/READ/LISTING/DELETE apiApp:CREATE/READ/LISTING/DELETE apiPublic:CREATE/READ/LISTING/DELETE apiServiceInternal:CREATE/READ/LISTING/DELETE"""
    if not BaseExportStatementCardCsvApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseExportStatementCardCsvApi.subclasses[0]().c_reate_export_statement_card_csv_for_user_card(userID, cardID, user_agent, x_bunq_client_authentication, export_statement_card_csv, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.delete(
    "/user/{userID}/card/{cardID}/export-statement-card-csv/{itemId}",
    responses={
        200: {"model": object, "description": "Used to serialize ExportStatementCardCsv apiAdmin:CREATE/READ/LISTING/DELETE apiApp:CREATE/READ/LISTING/DELETE apiPublic:CREATE/READ/LISTING/DELETE apiServiceInternal:CREATE/READ/LISTING/DELETE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["export-statement-card-csv"],
    summary="",
...
async def d_elete_export_statement_card_csv_for_user_card(
    userID: StrictInt = Path(..., description=""),
    cardID: StrictInt = Path(..., description=""),
    ...
    if not BaseExportStatementCardCsvApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseExportStatementCardCsvApi.subclasses[0]().d_elete_export_statement_card_csv_for_user_card(userID, cardID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/card/{cardID}/export-statement-card-csv",
    responses={
        200: {"model": List[ExportStatementCardCsvListing], "description": "Used to serialize ExportStatementCardCsv apiAdmin:CREATE/READ/LISTING/DELETE apiApp:CREATE/READ/LISTING/DELETE apiPublic:CREATE/READ/LISTING/DELETE apiServiceInternal:CREATE/READ/LISTING/DELETE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["export-statement-card-csv"],
    summary="",
...
async def list_all_export_statement_card_csv_for_user_card(
    ...
    x_bunq_client_request_id: Annotated[Optional[StrictStr], Field(description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer.")] = Header(None, description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer."),
    x_bunq_geolocation: Annotated[Optional[StrictStr], Field(description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued.")] = Header(None, description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued."),
) -> List[ExportStatementCardCsvListing]:
    """Used to serialize ExportStatementCardCsv apiAdmin:CREATE/READ/LISTING/DELETE apiApp:CREATE/READ/LISTING/DELETE apiPublic:CREATE/READ/LISTING/DELETE apiServiceInternal:CREATE/READ/LISTING/DELETE"""
    if not BaseExportStatementCardCsvApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseExportStatementCardCsvApi.subclasses[0]().list_all_export_statement_card_csv_for_user_card(userID, cardID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/card/{cardID}/export-statement-card-csv/{itemId}",
    responses={
        200: {"model": ExportStatementCardCsvRead, "description": "Used to serialize ExportStatementCardCsv apiAdmin:CREATE/READ/LISTING/DELETE apiApp:CREATE/READ/LISTING/DELETE apiPublic:CREATE/READ/LISTING/DELETE apiServiceInternal:CREATE/READ/LISTING/DELETE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["export-statement-card-csv"],
    summary="",
...
async def r_ead_export_statement_card_csv_for_user_card(
    userID: StrictInt = Path(..., description=""),
    cardID: StrictInt = Path(..., description=""),
    itemId: StrictInt = Path(..., description=""),
    user_agent: Annotated[StrictStr, Field(description="The User-Agent header field should contain information about the user agent originating the request. There are no restrictions on the value of this header.")] = Header(None, description="The User-Agent header field should contain information about the user agent originating the request. There are no restrictions on the value of this header."),
    ...
    if not BaseExportStatementCardCsvApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseExportStatementCardCsvApi.subclasses[0]().r_ead_export_statement_card_csv_for_user_card(userID, cardID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
