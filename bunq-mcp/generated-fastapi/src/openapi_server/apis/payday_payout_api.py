...
from openapi_server.apis.payday_payout_api_base import BasePaydayPayoutApi
...
from openapi_server.models.payday_payout_listing import PaydayPayoutListing
...
@router.get(
    "/user/{userID}/payday-payout",
    responses={
        200: {"model": List[PaydayPayoutListing], "description": "Get payday instances for auser. apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING"},
...
async def list_all_payday_payout_for_user(
...
) -> List[PaydayPayoutListing]:
    """Get payday instances for auser. apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING"""
    if not BasePaydayPayoutApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BasePaydayPayoutApi.subclasses[0]().list_all_payday_payout_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
