...
from openapi_server.apis.session_server_admin_okta_url_api_base import BaseSessionServerAdminOktaUrlApi
...
from openapi_server.models.session_server_admin_okta_url import SessionServerAdminOktaUrl
from openapi_server.models.session_server_admin_okta_url_create import SessionServerAdminOktaUrlCreate
...
@router.post(
    "/session-server-admin-okta-url",
    responses={
        200: {"model": SessionServerAdminOktaUrlCreate, "description": "Create a OIDC authentication url for a server device. apiAdmin:CREATE apiServiceInternal:CREATE apiProcess:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["session-server-admin-okta-url"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_session_server_admin_okta_url(
    ...
    session_server_admin_okta_url: SessionServerAdminOktaUrl = Body(None, description=""),
...
) -> SessionServerAdminOktaUrlCreate:
    """Create a OIDC authentication url for a server device. apiAdmin:CREATE apiServiceInternal:CREATE apiProcess:CREATE"""
    if not BaseSessionServerAdminOktaUrlApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseSessionServerAdminOktaUrlApi.subclasses[0]().c_reate_session_server_admin_okta_url(user_agent, x_bunq_client_authentication, session_server_admin_okta_url, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
