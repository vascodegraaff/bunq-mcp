...
from openapi_server.apis.mastercard_clearing_message_api_base import BaseMastercardClearingMessageApi
...
from openapi_server.models.master_card_clearing_message_listing import MasterCardClearingMessageListing
from openapi_server.models.master_card_clearing_message_read import MasterCardClearingMessageRead
...
@router.get(
    "/mastercard-action/{mastercard-actionID}/mastercard-clearing-message",
    responses={
        200: {"model": List[MasterCardClearingMessageListing], "description": "MasterCard clearing message view. apiAdmin:LISTING/READ apiServiceInternal:LISTING/READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["mastercard-clearing-message"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_mastercard_clearing_message_for_mastercard_action(
...
) -> List[MasterCardClearingMessageListing]:
    """MasterCard clearing message view. apiAdmin:LISTING/READ apiServiceInternal:LISTING/READ"""
    if not BaseMastercardClearingMessageApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseMastercardClearingMessageApi.subclasses[0]().list_all_mastercard_clearing_message_for_mastercard_action(mastercard-actionID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/mastercard-action/{mastercard-actionID}/mastercard-clearing-message/{itemId}",
    responses={
        200: {"model": MasterCardClearingMessageRead, "description": "MasterCard clearing message view. apiAdmin:LISTING/READ apiServiceInternal:LISTING/READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["mastercard-clearing-message"],
    summary="",
...
async def r_ead_mastercard_clearing_message_for_mastercard_action(
    mastercard-actionID: StrictInt = Path(..., description=""),
    ...
    if not BaseMastercardClearingMessageApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseMastercardClearingMessageApi.subclasses[0]().r_ead_mastercard_clearing_message_for_mastercard_action(mastercard-actionID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
