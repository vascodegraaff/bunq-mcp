...
from openapi_server.apis.tree_api_base import BaseTreeApi
...
from openapi_server.models.tree_user_listing import TreeUserListing
...
@router.get(
    "/user/{userID}/tree",
    responses={
        200: {"model": List[TreeUserListing], "description": "Endpoint for listing trees for a user. apiApp:LISTING"},
...
async def list_all_tree_for_user(
...
) -> List[TreeUserListing]:
    """Endpoint for listing trees for a user. apiApp:LISTING"""
    if not BaseTreeApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseTreeApi.subclasses[0]().list_all_tree_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
