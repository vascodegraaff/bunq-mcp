...
from openapi_server.apis.valuation_api_base import BaseValuationApi
...
from openapi_server.models.birdee_investment_portfolio_valuation_listing import BirdeeInvestmentPortfolioValuationListing
...
@router.get(
    "/user/{userID}/monetary-account-investment/{monetary-account-investmentID}/birdee-investment-portfolio/{birdee-investment-portfolioID}/valuation",
    responses={
        200: {"model": List[BirdeeInvestmentPortfolioValuationListing], "description": "Endpoint for interacting with the birdee investment portfolio valuation. apiApp:LISTING"},
...
async def list_all_valuation_for_user_monetary_account_investment_birdee_investment_portfolio(
...
) -> List[BirdeeInvestmentPortfolioValuationListing]:
    """Endpoint for interacting with the birdee investment portfolio valuation. apiApp:LISTING"""
    if not BaseValuationApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseValuationApi.subclasses[0]().list_all_valuation_for_user_monetary_account_investment_birdee_investment_portfolio(userID, monetary-account-investmentID, birdee-investment-portfolioID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
