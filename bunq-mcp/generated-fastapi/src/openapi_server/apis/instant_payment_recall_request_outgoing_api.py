...
from openapi_server.apis.instant_payment_recall_request_outgoing_api_base import BaseInstantPaymentRecallRequestOutgoingApi
...
from openapi_server.models.instant_payment_recall_request_outgoing import InstantPaymentRecallRequestOutgoing
from openapi_server.models.instant_payment_recall_request_outgoing_create import InstantPaymentRecallRequestOutgoingCreate
from openapi_server.models.instant_payment_recall_request_outgoing_read import InstantPaymentRecallRequestOutgoingRead
...
@router.post(
    "/instant-payment-recall-request-outgoing",
    responses={
        200: {"model": InstantPaymentRecallRequestOutgoingCreate, "description": "Endpoint to manage recall requests for SEPA Instant Payments. apiAdmin:CREATE/READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["instant-payment-recall-request-outgoing"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_instant_payment_recall_request_outgoing(
    ...
    instant_payment_recall_request_outgoing: InstantPaymentRecallRequestOutgoing = Body(None, description=""),
...
) -> InstantPaymentRecallRequestOutgoingCreate:
    """Endpoint to manage recall requests for SEPA Instant Payments. apiAdmin:CREATE/READ"""
    if not BaseInstantPaymentRecallRequestOutgoingApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseInstantPaymentRecallRequestOutgoingApi.subclasses[0]().c_reate_instant_payment_recall_request_outgoing(user_agent, x_bunq_client_authentication, instant_payment_recall_request_outgoing, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/instant-payment-recall-request-outgoing/{itemId}",
    responses={
        200: {"model": InstantPaymentRecallRequestOutgoingRead, "description": "Endpoint to manage recall requests for SEPA Instant Payments. apiAdmin:CREATE/READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["instant-payment-recall-request-outgoing"],
    summary="",
...
async def r_ead_instant_payment_recall_request_outgoing(
    ...
    x_bunq_client_request_id: Annotated[Optional[StrictStr], Field(description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer.")] = Header(None, description="This header must specify an ID with each request that is unique for the logged in user. There are no restrictions for the format of this ID. However, the server will respond with an error when the same ID is used again on the same DeviceServer."),
    x_bunq_geolocation: Annotated[Optional[StrictStr], Field(description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued.")] = Header(None, description="This header must specify the geolocation of the device. The format of this value is longitude latitude altitude radius country. The country is expected to be formatted of an ISO 3166-1 alpha-2 country code. When no geolocation is available or known the header must still be included but can be zero valued."),
) -> InstantPaymentRecallRequestOutgoingRead:
    """Endpoint to manage recall requests for SEPA Instant Payments. apiAdmin:CREATE/READ"""
    if not BaseInstantPaymentRecallRequestOutgoingApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseInstantPaymentRecallRequestOutgoingApi.subclasses[0]().r_ead_instant_payment_recall_request_outgoing(itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
