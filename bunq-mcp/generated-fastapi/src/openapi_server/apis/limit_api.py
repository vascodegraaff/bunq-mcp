...
from openapi_server.apis.limit_api_base import BaseLimitApi
...
from openapi_server.models.customer_limit_listing import CustomerLimitListing
...
@router.get(
    "/user/{userID}/limit",
    responses={
        200: {"model": List[CustomerLimitListing], "description": "Show the limits for the authenticated user. apiAdmin:LISTING apiApp:LISTING apiPublic:LISTING apiServiceInternal:LISTING"},
...
) -> List[CustomerLimitListing]:
    """Get all limits for the authenticated user."""
    if not BaseLimitApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseLimitApi.subclasses[0]().list_all_limit_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
