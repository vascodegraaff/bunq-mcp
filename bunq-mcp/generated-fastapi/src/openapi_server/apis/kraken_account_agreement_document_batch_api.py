...
from openapi_server.apis.kraken_account_agreement_document_batch_api_base import BaseKrakenAccountAgreementDocumentBatchApi
...
from openapi_server.models.kraken_account_agreement_document_batch import KrakenAccountAgreementDocumentBatch
from openapi_server.models.kraken_account_agreement_document_batch_create import KrakenAccountAgreementDocumentBatchCreate
...
@router.post(
    "/user/{userID}/kraken-account-agreement-document-batch",
    responses={
        200: {"model": KrakenAccountAgreementDocumentBatchCreate, "description": "Used to agree to documents required to open a Kraken account. apiApp:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["kraken-account-agreement-document-batch"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_kraken_account_agreement_document_batch_for_user(
    ...
    kraken_account_agreement_document_batch: KrakenAccountAgreementDocumentBatch = Body(None, description=""),
...
) -> KrakenAccountAgreementDocumentBatchCreate:
    """Used to agree to documents required to open a Kraken account. apiApp:CREATE"""
    if not BaseKrakenAccountAgreementDocumentBatchApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseKrakenAccountAgreementDocumentBatchApi.subclasses[0]().c_reate_kraken_account_agreement_document_batch_for_user(userID, user_agent, x_bunq_client_authentication, kraken_account_agreement_document_batch, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
