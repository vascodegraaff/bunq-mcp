...
from openapi_server.apis.insight_budget_warning_api_base import BaseInsightBudgetWarningApi
...
from openapi_server.models.insight_budget_warning_read import InsightBudgetWarningRead
...
@router.get(
    "/user/{userID}/insight-budget-warning/{itemId}",
    responses={
        200: {"model": InsightBudgetWarningRead, "description": "The endpoints for the insight budget warnings. apiApp:READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["insight-budget-warning"],
    summary="",
    response_model_by_alias=True,
)
async def r_ead_insight_budget_warning_for_user(
...
) -> InsightBudgetWarningRead:
    """The endpoints for the insight budget warnings. apiApp:READ"""
    if not BaseInsightBudgetWarningApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseInsightBudgetWarningApi.subclasses[0]().r_ead_insight_budget_warning_for_user(userID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
