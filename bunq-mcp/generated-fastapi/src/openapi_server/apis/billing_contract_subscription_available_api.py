...
from openapi_server.apis.billing_contract_subscription_available_api_base import BaseBillingContractSubscriptionAvailableApi
...
from openapi_server.models.billing_contract_subscription_available_listing import BillingContractSubscriptionAvailableListing
...
@router.get(
    "/user/{userID}/billing-contract-subscription-available",
    responses={
        200: {"model": List[BillingContractSubscriptionAvailableListing], "description": "List available subscription types for a user. apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["billing-contract-subscription-available"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_billing_contract_subscription_available_for_user(
...
) -> List[BillingContractSubscriptionAvailableListing]:
    """List available subscription types for a user. apiApp:LISTING"""
    if not BaseBillingContractSubscriptionAvailableApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseBillingContractSubscriptionAvailableApi.subclasses[0]().list_all_billing_contract_subscription_available_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
