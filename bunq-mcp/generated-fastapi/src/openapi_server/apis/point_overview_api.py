...
from openapi_server.apis.point_overview_api_base import BasePointOverviewApi
...
from openapi_server.models.point_overview_listing import PointOverviewListing
...
@router.get(
    "/user/{userID}/point-overview",
    responses={
        200: {"model": List[PointOverviewListing], "description": "Endpoint to retrieve an overview of points screen. apiAdmin:LISTING apiApp:LISTING"},
...
async def list_all_point_overview_for_user(
...
) -> List[PointOverviewListing]:
    """Endpoint to retrieve an overview of points screen. apiAdmin:LISTING apiApp:LISTING"""
    if not BasePointOverviewApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BasePointOverviewApi.subclasses[0]().list_all_point_overview_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
