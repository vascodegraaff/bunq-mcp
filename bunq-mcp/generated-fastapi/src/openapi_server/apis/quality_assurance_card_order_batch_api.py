...
from openapi_server.apis.quality_assurance_card_order_batch_api_base import BaseQualityAssuranceCardOrderBatchApi
...
from openapi_server.models.quality_assurance_card_order_batch_create import QualityAssuranceCardOrderBatchCreate
...
@router.post(
    "/quality-assurance-card-order-batch",
    responses={
        200: {"model": QualityAssuranceCardOrderBatchCreate, "description": "Update cards, for Quality Assurance purposes. apiQualityAssurance:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["quality-assurance-card-order-batch"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_quality_assurance_card_order_batch(
...
) -> QualityAssuranceCardOrderBatchCreate:
    """Update cards, for Quality Assurance purposes. apiQualityAssurance:CREATE"""
    if not BaseQualityAssuranceCardOrderBatchApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseQualityAssuranceCardOrderBatchApi.subclasses[0]().c_reate_quality_assurance_card_order_batch(user_agent, x_bunq_client_authentication, body, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
