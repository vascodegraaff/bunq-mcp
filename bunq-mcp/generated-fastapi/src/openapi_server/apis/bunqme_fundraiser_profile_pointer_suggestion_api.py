...
from openapi_server.apis.bunqme_fundraiser_profile_pointer_suggestion_api_base import BaseBunqmeFundraiserProfilePointerSuggestionApi
...
from openapi_server.models.bunq_me_fundraiser_profile_pointer_suggestion_listing import BunqMeFundraiserProfilePointerSuggestionListing
...
@router.get(
    "/user/{userID}/bunqme-fundraiser-profile-pointer-suggestion",
    responses={
        200: {"model": List[BunqMeFundraiserProfilePointerSuggestionListing], "description": "Endpoint for getting suggestions to be used as the variable part of bunq.me fundraiser URLs apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["bunqme-fundraiser-profile-pointer-suggestion"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_bunqme_fundraiser_profile_pointer_suggestion_for_user(
...
) -> List[BunqMeFundraiserProfilePointerSuggestionListing]:
    """Endpoint for getting suggestions to be used as the variable part of bunq.me fundraiser URLs apiApp:LISTING"""
    if not BaseBunqmeFundraiserProfilePointerSuggestionApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseBunqmeFundraiserProfilePointerSuggestionApi.subclasses[0]().list_all_bunqme_fundraiser_profile_pointer_suggestion_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
