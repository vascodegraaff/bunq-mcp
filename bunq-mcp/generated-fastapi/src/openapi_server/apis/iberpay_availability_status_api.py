...
from openapi_server.apis.iberpay_availability_status_api_base import BaseIberpayAvailabilityStatusApi
...
from openapi_server.models.iberpay_incoming_availability_status_create import IberpayIncomingAvailabilityStatusCreate
...
@router.post(
    "/iberpay-availability-status",
    responses={
        200: {"model": IberpayIncomingAvailabilityStatusCreate, "description": "Endpoint for Iberpay to send availability statuses concerning other participants to bunq. Iberpay service name: WS_DISP_DIFU. apiIberpay:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["iberpay-availability-status"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_iberpay_availability_status(
...
) -> IberpayIncomingAvailabilityStatusCreate:
    """Endpoint for Iberpay to send availability statuses concerning other participants to bunq. Iberpay service name: WS_DISP_DIFU. apiIberpay:CREATE"""
    if not BaseIberpayAvailabilityStatusApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseIberpayAvailabilityStatusApi.subclasses[0]().c_reate_iberpay_availability_status(user_agent, x_bunq_client_authentication, body, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
