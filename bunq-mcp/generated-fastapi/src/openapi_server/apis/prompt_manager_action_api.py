...
from openapi_server.apis.prompt_manager_action_api_base import BasePromptManagerActionApi
...
from openapi_server.models.prompt_manager_action import PromptManagerAction
from openapi_server.models.prompt_manager_action_create import PromptManagerActionCreate
...
@router.post(
    "/user/{userID}/prompt-manager-action",
    responses={
        200: {"model": PromptManagerActionCreate, "description": "Used to create actions involving prompts hosted on the prompt manager. apiAdmin:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["prompt-manager-action"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_prompt_manager_action_for_user(
    ...
    prompt_manager_action: PromptManagerAction = Body(None, description=""),
...
) -> PromptManagerActionCreate:
    """Used to create actions involving prompts hosted on the prompt manager. apiAdmin:CREATE"""
    if not BasePromptManagerActionApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BasePromptManagerActionApi.subclasses[0]().c_reate_prompt_manager_action_for_user(userID, user_agent, x_bunq_client_authentication, prompt_manager_action, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
