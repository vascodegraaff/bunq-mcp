...
from openapi_server.apis.user_api_key_api_base import BaseUserApiKeyApi
...
from openapi_server.models.user_api_key_read import UserApiKeyRead
...
@router.get(
    "/user-api-key/{itemId}",
    responses={
        200: {"model": UserApiKeyRead, "description": "Used to view OAuth request detais in events. apiApp:READ"},
...
async def r_ead_user_api_key(
    ...
    """Used to view OAuth request detais in events. apiApp:READ"""
    if not BaseUserApiKeyApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseUserApiKeyApi.subclasses[0]().r_ead_user_api_key(itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
