...
from openapi_server.apis.bizum_event_api_base import BaseBizumEventApi
...
from openapi_server.models.bizum_event_listing import BizumEventListing
...
@router.get(
    "/user/{userID}/bizum-event",
    responses={
        200: {"model": List[BizumEventListing], "description": "Used to list events specifically for the user&#39;s Bizum profile. apiApp:LISTING"},
...
async def list_all_bizum_event_for_user(
...
) -> List[BizumEventListing]:
    """Used to list events specifically for the user&#39;s Bizum profile. apiApp:LISTING"""
    if not BaseBizumEventApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseBizumEventApi.subclasses[0]().list_all_bizum_event_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
