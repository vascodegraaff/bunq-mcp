...
from openapi_server.apis.currency_conversion_api_base import BaseCurrencyConversionApi
...
from openapi_server.models.currency_conversion_listing import CurrencyConversionListing
from openapi_server.models.currency_conversion_read import CurrencyConversionRead
...
@router.get(
    ...
    responses={
        200: {"model": List[CurrencyConversionListing], "description": "Endpoint for managing currency conversions. apiApp:LISTING/READ apiPublic:LISTING/READ"},
...
) -> List[CurrencyConversionListing]:
    """Endpoint for managing currency conversions. apiApp:LISTING/READ apiPublic:LISTING/READ"""
    if not BaseCurrencyConversionApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseCurrencyConversionApi.subclasses[0]().list_all_currency_conversion_for_user_monetary_account(userID, monetary-accountID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)


@router.get(
    "/user/{userID}/monetary-account/{monetary-accountID}/currency-conversion/{itemId}",
    responses={
        200: {"model": CurrencyConversionRead, "description": "Endpoint for managing currency conversions. apiApp:LISTING/READ apiPublic:LISTING/READ"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["currency-conversion"],
    summary="",
...
async def r_ead_currency_conversion_for_user_monetary_account(
    userID: StrictInt = Path(..., description=""),
    monetary-accountID: StrictInt = Path(..., description=""),
    ...
    if not BaseCurrencyConversionApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseCurrencyConversionApi.subclasses[0]().r_ead_currency_conversion_for_user_monetary_account(userID, monetary-accountID, itemId, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
