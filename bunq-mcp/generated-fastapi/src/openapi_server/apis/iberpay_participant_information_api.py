...
from openapi_server.apis.iberpay_participant_information_api_base import BaseIberpayParticipantInformationApi
...
from openapi_server.models.iberpay_incoming_participant_information_create import IberpayIncomingParticipantInformationCreate
...
@router.post(
    "/iberpay-participant-information",
    responses={
        200: {"model": IberpayIncomingParticipantInformationCreate, "description": "Endpoint for Iberpay to send participant lists to bunq. Iberpay service name: WS_CARGAENTIDADESSCTINST. apiIberpay:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["iberpay-participant-information"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_iberpay_participant_information(
...
) -> IberpayIncomingParticipantInformationCreate:
    """Endpoint for Iberpay to send participant lists to bunq. Iberpay service name: WS_CARGAENTIDADESSCTINST. apiIberpay:CREATE"""
    if not BaseIberpayParticipantInformationApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseIberpayParticipantInformationApi.subclasses[0]().c_reate_iberpay_participant_information(user_agent, x_bunq_client_authentication, body, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
