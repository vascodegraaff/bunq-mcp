...
from openapi_server.apis.admin_assistant_conversation_escalation_expertise_available_api_base import BaseAdminAssistantConversationEscalationExpertiseAvailableApi
...
from openapi_server.models.admin_assistant_conversation_escalation_expertise_available_listing import AdminAssistantConversationEscalationExpertiseAvailableListing
...
@router.get(
    "/admin-assistant-conversation-escalation-expertise-available",
    responses={
        200: {"model": List[AdminAssistantConversationEscalationExpertiseAvailableListing], "description": "This call is used to list all active admin assistant conversation escalation on expertise. apiAdmin:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["admin-assistant-conversation-escalation-expertise-available"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_admin_assistant_conversation_escalation_expertise_available(
...
) -> List[AdminAssistantConversationEscalationExpertiseAvailableListing]:
    """This call is used to list all active admin assistant conversation escalation on expertise. apiAdmin:LISTING"""
    if not BaseAdminAssistantConversationEscalationExpertiseAvailableApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAdminAssistantConversationEscalationExpertiseAvailableApi.subclasses[0]().list_all_admin_assistant_conversation_escalation_expertise_available(user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
