...
from openapi_server.apis.mifid_national_client_identifier_requirement_api_base import BaseMifidNationalClientIdentifierRequirementApi
...
from openapi_server.models.mifid_national_client_identifier_requirement_listing import MifidNationalClientIdentifierRequirementListing
...
@router.get(
    "/user/{userID}/mifid-national-client-identifier-requirement",
    responses={
        200: {"model": List[MifidNationalClientIdentifierRequirementListing], "description": "The MiFID national client identifier requirements for a user. apiAdmin:LISTING apiApp:LISTING"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["mifid-national-client-identifier-requirement"],
    summary="",
    response_model_by_alias=True,
)
async def list_all_mifid_national_client_identifier_requirement_for_user(
...
) -> List[MifidNationalClientIdentifierRequirementListing]:
    """The MiFID national client identifier requirements for a user. apiAdmin:LISTING apiApp:LISTING"""
    if not BaseMifidNationalClientIdentifierRequirementApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseMifidNationalClientIdentifierRequirementApi.subclasses[0]().list_all_mifid_national_client_identifier_requirement_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
