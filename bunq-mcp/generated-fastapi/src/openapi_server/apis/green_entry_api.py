...
from openapi_server.apis.green_entry_api_base import BaseGreenEntryApi
...
from openapi_server.models.green_entry_listing import GreenEntryListing
...
@router.get(
    "/user/{userID}/green-entry",
    responses={
        200: {"model": List[GreenEntryListing], "description": "Used to view the green entries of a user. apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING"},
...
async def list_all_green_entry_for_user(
...
) -> List[GreenEntryListing]:
    """Used to view the green entries of a user. apiAdmin:LISTING apiApp:LISTING apiServiceInternal:LISTING"""
    if not BaseGreenEntryApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseGreenEntryApi.subclasses[0]().list_all_green_entry_for_user(userID, user_agent, x_bunq_client_authentication, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
