...
from openapi_server.apis.session_registry_installation_api_base import BaseSessionRegistryInstallationApi
...
from openapi_server.models.session_registry_installation import SessionRegistryInstallation
from openapi_server.models.session_registry_installation_create import SessionRegistryInstallationCreate
...
@router.post(
    ...
    responses={
        200: {"model": SessionRegistryInstallationCreate, "description": "Since endpoint used to register a device and create an anonymous user to be able to create Registries. Can also be used to refresh the device token upon expiry. apiApp:CREATE"},
        400: {"model": CREATE1brn400Response, "description": "This is how the error response looks like for 4XX response codes"},
    },
    tags=["session-registry-installation"],
    summary="",
    response_model_by_alias=True,
)
async def c_reate_session_registry_installation(
    ...
    session_registry_installation: SessionRegistryInstallation = Body(None, description=""),
...
) -> SessionRegistryInstallationCreate:
    """Since endpoint used to register a device and create an anonymous user to be able to create Registries. Can also be used to refresh the device token upon expiry. apiApp:CREATE"""
    if not BaseSessionRegistryInstallationApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseSessionRegistryInstallationApi.subclasses[0]().c_reate_session_registry_installation(user_agent, x_bunq_client_authentication, session_registry_installation, cache_control, x_bunq_language, x_bunq_region, x_bunq_client_request_id, x_bunq_geolocation)
